#Если ТолстыйКлиентОбычноеПриложение Тогда
	Процедура ЗарегистрироватьСобытие(Знач ИмяСобытия, Знач Уровень = Неопределено, Знач ОбъектМетаданных = Неопределено, Знач Данные = Неопределено, Знач Комментарий = "", Знач РежимТранзакции = Неопределено) Экспорт
		Попытка
			Если ПараметрыСеанса.ЖурналРегистрацииВsql Тогда
				ЗаписатьСобытие(Строка(ИмяСобытия), Строка(Уровень), строка(Комментарий), Данные, ОбщегоНазначенияПовтИсп.Компьютер());
			Иначе
				ЗаписьЖурналаРегистрации(ИмяСобытия, Уровень, ОбъектМетаданных, Данные, Комментарий, РежимТранзакции);	
			КонецЕсли;
		Исключение
			ЗаписьЖурналаРегистрации(ИмяСобытия, Уровень, ОбъектМетаданных, Данные, Комментарий, РежимТранзакции);
			ЗаписьЖурналаРегистрации("Ошибка выполнения",ПредопределенноеЗначение("УровеньЖурналаРегистрации.Ошибка"),,Данные,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
...
#Иначе
	Процедура ЗарегистрироватьСобытие(Знач ИмяСобытия, Знач Уровень = Неопределено, Знач ОбъектМетаданных = Неопределено, Знач Данные = Неопределено, Знач Комментарий = "", Знач РежимТранзакции = Неопределено) Экспорт
		...
		Попытка
			Если ПараметрыСеанса.ЖурналРегистрацииВsql Тогда
				ЗаписатьСобытие(Строка(ИмяСобытия), Строка(Уровень), Строка(Комментарий), Данные, ПолучитьТекущийСеансИнформационнойБазы().ИмяКомпьютера);
			Иначе
				ЗаписьЖурналаРегистрации(ИмяСобытия, Уровень, ОбъектМетаданных, Данные, Комментарий, РежимТранзакции);	
			КонецЕсли;
		Исключение
...
#Если ТонкийКлиент Тогда
	&НаСервере
#КонецЕсли
Функция ЗаписатьСобытие(Знач Событие = "", Знач Уровень = "Информация", Знач Комментарий = "", Знач пДанные = Неопределено, Знач Компьютер = Неопределено, Знач Пользователь = Неопределено, Знач ПредставлениеДанных = Неопределено, Знач Дата = Неопределено, Знач пМетаданные = Неопределено) Экспорт
		
	Тип = ТипЗнч(пДанные);
	Попытка
		лМетаданные = пДанные.Метаданные().ПолноеИмя();
		УИД = Строка(пДанные.УникальныйИдентификатор());	
		лДанные = "0x"+ирОбщий.ПолучитьГУИДИнверсныйИзПрямогоЛкс(УИД)
	Исключение
		лМетаданные = "";
		лДанные = "0x0";
		Если Комментарий = "" И пДанные <> Неопределено Тогда
			Попытка
				Если Тип = Тип("Структура") Тогда
					Комментарий = СтруктураВСтроку(пДанные);
				Иначе
					Если пДанные <> "" Тогда
						Комментарий = ЗначениеВСтрокуВнутр(пДанные);	
					КонецЕсли;
				КонецЕсли;
	...
	Если ПредставлениеДанных = Неопределено ИЛИ ПредставлениеДанных = "" Тогда
		ПредставлениеДанных = Строка(пДанные);
		Если ПустаяСтрока(ПредставлениеДанных) Тогда
			ПредставлениеДанных = Строка(лМетаданные);
		КонецЕсли;
	КонецЕсли;	
	...
	Если Дата = Неопределено Тогда
		Дата = "GetDate()";
	Иначе
		Дата = "convert(datetime, '"+Формат(Дата, "ДФ='yyyyMMdd HH:mm:ss'")+"', 104)"; 
	КонецЕсли;
	...
	, Уровень
	, Событие
	, ЖурналРегистрации.Экранировать(Строка(Комментарий))
	, лДанные
	, лев(лМетаданные,50)
	, ПредставлениеДанных
	, Дата
	...
	ТекстОшибки = "";
	Попытка
		SQL.ВыполнитьЗапросАсинхронно(ЖурналРегистрации.Коннект(), ТекстЗапроса, Отказ, ТекстОшибки);	
		Если Отказ Тогда
...
Процедура ЗарегистрироватьСобытиеОшибки(ИнформацияОбОшибке, Данные = Неопределено) Экспорт
	ЗарегистрироватьСобытие("Ошибка выполнения", ПредопределенноеЗначение("УровеньЖурналаРегистрации.Ошибка"),,Данные,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));	
КонецПроцедуры

#Если ТонкийКлиент Тогда
	&НаСервере
#КонецЕсли
Процедура ЗарегистрироватьИнтерактивноеДействие(Действие = "",  Комментарий = "", Данные = Неопределено) Экспорт
	ЗарегистрироватьСобытие(Действие, "ИнтерактивноеДействие",,Данные, Комментарий);	
КонецПроцедуры
...
// Получает строку, содержащую ключи и значения структуры, разделенные символом разделителя.
//
// Параметры:
//	Структура - Структура - Структура, ключи которой преобразуются в строку.
//	Разделитель - Строка - Разделитель, который вставляется в строку между ключами структуры.
...
//	Строка - Строка, содержащая ключи структуры разделенные разделителем.
//
Функция СтруктураВСтроку(Структура, Разделитель = ",") Экспорт
	
	Результат = "";
	
	Для Каждого Элемент Из Структура Цикл
		СимволРазделителя = ?(ПустаяСтрока(Результат), "", Разделитель);
		Результат = Результат + СимволРазделителя + Элемент.Ключ + ": " + Элемент.Значение;
	КонецЦикла;
