...
#Если ТонкийКлиент Тогда
	&НаСервере
#КонецЕсли
Функция ЗаписатьСобытие(Знач Событие = "", Знач Уровень = "Информация", Знач Комментарий = "", Знач пДанные = Неопределено, Знач Компьютер = Неопределено, Знач Пользователь = Неопределено, Знач ПредставлениеДанных = Неопределено, Знач Дата = Неопределено) Экспорт
