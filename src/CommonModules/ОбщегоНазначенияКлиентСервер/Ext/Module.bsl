...
#Иначе
	Процедура ЗарегистрироватьСобытие(Знач ИмяСобытия, Знач Уровень = Неопределено, Знач ОбъектМетаданных = Неопределено, Знач Данные = Неопределено, Знач Комментарий = "", Знач РежимТранзакции = Неопределено) Экспорт
		...
		Попытка
			Если ПараметрыСеанса.ЖурналРегистрацииВsql Тогда
				ЗаписатьСобытие(ИмяСобытия, Строка(Уровень), Комментарий, Данные, ПолучитьТекущийСеансИнформационнойБазы().ИмяКомпьютера);
			Иначе
				ЗаписьЖурналаРегистрации(ИмяСобытия, Уровень, ОбъектМетаданных, Данные, Комментарий, РежимТранзакции);	
			КонецЕсли;
		Исключение
...
#Если ТонкийКлиент Тогда
	&НаСервере
#КонецЕсли
Функция ЗаписатьСобытие(Знач Событие = "", Знач Уровень = "Информация", Знач Комментарий = "", Знач Данные = Неопределено, Знач Компьютер = Неопределено, Знач Пользователь = Неопределено, Знач ПредставлениеДанных = Неопределено, Знач Дата = Неопределено) Экспорт
	...
	, ЖурналРегистрации.ПреобразоватьДанные(Данные, лМетаданные)
	, лМетаданные
	, ПредставлениеДанных
	, Дата
	);
