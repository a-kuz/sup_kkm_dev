...
Функция СтруктураДанныхЛояльностиПоЗаказу_CALC(Источник, Ошибка, ТекстОшибки)
	...
	ИтогиО.Вставить("КГЛИнфо", Новый Массив);
	ИтогиКП.Вставить("КГЛИнфо", Новый Массив);
	Итоги.Вставить("Текстнфо", Новый Массив);
	ИтогиМ.Вставить("Текстнфо", Новый Массив);
	ИтогиО.Вставить("Текстнфо", Новый Массив);
	ИтогиКП.Вставить("Текстнфо", Новый Массив);
	...
	//СоответствиеИтогов.Вставить(0, "Простой текст");
	СоответствиеИтогов.Вставить(1, "За каждые 100г/мл");
	СоответствиеИтогов.Вставить(2, "За Акции");
	СоответствиеИтогов.Вставить(3, "По Программе ЛУЧ");
	СоответствиеИтогов.Вставить(4, "За персональные предложения");
	СоответствиеИтогов.Вставить(5, "За совместные акции");
	СоответствиеИтогов.Вставить(6, "За сумму покупки");
	Для каждого ТекущаяСтрока Из Источник.order.n_kgl_info Цикл
		...
		Если ТекущаяСтрока.type = 0 Тогда
			ТекущиеИтоги.ТекстИнфо.Добавить(ТекущаяСтрока.text);
		Иначе
			ТекстСтроки = СоответствиеИтогов.Получить(ТекущаяСтрока.type);
			Если ТекстСтроки <> Неопределено Тогда
				Строка = Новый Структура("Текст, КГЛ", , ТекущаяСтрока.kgl_add);
				ТекущиеИтоги.КГЛИнфо.Добавить(Строка);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
