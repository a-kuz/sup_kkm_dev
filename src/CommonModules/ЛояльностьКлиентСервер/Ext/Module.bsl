...
Функция СтруктураДанныхЛояльностиПоЗаказу(ТипДанных="INFO", Источник=Неопределено, Ошибка=Ложь, ТекстОшибки="") Экспорт
	...
	ИначеЕсли ТипДанных = "LUCH" Тогда
		Возврат СтруктураДанныхЛояльностиПоЗаказу_LUCH(Источник, Ошибка, ТекстОшибки);
...
Функция СтруктураДанныхЛояльностиПоЗаказу_LUCH(Источник, Ошибка, ТекстОшибки)
...
Функция СтруктураДанныхЛояльностиПоЗаказу_ACTIONS(Источник, Ошибка, ТекстОшибки)
	...
	Результат.Вставить("Ошибка"			,Ошибка);
	Результат.Вставить("ТекстОшибки"	,ТекстОшибки);
	Результат.Вставить("ЛУЧ"			,Ложь);
	Результат.Вставить("ЛУЧМ"			,Ложь);
	Результат.Вставить("ЛУЧО"			,Ложь);
	Результат.Вставить("ЛУЧКП"			,Ложь);
	Результат.Вставить("Купоны"			,Новый Массив);
	Результат.Вставить("Акции"			,Новый Массив);
	...
	// Данные о ЛУЧе гостя
	Результат.ЛУЧ = (Источник.guest.luch=1);
	Результат.ЛУЧМ = (Источник.guest.luch_m=1);
	Результат.ЛУЧО = (Источник.guest.luch_o=1);
	Результат.ЛУЧКП = (Источник.guest.luch_kp=1);
	...
	Для каждого ТекущаяСтрока Из Источник.guest.coupons_in Цикл
		...
		НоваяСтрока.Вставить("Проект", 0);//ТекущаяСтрока.type);
...
Функция СтруктураДанныхЛояльностиПоЗаказу_SALE(Источник, ТипЛояльности, Ошибка, ТекстОшибки) Экспорт
	...
	ИтогиМ = Новый Структура("КГЛНачислено, КГЛСписано", 0, 0);
	ИтогиО = Новый Структура("КГЛНачислено, КГЛСписано", 0, 0);
	ИтогиКП = Новый Структура("КГЛНачислено, КГЛСписано", 0, 0);
	...
	Если ТипЛояльности = 2 Тогда
		ТаблицаИтогов = Источник.s_kgl_info;
	Иначе
		ТаблицаИтогов = Источник.n_kgl_info;
	КонецЕсли;
		
	Для каждого ТекущаяСтрока Из ТаблицаИтогов Цикл
		Если ТекущаяСтрока.prj = 3 Тогда
			ТекущиеИтоги = ИтогиКП;
		ИначеЕсли ТекущаяСтрока.prj = 1 Тогда
			ТекущиеИтоги = ИтогиО;
		Иначе
			ТекущиеИтоги = ИтогиМ;
		КонецЕсли;
		ТекущиеИтоги.КГЛНачислено = ТекущиеИтоги.КГЛНачислено + ТекущаяСтрока.kgl_add;
		Если ТипЛояльности = 2 Тогда
			ТекущиеИтоги.КГЛСписано = ТекущиеИтоги.КГЛСписано + ТекущаяСтрока.kgl_rem;
		КонецЕсли;
		Строка = Новый Структура("ТипАкции, КГЛ", ТекущаяСтрока.action_type, ТекущаяСтрока.kgl_add);
		ТекущиеИтоги.КГЛИнфо.Добавить(Строка);
	КонецЦикла;
...
Функция СтруктураДанныхЛояльностиПоЗаказу_CALC(Источник, Ошибка, ТекстОшибки)
	...
	Результат.Вставить("КГЛНачислено"	,0);
	Результат.Вставить("КГЛСписано"		,0);
	Итоги = Новый Структура("КГЛНачислено, КГЛСписано", 0, 0);
	ИтогиМ = Новый Структура("КГЛНачислено, КГЛСписано", 0, 0);
	ИтогиО = Новый Структура("КГЛНачислено, КГЛСписано", 0, 0);
	ИтогиКП = Новый Структура("КГЛНачислено, КГЛСписано", 0, 0);
	Итоги.Вставить("КГЛИнфо", Новый Массив);
	ИтогиМ.Вставить("КГЛИнфо", Новый Массив);
	ИтогиО.Вставить("КГЛИнфо", Новый Массив);
	ИтогиКП.Вставить("КГЛИнфо", Новый Массив);
	Итоги.Вставить("ТекстИнфо", Новый Массив);
	ИтогиМ.Вставить("ТекстИнфо", Новый Массив);
	ИтогиО.Вставить("ТекстИнфо", Новый Массив);
	ИтогиКП.Вставить("ТекстИнфо", Новый Массив);
	...
	// Данные по итогам
	Для каждого ТекущаяСтрока Из Источник.order.n_kgl_info Цикл
		Если ТекущаяСтрока.prj = 1 Тогда
			ТекущиеИтоги = ИтогиО;
		ИначеЕсли ТекущаяСтрока.prj = 2 Тогда
			ТекущиеИтоги = ИтогиМ;
		ИначеЕсли ТекущаяСтрока.prj = 3 Тогда
			ТекущиеИтоги = ИтогиКП;
		Иначе
			ТекущиеИтоги = Итоги;
		КонецЕсли;
		Строка = Новый Структура("ТипАкции, КГЛ", ТекущаяСтрока.type, ТекущаяСтрока.kgl_add);
		ТекущиеИтоги.КГЛИнфо.Добавить(Строка);
		ТекущиеИтоги.КГЛНачислено = ТекущиеИтоги.КГЛНачислено + ТекущаяСтрока.kgl_add;
	КонецЦикла;
	
	Для каждого ТекущаяСтрока Из Источник.order.n_kgl_addinfo Цикл
		Если ТекущаяСтрока.prj = 1 Тогда
			ТекущиеИтоги = ИтогиО;
		ИначеЕсли ТекущаяСтрока.prj = 2 Тогда
			ТекущиеИтоги = ИтогиМ;
		...
		Иначе
			ТекущиеИтоги = Итоги;
		КонецЕсли;
		ТекущиеИтоги.ТекстИнфо.Добавить(Новый Структура("ТекстГостя,ТекстСтанции", СтрЗаменить(ТекущаяСтрока.guest_info,"\n", Символы.ПС), СтрЗаменить(ТекущаяСтрока.station_info,"\n", Символы.ПС)));
	КонецЦикла;
	
	// Купоны
	Если Источник.Свойство("coupons") Тогда
		Для каждого ТекущаяСтрока Из Источник.coupons Цикл
			НоваяСтрока = Новый Структура("Код, Тип, Проект, Наименование, ИнфоГостя, ИнфоСтанции, Статус");
			НоваяСтрока.Код = ТекущаяСтрока.code;
			НоваяСтрока.Тип = ТекущаяСтрока.type;
			НоваяСтрока.Проект = ТекущаяСтрока.prj;
			НоваяСтрока.Наименование = ТекущаяСтрока.name;
			НоваяСтрока.Статус = ТекущаяСтрока.status;
			НоваяСтрока.ИнфоГостя = СтрЗаменить(ТекущаяСтрока.guest_info,"\n", Символы.ПС);
			НоваяСтрока.ИнфоСтанции = СтрЗаменить(ТекущаяСтрока.station_info,"\n", Символы.ПС);
			//НоваяСтрока.Добавлен = (ТекущаяСтрока.Свойство("inserted") И ТекущаяСтрока.inserted = 1);
			Результат.Купоны.Добавить(НоваяСтрока);
		КонецЦикла;
	КонецЕсли;
	
	// Акции
	Если Источник.Свойство("actions") Тогда
		Для каждого ТекущаяСтрока Из Источник.actions Цикл
			НоваяСтрока = Новый Структура("Код, Тип, Проект, Наименование, ИнфоГостя, ИнфоСтанции, Статус");
			НоваяСтрока.Код = ТекущаяСтрока.code;
			НоваяСтрока.Тип = ТекущаяСтрока.type;
			НоваяСтрока.Проект = ТекущаяСтрока.prj;
			...
			НоваяСтрока.ИнфоГостя = СтрЗаменить(ТекущаяСтрока.guest_info,"\n", Символы.ПС);
			НоваяСтрока.ИнфоСтанции = СтрЗаменить(ТекущаяСтрока.station_info,"\n", Символы.ПС);
			Результат.Акции.Добавить(НоваяСтрока);
		КонецЦикла;
	КонецЕсли;
	
	// Всплывающие уведомления
	Если Источник.Свойство("popup") Тогда
		Для каждого ТекущаяСтрока Из Источник.popup Цикл
			НоваяСтрока = Новый Структура("Проект, Наименование, ИнфоГостя, ИнфоСтанции, Статус");
			НоваяСтрока.Проект = ТекущаяСтрока.prj;
			НоваяСтрока.Наименование = ТекущаяСтрока.name;
			НоваяСтрока.Статус = ТекущаяСтрока.status;
			НоваяСтрока.ИнфоГостя = СтрЗаменить(ТекущаяСтрока.guest_info,"\n", Символы.ПС);
			...
			Результат.Алерты.Добавить(НоваяСтрока);
		КонецЦикла;
	КонецЕсли;
...
Функция ПолучитьДанныеГостя(НомерКарты, Тест=Ложь) Экспорт
	...
	ДанныеЗапроса.Вставить("protocol_version", 5);
...
// Функция отправляет запрос сервису Лояльности для получения списка товаров в ЛУЧе Гостя
//
// Параметры:
// 	НомерКарты  - Строка - проверяемый номер
//
...
//	 	- Ошибка		- Булево
//		- ТекстОшибки	- Строка - описание ошибки
Функция ПолучитьЛУЧГостя(НомерКарты, Тест=Ложь) Экспорт
	
	Если ПустаяСтрока(НомерКарты) Тогда
		Возврат СтруктураДанныхЛояльностиПоЗаказу("LUCH");
	КонецЕсли;
	...
	ДанныеЗапроса = Новый Структура;
	ДанныеЗапроса.Вставить("request", "LUCH");
	ДанныеЗапроса.Вставить("protocol_version", 5);
	ДанныеЗапроса.Вставить("region", ПолучитьТекущийРегионСтрокой());
	ДанныеЗапроса.Вставить("tt", 1); //!!! Неплохо бы брать из настроек базы через повт. исп.
	ДанныеЗапроса.Вставить("station_type", 0);
	...
	Если ОтветJSON.Ошибка Тогда
		// При ошибке сервиса возвращаем пустую структуру, в регистр не пишем
		Возврат СтруктураДанныхЛояльностиПоЗаказу("LUCH",, Истина, ОтветJSON.Ответ);
	КонецЕсли;
	ДанныеОтвета = РазборJSON(ОтветJSON.Ответ);
	Если ДанныеОтвета=Неопределено Тогда
		Возврат СтруктураДанныхЛояльностиПоЗаказу("LUCH",, Истина, ОтветJSON.Ответ);
	КонецЕсли;
	Если ДанныеОтвета.err<0 Тогда
		Возврат СтруктураДанныхЛояльностиПоЗаказу("LUCH",, Истина, ДанныеОтвета.err_msg);
	КонецЕсли;
		
	Возврат СтруктураДанныхЛояльностиПоЗаказу("LUCH", ДанныеОтвета);
...
Функция ПолучитьАкцииГостя(НомерКарты, Тест=Ложь) Экспорт
	...
	ОтветJSON = ОтправитьЗапросСервисуОбработкиКартыГостя(ЗапросJSON, "GET", Тест);
	Если ОтветJSON.Ошибка Тогда
		ЗарегистрироватьСобытие("Лояльность.Сервисы", УровеньЖурналаРегистрации.Ошибка,,,"ACTIONS: "+ОтветJSON.Ответ);
		// При ошибке сервиса возвращаем пустую структуру, в регистр не пишем
...
Функция ПредварительныйРасчетЗаказа(ЗаказОбъект, Тест = Ложь) Экспорт
	...
	Если ЛояльностьКлиентСервер.РежимОффлайн() Тогда
		Возврат СтруктураДанныхЛояльностиПоЗаказу("CALC");
	КонецЕсли;
	...
	ИначеЕсли ВерсияПротокола = 5 Тогда
 		ДанныеЗапроса = СформироватьJSONДляПредварительногоРасчетаЗаказа_Протокол5(ЗаказОбъект);
	Иначе
		Возврат СтруктураДанныхЛояльностиПоЗаказу("CALC");
	КонецЕсли;
...
Процедура ЗаполнитьЗаказПоДаннымАкцийГостя_УДАЛИТЬ(ЗаказОбъект, ДанныеАкций) Экспорт
...
Функция РазобратьМаску_ПроектТипДисплея(Знач Значение)
	Значение = Формат(Значение, "ЧЦ=8; ЧН=00000000; ЧВН=");
	
	Маска = "#П0#ТД0#П1#ТД1#П2#ТД2#П3#ТД3";
	Количество = СтрДлина(Значение);
	Для Индекс = 1 По Количество Цикл
		ТекущееЗначение = Сред(Значение, Индекс, 1);
		Если ТекущееЗначение = "0" Тогда
			Продолжить;
		КонецЕсли;
		
		ТекущаяГруппа = Сред(Маска, (Индекс-1)*3+1, 3);
		Если Лев(ТекущаяГруппа, 2) = "#П" Тогда
			Проект = Число(Прав(ТекущаяГруппа,1));
		ИначеЕсли Лев(ТекущаяГруппа, 3) = "#ТД" Тогда
			ТипДисплея = Число(Прав(ТекущаяГруппа,1));
		КонецЕсли;		
	КонецЦикла;
	
	Возврат Новый Структура("Проект, ТипДисплея", Проект, ТипДисплея);;
КонецФункции
...
Функция РежимОффлайн() Экспорт
	Возврат Лояльность.РежимОффлайн();
КонецФункции
...
// Стабильная версия лояльности, используемая, по-умолчанию
...
// Версия протокола, соответствующая стабильной версия лояльности, используемой, по-умолчанию
...
Функция ВерсияПротоколаИзВерсииЛояльности(Знач Версия="") Экспорт
	Если Не ЗначениеЗаполнено(Версия) Тогда
		Возврат ТекущаяВерсияПротокола();
	КонецЕсли;
	
	Если ТипЗнч(Версия) = Тип("Строка") Тогда
		Версия = ОставитьТолькоЦифры(Версия);
		ВерсияПротокола = Лев(Версия, 1);
		
		Попытка
			ВерсияПротокола = Число(ВерсияПротокола);
		Исключение
...
Функция ОпределитьРегионПоНомеруКарты(НомерКарты) Экспорт
	Если Лев(НомерКарты, 2) = "50" ИЛИ Лев(НомерКарты, 2) = "52" Тогда
		Возврат "52";
	ИначеЕсли Лев(НомерКарты, 2) = "70" ИЛИ Лев(НомерКарты, 2) = "77" ИЛИ Лев(НомерКарты, 2) = "99" ИЛИ Лев(НомерКарты, 2) = "90" ИЛИ Лев(НомерКарты, 2) = "97" Тогда
		Возврат "77";
	Иначе
		Возврат "";
	КонецЕсли;
...
Процедура Логирование(Уровень, Место, Текст, Раздел="") Экспорт
...
#КонецЕсли
			...
			ИмяФайла = ПутьДляЛогирования + "\lol" + Раздел + Каталог + ".log";
		...
		КонецЕсли;
	Исключение
		ЗарегистрироватьСобытие("Ошибка записи лога Лояльности", УровеньЖурналаРегистрации.Предупреждение,,,"ПутьДляЛогирования="+ПутьДляЛогирования+", ИмяФайла="+ИмяФайла+", ОписаниеОшибки="+ОписаниеОшибки());
	КонецПопытки;
...
Функция ПолучитьВерсиюДанныхЛояльностиПоЗаказу(ЗаказСсылка) Экспорт
	Возврат Лояльность.ПолучитьВерсиюДанныхЛояльностиПоЗаказу(ЗаказСсылка);
КонецФункции
...
Процедура ОткрытьЗаказ(ЗаказОбъект, Тест) Экспорт
	...
	Иначе
		ПараметрыЗапроса = ЛояльностьКлиентСервер.СформироватьJSONДляОткрытияЗаказа_Протокол5(ЗаказОбъект);
	КонецЕсли;
...
#КонецЕсли
	...
	Если Не ПустаяСтрока(НомерКартыЛояльности) Тогда
		...
		ИначеЕсли ДанныеГостя.Ошибка Тогда
			...
			//ЗарегистрироватьСобытие("Лояльность.Получение данных КГЛБ", УровеньЖурналаРегистрации.Ошибка,,,ДанныеГостя.ТекстОшибки);
		Иначе
...
#КонецЕсли
	...
	Если ТипЗапроса = "SALE" Тогда
		МассивОплаты = Лояльность.ПолучитьМассивОплаты(ЗаказОбъект.Ссылка);
...
Функция СформироватьJSONПоЗаказу_Протокол2(ТипЗапроса = "SALE", ЗаказОбъект, ТипЧекаЛояльность, СтруктураАСКов = Неопределено, РежимСПУЛ = Ложь, Тест = Ложь) Экспорт
	...
	Результат.Вставить("loyality_version", ВерсияЛояльности(Тест));
	
	//Чек.Вставить("debug"	, 1);
	Чек.Вставить("request"	, ТипЗапроса);
	Чек.Вставить("request_id", Строка(ЗаказОбъект.Ссылка.УникальныйИдентификатор()));
...
Функция СформироватьJSONДляРасчетаЗаказа_Протокол5(ЗаказОбъект, ДанныеЛояльностиПоЗаказу=Неопределено, СтруктураАСКов = Неопределено) Экспорт
	...
	ТаблицаТовары.Колонки.Добавить("Количество", ТипЧисло);
	ТаблицаТовары.Колонки.Добавить("Цена", ТипЧисло);
	ТаблицаТовары.Колонки.Добавить("ЦенаРеализации", ТипЧисло);
	ТаблицаТовары.Колонки.Добавить("ПДФ", ТипСтрока);
	ТаблицаТовары.Колонки.Добавить("ШК", ТипСтрока);
	ТаблицаТовары.Колонки.Добавить("ВремяДобавления", ТипДата);
	...
	ТаблицаСпецифики.Колонки.Добавить("Количество", ТипЧисло);
	ТаблицаСпецифики.Колонки.Добавить("Цена", ТипЧисло);
	ТаблицаСпецифики.Колонки.Добавить("ЦенаРеализации", ТипЧисло);
	...
	|	ТаблицаТоваров.Количество КАК Количество,
	|	ТаблицаТоваров.Цена КАК Цена,
	|	ТаблицаТоваров.ЦенаРеализации КАК ЦенаРеализации,
	|	ТаблицаТоваров.ПДФ КАК ПДФ,
	|	ТаблицаТоваров.ШК КАК ШК,
	|	ТаблицаТоваров.ВремяДобавления КАК ВремяДобавления,
	...
	|	ТаблицаСпецифики.Количество КАК Количество,
	|	ТаблицаСпецифики.Цена КАК Цена,
	|	ТаблицаСпецифики.ЦенаРеализации КАК ЦенаРеализации,
	...
	|	Влож.Цена КАК Цена,
	|	Влож.Сумма КАК Сумма,
	|	Влож.ЦенаФакт КАК ЦенаФакт,
	|	Влож.СуммаФакт КАК СуммаФакт,
	|	Влож.Наименование КАК Наименование,
	|	Влож.Товар КАК Товар,
	|	Влож.Специфика КАК Специфика,
	|	Влож.НомерСтрокиСпец КАК НомерСтрокиСпец,
	...
	|		ЗаказТовары.Цена КАК Цена,
	|		ЗаказТовары.Цена * ЗаказТовары.Количество КАК Сумма,
	|		ЗаказТовары.ЦенаРеализации КАК ЦенаФакт,
	|		ЗаказТовары.ЦенаРеализации * ЗаказТовары.Количество КАК СуммаФакт,
	|		ЗаказТовары.Товар.Наименование КАК Наименование,
	|		ЗаказТовары.Товар КАК Товар,
	|		0 КАК Специфика,
	|		NULL КАК НомерСтрокиСпец,
	...
	|		ЗаказТовары.ИдСтроки,
	|		ЗаказСпецифики.Специфика.Номенклатура.КодСУП,
	|		ЗаказСпецифики.Количество * ЗаказТовары.Количество,
	|		ЗаказСпецифики.Цена,
	|		ЗаказСпецифики.Цена * ЗаказСпецифики.Количество * ЗаказТовары.Количество,
	|		ЗаказСпецифики.ЦенаРеализации,
	|		ЗаказСпецифики.ЦенаРеализации * ЗаказСпецифики.Количество * ЗаказТовары.Количество,
	|		ЗаказСпецифики.Специфика.Наименование,
	|		NULL,
	|		1,
	|		ЗаказСпецифики.НомерСтроки,
	...
	Пока ВыборПоНомеру.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.СтатусОплаты=1 Тогда
				// У всех оплаченных строчек оставляем статус лояльности
			...
			ИначеЕсли Выборка.ЛояльностьСтатус = 1 И //Выборка.Сумма > Выборка.СуммаФакт Тогда
				(Выборка.ЛояльностьСтатусАкции = 1 ИЛИ Выборка.ЛояльностьСтатусАкции = 11 ИЛИ Выборка.ЛояльностьСтатусАкции = 12
				ИЛИ Выборка.ЛояльностьГруппаАкции = 26) Тогда
				// Если применена лояльность и сумма фактическая меньше суммы прайсовой, то значит пременилась одна из акций +1, ПЗП, купон на скидку...
				//!!! Неизвестно как будет работать со спецификами
				СтрокаЧека.Вставить("loyality_status"	, Выборка.ЛояльностьСтатус);
				СтрокаЧека.Вставить("action_status"		, Выборка.ЛояльностьСтатусАкции);
				СтрокаЧека.Вставить("action_status_d"	, Выборка.ЛояльностьДопСтатусАкции);
				СтрокаЧека.Вставить("action_group"		, Выборка.ЛояльностьГруппаАкции);
	...
	//Запрос.Вставить("region", Строка(Лояльность.ПолучитьТекущийРегион())); //### Только для разработки!
	Запрос.Вставить("station_type", Станция);
	//Запрос.Вставить("debug", 1);
	//Запрос.Вставить("delay", 1000);
	Запрос.Вставить("order", Новый Структура);
...
#КонецЕсли
	...
	Для Каждого ТекущийКупон ИЗ ЗаказОбъект.Купоны Цикл
		...
		СтрокаКупонов.Вставить("status", ?(ТекущийКупон.СтатусКупона=3,3,1));
		Купоны.Добавить(СтрокаКупонов);
	КонецЦикла;
	
	АкцииПоЗаказу = Лояльность.ПолучитьДанныеЛояльностиПоЗаказу(ЗаказОбъект.Ссылка, "ACTIONS");
	ЗаказОбъектКупоны = ЗаказОбъект.Купоны;
#Если НЕ ТонкийКлиент Тогда
	Если ТипЗнч(ЗаказОбъект) = Тип("Структура") Тогда
		ЗаказОбъектКупоны = МассивСтруктурВТаблицуЗначений(ЗаказОбъект.Купоны);
	КонецЕсли;
#КонецЕсли
	Для каждого ТекущийКупон Из АкцииПоЗаказу.Купоны Цикл
		// Проверяем если купон уже есть в заказе, то его уже добавили в массив
		Если НЕ ЗаказОбъектКупоны = Неопределено Тогда
			ПоискДублей = ЗаказОбъектКупоны.Найти(ТекущийКупон.Код, "Купон");
			Если ПоискДублей <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		СтрокаКупонов = Новый Структура;
		СтрокаКупонов.Вставить("code", ТекущийКупон.Код);
		СтрокаКупонов.Вставить("type", ТекущийКупон.Тип);
		СтрокаКупонов.Вставить("status", ТекущийКупон.Статус);
		Купоны.Добавить(СтрокаКупонов);
	КонецЦикла;
...
Функция СформироватьJSONДляПредварительногоРасчетаЗаказа_Протокол5(ЗаказОбъект) Экспорт
	...
	Пока ВыборПоНомеру.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			// Только у оплаченных строчек оставляем статусы лояльности
			Если Выборка.СтатусОплаты=1 Тогда
				СтрокаЧека.Вставить("loyality_status"	, Выборка.ЛояльностьСтатус);
				СтрокаЧека.Вставить("action_status"		, Выборка.ЛояльностьСтатусАкции);
				СтрокаЧека.Вставить("action_status_d"	, Выборка.ЛояльностьДопСтатусАкции);
...
// Формирование JSON по объекту заказа или структуре
Функция СформироватьJSONДляОткрытияЗаказа_Протокол5(ЗаказОбъект) Экспорт
...
КонецЕсли;
	...
	МассивОплаты = Лояльность.ПолучитьМассивОплаты(ВозвратОбъект.Ссылка);
	Чек.Вставить("pay"			, МассивОплаты);
...
Функция ПолучитьДанныеДляЦенников(Запрос) Экспорт
	
	Ответ = Новый Структура;
	Ответ.Вставить("Результат", 1);
	Ответ.Вставить("ОписаниеОшибки", "");
	Ответ.Вставить("СписокТоваров", Новый Массив);
	
	Регион = Лояльность.ПолучитьТекущийРегион();
	
	ДанныеЗапроса = Новый Структура;
	ДанныеЗапроса.Вставить("request", "PRICE");
	ДанныеЗапроса.Вставить("protocol_version", 5);//ЛояльностьКлиентСервер.ВерсияЛояльности());
	ТТ = ЛояльностьКлиентСервер.НомерТТ(ПланыОбмена.Основной.ЭтотУзел().ИнформационнаяБаза.Код);
	ДанныеЗапроса.Вставить("tt", ТТ);
	ДанныеЗапроса.Вставить("price_date", ТекущаяДата());
	ДанныеЗапроса.Вставить("price_str", Новый Массив);	
	ДанныеЗапроса.Вставить("password", ПарольСервиса_Чеки());
	
	Для каждого ТекущаяСтрока Из Запрос.СписокТоваров Цикл
		НоваяСтрока = Новый Структура;
		НоваяСтрока.Вставить("product_code", ТекущаяСтрока.КодСУП);
		НоваяСтрока.Вставить("price", ТекущаяСтрока.Цена);
		НоваяСтрока.Вставить("quantity", ТекущаяСтрока.Количество);
		ДанныеЗапроса.price_str.Добавить(НоваяСтрока);
	КонецЦикла;
	
	ЗапросJSON = СформироватьJSON_Новый(ДанныеЗапроса);
	
	Тест = Ложь;
	ОтветСервера = Лояльность.ОтправитьЗапросКСервисуРасчетаЧеков(ЗапросJSON, "PRICE_LIST", Тест);

	Если ОтветСервера.Ошибка ИЛИ ОтветСервера.Ответ = "" ИЛИ ОтветСервера.Ответ = Неопределено Тогда
		// сервис недоступен или не смог обработать запрос
		Ответ.Результат = 0;
		Ответ.ОписаниеОшибки = "Ошибка сервиса Лояльности";
		Возврат СформироватьJSON_Новый(Ответ);
	КонецЕсли;
	
	ДанныеОтвета = РазборJSON(ОтветСервера.Ответ);
	
	Если ДанныеОтвета.err<>0  Тогда
		Ответ.Результат = 0;
		Ответ.ОписаниеОшибки = ДанныеОтвета.err_msg;
		Возврат СформироватьJSON_Новый(Ответ);
	КонецЕсли;
	
	Для каждого ТекущаяСтрока Из ДанныеОтвета.price_str Цикл
		НоваяСтрока = Новый Структура;
		НоваяСтрока.Вставить("КодСУП", ТекущаяСтрока.product_code);
		НоваяСтрока.Вставить("Цена", ТекущаяСтрока.price);
		НоваяСтрока.Вставить("Количество", ТекущаяСтрока.quantity);
		НоваяСтрока.Вставить("МРЦ", ТекущаяСтрока.mrc);
		НоваяСтрока.Вставить("КГЛНачисление", ТекущаяСтрока.kgl_add);
		НоваяСтрока.Вставить("КГЛСписание", ТекущаяСтрока.kgl_rem);
		НоваяСтрока.Вставить("СтатусАкции", ТекущаяСтрока.action_status);
		НоваяСтрока.Вставить("ГруппаАкции", ТекущаяСтрока.action_group);
		Ответ.СписокТоваров.Добавить(НоваяСтрока);
	КонецЦикла;
...
Функция СформироватьJSON(Данные, ПараметрыФормирования=Неопределено) Экспорт
	...
	ИсключитьСвойства = Новый Массив;
	
	ЗаписатьДанныеВJSON(ЗаписьJSON, Данные, ПараметрыФормирования, ИсключитьСвойства);
...
Функция СформироватьJSON_Новый(Данные, ПараметрыФормирования=Неопределено, ИсключитьСвойства=Неопределено) Экспорт
	
	Если ПараметрыФормирования = Неопределено Тогда
		ПараметрыФормирования = ПолучитьПараметрыФормированияJSON_Протокол3();
	КонецЕсли;
	Если ИсключитьСвойства = Неопределено Тогда
		ИсключитьСвойства = Новый Массив;
	КонецЕсли;
...
Функция ЗаписатьДанныеВJSON(ЗаписьJSON, Данные, ПараметрыФормирования, ИсключитьСвойства)
	Если ТипЗнч(Данные) = Тип("Структура") Тогда
		ЗаписьJSON.ЗаписатьНачалоОбъекта();
		Для каждого ТекущиеДанные Из Данные Цикл
			Если ИсключитьСвойства.Найти(ТекущиеДанные.Ключ) <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
			ЗаписьJSON.ЗаписатьИмяСвойства(ТекущиеДанные.Ключ);
			ЗаписатьДанныеВJSON(ЗаписьJSON, ТекущиеДанные.Значение, ПараметрыФормирования, ИсключитьСвойства);
		КонецЦикла;
		ЗаписьJSON.ЗаписатьКонецОбъекта();
	ИначеЕсли ТипЗнч(Данные) = Тип("Массив") Тогда
		ЗаписьJSON.ЗаписатьНачалоМассива();
		Для каждого ТекущиеДанные Из Данные Цикл
			ЗаписатьДанныеВJSON(ЗаписьJSON, ТекущиеДанные, ПараметрыФормирования, ИсключитьСвойства);
		КонецЦикла;
	...
	ИначеЕсли ТипЗнч(Данные) = Тип("ТаблицаЗначений") Тогда
		...
		Для каждого ТекущиеДанные Из Данные Цикл
			...
			Для каждого ТекущаяКолонка Из Данные.Колонки Цикл
				Если ИсключитьСвойства.Найти(ТекущаяКолонка.Имя) <> Неопределено Тогда
					Продолжить;
				КонецЕсли;
				ЗаписьJSON.ЗаписатьИмяСвойства(ТекущаяКолонка.Имя);
				ЗаписатьДанныеВJSON(ЗаписьJSON, ТекущиеДанные[ТекущаяКолонка.Имя], ПараметрыФормирования, ИсключитьСвойства);
			КонецЦикла;
			ЗаписьJSON.ЗаписатьКонецОбъекта();
		КонецЦикла;
		ЗаписьJSON.ЗаписатьКонецМассива();
	Иначе
		...
		Иначе
			...
			Исключение
				ЗаписьJSON.ЗаписатьЗначение(Строка(Данные));
			КонецПопытки;
		КонецЕсли;		
	КонецЕсли;
КонецФункции
