...
Функция ПолучитьДанныеГостя(НомерКарты, Тест=Ложь) Экспорт
	...
	ВерсияЛояльности = ВерсияЛояльности();
 	ВерсияПротокола = ВерсияПротоколаИзВерсииЛояльности(ВерсияЛояльности);
	
	ДанныеЗапроса = Новый Структура;
	ДанныеЗапроса.Вставить("request", "INFO");
	ДанныеЗапроса.Вставить("protocol_version", ВерсияПротокола);
