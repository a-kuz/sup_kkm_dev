...
Функция СтруктураДанныхЛояльностиПоЗаказу_CALC(Источник, Ошибка, ТекстОшибки)
	...
	Для каждого ТекущаяСтрока Из Источник.order.n_kgl_info Цикл
		Если ТекущаяСтрока.prj = 1 Тогда
			Строка = Новый Структура("Текст, КГЛ", ТекущаяСтрока.text, ТекущаяСтрока.kgl_add);
			ИтогиО.КГЛИнфо.Добавить(Строка);
		ИначеЕсли ТекущаяСтрока.prj = 2 Тогда
			Строка = Новый Структура("Текст, КГЛ", ТекущаяСтрока.text, ТекущаяСтрока.kgl_add);
			ИтогиМ.КГЛИнфо.Добавить(Строка);
		ИначеЕсли ТекущаяСтрока.prj = 3 Тогда
			Строка = Новый Структура("Текст, КГЛ", ТекущаяСтрока.text, ТекущаяСтрока.kgl_add);
			ИтогиКП.КГЛИнфо.Добавить(Строка);
		Иначе
			Строка = Новый Структура("Текст, КГЛ", ТекущаяСтрока.text, ТекущаяСтрока.kgl_add);
