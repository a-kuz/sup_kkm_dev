...
Функция СтруктураДанныхЛояльностиПоЗаказу_ACTIONS(Источник, Ошибка, ТекстОшибки)
	...
	Результат.ЛУЧМ = (Источник.guest.luch_o=1);
	Результат.ЛУЧО = (Источник.guest.luch_m=1);
...
Функция СтруктураДанныхЛояльностиПоЗаказу_CALC(Источник, Ошибка, ТекстОшибки)
	...
	Для каждого ТекущаяСтрока Из Источник.order.n_kgl_info Цикл
		...
		Иначе
			ТекущиеИтоги = Итоги;
		КонецЕсли;
		//Если ТекущаяСтрока.type = 0 Тогда
		//	ТекущиеИтоги.ТекстИнфо.Добавить(СтрЗаменить(ТекущаяСтрока.text,"\n", Символы.ПС));
		//Иначе		
	...
	ИтогиО.ТекстИнфо.Добавить(Новый Структура("ТекстГостя,ТекстСтанции", "Тестим раз", "Тестим раз"));
	ИтогиО.ТекстИнфо.Добавить(Новый Структура("ТекстГостя,ТекстСтанции", "Тестим два", "Тестим два"));
	ИтогиО.ТекстИнфо.Добавить(Новый Структура("ТекстГостя,ТекстСтанции", "", "Тестим три"));
	ИтогиМ.ТекстИнфо.Добавить(Новый Структура("ТекстГостя,ТекстСтанции", "Тестим раз", "Тестим раз"));
	ИтогиМ.ТекстИнфо.Добавить(Новый Структура("ТекстГостя,ТекстСтанции", "Тестим два", "Тестим два"));
	ИтогиМ.ТекстИнфо.Добавить(Новый Структура("ТекстГостя,ТекстСтанции", "Тестим три", ""));
	
	//Итоги.Вставить("ИнфоГостя", Новый Массив);
	//Итоги.Вставить("ИнфоСтанции", Новый Массив);
	//ИтогиМ.Вставить("ИнфоГостя", Новый Массив);
	//ИтогиМ.Вставить("ИнфоСтанции", Новый Массив);
	//ИтогиО.Вставить("ИнфоГостя", Новый Массив);
	//ИтогиО.Вставить("ИнфоСтанции", Новый Массив);
	//ИтогиКП.Вставить("ИнфоГостя", Новый Массив);
	//ИтогиКП.Вставить("ИнфоСтанции", Новый Массив);
