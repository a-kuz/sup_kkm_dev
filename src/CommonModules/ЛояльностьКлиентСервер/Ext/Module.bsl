...
Функция СтруктураДанныхЛояльностиПоЗаказу_CALC(Источник, Ошибка, ТекстОшибки)
	...
	Для каждого ТекущаяСтрока Из КГЛБИнфо.kgl_str Цикл
		...
		ИначеЕсли ТекущаяСтрока.sale_place_prj = 3 Тогда
			...
			КонецЕсли;
		Иначе
			Итоги.КГЛНачислено = Итоги.КГЛНачислено + ТекущаяСтрока.kgl_add;
			Если ТекущаяСтрока.action_status < 9 Тогда
				Итоги.КГЛЗаТовар = Итоги.КГЛЗаТовар + ТекущаяСтрока.kgl_add;
			Иначе
				Итоги.КГЛЗаСерт = Итоги.КГЛЗаСерт + ТекущаяСтрока.kgl_add;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	...
	Для каждого ТекущаяСтрока Из КГЛБИнфо.kgl_total Цикл
		...
		Иначе
			Итоги.КГЛНачислено = Итоги.КГЛНачислено + ТекущаяСтрока.kgl_add;
			Итоги.КГЛЗаСумму = Итоги.КГЛЗаСумму + ТекущаяСтрока.kgl_add;
		КонецЕсли;
	КонецЦикла;
	
	Если Источник.Свойство("coupon_in") Тогда
		Для каждого ТекущаяСтрока Из Источник.coupon_in Цикл
			НоваяСтрока = Новый Структура("Код, Тип, Проект, Наименование, ИнфоГостя, ИнфоСтанции, Статус, Добавлен");
	...
	Если Источник.Свойство("popup") Тогда
		Для каждого ТекущаяСтрока Из Источник.popup Цикл
			...
			НоваяСтрока.Наименование = ТекущаяСтрока.name;
			НоваяСтрока.Статус = ТекущаяСтрока.status;
			НоваяСтрока.ИнфоГостя = ТекущаяСтрока.guest_info;
			НоваяСтрока.ИнфоСтанции = ТекущаяСтрока.station_info;
...
#КонецЕсли
	...
	//АкцииПоЗаказу = ПолучитьДанныеЛояльностиПоЗаказу(ЗаказОбъект.Ссылка, "ACTIONS");
	//Для каждого ТекущийКупон Из АкцииПоЗаказу.КупоныВыданные Цикл
