...
Функция ОбработатьНачисление_Протокол5(ЗаказОбъект, ПротоколОбъект, ДанныеЗаказа, МестоРеализации = Неопределено)
	...
	Для каждого ТекущееМестоРеализации Из ДанныеЗаказа.sale_places Цикл
		...
		Если ТекущееМестоРеализации.Свойство("coupons_n") Тогда
			ОбработатьКупоны_Протокол4(ЗаказОбъект, ТекущееМестоРеализации.coupons_n);
		КонецЕсли;
...
Функция ОбработатьСписание_Протокол5(ЗаказОбъект, ПротоколОбъект, ДанныеЗаказа, МестоРеализации=Неопределено)
	...
	Для каждого ТекущееМестоРеализации Из ДанныеЗаказа.sale_places Цикл
		...
		// Обработка купонов
		Если ТекущееМестоРеализации.Свойство("coupon_s") Тогда
			ОбработатьКупоны_Протокол4(ЗаказОбъект, ТекущееМестоРеализации.coupon_s);
		КонецЕсли;
