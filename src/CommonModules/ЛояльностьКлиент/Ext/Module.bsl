...
Функция РасчетЧека_Протокол2(ЗаказОбъект, Тест)
	...
	Пока НЕ ЗакончитьРасчет Цикл
		...
		Если Чек.err <> 0 Тогда
			...
			Ответ = Неопределено;
			Ответ = ОбработкаОшибокЛОЛ(ЗаказОбъект, Чек.err, Чек.err_msg);
...
Функция ЗакрытьЗаказ(ЗаказОбъект, ПротоколСсылка=Неопределено) Экспорт
	...
	//ЛояльностьКлиентСервер.Логирование(1, "закрытие заказа", "начало ("+ЗаказСсылка+")");
	//
	//Лояльность.ЗапуститьЗакрытиеЗаказа(ЗаказСсылка, ПротоколСсылка);	
	//
	//ЛояльностьКлиентСервер.Логирование(1, "закрытие заказа", "конец ("+ЗаказСсылка+")");
	...
	Пока НЕ ЗакончитьРасчет Цикл
		...
		Если НЕ ОтправкаУспешна Тогда
			...
			Ответ = Неопределено;
			//Ответ = ВыводОкнаЛОЛ(ТекстЗаголовка, Текст, "ОК", "Повторить запрос", ТекстИнформационноеПоле);
			//Ответ = ВыводОкнаЛОЛ(ТекстЗаголовка, Текст, "ОК",, ТекстИнформационноеПоле);
...
Процедура ПрименитьЛояльность_КП(Обработка, ЕстьДеньРожденияВЗаказе) Экспорт
	...
	Если НЕ ОшибкаЛояльности Тогда
		...
		Если ОтветСервера.Ошибка Тогда
			...
			ЗаписьЖурналаРегистрации("Лояльность.Ошибка", УровеньЖурналаРегистрации.Ошибка, , Обработка.Заказ.Ссылка, ОписаниеОшибки);
	...
	Если ОшибкаЛояльности Тогда
		...
		Если Ответ = "НАЧИСЛИТЬ ПОЗЖЕ" Тогда //Если ответ = "НЕТ" тогда
			...
			ЗаписьЖурналаРегистрации("Лояльность.Обмен", УровеньЖурналаРегистрации.Информация, , Обработка.Заказ.Ссылка, "Выбрана оплата без лояльности");
		иначе
	...
	Если ответ= "СПИСАТЬ" ИЛИ ответ= "НАЧИСЛИТЬ" Тогда
		...
		Для каждого ТекущаяСтрока Из СтрокиЗаказа Цикл
			...
			Иначе
				...
				ЗаписьЖурналаРегистрации("Лояльность.Ошибка", УровеньЖурналаРегистрации.Ошибка, , Обработка.Заказ.Ссылка, "Некорректный ответ лояльности");
...
Функция ПолучитьДанныеЛояльностиПоЗаказу(ЗаказОбъект, ВерсияДанных=Неопределено) Экспорт
	...
	Возврат Лояльность.ПолучитьПревьюЗаказа(ЗаказОбъект.Ссылка, ВерсияДанных);
	
КонецФункции

// Идентификатор - УникальныйИдентификатор формы (чтобы при закрытии данные из хранилиза удалились)
Функция ПолучитьДанныеЛояльностиПоЗаказу_Асинхронно(ЗаказОбъект, ВерсияДанных, Идентификатор) Экспорт
...
Функция РасчетЧека_УДАЛИТЬ(ТипЗапроса = "SALE", ЗаказОбъект, Станция=Неопределено) Экспорт

	Если Станция = ПредопределенноеЗначение("Перечисление.ТипыРМ.Автокасса") Тогда
		Возврат РасчетЧека_Автокасса_УДАЛИТЬ(ТипЗапроса, ЗаказОбъект);
	Иначе
		Возврат РасчетЧека_Протокол3_УДАЛИТЬ(ТипЗапроса, ЗаказОбъект);
	КонецЕсли;
	
КонецФункции

Функция РасчетЧека_Автокасса_УДАЛИТЬ(ТипЗапроса = "PRESALE", ЗаказОбъект)
	...
	Попытка
		...
		Если Не ЗначениеЗаполнено(ЗаказОбъект.РабочееМесто) Тогда
			Тест = Истина;
		Иначе
			Тест = ЗаказОбъект.РабочееМесто.Тест;
		КонецЕсли;
		
		ОтветСервера = Лояльность.ОтправитьЧекНаОбработку_Протокол3(ПараметрыЗапроса.ТекстЗапроса, РежимОтправки, Тест);
		ОтправкаУспешна = НЕ ОтветСервера.Ошибка;
	Исключение
	КонецПопытки;
...
Функция РасчетЧека_Протокол3_УДАЛИТЬ(ТипЗапроса = "PRESALE", ЗаказОбъект)
	...
	Пока НЕ ЗакончитьРасчет Цикл
		...
		Если ДанныеОтвета.err <> 0 Тогда
			...
			Ответ = Неопределено;
			Ответ = ОбработкаОшибокЛОЛ(ЗаказОбъект, ДанныеОтвета.err, ДанныеОтвета.errmsg);
...
Функция РасчетЧека_Протокол2_УДАЛИТЬ(ТипЗапроса = "SALE", ЗаказОбъект)
	
	//ТипЗапроса = "PRESALE";
	
	//Возврат РасчетЧека_Протокол3(ТипЗапроса, ЗаказОбъект);
	...
	Пока НЕ ЗакончитьРасчет Цикл
		...
		ПараметрыЗапроса = ЛояльностьКлиентСервер.СформироватьJSONПоЗаказу_Протокол2(ТипЗапроса, ЗаказОбъект, ЗаказОбъект.ТипЧекаЛояльность, СтруктураАСКов);
		Если ПараметрыЗапроса.Ошибка Тогда
			ЛояльностьКлиентСервер.Логирование(1, "расчет", "ошибка формирования JSON("+ПараметрыЗапроса.ТекстОшибки+")");
			ОбработкаОшибокЛОЛ(ЗаказОбъект, 1, "Нет связи с принтером кассы");
			Возврат ВозвратВЧек(СтруктураВозврата);
		...
		Попытка
			ЛояльностьКлиентСервер.Логирование(1, "расчет", "отправка запроса (Режим="+?(РежимОтправки=0,"центр","локальный")+",текст="+ПараметрыЗапроса.ТекстЗапроса+")");
			ОтветСервера = Лояльность.ОтправитьЧекНаОбработку(ПараметрыЗапроса.ТекстЗапроса, РежимОтправки, ЗаказОбъект.РабочееМесто.Тест);
			ОтправкаУспешна = НЕ ОтветСервера.Ошибка;
		Исключение
		КонецПопытки;
		...
		Если Чек.chek_type = 0 Тогда
			...
			Если Чек.chek_s_kgl_rem > 0 Тогда
				...
				Если		Ответ = "Списать" Тогда
					...
					ЛояльностьКлиентСервер.Логирование(1, "расчет", "ОбработатьСписание");
					Если НЕ ОбработатьСписание(ЗаказОбъект, ,Чек) Тогда
						Возврат ВозвратВЧек(СтруктураВозврата);
					КонецЕсли;
					ЗакончитьРасчет = Истина;
				ИначеЕсли	Ответ = "Начислить" Тогда
					ЛояльностьКлиентСервер.Логирование(1, "расчет", "ОбработатьНачисление");
					Если НЕ ОбработатьНачисление(ЗаказОбъект, ,Чек) Тогда
						Возврат ВозвратВЧек(СтруктураВозврата);
					КонецЕсли;
					ЗакончитьРасчет = Истина;
				Иначе
...
Функция ОбработатьНачисление_УДАЛИТЬ(ЗаказОбъект, ДанныеЗаказа) Экспорт
	
	Возврат ОбработатьНачисление_Протокол3(ЗаказОбъект, ДанныеЗаказа);
	
КонецФункции

Функция ОбработатьНачисление_Протокол3_УДАЛИТЬ(ЗаказОбъект, ДанныеЗаказа)
...
Функция ОбработатьНачисление_Протокол2_УДАЛИТЬ(ЗаказОбъект, ДанныеЧека)
...
Функция ОбработатьСписание_УДАЛИТЬ(ЗаказОбъект, ДанныеЗаказа) Экспорт
	
	Возврат ОбработатьСписание_Протокол3(ЗаказОбъект, ДанныеЗаказа);
	
КонецФункции

Функция ОбработатьСписание_Протокол3_УДАЛИТЬ(ЗаказОбъект, ДанныеЗаказа)
...
Функция ОбработатьСписание_Протокол2_УДАЛИТЬ(ЗаказОбъект, ДанныеЧека)
...
Процедура СформироватьПодвалНачисления_Протокол2_УДАЛИТЬ(ЗаказОбъект, ДанныеЧека)
	ЗаказОбъект.ПодвалЧека = "";
...
Процедура СформироватьПодвалНачисления_Протокол3_УДАЛИТЬ(ЗаказОбъект, ДанныеЗаказа)
	ЗаказОбъект.ПодвалЧека = "";
...
Процедура СформироватьПодвалСписания_Протокол2_УДАЛИТЬ(ЗаказОбъект, ДанныеЧека)
	ЗаказОбъект.ПодвалЧека = "";
...
Процедура СформироватьПодвалСписания_Протокол3_УДАЛИТЬ(ЗаказОбъект, ДанныеЗаказа)
	ЗаказОбъект.ПодвалЧека = "";
...
Функция ЗаполнитьТаблицуТоваров_Протокол3_УДАЛИТЬ(ЗаказОбъект, МассивТоваров)
	Успех = Истина;
...
Функция ЗаполнитьТаблицуТоваров_Протокол2_УДАЛИТЬ(ЗаказОбъект, МассивТоваров)
	Успех = Истина;
