...
Функция РасчетЧека_Протокол5(ЗаказОбъект, ПротоколОбъект, Тест)
		...
		//Если ДанныеОтвета.request = "PRESALE" Тогда
			...
			Если Заказ.s_kgl_rem > 0 Тогда
				...
				Если		Ответ = "Списать" Тогда
					...
					Если НЕ ОбработатьСписание_Протокол5(ЗаказОбъект, ПротоколОбъект, Заказ) Тогда
						Возврат ВозвратВЧек(СтруктураВозврата);
					КонецЕсли;
				...
				ИначеЕсли	Ответ = "Начислить" Тогда
					...
					Если НЕ ОбработатьНачисление_Протокол5(ЗаказОбъект, ПротоколОбъект, Заказ) Тогда
						Возврат ВозвратВЧек(СтруктураВозврата);
					КонецЕсли;
					ЗакончитьРасчет = Истина;
				Иначе
	...
	Иначе
		...
		Если ПротоколОбъект.ТипЧекаЛояльность = 0 Тогда
			ОбработатьНачисление_Протокол5(ЗаказОбъект, ПротоколОбъект, Заказ);
		КонецЕсли;
...
Функция ОбработатьНачисление(ЗаказОбъект, ПротоколОбъект, ДанныеЗаказа, МестоРеализации=Неопределено) Экспорт
	...
	ИначеЕсли ВерсияПротокола = 5 Тогда
		...
		Возврат ОбработатьНачисление_Протокол5(ЗаказОбъект, ПротоколОбъект, ДанныеЗаказа);//???, МестоРеализации);
	КонецЕсли;
...
Функция ОбработатьНачисление_Протокол5(ЗаказОбъект, ПротоколОбъект, ДанныеЗаказа, МестоРеализации = Неопределено)
	...
	Для каждого ТекущееМестоРеализации Из ДанныеЗаказа.sale_places Цикл
		...
		// Обработка купонов
		Если ТекущееМестоРеализации.Свойство("coupons_n") Тогда
			ОбработатьКупоны_Протокол5(ЗаказОбъект, ТекущееМестоРеализации.coupons_n);
		КонецЕсли;
...
Функция ОбработатьСписание(ЗаказОбъект, ПротоколОбъект, ДанныеЗаказа, МестоРеализации=Неопределено) Экспорт
	...
	ИначеЕсли ВерсияПротокола = 5 Тогда
		...
		Возврат ОбработатьСписание_Протокол5(ЗаказОбъект, ПротоколОбъект, ДанныеЗаказа);
	КонецЕсли;
...
Функция ОбработатьСписание_Протокол5(ЗаказОбъект, ПротоколОбъект, ДанныеЗаказа, МестоРеализации=Неопределено)
	...
	Для каждого ТекущееМестоРеализации Из ДанныеЗаказа.sale_places Цикл
		...
		// Обработка купонов
		Если ТекущееМестоРеализации.Свойство("coupons_s") Тогда
			ОбработатьКупоны_Протокол5(ЗаказОбъект, ТекущееМестоРеализации.coupons_s);
		КонецЕсли;
...
Процедура ОбработатьКупоны_Протокол5(ЗаказОбъект, МассивКупонов)
	Если МассивКупонов = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Для каждого ТекущаяСтрока Из МассивКупонов Цикл
		Если ТекущаяСтрока.status <> 2 Тогда
			Продолжить;
		КонецЕсли;
		СтрокаКупона = ЗаказОбъект.Купоны.Найти(ТекущаяСтрока.code, "Купон");
		Если СтрокаКупона = Неопределено Тогда
...
Функция ПолучитьДанныеЛояльностиПоЗаказу(ЗаказОбъект, ВерсияДанных=Неопределено) Экспорт
	...
	КоличествоСпецифик = ЗаказОбъект.Специфики.Количество();
	МассивОплаченных = Новый Массив; // ИД оплаченных строк товаров
	...
	Для каждого ТекущаяСтрока Из ЗаказОбъект.Товары Цикл
		Если ЗначениеЗаполнено(ТекущаяСтрока.ДокументОплаты) Тогда
			МассивОплаченных.Добавить(ТекущаяСтрока.НомерСтроки);
			Продолжить;
		КонецЕсли;
		...
		// Ищем строки специфик
		Если КоличествоСпецифик>0 Тогда
			НайденныеСтроки = ЛояльностьКлиентСервер.НайтиСтрокиВМассивеСтруктур(ПревьюЗаказа.СоставЗаказа, Новый Структура("ИДСтрокиТовара", ТекущаяСтрока.ИдСтроки));
			Для каждого ТекущаяСпецифика Из НайденныеСтроки Цикл
				ТекущаяСтрока.СуммаРеализации = ТекущаяСтрока.СуммаРеализации + ТекущаяСпецифика.СуммаФакт;
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
	
	// Обновим таблицу специфик
	Для каждого ТекущаяСтрока Из ЗаказОбъект.Специфики Цикл
		Если МассивОплаченных.Найти(ТекущаяСтрока.НомерСтрокиТовара)<>Неопределено Тогда
			Продолжить;
		КонецЕсли;
