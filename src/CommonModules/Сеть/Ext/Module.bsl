...
Функция КопироватьФайлУдаленно(Хост, Источник, Приемник) Экспорт
	
	Если СтрНайти(Хост, "serv") Тогда
		Пользователь = "Администратор ТТ";
		Пароль = "1";
	ИначеЕсли СтрНайти(Хост, "ost") Тогда
	...
	Команда = СтрШаблон("psexec \\%1 -u %2 -p %3 cmd /ccopy %4 %5", Хост, Пользователь, Пароль, Источник, Приемник);
...
Функция ИмяХостаРМ(РМ, ДобавитьСуффикс = Ложь) Экспорт
	...
	Совпадения = V8RegEx.Выполнить(ПрофильВхода);
	Если Совпадения.Количество Тогда
		ИмяКомпьютера = Совпадения[0].Подгруппы[0];
	КонецЕсли;
...
Функция ПодключитьШару(Хост, ЭтоСервер = Ложь, Пользователь = "", Пароль = "") Экспорт
	Если СтрНайти(НРег(Хост),НРег(ИмяКомпьютера())) Тогда
		Возврат Истина;
	КонецЕсли;
	
	Если не ХостДоступен(Хост) Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Команда = СтрШаблон("net use \\%1\c$ /user:""%2"" %3", Хост, Пользователь, Пароль);
	ир.ЗапуститьСкрытоеПриложениеИДождатьсяЗавершения(Команда, , , КраткоеИмяФайлаРезультата, Истина);
	ВыводКоманды = РаботаСФайлами.ИзвлечьТекст(КаталогВременныхФайлов()+КраткоеИмяФайлаРезультата, КодировкаТекста.OEM, 1);
	Если СтрНайти(ВыводКоманды, "успешно") Тогда //100% потерь
		Возврат Истина;
	Иначе
		...
		КонецЕсли;		
		Если Не Пароль = "F54eTylfm8" Тогда
			Возврат Сеть.ПодключитьШару(Хост, ЭтоСервер, "Администратор", "F54eTylfm8");					
		КонецЕсли;
		Возврат Ложь;
...
Функция ХостДоступен(Хост, Таймаут = 200) Экспорт
	ир = ирКэш.Получить(); //:ир = Обработки.ирПлатформа.Создать();
 
	Команда = СтрШаблон("ping %1 -4 -w %2 -n 1 -l 1", Хост, Формат(Таймаут, "ЧГ=0"));
	КраткоеИмяФайлаРезультата = Строка(Новый УникальныйИдентификатор);
	ир.ЗапуститьСкрытоеПриложениеИДождатьсяЗавершения(Команда, КаталогВременныхФайлов(), , КраткоеИмяФайлаРезультата, , );
	
	ВыводКоманды = РаботаСФайлами.ИзвлечьТекст(КаталогВременныхФайлов()+КраткоеИмяФайлаРезультата, КодировкаТекста.OEM, 1);
	Если СтрНайти(ВыводКоманды, "100%") Тогда //100% потерь
		Возврат Ложь;
	Иначе
		Возврат Истина;
	КонецЕсли;
...
Функция СкопироватьЛокальнуюКопиюСсервераТТнаРМ(ИнформационнаяБаза, РМ, Молча = Ложь) Экспорт
	//:ИнформационнаяБаза = Справочники.ИнформационныеБазы.ПустаяСсылка();
	ир = ирКэш.Получить();
	Команда = СтрШаблон("psexec  \\%1  -u ""Администратор ТТ"" -p 1 -c -f c:\sup_kkm_addins\copyLocalBaseToKassa.bat %2", ИнформационнаяБаза.СерверХост, Сеть.ИмяХостаРМ(РМ));
	Если Не Молча Тогда
		Сообщить(Команда);	
	КонецЕсли;            	
	ФайлРезультат = Строка(Новый УникальныйИдентификатор);
	ир.ЗапуститьСкрытоеПриложениеИДождатьсяЗавершения(Команда,,,ФайлРезультат);
	РезультатБатника = РаботаСФайлами.ИзвлечьТекст(КаталогВременныхФайлов() + ФайлРезультат,КодировкаТекста.OEM);
	Если Не Молча Тогда
		Сообщить(РезультатБатника);	
	КонецЕсли;         
	Возврат РезультатБатника;
КонецФункции
