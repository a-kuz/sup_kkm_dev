...
#Если ТонкийКлиент Тогда
	&НаСервере
#КонецЕсли
Процедура ЗагрузитьЖРвЖР() Экспорт
	ТЗ = Новый ТаблицаЗначений;
	Отбор = Новый Структура();
	
	ВыгрузитьЖурналРегистрации(ТЗ, Отбор, "Уровень,Дата,ИмяПользователя ,Компьютер,ПредставлениеСобытия,Комментарий,Данные,ПредставлениеПриложения,ПредставлениеДанных",,10000);
	Если ТЗ.Количество() Тогда
		НачДата = ТЗ[0].Дата;
		Для каждого Т Из ТЗ Цикл
			ЗаписатьСобытие(Т.ПредставлениеСобытия, Т.уровень,Т.комментарий, Т.Данные, Т.Компьютер, Т.имяПользователя, Т.ПредставлениеДанных,Т.дата);	
		КонецЦикла;
		КонДата = Т.Дата;
