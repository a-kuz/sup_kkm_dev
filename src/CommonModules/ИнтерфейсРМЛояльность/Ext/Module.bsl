...
Процедура ДействиеРецептыГостя(ГлавнаяФорма) Экспорт 
	...
	//получим текущий товар, определим можно ли для текущего товара сохранять рецепты 
	...
	Если ТекДанные.Товар.Владелец = Справочники.КаталогиТоваров.Мяснов или ТекДанные.Товар.Владелец = Справочники.КаталогиТоваров.Отдохни тогда
		ИнтерфейсРМ.ВопросПредупреждение("Любимые рецепты гостя INSPIRED", "По этому товару нельзя создать рецепт! Выберите другое блюдо inspired.", "", "", "Esc=OK", "");
		Возврат;
	КонецЕсли;
