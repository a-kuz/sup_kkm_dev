...
Процедура ОбновитьПлашки(Форма, ВерсияДанных = "", ДанныеЛояльности = Неопределено) Экспорт
	...
	Если ДанныеЛояльности = Неопределено И НЕ ЗаказСсылка.Пустая() Тогда
...
Процедура ПоказатьИнформациюОТоваре(Товар, ЛояльностьДанныеЗаказа = Неопределено, ИдСтроки = "", РежимПредпросмотра = Ложь) Экспорт 
	...
	РежимПредпросмотра = Обработка.ПолучитьОсновнуюФорму().ОткрытьМодально();
КонецПроцедуры
