...
Процедура ФормаПриОткрытии(Форма) Экспорт 
	...
	Если ТабличноеПоле.Колонки.Найти("ГруппаАкции") = Неопределено Тогда
		...
		НовКолонка.Ширина = 5;
//ЛогоАкций	НовКолонка.КартинкиСтрок = БиблиотекаКартинок.ЛоготипАкций;
	КонецЕсли;
	
КонецПроцедуры

Процедура тпТоварыПриПолученииДанных(Форма, Элемент, ОформленияСтрок) Экспорт
...
//ЛогоАкций	ОформлениеСтроки.Ячейки.ГруппаАкции.ОтображатьКартинку = Истина;
//ЛогоАкций	ОформлениеСтроки.Ячейки.ГруппаАкции.ОтображатьТекст = Ложь;
//ЛогоАкций	Попытка
//ЛогоАкций		ОформлениеСтроки.Ячейки.ГруппаАкции.ИндексКартинки = ТекГруппаАкции;
...
//ЛогоАкций	КонецПопытки; 
		...
		Для каждого ТекСтрока Из ДанныеЛояльности.СоставЗаказа Цикл
			Если ТекСтрока.ИдСтроки = ИдСтроки Тогда
				ОформлениеСтроки.Ячейки.КГЛНачислено.УстановитьТекст(Формат(ТекСтрока.КГЛНачислено,"ЧН=' '"));
				Прервать;
			КонецЕсли;
		КонецЦикла;
