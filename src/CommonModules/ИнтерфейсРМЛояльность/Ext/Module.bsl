...
Функция ПолучитьТабДокЛояльность_МониторГостя(Форма, ВерсияДанных = Неопределено, ДанныеЛояльности = Неопределено) Экспорт
	...
	// Приветствие
	ДанныеГостя = ЛояльностьКлиентСервер.ПолучитьДанныеГостя(НомерКартыЛояльности, глПараметрыРМ.Тест);
	ДоброПожаловать = "Добро пожаловать в КуулКЛЕВЕР Гурмэ!";
	Если НЕ ДанныеГостя.Ошибка  Тогда
		Если НЕ ПустаяСтрока(ДанныеГостя.ФИО) Тогда 
			Имя = СтрРазделить(ДанныеГостя.ФИО, " ")[0];
			ДоброПожаловать = Имя + ", добро пожаловать в КуулКЛЕВЕР Гурмэ!";	
		КонецЕсли;
		НомерКартыЛояльности = СокрЛП(Формат(ДанныеГостя.НомерКарты, "ЧГ=0"));
	КонецЕсли;
	...
	Для каждого ТекСтрока Из ТекИтоги.КГЛИнфо Цикл
		ВывестиСтрокуИтогов(ТабДок, Макет, ВыведеноСтрок, ТекСтрока.Текст, ТекСтрока.КГЛНачислено);
	КонецЦикла;
	
	//Если ВыведеноСтрок < 7 Тогда
		Пока ВыведеноСтрок < 7 Цикл
...
Процедура ВывестиСтрокуИтогов(ТабДок, Макет, ВыведеноСтрок, Текст = "", КГЛ = 0, Итоги = Ложь)
