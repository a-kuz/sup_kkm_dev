...
Процедура ФормаПриОткрытии(Форма) Экспорт 
	...
	СоответствиеЗаголовков = ЛояльностьКлиентСерверПовтИсп.СоответствиеЗаголовковПлашек_Активная(глПараметрыРМ.МестоРеализации);
	Попытка
...
Процедура Плашка7Нажатие(Форма, Элемент)
	...
	Параметры.Вставить("ПроектФильтр", ОпределитьЗначениеОтбораПоПроекту(Форма));
	Параметры.Вставить("ЗаголовокГостя", "Все персональные предложения для Вас");
	Параметры.Вставить("ЗаголовокСтанции", "Персональные предложения гостя");
...
Процедура Плашка8Нажатие(Форма, Элемент)
	...
	Если глПараметрыРМ.МестоРеализации = ПредопределенноеЗначение("Справочник.МестаРеализации.Ресторан") Тогда
		...
		ОбработкаРецептыГостя.ГлавнаяФорма = Форма;
		
		РезультатДействия = ОбработкаРецептыГостя.ВыбратьВыполнитьДействие("ВыбратьРецептГостя");
...
Процедура Плашка12Нажатие(Форма, Элемент)
	...
	Параметры.Вставить("ПроектФильтр", ОпределитьЗначениеОтбораПоПроекту(Форма));
	Параметры.Вставить("ЗаголовокГостя", "Акции для Вас");
	Параметры.Вставить("ЗаголовокСтанции", "Акции");
	КодКупона = ОткрытьФормуМодально("Обработка.ОбработкаЛояльности.Форма.СписокАкцийИПП", Параметры, Форма);
...
#Область ОбновлениеИнтерфейсаГлавнойФормы
...
Процедура ОбновитьНадписьПриветствие(Форма, ВерсияДанных = "") Экспорт 
	...
	Иначе	
		...
		Если ЗначениеЗаполнено(НомерКартыЛояльности) Тогда
			Заголовок = СтрШаблон("Здравствуйте, %1!
	...
	Форма.ЭлементыФормы.НадписьЛояльность1.Заголовок = Заголовок;
...
Процедура ОбновитьКупоны(Форма, ВерсияДанных = "", ДанныеЛояльности = Неопределено) Экспорт 
	...
	ПроектФильтр = ОпределитьЗначениеОтбораПоПроекту(Форма);
...
Процедура ОбновитьПлашки(Форма, ВерсияДанных = "", ДанныеЛояльности = Неопределено) Экспорт
	...
	КонецЕсли;
	Если НЕ ДанныеЛояльности.Луч = Истина И НЕ ПроектФильтр = 3 Тогда
		//ПереключитьАктивностьКнопки(Форма, 8, Ложь, Истина);
		ПереключитьАктивностьКнопки(Форма, 8, Ложь);
	КонецЕсли;
	Если ПроектФильтр Тогда
		Форма.ЭлементыФормы.Плашка9.Заголовок = "Расчет КеГЛей " + Символы.ПС + ЛояльностьКлиентСерверПовтИсп.МестоРеализацииПоПроекту(ПроектФильтр, Истина);
	Иначе
		Форма.ЭлементыФормы.Плашка9.Заголовок = "Расчет
		|КеГЛей";
	КонецЕсли;
...
Процедура ОбновитьТабДокЛояльность(Форма, ВерсияДанных = "", ДанныеЛояльности = Неопределено) Экспорт 
	...
	Попытка
		...
		ОбластьСтрокаПороги = Макет.ПолучитьОбласть("СтрокаПороги");
	...
	КонецЦикла;
	Для каждого ТекСтрока Из ТекИтоги.ТекстИнфо Цикл
		ОбластьСтрокаПороги.Параметры.Заполнить(ТекСтрока);
		ТабДок.Вывести(ОбластьСтрокаПороги);
	КонецЦикла;
...
Процедура ПереключитьАктивностьКнопки(Форма, Инд, Активность = Истина, Доступность = Неопределено)
	...
	Если Активность И Доступность Тогда
		ТекЭлемент.Заголовок = ЛояльностьКлиентСерверПовтИсп.СоответствиеЗаголовковПлашек_Активная(глПараметрыРМ.МестоРеализации)[Инд];
	...
	ИначеЕсли Доступность Тогда	
		ТекЭлемент.Заголовок = ЛояльностьКлиентСерверПовтИсп.СоответствиеЗаголовковПлашек_Неактивная(глПараметрыРМ.МестоРеализации)[Инд];
...
Функция ОпределитьЗначениеОтбораПоПроекту(Форма)
	...
	Исключение
		ОтборПоФирме = Ложь;
	КонецПопытки;
	Если ОтборПоФирме = Ложь Тогда
		Попытка
			ОтборПоФирме = ЗначениеЗаполнено(Форма.ОбработкаОбъект.ДопПараметрыИнфо.ФирмаФильтр);
		Исключение
		КонецПопытки;
	КонецЕсли;
	
	Возврат ?(ОтборПоФирме, ЛояльностьКлиентСерверПовтИсп.ПроектПоМестуРеализации(глПараметрыРМ.Фирма.МестоРеализации), 0);
...
#Область ОтображениеИнформацииНаМонитореГостя
...
Функция ПолучитьТабДокЛояльность_МониторГостя(ЗаказСсылка, ВерсияДанных = "", ДанныеЛояльности = Неопределено, Фирма = Неопределено) Экспорт
	...
	КонецЕсли;
	ФирмаКратко = ЛояльностьКлиентСерверПовтИсп.МестоРеализацииПоПроекту(ПроектФильтр, Истина, 3);
	...
	ОблМакета.Параметры.Текст = ДанныеЛУЧ_Inspired(ЕстьЛуч, НомерКартыЛояльности, ФирмаКратко);
	ТабДок.Вывести(ОблМакета);
	...
	Для каждого ТекСтрока Из ТекИтоги.КГЛИнфо Цикл
		ВывестиСтрокуИтогов(ТабДок, Макет, ВыведеноСтрок, ТекСтрока.ТекстГостя, ТекСтрока.КГЛ);
	КонецЦикла;
	Для каждого ТекСтрока Из ТекИтоги.ТекстИнфо Цикл
		ВывестиСтрокуИтогов(ТабДок, Макет, ВыведеноСтрок, ТекСтрока.ТекстГостя, Неопределено);
	КонецЦикла;
...
Процедура ВывестиСтрокуИтогов(ТабДок, Макет, ВыведеноСтрок, Знач Текст = "", КГЛ = 0, Итоги = Ложь)
	...
	СтруктураПараметров = Новый Структура("ТекстГостя,КГЛ", Текст, КГЛ);
	Если Итоги Тогда
		ИмяОбласти = "ИтогиМР";
	ИначеЕсли КГЛ = Неопределено Тогда	
		ИмяОбласти = "ИтогиПороги";
...
// Заполняет колонки КГЛ и ГруппаАкции в строке дерева товаров, отправляемого на монитор гостя
Процедура ЗаполнитьДанныеЛояльностиВСтроке_МониторГостя(СтрокаДереваТоваров, ЛояльностьДанныеЗаказа) Экспорт
	
	Если НЕ ТипЗнч(ЛояльностьДанныеЗаказа) = Тип ("Структура") ИЛИ ЛояльностьДанныеЗаказа.Пустой ИЛИ ЛояльностьДанныеЗаказа.Ошибка Тогда
		Возврат;
	КонецЕсли;
	
	ТаблицаСоставЗаказа = МассивСтруктурВТаблицуЗначений(ЛояльностьДанныеЗаказа.СоставЗаказа);
	Если ТаблицаСоставЗаказа = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	СтрокаТовара = ТаблицаСоставЗаказа.Найти(СтрокаДереваТоваров.ИдСтроки, "ИдСтроки");
	Если НЕ СтрокаТовара = Неопределено Тогда
		СтрокаДереваТоваров.ГруппаАкции = СтрокаТовара.ГруппаАкции;
		
		КГЛНачислено = СтрокаТовара.КГЛНачислено;
		СпецификиСтроки = ТаблицаСоставЗаказа.НайтиСтроки(Новый Структура("Специфика,ИДСтрокиТовара", 1, СтрокаДереваТоваров.ИдСтроки));
		Для каждого ТекСтрокаСпецифики Из СпецификиСтроки Цикл
			КГЛНачислено = КГЛНачислено + ТекСтрокаСпецифики.КГЛНачислено;
		КонецЦикла;
		
		СтрокаДереваТоваров.КГЛ = (Формат(КГЛНачислено,"ЧН=' '"));
	КонецЕсли;
...
Процедура ДействиеРецептыГостя(ГлавнаяФорма) Экспорт 
	...
	Если НЕ ЗначениеЗаполнено(НомерКартыЛояльности) Тогда
		ИнтерфейсРМ.ВопросПредупреждение("Любимые рецепты гостя INSPIRED", "Возможность сохранять любимые рецепты доступна только владельцам карты лояльности."
				+Символы.ПС+" Чтобы сохранить любимый рецепт, привяжите карту лояльности к бейджу гостя", "", "", "Esc=OK", "");
		Возврат;
	КонецЕсли;
	...
	ИначеЕсли РезультатДействия.Свойство("РедактироватьРецептГостя") И ЗначениеЗаполнено(РезультатДействия.РедактироватьРецептГостя) Тогда
		...
		Если Результат.Ошибка Тогда
			ИнтерфейсРМ.ВопросПредупреждение("Любимые рецепты гостя INSPIRED", Результат.Текст1, "", "", "Esc=OK", "");
		Иначе
			Ответ = ИнтерфейсРМ.ВопросПредупреждение("Любимые рецепты гостя INSPIRED", Результат.Текст1, Результат.Текст2, "Да", "", "Esc=Нет");
			Если Ответ = "Да" Тогда
				СтруктураВыбораРецепта = Новый Структура;
	...
	ИначеЕсли РезультатДействия.Свойство("СохранитьРецептГостя") И ЗначениеЗаполнено(РезультатДействия.СохранитьРецептГостя) Тогда
		...
		ИнтерфейсРМ.ВопросПредупреждение("Любимые рецепты гостя INSPIRED", РезультатДействия.СохранитьРецептГостя, "", "", "Esc=OK", "");
		
	Иначе
		
		ГлавнаяФорма.ОбработкаОбъект.ПоказатьЗаказНаМонитореГостя();
		
	КонецЕсли;
	
КонецПроцедуры

Функция ДанныеЛУЧ_Inspired(ЕстьЛУЧ, НомерКартыЛояльности, ФирмаКратко = "")
	...
	Если глПараметрыРМ.МестоРеализации = ПредопределенноеЗначение("Справочник.МестаРеализации.Ресторан") Тогда
		...
		Если НЕ ЗначениеЗаполнено(НомерКартыЛояльности) Тогда
			ЕстьЛУЧ = Ложь;
			Результат = "Привяжите карту КеГеЛьБУМ к Вашему бейджу, и Вы сможете сохранять понравившиеся рецепты!";
		КонецЕсли;
		
		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("ТТ", глПараметрыРМ.ИнформационнаяБаза);
		Запрос.УстановитьПараметр("НомерКартыЛояльности", НомерКартыЛояльности);
		
		Запрос.Текст = 
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	РецептГостя.Ссылка КАК Ссылка
		|ИЗ
		|	Документ.РецептГостя КАК РецептГостя
		|ГДЕ
		|	РецептГостя.ТТ = &ТТ
		|	И РецептГостя.НомерКартыЛояльности = &НомерКартыЛояльности
		|	И РецептГостя.Проведен";
		
		ЕстьЛУЧ = НЕ Запрос.Выполнить().Пустой();
		Если ЕстьЛУЧ Тогда
			Результат = "У Вас есть любимые рецепты inspired. Попросите шефа станции приготовить по Вашему рецепту.";
		Иначе
			Результат = "Создавайте блюда на свой вкус и сохраняйте рецепты! Мы повторим понравившиеся в следующий раз!";
		КонецЕсли;
		
	ИначеЕсли ЕстьЛуч = Истина Тогда
		Результат = СтрШаблон("Вы - ЛУЧший покупатель %1. 
			|Попросите продавца показать список Ваших любимых продуктов.",
			ФирмаКратко);
	Иначе
		Результат = "Хотите в 5 раз больше КеГЛей на любимые продукты? Подключите программу ЛУЧший покупатель.";
	КонецЕсли;
