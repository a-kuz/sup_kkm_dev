...
Процедура ОбновитьПлашки(Форма, ВерсияДанных = Неопределено, ДанныеЛояльности = Неопределено) Экспорт
	...
	Если ДанныеЛояльности = Неопределено ИЛИ ДанныеЛояльности.Пустой ИЛИ ДанныеЛояльности.Ошибка Тогда
		Для Инд = 7 По 12 Цикл
...
//
Функция ПолучитьТабДокЛояльность_МониторГостя(ЗаказСсылка, ВерсияДанных = Неопределено, ДанныеЛояльности = Неопределено, Фирма = Неопределено) Экспорт
	...
	Если НЕ ДанныеГостя.Ошибка  Тогда
		...
		НомерКартыЛояльности = СокрЛП(Формат(ДанныеГостя.НомерКарты, "ЧГ=0"));
	КонецЕсли;
	ОблМакета = Макет.ПолучитьОбласть("Шапка|ВидимаяОбласть");
	...
	// ЛУЧ
	ОблМакета = Макет.ПолучитьОбласть("ЛУЧ|ВидимаяОбласть");
	Если ЕстьЛуч = Истина Тогда
		ОблМакета.Параметры.Текст = 
			...
			ФирмаКратко);
	Иначе
		ОблМакета.Параметры.Текст = "Хотите получать в 5 раз больше КеГЛей на любимые продукты?  Подключите программу ЛУЧший покупатель.";
	КонецЕсли;
	ТабДок.Вывести(ОблМакета);
	
	// Итоги
	ОблМакета = Макет.ПолучитьОбласть("ИтогиШапка|ВидимаяОбласть");
...
//1102	КонецЦикла;
	...
	Если ПустаяСтрока(НомерКартыЛояльности) Тогда
		ОблМакета = Макет.ПолучитьОбласть("ПодвалБезКарты|ВидимаяОбласть");
	Иначе
		ОблМакета = Макет.ПолучитьОбласть("ПодвалСКартой|ВидимаяОбласть");
	КонецЕсли;
	ТабДок.Вывести(ОблМакета);
