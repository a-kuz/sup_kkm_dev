...
Функция ПолучитьТабДокЛояльность_МониторГостя(ЗаказСсылка, ВерсияДанных = Неопределено, ДанныеЛояльности = Неопределено) Экспорт
	...
	ОблМакета.Параметры.Текст = "Вы - ЛУЧший покупатель МясновЪ. Напомнить список подключенных любимых продуктов? Спросите у продавца!";
	ТабДок.Вывести(ОблМакета);
...
Процедура ДействиеРецептыГостя(ГлавнаяФорма) Экспорт 
	...
	Если РезультатДействия.Свойство("ВыбратьРецептГостя") И ЗначениеЗаполнено(РезультатДействия.ВыбратьРецептГостя) Тогда
		...
		СохранитьЗначение("ВыбранныйРецептГостя", РецептСсылка);
		ПодключитьОбработчикОжидания("ИнтерфейсРМЛояльность.Оповестить_Ожидание", 0.5, Истина);
		//ДополнительныеПараметры = Новый Структура("РецептСсылка", РецептСсылка);
		//Оповещение = Новый ОписаниеОповещения("Оповестить_Ожидание", ИнтерфейсРМЛояльность, ДополнительныеПараметры);
...
Процедура Оповестить_Ожидание() Экспорт //Результат, ДополнительныеПараметры) Экспорт 
	
	ФормаОткрыта = Ложь;
	Для каждого ТекОкно Из глСтекОкон Цикл
		Если ТипЗнч(ТекОкно.Форма.ОбработкаОбъект) = Тип("ОбработкаОбъект.ВыборСпецифик") Тогда
			ФормаОткрыта = Истина;
			Прервать;
		КонецЕсли;		
	КонецЦикла;
	...
	Если ФормаОткрыта Тогда
		РецептСсылка = ВосстановитьЗначение("ВыбранныйРецептГостя");
		Оповестить("ВыбратьРецептГостя", РецептСсылка);
		//Оповестить("ВыбратьРецептГостя", ДополнительныеПараметры.РецептСсылка);
	Иначе
		ПодключитьОбработчикОжидания("ИнтерфейсРМЛояльность.Оповестить_Ожидание", 0.5, Истина);
		//Оповещение = Новый ОписаниеОповещения("Оповестить_Ожидание", ИнтерфейсРМЛояльность, ДополнительныеПараметры);
		//ВыполнитьОбработкуОповещения(Оповещение);
