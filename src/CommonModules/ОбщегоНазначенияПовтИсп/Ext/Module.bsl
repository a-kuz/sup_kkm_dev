...
Функция СвойстваЗаказДопИнф() Экспорт
	Запрос = Новый Запрос("Выбрать * Из РегистрСведений.ЗаказДопИнф Где 1=0");
	Возврат СтрокаТаблицыЗначенийВСтруктуру(Запрос.Выполнить().Выгрузить().Добавить());
КонецФункции

Функция СвойстваЗаказТоварыДопИнф() Экспорт
	Запрос = Новый Запрос("Выбрать * Из РегистрСведений.ЗаказТоварыДопИнф Где 1=0");
	Возврат СтрокаТаблицыЗначенийВСтруктуру(Запрос.Выполнить().Выгрузить().Добавить());
КонецФункции

Функция ЭтоТоварМяснов(Товар) Экспорт
	//:Товар = Справочники.Товары.ПустаяСсылка();
	Возврат Товар.Владелец.МестоРеализации = Справочники.МестаРеализации.Мяснов;
КонецФункции
