...
Функция ПродолжитьЗапускРабочегоМеста(Тестирование = Ложь) Экспорт	
	...
	ИначеЕсли ТекущееРМ.Тип = Перечисления.ТипыРМ.СтанцияОплатыМОКП Тогда
		ГлавнаяФорма = Обработки.ГлавнаяФормаМОКП.Создать();
	...
	ЭкранШирина = ИОЭ[0].Ширина;
	ЭкранВысота = ИОЭ[0].Высота;
	глПараметрыРМ.Вставить("ЭкранШирина", ЭкранШирина);
	глПараметрыРМ.Вставить("ЭкранВысота", ЭкранВысота);
	глПараметрыРМ.Вставить("КоличествоМониторов", ИОЭ.Количество());
	...
	ИначеЕсли  глПараметрыРМ.Тип = Перечисления.ТипыРМ.СтанцияПовараМОКП Тогда
		ИмяФормы = "ФормаПовара";		
...
Процедура ОбрезкаОкна(Заголовок, Ширина=0) Экспорт 
	Попытка
	...
	//Если глПараметрыРМ.ТениОкон ИЛИ глПараметрыРМ.ОбрезкаОкон Тогда
		...
		Попытка
			...
			Если Ширина <> 0 Тогда
				Скрипт = СтрЗаменить(Скрипт, ";ширина ","");
			КонецЕсли;
...
Функция ВопросПредупреждение(ЗаголовокФормы, Текст1, Текст2, Кнопка1, Кнопка2="", Кнопка3="", 
	...
	Иначе
		 ЭлементыФормы.Текст2.ГоризонтальноеПоложение = ГоризонтальноеПоложение.Центр;
	КонецЕсли;
	//Z+
	...
	Для НомКнопки=1 По 3 Цикл
		...
		Если ПустаяСтрока(ТекстКнопки) Тогда
			...
			Если ЭлементыФормы.Найти("Кнопка"+НомКнопки+"Рамка")<>Неопределено Тогда
				ЭлементыФормы["Кнопка"+НомКнопки+"Рамка"].Видимость = Ложь;
			КонецЕсли;                                                     			
		Иначе
	...
	Если ДанныеДляЖР = Неопределено Тогда
		глОбработки = глОбработки;
		Если ТипЗнч(глОбработки) = Тип("Структура") Тогда
			Если глОбработки.Свойство("Заказ") Тогда
			ИначеЕсли глОбработки.Свойство("ГлавнаяФорма") Тогда
				Попытка
					ДанныеДляЖР = глОбработки.ГлавнаяФорма.ТекущийДокумент.Ссылка;
				Исключение
				КонецПопытки;				
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	//Если ДанныеДляЖР <> Неопределено Тогда
		ЗарегистрироватьСобытие("Вопрос."+Текст1,,,ДанныеДляЖР,Текст2 + Символы.ПС + "Ответ: " + Ответ);
	//КонецЕсли;
...
Функция ИдентификацияПользователя(ЗапретБыстрогоВыбора = Ложь) Экспорт
	...
	Иначе
		...
		Иначе
			ЗарегистрироватьСотрудникаВпоследних(Пользователь);
			Возврат Пользователь;
			
		КонецЕсли;
		
		ФлагПовтора = ИнтерфейсРМ.ВопросПредупреждение("Ошибка",Текст1,Текст2,"","OK","") = "Повтор";
		ФлагПовтора = Истина;
...
Функция ЗарегистрироватьСотрудникаВпоследних(Сотрудник) Экспорт
	
	Если глПараметрыРМ.Свойство("Тест") Тогда
		Тест = глПараметрыРМ.Тест;
	Иначе
		Тест = глРабочееМесто.Тест;
	КонецЕсли;
	
	Если Не Тест Тогда
		Если СтрНайти(нрег(Сотрудник),"администра") Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
	//:глПоследниеСотрудники = Новый Массив
	БыстраяАвторизация = Ложь;
	глПараметрыРМ.Свойство("БыстраяАвторизация", БыстраяАвторизация);
	Если БыстраяАвторизация <> Истина Тогда
		Возврат Ложь;
	КонецЕсли;
...
//Функция ИдентификацияКлиента(КодКлиента=Неопределено, ЗапретПустого=Ложь, БезПодтверждения=Ложь) Экспорт
	...
	Если КодКлиента=Неопределено Тогда
		...
		КодКлиента = ВводЧисла("Бейдж гостя", "Число",4,,,,"Клиент");
...
Процедура ЗаписьСобытия(СобытиеСсылка, Данные=Неопределено, Парам1="", Парам2="", Парам3="", Парам4="", Парам5="") Экспорт
	...
	ЗарегистрироватьСобытие(Событие.Событие, Событие.Уровень, ОбъектМетаданных, Данные, Комментарий);
...
//	Тип.Вид		- описание
//
Функция ТекущаяСменаЗапрос(МестоРеализации = Неопределено) Экспорт
...
Функция ВыводНаДП(Знач Действие, Знач Текст1="", Сумма1=Неопределено, Знач Текст2="", Сумма2=Неопределено ) Экспорт
	...
	Исключение
		ЗарегистрироватьСобытие("Ошибка выполнения",УровеньЖурналаРегистрации.Ошибка,,,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())); // иногда ошибка доступа к файлу
		Возврат Ложь;
	КонецПопытки;
...
Функция ПопыткаДействияСОбъектом(Объект, Действие, ФоновыйРежим = Истина, Знач Попыток = 3, ОбъектДляЖурнала = Неопределено) Экспорт
	...
	Пока ПовторДействия Цикл
		...
		Исключение
			...
			ЗарегистрироватьСобытие("Ошибка выполнения. " + Действие, УровеньЖурналаРегистрации.Ошибка, , ОбъектДляЖурнала, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));			
			Результат = Ложь;
...
Функция ЗаблокироватьОбъект(Объект, КемЗаблокирован="") Экспорт
	...
	Попытка
		...
		Если ТипЗнч(Объект) = Тип("ДокументОбъект.Заказ") Тогда
			...
			ЗарегистрироватьСобытие("Заказ.Блокировка по фирме", УровеньЖурналаРегистрации.Информация, , Объект.Ссылка, СтрШаблон("Фирма=%1;Заблокирован=%2;Успех=%3", глПараметрыРМ.Фирма, КемЗаблокирован, РезультатБлокировки.Успех));			
			
			Возврат РезультатБлокировки.Успех;
		КонецЕсли;
	Исключение
		ЗарегистрироватьСобытие("Заказ.Блокировка по фирме", УровеньЖурналаРегистрации.Ошибка, , Объект.Ссылка, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));			
	КонецПопытки;
	...
	Попытка
		...
		ЗарегистрироватьСобытие("Заказ.Блокировка", УровеньЖурналаРегистрации.Информация, , Объект.Ссылка, "Успех=Да");			
	...
	Исключение
		...
		ЗарегистрироватьСобытие("Заказ.Блокировка", УровеньЖурналаРегистрации.Информация, , Объект.Ссылка, СтрШаблон("Заблокирован=%1;Успех=Нет", КемЗаблокирован));			
		Результат = Ложь;
...
Процедура ОбработкаОчередиСобытий(РабочееМесто, ОчиститьОчередь=Ложь) Экспорт
	...
	Если ВыборкаСобытий.Следующий() Тогда
		...
		ИначеЕсли ТипСобытия = Перечисления.ТипыСобытий.ТребованиеСбросаЗаказа Тогда	
			
			// Оповестить РМ о изменениях в марках
			Если ЗначениеЗаполнено(глРабочееМесто) Тогда
				//ТекущаяФорма = глСтекОкон[0];
				Оповестить("СброситьЗаказ", ВыборкаСобытий.Описание);
			КонецЕсли;
...
Функция ЗаполнитьПодвал(ТабПоле, ЗаказОбъект,Макет = Неопределено) Экспорт
	...
	ФирмыРМ = ОбщегоНазначенияПовтИсп.ФирмыРабочегоМеста();
	...
	ВсеФирмы = ОбщегоНазначенияПовтИсп.ФирмыИнформационнойБазы();
	Для Каждого Фирма Из ВсеФирмы Цикл
		...
		СтрокаФирма.ЭтоНашаФирма = ФирмыРМ.Найти(Фирма) <> Неопределено;
	...
	Для Каждого стФирма Из СуммыПоФирмам Цикл
		...
		ОбластьСтрока.Параметры.Фирма = ?(стФирма.Фирма.МестоРеализации = ПредопределенноеЗначение("Справочник.МестаРеализации.Ресторан"), "Кухня Полли", стФирма.Фирма.МестоРеализации);
		Если Не стФирма.ЭтоНашаФирма Тогда
			Если ЗначениеЗаполнено(стФирма.КОплате) Тогда
				кОплатеПоДругимФирмам=кОплатеПоДругимФирмам+стФирма.КОплате;
				ОбластьСтрока.Параметры.кОплатеПоДругимФирмам = "+ " + Формат(кОплатеПоДругимФирмам, "ЧДЦ=2; ЧРГ=; ЧН=' '");
				
			Иначе
				ОбластьСтрока.Параметры.кОплатеПоДругимФирмам = "";
			КонецЕсли;
		КонецЕсли;
	...
	//:СуммыПоФирмам = Новый ТаблицаЗначений
	тзСуммаПоНашимФирмам = СуммыПоФирмам.Скопировать(,"ЭтоНашаФирма,КОплате");
	тзСуммаПоНашимФирмам.Свернуть("ЭтоНашаФирма", "КОплате");
	СтрокаСуммаПоНашимФирмам = тзСуммаПоНашимФирмам.Найти(1, "ЭтоНашаФирма");
	Если СтрокаСуммаПоНашимФирмам <> Неопределено Тогда
		СуммаПоНашимФирмам = СтрокаСуммаПоНашимФирмам.КОплате;
	Иначе     
		СуммаПоНашимФирмам = 0;
	КонецЕсли;
	...
	ТабДок.Вывести(ОбластьИтог);
 	Если кОплатеПоДругимФирмам Тогда
		Область = Табдок.Область("R2C6:R3C6");
		Область.Объединить();
		Область.Текст = Формат(СуммаПоНашимФирмам, "ЧДЦ=2; ЧРГ=; ЧН=' '");
	Иначе
		Область = Табдок.Область("R2C6:R5C6");
		Область.Объединить();
		Область.Текст = "= " + Формат(СуммаПоНашимФирмам, "ЧДЦ=2; ЧРГ=; ЧН=' '");
	КонецЕсли;
...
Функция ЗаполнитьПодвалГостя(ТабПоле, ЗаказОбъект, Макет = Неопределено, ДанныеЛояльности = Неопределено, Кратко = Ложь) Экспорт
	ТипСумма = ПолучитьОписаниеТиповЧисла(12,2);
	...
	Если НЕ Кратко Тогда
		// для КП со спецификами 5х3 не показываем
		...
		Если СуммыПоФирмам.Количество() < 3 Тогда
			Для Сч = СуммыПоФирмам.Количество() По 2 Цикл
				ОбластьСтрока = Макет.ПолучитьОбласть("СтрокаДругаяФирма");
				ТабДок.Вывести(ОбластьСтрока);	
			КонецЦикла;
		КонецЕсли;
