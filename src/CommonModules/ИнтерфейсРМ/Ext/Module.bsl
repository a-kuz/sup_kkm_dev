...
Функция ЗапуститьРабочееМесто(ТекущееРМ=Неопределено) Экспорт
	...
	глОтладкаУровень = ?(Справочники.МестаРеализации.КМ = МестоРеализации,1,0);
			
	Если (МестоРеализации = Справочники.МестаРеализации.Ресторан Или Истина) Тогда
		...
		Возврат Истина;	
	Иначе
		Возврат ПродолжитьЗапускРабочегоМеста();	
	КонецЕсли;
...
//			РаботаСокнами.ShowCaption(0,0);
		...
		Иначе
			ИмяФормы = "Форма9";
		КонецЕсли;
	ИначеЕсли глПараметрыРМ.Тип = Перечисления.ТипыРМ.СтанцияОплатыКМ Тогда
...
Функция ВопросПредупреждение(ЗаголовокФормы, Текст1, Текст2, Кнопка1, Кнопка2, Кнопка3, 
	Текст2ГоризонтальноеПоложение=Неопределено, РазрешитьВнешнееСобытие=Ложь, Таймаут=0,РазмерШрифта=0,ДанныеДляЖР = Неопределено) Экспорт
...
//
Функция ВводЧисла(ЗаголовокФормы, ТипЗначения, Длина=10, Точность=0, НачЗначение=Неопределено, Кратность=0, ВидЗначения="", ЗапретОтмены = Ложь, Маска = "", ИнформацияДисплейГостя=Неопределено) Экспорт
...
//	Введенное значение. Если отказ от ввода, то Неопределено
//
Функция Авторизация(ЗаголовокФормы, ТипЗначения, Длина=10, Точность=0, НачЗначение=Неопределено, Кратность=0, ВидЗначения="", ЗапретОтмены = Ложь) Экспорт
...
//
Процедура СменаПользователя(НовыйПользователь=Неопределено) Экспорт
	
	БылПользователь = глПользователь;
	//глПользователь = ?(НовыйПользователь=Неопределено, ИдентификацияПользователя(), НовыйПользователь);
	ВремПользователь = ?(НовыйПользователь=Неопределено, ИдентификацияПользователя(), НовыйПользователь);
...
//
Функция ИдентификацияПользователя() Экспорт
	
	КодДоступа = Авторизация("АВТОРИЗАЦИЯ", "Пароль",4,,,,"Сотрудник",Истина);
	...
	Если Пользователь = Неопределено Тогда
		Если ФлагПовтора Тогда
			Возврат ИдентификацияПользователя();
		Иначе
			Возврат Справочники.Сотрудники.ПустаяСсылка();
		КонецЕсли;
	КонецЕсли; 
...
Процедура ЗаписьСобытия(СобытиеСсылка, Данные=Неопределено, Парам1="", Парам2="", Парам3="", Парам4="", Парам5="") Экспорт
	...
	Если 0 Тогда
		...
		Док = Документы.РегистрацияСобытия.СоздатьДокумент();
		Док.Дата			= ТекущаяДата();
		Док.РабочееМесто	= глРабочееМесто;
		Док.Автор			= глПользователь;
		Док.Событие			= СобытиеСсылка;
		Док.Заказ			= Данные;
		...
		Попытка 
			Док.Записать();
		Исключение
			ЗаписьЖурналаРегистрации(Событие.Событие, УровеньЖурналаРегистрации.Ошибка, ОбъектМетаданных, Данные, "Ошибка регистрации: "+ОписаниеОшибки() );
		КонецПопытки;
	КонецЕсли; 
...
Функция ОтчетСторно() Экспорт
	...
	Настройки.ПараметрыДанных.УстановитьЗначениеПараметра("Дата", НачалоДня(ТекущаяДата()));
...
Функция ЗаполнитьПодвалГостя(ТабПоле, ЗаказОбъект, Макет = Неопределено) Экспорт
	ТипСумма = ПолучитьОписаниеТиповЧисла(12,2);
