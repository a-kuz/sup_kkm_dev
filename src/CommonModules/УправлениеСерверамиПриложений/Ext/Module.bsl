...
Функция СоздатьИндексНаТаблицеМетаданных(ИнформационнаяБаза, ИмяТаблицыМетаданных, ПоляИндекса, ВключяемыеПоля, Отказ, ОписаниеОшибки) Экспорт
	ОписаниеОшибки = "";
	...
	Попытка
		...
		СтруктураХранения = КомКоннект.ирОбщий.ПолучитьСтруктуруХраненияБДЛкс(ОбъектыМД, Истина);
		
		стрСтруктураХранения = КомКоннект.ЗначениеВСтрокуВнутр(СтруктураХранения);
		локСтруктураХранения = ЗначениеИзСтрокиВнутр(стрСтруктураХранения);
		...
		Запрос = СтрШаблон("CREATE NONCLUSTERED INDEX %1 ON %2 --%3
		...
		Результат = запрос;
		SQL.ВыполнитьЗапросВыборки(Коннект, Запрос,Отказ,ОписаниеОшибки,6000);
		Если Не Отказ Тогда
			флагРезультат = 1;
		Иначе
