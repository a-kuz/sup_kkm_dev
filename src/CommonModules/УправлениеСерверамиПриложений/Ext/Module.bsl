...
Функция ОбновитьКонфигурациюБД(ИнформационнаяБаза, Синхронно = Ложь) Экспорт
	...
	ПутьКплатформе 			= ?(ЭтоX64, "C:\Program Files (x86)\1cv8\8.3.9.2170\bin\1cv8.exe", "C:\Program Files\1cv8\8.3.9.2170\bin\1cv8.exe");
	...
	Платформа.ЗапуститьСкрытоеПриложениеИДождатьсяЗавершения(Команда,,,,Синхронно);
	Платформа.ЗапуститьСкрытоеПриложениеИДождатьсяЗавершения(СтрЗаменить(Команда,"8.3.9.2170","8.3.10.2561"),,,,Синхронно);
...
Функция ЗапуститьОбменДанными(ИнформационнаяБаза, Синхронно = Ложь) Экспорт
	...
	Платформа.ЗапуститьСкрытоеПриложениеИДождатьсяЗавершения(СтрЗаменить(Команда,"8.3.9.2170","8.3.10.2561"), КаталогВременныхФайлов(), , Строка(Новый УникальныйИдентификатор())+".txt", Синхронно);
...
Функция ВыгрузитьDT(ИнформационнаяБаза) Экспорт
	...
	Исключение
	КонецПопытки;
	ПутьКзапускателюПлатформы = ПутьКзапускателюПлатформы(ИнформационнаяБаза);
	Команда = "psexec \\" + Хост + " -accepteula -i " + """C:\Program Files\1cv8\8.3.9.2170\bin\1cv8.exe"" DESIGNER /S" + 
	Хост + "\sup_kkm /NАдминистратор /p""19643003"" /UC123 /DumpIB " + локПутьКфайлуВыгрузки + " /Out" + локПутьКфайлуАут;
	
	Платформа.ЗапуститьСкрытоеПриложениеИДождатьсяЗавершения(Команда, КаталогВременныхФайлов(), , Строка(Новый УникальныйИдентификатор())+".txt", Истина);
...
Функция ОбрезатьЖурналРегистрации(ИнформационнаяБаза, Синхронно = Истина) Экспорт
	...
	ДатаОбрезки = Формат(ТекущаяДата()-86400*21,"ДФ=yyyy-MM-dd");
	Команда = "psexec \\" + Хост + " -accepteula -i " + """C:\Program Files\1cv8\8.3.9.2170\bin\1cv8.exe"" DESIGNER /S" + 
...
Функция ЗавершитьСеансы(ИнформационнаяБаза) Экспорт
	Соединитель = Новый COMObject("V83.COMConnector_8.3.9.2170");
	Попытка
...
Функция СоздатьБазуИзDT(ИнформационнаяБаза, Хост) Экспорт
	...
	Команда = "psexec \\" + Хост + " -accepteula -i " + """C:\Program Files\1cv8\8.3.9.2170\bin\1cv8.exe""" + 
...
Функция СоздатьЛокальнуюФайловуюБазу() Экспорт
	...
	Если ПапкаНаСервере.Существует() Тогда
		...
		Для Каждого Файл Из Файлы Цикл
			Сеть.ПодключитьШару(Cервер, Истина);
			КомандаКопирования = СтрШаблон("xcopy %1\1cv8.1CD %2 /I /Y", ПутьКпапкеСбазойНаСервере, локПутьКпапкеСбазой);
