...
Функция ОбновитьКонфигурациюБД(ИнформационнаяБаза, Синхронно = Ложь) Экспорт
	...
	Команда = "psexec \\"+Сервер + " -u """ + Пользователь + """ -p " + Пароль + " -accepteula -i 0 " + """"+ПутьКплатформе+""" designer /S" + Сервер + "\sup_kkm /NАдминистратор /p""19643003"" /DisableStartupMessages  /UC123 /UpdateDBCfg -Server /DumpResult" + локПутьКфайлуРезультата + " /Out" + локПутьКфайлуАут;
...
Функция ЗапуститьОбменДанными(ИнформационнаяБаза, Синхронно = Ложь) Экспорт
	...
	Команда = "psexec \\"+Сервер + " -u """ + Пользователь + """ -p " + Пароль + " -accepteula" + " -i 0 " + """"+ ПутьКплатформе + """ enterprise /S" + 
...
Функция ВыполнитьЗагрузкуТоваров(ИнформационнаяБаза) Экспорт
	...
	Команда = "psexec \\"+Сервер + " -u """ + Пользователь + """ -p " + Пароль + " -accepteula" + " -i 0 " + """C:\Program Files\1cv8\common\1cestart.exe"" enterprise /S" + 
...
Функция ВыгрузитьDT(ИнформационнаяБаза) Экспорт
	...
	Команда = "psexec \\" + Хост + " -accepteula -i 0 " + """C:\Program Files\1cv8\8.3.10.2561\bin\1cv8.exe"" DESIGNER /S" + 
...
Функция СоздатьБазуИзDT(ИнформационнаяБаза, Хост) Экспорт
	...
	ПутьКфайлуАут 			= "\\" + Хост + "\c$\temp\CreateIBout.txt";
	локПутьКфайлуАут 		= "c:\temp\CreateIBout.txt";
	...
	Команда = "psexec \\" + Хост + " -accepteula -i 0 " + """C:\Program Files\1cv8\8.3.10.2561\bin\1cv8.exe""" + 
...
Функция СоздатьЛокальнуюФайловуюБазу() Экспорт
	...
	ЗарегистрироватьСобытие("СоздатьЛокальнуюФайловуюБазу",,,,"Вход");
	Если ПроцедурыОбменаДанными.ОпределитьЭтаИнформационнаяБазаФайловая() Тогда
		Возврат Ложь;
	КонецЕсли;
	
	
	ЗарегистрироватьСобытие("СоздатьЛокальнуюФайловуюБазу",,,,"Платформа = ирКэш.Получить()");
	Cервер = ПараметрыСеанса.ТекущаяИБ.СерверХост;
	Сеть.ОтключитьШары();
	ЗарегистрироватьСобытие("СоздатьЛокальнуюФайловуюБазу",,,,"Сеть.ОтключитьШары()");
	Сеть.ПодключитьШару(Cервер, Истина);
	ЗарегистрироватьСобытие("СоздатьЛокальнуюФайловуюБазу",,,,"Сеть.ПодключитьШару(Cервер, Истина)");
	
	ПутьКпапкеСбазойНаСервере = СтрШаблон("\\%1\c$\sup_kkm", Cервер);
	локПутьКпапкеСбазой = "c:\sup_kkm";
	СоздатьКаталог(локПутьКпапкеСбазой);
	ЗарегистрироватьСобытие("СоздатьЛокальнуюФайловуюБазу",,,,"СоздатьКаталог(локПутьКпапкеСбазой);");
	...
	Если ПапкаНаСервере.Существует() Тогда
		...
		Для Каждого Файл Из Файлы Цикл
			//:Файл=Новый Файл
			Если Файл.Размер() > 5000000 И НРег(Файл.ИмяБезРасширения) = "1cv8" Тогда
				Попытка
					sh = New ComObject("shell.application");
					ns = sh.NameSpace("C:\Sup_kkm");
					ns.CopyHere(СтрШаблон("%1\1cv8.1CD",ПутьКпапкеСбазойНаСервере),16);
				Исключение
					...
					ЗарегистрироватьСобытие("СоздатьЛокальнуюФайловуюБазу",,,,"Платформа.ЗапуститьСкрытоеПриложениеИДождатьсяЗавершения(КомандаКопирования,,,,Ложь);");
				КонецПопытки;
				Прервать;
			КонецЕсли;			
		КонецЦикла;
	...
	Иначе
		Результат = Ложь;
	КонецЕсли;
	ЗарегистрироватьСобытие("СоздатьЛокальнуюФайловуюБазу",,,,"Выход");
	Возврат Результат;
КонецЕсли;
КонецФункции
...
Процедура СоздатьФайловуюБазу(ИнформационнаяБаза) Экспорт
	...
	СоздатьИндексы(ИнформационнаяБаза);
