...
Функция ОбновитьКонфигурациюБД(ИнформационнаяБаза, Синхронно = Ложь) Экспорт
	...
	Команда = "psexec \\"+Сервер + " -u """ + Пользователь + """ -p " + Пароль + " -accepteula -i 2 " + """"+ПутьКплатформе+""" designer /S" + Сервер + "\sup_kkm /NАдминистратор /p""19643003"" /DisableStartupMessages  /UC123 /UpdateDBCfg -Server /DumpResult" + локПутьКфайлуРезультата + " /Out" + локПутьКфайлуАут;
	Платформа.ЗапуститьСкрытоеПриложениеИДождатьсяЗавершения(СтрЗаменить(Команда,"8.3.10.2561","8.3.10.2561"),,,,Синхронно);
