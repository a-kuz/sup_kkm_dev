...
Функция ПоПейджеруЕстьМаркиКВыдаче(Пейджер, Станция=Неопределено)
	...
	Запрос.УстановитьПараметр("ДатаГотово",	ТекущаяДата()-60*60*12); // за последние 12 часов; как в мониторе марок
...
Функция ПолностьюОбработан(ИмяПараметра, МассивЗначений, Станция=Неопределено) Экспорт
	...
	Запрос.УстановитьПараметр("ДатаОтсчета",	ТекущаяДата()-60*60*12); // за последние 12 часов
...
Процедура РегламентСнятьСтарыеМаркиСУчета() Экспорт 
	...
	Запрос.УстановитьПараметр("Дата", ТекущаяДата()-60*60*12); // за последние 24 часов (в мониторе марки выводятся за последние 12 часов).
...
Функция ЭтоРМСтанция() Экспорт
	Если глПараметрыРМ = Неопределено Тогда
		Возврат Истина;
	КонецЕсли;
	Возврат (глПараметрыРМ.Тип = Перечисления.ТипыРМ.Производство) И ЗначениеЗаполнено(глРабочееМесто.Станция);
КонецФункции 
