...
Функция ЗаказПоКартеДоступа(КартаДоступа, Знач МожноСлужебныйБейдж = Ложь) Экспорт
	...
	Если Не МожноСлужебныйБейдж И КартаДоступа.СлужебныйБейдж Тогда
	...
	|ИЗ
	|	т КАК т";
	Запрос.УстановитьПараметр("Дата", ТекущаяДата()-3600*12);
	...
	Если Выборка.Следующий() Тогда
		...
		Если НаборЗаписей.Количество() > 0 Тогда 
			...
			НоваяЗапись.ТекущийАвтор = глПользователь;
...
Функция ОсвободитьКартуДоступаОтЗаказа(Заказ) Экспорт
	...
	Для Каждого РМ Из ВсеРМ Цикл
		Если РМ <> глРабочееМесто Тогда
			РегистрыСведений.ОбработкаСобытий.СоздатьЗапись(РМ , "",ТекущаяДатаНаСервере(), ПредопределенноеЗначение("Перечисление.ТипыСобытий.ТребованиеСбросаЗаказа"), Заказ.Номер, РМ);
		КонецЕсли;
	КонецЦикла;
	Возврат Истина;
...
Процедура ВозвратПейджеровПоЗаказам(МассивЗаказов, Станция, ВыводитьПредупреждение=Истина, Ответ="Да") Экспорт
	...
	Если Не ЗначениеЗаполнено(Станция) Тогда
		...
		Оператор = СХ.ПакетЗапросов[0].Операторы[0];
		Оператор.Отбор.Удалить(1);
		Запрос.Текст = СХ.ПолучитьТекстЗапроса();
	КонецЕсли;
	Выборка = Запрос.Выполнить().Выбрать();
...
Функция РегистрацияНовыхМарок(Заказ, Станция) Экспорт 
	...
	Пока ВыборкаМарки.Следующий() Цикл
		...
		Если НЕ МенеджерЗаписи.Выбран() Тогда
			...
			Если Не ЗначениеЗаполнено(ВыборкаМарки.Станция) Тогда
				Продолжить;
			КонецЕсли;
			...
			МенеджерЗаписи.Заказ							= Заказ; 
			МенеджерЗаписи.НомерМарки						= ВыборкаМарки.НомерМарки; 
			МенеджерЗаписи.Станция							= ВыборкаМарки.Станция; 
...
Функция ПоЗаказуЕстьМарки(МассивЗаказов, Станция=Неопределено, КромеУдаленных = Ложь) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
	               ...
	               |ИЗ
	               |	РегистрСведений.Марки КАК Марки
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЗаказТоварыДопИнф КАК ЗаказТоварыДопИнф
	               |		ПО Марки.Заказ = ЗаказТоварыДопИнф.Заказ
	               |			И Марки.НомерМарки = ЗаказТоварыДопИнф.НомерМарки
	               |ГДЕ
	               |	Марки.Заказ В(&Заказы)
	               |	И (Марки.Станция = &Станция
	               |			ИЛИ НЕОПРЕДЕЛЕНО = &Станция)
	               |	И (&КромеУдаленных = ЛОЖЬ
	               |			ИЛИ ЕСТЬNULL(ЗаказТоварыДопИнф.Статус, 0) <> &СтатусУдалено)";
	Запрос.УстановитьПараметр("СтатусУдалено", Перечисления.СтатусыПозицийЗаказа.Удалено);	
	Запрос.УстановитьПараметр("КромеУдаленных", КромеУдаленных);	
...
Функция СнятьМаркиСУчета(ИмяПараметра, МассивЗначений, Станция=Неопределено) Экспорт 
	...
	|	ЗаказТоварыДопИнф.Заказ В(&Заказы)
	|	И ЗаказТоварыДопИнф.НомерМарки <> 0
	|	И (&Станция = НЕОПРЕДЕЛЕНО
	|			ИЛИ ЗаказТоварыДопИнф.Станция = &Станция)");

	Запрос.УстановитьПараметр("Заказы", МассивЗначений);
	Запрос.УстановитьПараметр("Станция", Станция);
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		МЗ = РегистрыСведений.ЗаказТоварыДопИнф.СоздатьМенеджерЗаписи();
		МЗ.Заказ = Выборка.Заказ;
		МЗ.ИдСтроки = Выборка.ИдСтроки;
		МЗ.Прочитать();
		МЗ.НомерМарки = 0;
		Если МЗ.Статус = Перечисления.СтатусыПозицийЗаказа.Заказано Или МЗ.Статус = Перечисления.СтатусыПозицийЗаказа.Готово Тогда
			МЗ.Статус = Перечисления.СтатусыПозицийЗаказа.Выдано;
		КонецЕсли;
		МЗ.Записать();
	КонецЦикла;
	Возврат РезультатВыполнения;
