...
// Преобразует массив структур с идентичным набором полей в таблицу значений
Функция МассивСтруктурВТаблицуЗначений(МассивСтруктур) Экспорт 
	...
	ТекСтруктура = МассивСтруктур[0];
	Для каждого КлючИЗначение Из ТекСтруктура Цикл
		КолонкиТаблицы.Добавить(КлючИЗначение.Ключ);
	КонецЦикла;
	
	Для каждого ТекСтруктура Из МассивСтруктур Цикл
		ЗаполнитьЗначенияСвойств(ТаблицаЗначений.Добавить(), ТекСтруктура);
	КонецЦикла;
...
Функция АтрибутыЗаказа(ЗаказСсылка, Знач Атрибуты) Экспорт 
	...
	КонецЕсли;
	ТекстЗапроса = СтрШаблон("ВЫБРАТЬ %1 ИЗ РегистрСведений.ЗаказДопИнф ГДЕ Заказ = &Заказ", стрПоля);
...
// 	Неопределено
//
Функция УстановитьАтрибутыЗаказа(ЗаказСсылка, ЗначенияАтрибутов) Экспорт
...
// 	Неопределено
//
Функция УстановитьАтрибутЗаказа(ЗаказСсылка, Атрибут, Значение) Экспорт
	МЗ = РегистрыСведений.ЗаказДопИнф.СоздатьМенеджерЗаписи();
	МЗ.Заказ = ЗаказСсылка;
	МЗ.Прочитать();
	МЗ[Атрибут] = Значение;
...
Процедура ПодключитьсяКвнешнемуИсточникуСупЦентр(Регион, Переподключить = Ложь) Экспорт
	...
	Исключение
		...
		ОП = ВнешниеИсточникиДанных.СУП_Центр.ПолучитьОбщиеПараметрыСоединения();
		...
		ОП.Пароль = "ser09l";
		Попытка
			ВнешниеИсточникиДанных.СУП_Центр.УстановитьОбщиеПараметрыСоединения(ОП);
		ВнешниеИсточникиДанных.СУП_Центр.УстановитьПараметрыСоединенияПользователя(ИмяПользователя(), ОП);
		ВнешниеИсточникиДанных.СУП_Центр.УстановитьПараметрыСоединенияСеанса(ОП);
		...
		Попытка
			ВнешниеИсточникиДанных.СУП_Центр.УстановитьСоединение();
		Исключение
		КонецПопытки;
	КонецПопытки;
КонецПроцедуры
