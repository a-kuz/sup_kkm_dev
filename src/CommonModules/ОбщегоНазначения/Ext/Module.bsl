...
Функция ЦенаТовара(Товар, ТипЦен=Неопределено, ДатаЦены=Неопределено) Экспорт
	...
	Отбор = Новый Структура("ТипЦен,Товар", ТипЦен, Товар);
	
	Возврат РегистрыСведений.Цены.ПолучитьПоследнее(ДатаЦены, Отбор);
...
#Если Клиент Тогда
	...
	Процедура ЗавершитьРаботуПользователей() Экспорт
		...
		// Получение текущего значения параметров блокировки.
		ТекущийРежим = ПолучитьБлокировкуУстановкиСоединений();
		БлокировкаУстановлена = ТекущийРежим.Установлена;
		ВремяНачалаБлокировки = ТекущийРежим.Начало;
