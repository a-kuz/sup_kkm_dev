Процедура ЗагрузкаСправочникаСотрудников() Экспорт
	...
	Иначе
		...
		ЗаписьЖурналаРегистрации("Обновление справочника Сотрудники",УровеньЖурналаРегистрации.Ошибка,,,"Не найдена внешняя элемент справочника ВнешниеОбработки ('ЗагрузкаСправочникаСотрудников')");
...
Процедура ОбработатьСпулExcise() Экспорт
	...
	Для каждого Т Из ТЗ Цикл
		...
		Исключение
			ЗаписьЖурналаРегистрации("Спул Excise.Ошибка",УровеньЖурналаРегистрации.Ошибка,,Т.Заказ,ОписаниеОшибки());
		КонецПопытки;
		Попытка
			...
			Если Найти(Профиль,ИмяКомпьютера) > 0 Тогда
				...
				для каждого эл из СписокМасок Цикл
					...
					Для Каждого ИмяФайла из Файлы Цикл
						...
						Исключение
							ЗаписьЖурналаРегистрации("Загрузка файлов регламент",УровеньЖурналаРегистрации.Ошибка,Объект,,"ошибка записи ""объекта""" + ИмяФайла.ПолноеИмя);
...
Процедура ВыгрузитьЦеныПоРеестрамЦен() Экспорт
		...
		Пока Выб.Следующий() Цикл
			...
			Если ТипЗнч(ОбъектДляВыгрузки) = Тип("ДокументОбъект.РеестрЦен") Тогда
				...
				Если ВыгрузкаУспешна = Истина Тогда
					...
					ЗаписьЖурналаРегистрации("Выгружены цены на ТТ", УровеньЖурналаРегистрации.Информация,,ОбъектДляВыгрузки.Ссылка);
				Иначе
					Сообщить(ОписаниеОшибки);
					ЗаписьЖурналаРегистрации("Ошибка выгрузки цен на ТТ", УровеньЖурналаРегистрации.Ошибка,,ОбъектДляВыгрузки.Ссылка);
				КонецЕсли;
	...
	Исключение
		ЗаписьЖурналаРегистрации("Ошибка удаления цен на ТТ", УровеньЖурналаРегистрации.Ошибка,Метаданные.Документы.РеестрЦен,,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Процедура ОбработатьЦеныПомеченныеНаУдаление() Экспорт
	...
	Для Каждого Т Из ТаблицаУдаляемыхЦен Цикл
		...
		Если Соединение = Неопределено Тогда
			ЗаписьЖурналаРегистрации("Ошибка удаления цен на ТТ", УровеньЖурналаРегистрации.Ошибка,Метаданные.Документы.РеестрЦен,,"Не удалось установить соединение с " + ИБ.Наименование);
			Продолжить;
		КонецЕсли;
		...
		ЗапросТовары.УстановитьПараметр("КодСУП", Т.КодСУП);
		ТоварыИБ = ЗапросТовары.Выполнить().Выгрузить().ВыгрузитьКолонку(0);
		Для Каждого ТоварИБ Из ТоварыИБ Цикл
			
			МЗ = Соединение.РегистрыСведений.Цены.СоздатьМенеджерЗаписи();
	...
	Если ДокументОбъект.Проведен Тогда
		...
		Для Каждого Т Из ДокументОбъект.Товары Цикл
			Если Не ЗначениеЗаполнено(Т.Номенклатура.КодСУП) Тогда
				Продолжить;
			КонецЕсли;
			ЗапросТовары.УстановитьПараметр("КодСУП", Т.Номенклатура.КодСУП);
			ТоварыИБ = ЗапросТовары.Выполнить().Выгрузить().ВыгрузитьКолонку(0);
			Если Не ТоварыИБ.Количество() Тогда
				ОписаниеОшибки = ОписаниеОшибки + "Отсутствуют товары с кодом СУП = " + Т.Номенклатура.КодСУП + Символы.ПС;
			КонецЕсли;
			...
			Если Не фОшибка Тогда
				...
				Для Каждого ТоварИБ Из ТоварыИБ Цикл
					МЗ = Соединение.РегистрыСведений.Цены.СоздатьМенеджерЗаписи();
					МЗ.Валюта = Соединение.Константы.ОсновнаяВалюта.Получить();
					...
					МЗ.Записать(Истина);
					ПОпытка
						ЗарегистрироватьСобытие("Выгружена цена на товар",УровеньЖурналаРегистрации.Информация,,ДокументОбъект.ссылка,ТоварИБ.Наименование + " (" + ТоварИБ.Владелец.Наименование + ")");
					Исключение
					КонецПопытки;
...
Функция ТаблицаСсылокДляВыгрузки(ИБ) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |	ИЗ
	                      |		РегистрСведений.ДоступностьКаталоговТоваров.Изменения КАК ДоступностьКаталоговТоваровИзменения) КАК Т
	                      |ГДЕ
...
Процедура ВыгрузитьПоТаблицеСсылок(ТаблицаСсылок) Экспорт
	...
	Для Каждого Т Из ТаблицаСсылок Цикл
		...
		ИначеЕсли СтрНайти(Т.Тип, "лояльности") Тогда
			...
			Исключение
				...
				Исключение
				КонецПопытки;
				ЗаписьЖурналаРегистрации("Ошибка выгрузки на ТТ", УровеньЖурналаРегистрации.Ошибка,,Данные,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			КонецПопытки;
		...
		Иначе
			фОшибка = Ложь;
			Попытка
				ЗаписатьОбъектСсылочногоТипа(Соединение, Т.Ссылка.ПолучитьОбъект());
				ПланыОбмена.УдалитьРегистрациюИзменений(ИБ.ПолучитьУзелРИБ(), Т.Ссылка);
			Исключение
				Сообщить("Ошибка выгрузки "  + Т.Ссылка + ":");
				Сообщить(ОписаниеОшибки());
...
Функция ЗаписатьОбъектСсылочногоТипа(ComConnection, Объект) Экспорт
	//:Объект = Справочники.Товары.СоздатьЭлемент();
	
	обМетаданные = Объект.Метаданные();
	Структура = Новый Структура;
	
	Поля = Массив("Код","Наименование", "Ссылка", "ПометкаУдаления", "Владелец", "Родитель");
