...
Процедура ВыполнитьВыгрузкуНаТТ() Экспорт
...
Процедура ВыгрузитьПоТаблицеСсылок(ТаблицаСсылок) Экспорт
	...
	Для Каждого Т Из ТаблицаСсылок Цикл
		...
		Если Т.Ссылка <> Null И Т.Ссылка <> Неопределено Тогда
...
Функция ЗаписатьОбъектСсылочногоТипа(ComConnection, Объект) Экспорт
	...
	Поля = Массив("Код","Наименование", "Ссылка");
	...
	Иначе
		...
		Если comОбъект = Неопределено Тогда
			comОбъект = ComConnection.CreateItem[обМетаданные.Имя].СоздатьОбъект();
			comОбъект.УстановитьСсылкуНового(comСсылка);
		КонецЕсли;
	КонецЕсли;
