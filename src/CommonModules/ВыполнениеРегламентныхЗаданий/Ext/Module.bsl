...
Функция ТаблицаСсылокДляВыгрузки(ИБ) Экспорт
	...
	|	ИЗ
	|		РегистрСведений.НастройкаУслугПоПриготовлению.Изменения КАК НастройкаУслугПоПриготовлениюИзменения) КАК Т
	|ГДЕ
...
Процедура ВыгрузитьПоТаблицеСсылок(ТаблицаСсылок, Соединение = Неопределено) Экспорт
	...
	Для Каждого Т Из ТаблицаСсылок Цикл
		...
		ИначеЕсли СтрНайти(Т.Тип, "Привязка товаров к станциям") Тогда
			НЗ = РегистрыСведений.ПривязкаТоваровКстанциям.СоздатьНаборЗаписей();
			НЗ.Отбор.КаталогТоваров.Установить(Т.КаталогТоваров);
			НЗ.Отбор.ИнформационнаяБаза.Установить(Т.ИнформационнаяБазаГруппа);
			НЗ.Прочитать();
			Стр = ЗаписатьОбъектВстроку(НЗ);
			об = Соединение.ПрочитатьОбъектИзСтроки(Стр);
			об.ОбменДанными.Загрузка = Истина;	
		...
		ИначеЕсли СтрНайти(Т.Тип, "Настройка у") Тогда
			НЗ = РегистрыСведений.НастройкаУслугПоПриготовлению.СоздатьНаборЗаписей();
			НЗ.Отбор.Товар_КодСУП.Установить(Т.Товар_КодСУП);
			НЗ.Отбор.ИнформационнаяБаза.Установить(Т.ИнформационнаяБаза);
			НЗ.Прочитать();
			Стр = ЗаписатьОбъектВстроку(НЗ);
			об = Соединение.ПрочитатьОбъектИзСтроки(Стр);
