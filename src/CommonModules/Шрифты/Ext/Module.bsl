...
Функция ПолучитьКартуШрифта() Экспорт;
	...
	fa=Новый Структура(Ключи,61506,61808,61495,61497,61494,61496,61689,61757,61819,61961,61699,61696,61697,61698,61703,61700,61701,
...
Функция ПолучитьСимволыШрифта() Экспорт;
	...
	fa=ПолучитьКартуШрифта();
	Для каждого СимволШрифта Из fa Цикл
	
		fa.Вставить(СимволШрифта.Ключ, Символ(СимволШрифта.Значение));
...
Функция ПолучитьСимвол(Ключ) Экспорт
	Символ = "";
	СимволыШрифта = ПолучитьСимволыШрифта();	
	СимволыШрифта.Свойство(Ключ, Символ);
	Возврат Символ;
КонецФункции

Функция ШрифтУстановлен(ИД="FontAwesome Regular (TrueType)") Экспорт;
	Попытка
		WshShell.RegRead("HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Fonts");
	Исключение
		// нет административных прав, установить не получиться
		Возврат Истина;
	КонецПопытки;
	Попытка
		Результат = WshShell.RegRead("HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Fonts\"+ИД);
...
Процедура УстановитьШрифт() Экспорт
	...
	Попытка
		...
		#Если ТонкийКлиент Тогда
			Предупреждение("Установка шрифта в тонком клиенте не возможна!
			|Что-то будет отображаться некорректно.");
		#Иначе
			
			ИмяВременногоФайла = КаталогВременныхФайлов()+"fontawesome-webfont.ttf";
			ПолучитьОбщийМакет("ШрифтFontAwesome").Записать(ИмяВременногоФайла);
			КопироватьФайл(ИмяВременногоФайла,"C:\Windows\Fonts\fontawesome-webfont.ttf");
			ТекстReg = "REGEDIT4
			|[HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Fonts]
			|""FontAwesome (OpenType)""=""fontawesome.otf""
			|""FontAwesome Regular (TrueType)""=""fontawesome-webfont.ttf""";
			ТекстовыйДокумент = Новый ТекстовыйДокумент;
			ТекстовыйДокумент.УстановитьТекст(ТекстReg);
			ИФ = ПолучитьИмяВременногоФайла("reg");
			
			ТекстовыйДокумент.Записать(иф,КодировкаТекста.ANSI);
			КомандаСистемы("regedit /s """+ИФ+"""");
		#КонецЕсли
