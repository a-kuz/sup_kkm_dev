...
Процедура УдалитьИзСПУЛа_Закрытие(ДокументСсылка, ПротоколСсылка) Экспорт
	...
	// Если по протоколу установлен признак финализации, тогда не удаляем, а сбрасываем признак закрытия
	НаборЗаписей.Прочитать();
	Для каждого ТекущаяЗапись Из НаборЗаписей Цикл
		ЕстьФинализация = ТекущаяЗапись.СПУЛ;
		ТекущаяЗапись.ОбработатьЗакрытиеЗаказа = Ложь;
	КонецЦикла;
	Если Не ЕстьФинализация Тогда
		НаборЗаписей.Очистить();
	КонецЕсли;
