...
Функция ПредварительныйРасчетЗаказаПоСсылке(ЗаказСсылка, Тест) Экспорт
	...
	ИначеЕсли ВерсияПротокола = 5 Тогда
		ДанныеЗапроса = СформироватьJSONДляПредварительногоРасчетаЗаказа_Протокол5(СобратьДанныеДляПредварительногоРасчетаЗаказа_Протокол4(ЗаказСсылка));
	Иначе
...
Функция СформироватьJSONДляЗакрытияЗаказа_Протокол5(Данные, ДанныеЛояльностиПоЗаказу=Неопределено, Тест = Ложь)
