...
Функция ПолучитьДанныеЛояльностиПоЗаказу(ДокументСсылка, ТипДанных=Неопределено) Экспорт //### Переходный режим
	...
	Исключение
		ЗаписьЖурналаРегистрации("Лояльность.Данные заказа",УровеньЖурналаРегистрации.Ошибка,,,"Ошибка получения данных заказа: " + КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ДополнитьПревьюЗаказаАкциямиГостя(ЗаказСсылка, ПревьюЗаказа)
	...
	Иначе
		...
		Для каждого ТекущаяСтрока Из ДобавленныеКупоны Цикл
			...
			Если ЛояльностьКлиентСервер.НайтиСтрокуВМассивеСтруктур(Данные.КупоныВыданные, "Код", ТекущаяСтрока.Код) <> Неопределено Тогда
				Продолжить; // Избавляемся от дублей
			КонецЕсли;
			Данные.КупоныВыданные.Добавить(ЛояльностьКлиентСервер.КопияСтруктуры(ТекущаяСтрока));
		КонецЦикла;
	...
	Для каждого ТекущаяСтрока Из Данные.КупоныВыданные Цикл
...
Функция ПолучитьПревьюЗаказа(ЗаказСсылка, ВерсияДанных) Экспорт
	...
	Тест = ЛояльностьКлиентСервер.ТестовыйРежим(ПолучитьРабочееМестоЗаказа(ЗаказСсылка));
	...
	ДополнитьПревьюЗаказаАкциямиГостя(ЗаказСсылка, Данные);
	Если Отладка Тогда
	...
	ЗаписатьДанныеЛояльностиПоЗаказу(ЗаказСсылка, "PREVIEW", Данные, ВерсияЗаказа);
...
Процедура УстановитьПризнакОбработкиСПУЛ_ФинализацияЧека(ДокументСсылка, ПротоколСсылка, Признак=Истина) Экспорт
	НаборЗаписей = РегистрыСведений.СПУЛЛояльности.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.ДокументСсылка.Установить(ДокументСсылка);
	НаборЗаписей.Отбор.Протокол.Установить(ПротоколСсылка);
...
Процедура УстановитьПризнакОбработкиСПУЛ_ЗакрытиеЗаказа(ДокументСсылка, ПротоколСсылка, Признак=Истина) Экспорт
	НаборЗаписей = РегистрыСведений.СПУЛЛояльности.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.ДокументСсылка.Установить(ДокументСсылка);
	НаборЗаписей.Отбор.Протокол.Установить(ПротоколСсылка);
...
Процедура ОбработатьСпулЛояльности(ЗаказСсылка=Неопределено, ПротоколСсылка=Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ЗаказСсылка) И ЗначениеЗаполнено(ПротоколСсылка) Тогда
		Попытка
			...
			КонецЕсли;
		Исключение
			ЗаписьЖурналаРегистрации("Лояльность.Обработка СПУЛ",УровеньЖурналаРегистрации.Ошибка,,ЗаказСсылка,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
	Иначе
		...
		Для Каждого Стр ИЗ ТаблицаЗаказов Цикл
			Попытка
				...
				КонецЕсли;
			Исключение
				ЗаписьЖурналаРегистрации("Лояльность.Обработка СПУЛ",УровеньЖурналаРегистрации.Ошибка,,Стр.ЗаказСсылка,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			КонецПопытки;
		КонецЦикла;
		
		ТаблицаВозвратов = ПолучитьСпулВозвратов();
		Для Каждого Стр ИЗ ТаблицаВозвратов Цикл
			Попытка
				ВозвратОбъект = Стр.ВозвратСсылка.ПолучитьОбъект();
				ЛояльностьКлиентСервер.ОбработатьВозврат(ВозвратОбъект, Истина);
			Исключение
				ЗаписьЖурналаРегистрации("Лояльность. Ошибка обработки СПУЛа",УровеньЖурналаРегистрации.Ошибка,,Стр.ВозвратСсылка,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			КонецПопытки;
		КонецЦикла;
		...
		//	Исключение
		//		ЗаписьЖурналаРегистрации("Лояльность. Ошибка обработки СПУЛа",УровеньЖурналаРегистрации.Ошибка,,Стр.ВозвратСсылка,ПодробноеПредставлениеОшибки(ОписаниеОшибки()));
		//	КонецПопытки;
		//КонецЦикла;
	КонецЕсли;
КонецПроцедуры
...
Процедура СкорректироватьЗаказПоПротоколуРасчетов(ЗаказСсылка, ПротоколСсылка)
	...
	Исключение
		ЗаписьЖурналаРегистрации("Лояльность. Ошибка обработки СПУЛа",УровеньЖурналаРегистрации.Ошибка,,ЗаказСсылка,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
	
КонецПроцедуры

Процедура СкорректироватьПротоколРасчетовПоЗаказу(ЗаказСсылка, ПротоколСсылка)
	...
	Исключение
		ЗаписьЖурналаРегистрации("Лояльность. Ошибка обработки СПУЛа",УровеньЖурналаРегистрации.Ошибка,,ПротоколСсылка,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
	
КонецПроцедуры

Процедура ПроверитьНеобходимостьЗакрытияЗаказа(ЗаказСсылка, ПротоколСсылка)
...
Процедура ФинализацияЧека_Протокол4(ЗаказСсылка, ПротоколСсылка, РежимСПУЛ = Ложь, ЗаписьЛога = "", Тест=Ложь)
	...
	//### Это затычка для правильной финализации заказа 4го протокола. Ведь сюда интерфейсы еще не передают протокол
	//Если Не ЗначениеЗаполнено(ПротоколСсылка) Тогда
	//	//### Поэтому ищем протокол сами
	//	ПротоколСсылка = НайтиПротоколРасчетовПоЗаказу(ЗаказСсылка);
	//	Если Не ЗначениеЗаполнено(ПротоколСсылка) Тогда
	//		ЗаписьЖурналаРегистрации("Лояльность.Финализация чека",УровеньЖурналаРегистрации.Ошибка,,ЗаказСсылка,"Не найден протокол расчетов");
	//		Возврат;
	//	КонецЕсли;
	//	//### Поэтому считаем, что есть только один протокол оплаты, вот его и запишем в нужные места
	//КонецЕсли;
	
	//### Кроме того, нужно и сам протокол оплаты проапдейтить
	//СкорректироватьПротоколРасчетовПоЗаказу(ЗаказСсылка, ПротоколСсылка);
	//СкорректироватьЗаказПоПротоколуРасчетов(ЗаказСсылка, ПротоколСсылка);
	//ПроверитьНеобходимостьЗакрытияЗаказа(ЗаказСсылка, ПротоколСсылка);	
	...
	//ЭтоСПУЛ = Лояльность.ПолучитьПризнакСПУЛа(ЗаказСсылка);
	//
	...
	Если ПараметрыЗапроса.Ошибка Тогда		
		...
		ЗаписьЖурналаРегистрации("Лояльность.Финализация чека", УровеньЖурналаРегистрации.Ошибка,, ЗаказСсылка,"ОписаниеОшибки=ошибка формирования JSON");
	Иначе		
		Попытка
	...
	Если НЕ ОтправкаУспешна Тогда
		...
		ЗаписьЖурналаРегистрации("Лояльность.Финализация чек", УровеньЖурналаРегистрации.Ошибка, , ЗаказСсылка,"ОписаниеОшибки="+ОписаниеОшибки);
	...
	Иначе
		Если ОтветСервера.Ошибка ИЛИ ОтветСервера.Ответ = "" ИЛИ ОтветСервера.Ответ = Неопределено Тогда
			// сервис недоступен или не смог обработать запрос
			ЗаписьЖурналаРегистрации("Лояльность.Финализация чека", УровеньЖурналаРегистрации.Ошибка, , ЗаказСсылка,"ответ="+ОтветСервера.Ответ);
...
Процедура ФинализацияЧека_Протокол3(ЗаказОбъект, РежимСПУЛ = Ложь, ЗаписьЛога = "", Тест=Ложь)
	...
	Если ПараметрыЗапроса.Ошибка Тогда
		...
		ЗаписьЛога = ЗаписьЛога+Символы.ПС+""+ТекущаяДата()+";"+"финализация;ошибка формирования JSON("+ПараметрыЗапроса.ТекстОшибки+")";
		ЗаписьЖурналаРегистрации("Ошибка финализации заказа (Лояльность)", УровеньЖурналаРегистрации.Ошибка, ЗаказОбъект, ЗаказОбъект.Ссылка,"ОписаниеОшибки=ошибка формирования JSON");
	...
	Если НЕ ОтправкаУспешна Тогда
		...
		ЗаписьЖурналаРегистрации("Ошибка финализации заказа (Лояльность)", УровеньЖурналаРегистрации.Ошибка, ЗаказОбъект, ЗаказОбъект.Ссылка,"ОписаниеОшибки="+ОписаниеОшибки);
		ЗаписьЛога = ЗаписьЛога+Символы.ПС+""+ТекущаяДата()+";"+"финализация;ошибка отправки (ответ="+ОписаниеОшибки+")";
	Иначе
		Если ОтветСервера.Ошибка ИЛИ ОтветСервера.Ответ = "" ИЛИ ОтветСервера.Ответ = Неопределено Тогда
			// сервис недоступен или не смог обработать запрос
			ЗаписьЖурналаРегистрации("Ошибка финализации заказа (Лояльность)", УровеньЖурналаРегистрации.Ошибка, ЗаказОбъект, ЗаказОбъект.Ссылка,"ответ="+ОтветСервера.Ответ);
			ЗаписьЛога = ЗаписьЛога+Символы.ПС+""+ТекущаяДата()+";"+"финализация;ошибка отправки (ответ="+ОтветСервера.Ответ+")";
			ОтправкаУспешна = Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура ФинализацияЧека_Протокол2(ЗаказОбъект, РежимСПУЛ = Ложь, ЗаписьЛога = "", Тест=Ложь)
	...
	///////////////////////////////////////////////////////////////////////////////////
	// ФОРМИРОВАНИЕ И ОТПРАВКА ЗАПРОСА
	ПараметрыЗапроса = ЛояльностьКлиентСервер.СформироватьJSONПоЗаказу_Протокол2(ТипЗапроса, ЗаказОбъект, ТипЧекаЛояльность,,РежимСПУЛ);
...
Процедура ЗакрытиеЗаказа_Протокол4(ЗаказСсылка, ПротоколСсылка, Тест = Ложь)
	...
	Если ПараметрыЗапроса.Ошибка Тогда		
		...
		ЗаписьЖурналаРегистрации("Лояльность.Закрытие заказа", УровеньЖурналаРегистрации.Ошибка,, ЗаказСсылка,"ОписаниеОшибки=ошибка формирования JSON");
	Иначе		
		Попытка
	...
	Если НЕ ОтправкаУспешна Тогда
		...
		ЗаписьЖурналаРегистрации("Лояльность.Закрытие заказа", УровеньЖурналаРегистрации.Ошибка, , ЗаказСсылка,"ОписаниеОшибки="+ОписаниеОшибки);
		ЗаписьЛога = ЗаписьЛога+Символы.ПС+""+ТекущаяДата()+";"+"закрытие заказа;ошибка отправки (ответ="+ОписаниеОшибки+")";
	Иначе
		Если ОтветСервера.Ошибка ИЛИ ОтветСервера.Ответ = "" ИЛИ ОтветСервера.Ответ = Неопределено Тогда
			// сервис недоступен или не смог обработать запрос
			ЗаписьЖурналаРегистрации("Лояльность.Закрытие заказа", УровеньЖурналаРегистрации.Ошибка, , ЗаказСсылка,"ответ="+ОтветСервера.Ответ);
...
Процедура ОбработатьВозврат(ВозвратСсылка, ПротоколСсылка = Неопределено, РежимСПУЛ = Ложь, ЗаписьЛога = "") Экспорт
	...
	Тест = ЛояльностьКлиентСервер.ТестовыйРежим(ПолучитьРабочееМестоВозврата(ВозвратСсылка));
	ВерсияЛояльности = ЛояльностьКлиентСервер.ВерсияЛояльности(Тест);
	ВерсияПротокола = ЛояльностьКлиентСервер.ВерсияПротоколаИзВерсииЛояльности(ВерсияЛояльности);
	Если ВерсияПротокола < 3 Тогда
...
Процедура ОбработатьВозврат_Протокол4(ВозвратСсылка, ПротоколСсылка, РежимСПУЛ = Ложь, ЗаписьЛога = "", Тест = Ложь)
	...
	Если ПараметрыЗапроса.Ошибка Тогда
		...
		ЗаписьЖурналаРегистрации("Ошибка финализации возврата (Лояльность)", УровеньЖурналаРегистрации.Ошибка, , ВозвратСсылка,"ОписаниеОшибки=ошибка формирования JSON");
	...
	Иначе
		...
		Если НЕ ОтправкаУспешна Тогда
			...
			ЗаписьЖурналаРегистрации("Ошибка финализации возврата (Лояльность)", УровеньЖурналаРегистрации.Ошибка, , ВозвратСсылка, "ОписаниеОшибки="+ОписаниеОшибки);
	...
	Если ОтправкаУспешна Тогда
		Если ОтветСервера.Ошибка ИЛИ ОтветСервера.Ответ = "" ИЛИ ОтветСервера.Ответ = Неопределено Тогда
			// сервис недоступен или не смог обработать запрос
			ЗаписьЖурналаРегистрации("Ошибка финализации заказа (Лояльность)", УровеньЖурналаРегистрации.Ошибка, , ВозвратСсылка,"ответ="+ОтветСервера.Ответ);
	...
	Если ОтправкаУспешна Тогда
		...
		Иначе
			// удаляем из СПУЛа
			Лояльность.УстановитьПризнакОбработкиСПУЛ_ФинализацияЧека(ВозвратСсылка, ПротоколСсылка, Ложь);
		КонецЕсли;
...
Процедура ОбработатьВозврат_Протокол3(ВозвратОбъект, РежимСПУЛ = Ложь, ЗаписьЛога = "", Тест = Ложь)
	...
	Если ПараметрыЗапроса.Ошибка Тогда
		...
		ЗаписьЛога = ЗаписьЛога+Символы.ПС+""+ТекущаяДата()+";"+"финализация возврата;ошибка формирования JSON("+ПараметрыЗапроса.ТекстОшибки+")";
		ЗаписьЖурналаРегистрации("Ошибка финализации возврата (Лояльность)", УровеньЖурналаРегистрации.Ошибка, ВозвратОбъект, ВозвратОбъект.Ссылка,"ОписаниеОшибки=ошибка формирования JSON");
	...
	Иначе
		...
		Если НЕ ОтправкаУспешна Тогда
			...
			ЗаписьЖурналаРегистрации("Ошибка финализации возврата (Лояльность)", УровеньЖурналаРегистрации.Ошибка, ВозвратОбъект, ВозвратОбъект.Ссылка,"ОписаниеОшибки="+ОписаниеОшибки);
			ЗаписьЛога = ЗаписьЛога+Символы.ПС+""+ТекущаяДата()+";"+"финализация возврата;ошибка отправки (Режим=локальный, ответ="+ОписаниеОшибки+")";
		КонецЕсли;
	...
	Если ОтправкаУспешна Тогда
		Если ОтветСервера.Ошибка ИЛИ ОтветСервера.Ответ = "" ИЛИ ОтветСервера.Ответ = Неопределено Тогда
			// сервис недоступен или не смог обработать запрос
			ЗаписьЖурналаРегистрации("Ошибка финализации заказа (Лояльность)", УровеньЖурналаРегистрации.Ошибка, ВозвратОбъект, ВозвратОбъект.Ссылка,"ответ="+ОтветСервера.Ответ);
			ЗаписьЛога = ЗаписьЛога+Символы.ПС+""+ТекущаяДата()+";"+"финализация возврата;ошибка отправки (ответ="+ОтветСервера.Ответ+")";
			ОтправкаУспешна = Ложь;
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьПараметрыСервисаРасчетаЧеков(Тест = Ложь) Экспорт
...
Функция СформироватьJSONДляЗакрытияЗаказа_Протокол4(Данные, ДанныеЛояльностиПоЗаказу=Неопределено, Тест = Ложь)
	...
	Пока Данные_Товары.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			СтрокаЧека.Вставить("mol"				, Данные.Протокол.КодКассира);
			СтрокаЧека.Вставить("sale_place"		, Выборка.МестоРеалиации);
...
Функция ВерсияЛояльности(Тест=Ложь) Экспорт
	...
	Возврат ПараметрыСеанса.ВерсияЛояльности;
			
КонецФункции

Функция ПолучитьВерсиюЛояльности()
	
	ТекущаяИБ = ПараметрыСеанса.ТекущаяИБ;
	
	Если ЗначениеЗаполнено(ТекущаяИБ) Тогда
...
Процедура ФормированиеТаблицыЗаявокЛУЧ(Дата1, Дата2) Экспорт
	...
	Если Подключение = Неопределено Тогда
		ЗаписьЖурналаРегистрации("Выгрузка ЛУЧ",УровеньЖурналаРегистрации.Ошибка,,,"Ошибка подключения к серверу");
		Возврат;
	КонецЕсли;
	...
	Если Отказ Тогда
		ЗаписьЖурналаРегистрации("Выгрузка ЛУЧ",УровеньЖурналаРегистрации.Ошибка,,,"Ошибка получения данных (" + ТекстОшибки + ")");
		Возврат;
	КонецЕсли;
		...
		Если Стр.doc_number <> ТекущийНомерДокумента Тогда
			...
			Если ТоварыДЖСОН.Количество() > 0 Тогда
				...
				Если Отказ Тогда
					...
					//ЗаписьЖурналаРегистрации("Выгрузка ЛУЧ",УровеньЖурналаРегистрации.Ошибка,,,"Ошибка записи данных (" + ТекстОшибки + ")");
				КонецЕсли;
			КонецЕсли;
...
Процедура ОтправкаЗаявленийЛУЧ(Дата1, Дата2) Экспорт
	...
	Если Подключение = Неопределено Тогда
		ЗаписьЖурналаРегистрации("Отправка ЛУЧ",УровеньЖурналаРегистрации.Ошибка,,,"Ошибка подключения к серверу");
		Возврат;
	КонецЕсли;
	...
	Результат = SQL.ВыполнитьЗапросВыборки(Подключение, ТекстЗапроса, Отказ, ТекстОшибки);
	Если Отказ Тогда
		ЗаписьЖурналаРегистрации("Отправка ЛУЧ",УровеньЖурналаРегистрации.Ошибка,,,"Ошибка получения данных (" + ТекстОшибки + ")");
		Возврат;
	КонецЕсли;
	...
	Для Каждого Стр ИЗ Результат Цикл
		...
		Если ОтветСервера = Неопределено ИЛИ ОтветСервера.Ошибка ИЛИ ОтветСервера.Ответ = "" Тогда
			ЗаписьЖурналаРегистрации("Отправка ЛУЧ",УровеньЖурналаРегистрации.Ошибка,,,"Ошибка отправки");
			Продолжить;
		КонецЕсли;
		...
		Если Ошибка = 0 Тогда
			...
			SQL.ВыполнитьЗапрос(Подключение, ТекстЗапроса, Отказ, ТекстОшибки);
			Если Отказ Тогда
				ЗаписьЖурналаРегистрации("Отправка ЛУЧ",УровеньЖурналаРегистрации.Ошибка,,,"Ошибка обновления данных (" + ТекстОшибки + ")");
			КонецЕсли;
		Иначе
			ТекстОшибки = "";
...
Процедура ЛогированиеЛОЛ(Подключение = Неопределено, Тип, ТекстОшибки = "") Экспорт
	...
	SQL.ВыполнитьЗапрос(Подключение, ТекстЗапроса, Отказ, ТекстОшибки);
	Если Отказ Тогда
		ЗаписьЖурналаРегистрации("ЛогированиеЛОЛ",УровеньЖурналаРегистрации.Ошибка,,,"Ошибка записи лога ошибок");
	КонецЕсли;
...
Процедура ЗарегистрироватьЗаказГостя(ДанныеЗапроса)
	...
	Если РезультатЗапроса.Пустой() Тогда
		ДанныеЗапроса.err = 1;
		ДанныеЗапроса.err_msg = "Карта гостя не найдена";
	...
	Иначе
		...
		Если Не ПустаяСтрока(ДанныеЗаказа.НомерКартыЛояльности) И ДанныеЗаказа.НомерКартыЛояльности <> ДанныеЗапроса.guest.card_number Тогда
			// Текущий заказ привязан к другой карте гостя
			//НачатьТранзакцию();
			//Попытка
			//	ЗаказОбъект = ДанныеЗаказа.Ссылка.ПолучитьОбъект();
			//	ЗаказОбъект.НомерКартыЛояльности = ДанныеЗапроса.guest.card_number;
			//	ЗаказОбъект.Записать();
			//	ЗаказОбъект = Неопределено;
			//	Лояльность.ЗаписатьДанныеЛояльностиПоЗаказу(ДанныеЗаказа.Ссылка, "INFO", ДанныеЗапроса);
			//	ЗафиксироватьТранзакцию();
			//Исключение
			//	ОтменитьТранзакцию();
			//КонецПопытки;
			
			//### Запускаем задание на получения данных по акциям Гостя + CALC заказа
...
Функция Подключиться_loyality77()
	...
	Except
		...
		ЗаписьЖурналаРегистрации("Подключение к базе", УровеньЖурналаРегистрации.Ошибка,,,ТекстСообщения);
...
Процедура ЗаписатьДанныеЛояльностиПоЗаказу_УДАЛИТЬ(ЗаказСсылка, ДанныеЛояльности) Экспорт
	...
	НоваяЗапись.ВерсияДанных = ЛояльностьКлиентСервер.ТекущаяВерсияЛояльностиЧислом();	
	НаборЗаписей.Записать();
