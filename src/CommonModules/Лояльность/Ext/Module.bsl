...
Процедура ОбработатьСпулЛояльности(ЗаказСсылка=Неопределено, ПротоколСсылка=Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ЗаказСсылка) И ЗначениеЗаполнено(ПротоколСсылка) Тогда
		Попытка
			...
			КонецЕсли;
			//### Временная затычка, ведь в СПУЛ лояльности закрытие заказа не записывается
			//ДанныеСПУЛа = ПолучитьДанныеСПУЛа(ЗаказСсылка, ПротоколСсылка);
...
Процедура СброситьДанныеЛояльностиПоЗаказу(ЗаказОбъект) Экспорт
	// Убираем все данные из регистра
	Лояльность.УдалитьДанныеЛояльностиПоЗаказу(ЗаказОбъект.Ссылка, "INFO");
	Лояльность.УдалитьДанныеЛояльностиПоЗаказу(ЗаказОбъект.Ссылка, "ACTIONS");
	Лояльность.УдалитьДанныеЛояльностиПоЗаказу(ЗаказОбъект.Ссылка, "CALC");
...
Процедура ОбработатьВозврат(ВозвратОбъект, РежимСПУЛ = Ложь, ЗаписьЛога = "") Экспорт
	...
	Тест = ЛояльностьКлиентСервер.ТестовыйРежим(ВозвратОбъект.РабочееМесто);
	ВерсияЛояльности = ЛояльностьКлиентСервер.ВерсияЛояльности(Тест);
	ВерсияПротокола = ЛояльностьКлиентСервер.ВерсияПротоколаИзВерсииЛояльности(ВерсияЛояльности);
	Если ВерсияПротокола < 3 Тогда
...
#КонецЕсли
	...
	Пока Данные_Товары.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			СтрокаЧека.Вставить("mol"				, Данные.Заказ.КодКассира);
...
Функция СформироватьJSONДляПредварительногоРасчетаЗаказа_Протокол4(Данные, ДанныеЛояльностиПоЗаказу=Неопределено)
	...
	Пока Данные_Товары.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			СтрокаЧека.Вставить("pay_status"		, ?(ЗначениеЗаполнено(Выборка.ДокументОплаты),1,0));
...
Функция ПолучитьДанныеЛояльностиПоЗаказу_УДАЛИТЬ(ЗаказСсылка) Экспорт
	...
	|	РегистрСведений.ДанныеЛояльностиПоЗаказу КАК ДанныеЛояльностиПоЗаказу
	|ГДЕ
	|	ДанныеЛояльностиПоЗаказу.Заказ = &Заказ";
	Запрос.УстановитьПараметр("Заказ", ЗаказСсылка);
	РезультатЗапроса = Запрос.Выполнить();
	Если РезультатЗапроса.Пустой() Тогда
