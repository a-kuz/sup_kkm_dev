...
Процедура ДополнитьПревьюЗаказаДаннымиФинализации(ЗаказСсылка, ПревьюЗаказа)
	...
	Пока ВыборкаЗапроса.Следующий() Цикл
		...
		ПревьюЗаказа.Итоги.КГЛНачислено = ПревьюЗаказа.Итоги.КГЛНачислено + ТекущиеДанные.ИтогиМ.КГЛНачислено;
		ПревьюЗаказа.Итоги.КГЛСписано = ПревьюЗаказа.Итоги.КГЛСписано + ТекущиеДанные.ИтогиМ.КГЛСписано;
		...
		ПревьюЗаказа.Итоги.КГЛНачислено = ПревьюЗаказа.Итоги.КГЛНачислено + ТекущиеДанные.ИтогиО.КГЛНачислено;
		ПревьюЗаказа.Итоги.КГЛСписано = ПревьюЗаказа.Итоги.КГЛСписано + ТекущиеДанные.ИтогиО.КГЛСписано;
		...
		ПревьюЗаказа.Итоги.КГЛНачислено = ПревьюЗаказа.Итоги.КГЛНачислено + ТекущиеДанные.ИтогиКП.КГЛНачислено;
		ПревьюЗаказа.Итоги.КГЛСписано = ПревьюЗаказа.Итоги.КГЛСписано + ТекущиеДанные.ИтогиКП.КГЛСписано;
		
		Для каждого ТекущаяСтрока Из ТекущиеДанные.ИтогиМ.КГЛИнфо Цикл
			ПревьюЗаказа.ИтогиМ.КГЛИнфо.Добавить(ЛояльностьКлиентСервер.КопияСтруктуры(ТекущаяСтрока));
			ПревьюЗаказа.Итоги.КГЛИнфо.Добавить(ЛояльностьКлиентСервер.КопияСтруктуры(ТекущаяСтрока));
		КонецЦикла;		
		Для каждого ТекущаяСтрока Из ТекущиеДанные.ИтогиО.КГЛИнфо Цикл
			ПревьюЗаказа.ИтогиО.КГЛИнфо.Добавить(ЛояльностьКлиентСервер.КопияСтруктуры(ТекущаяСтрока));
			ПревьюЗаказа.Итоги.КГЛИнфо.Добавить(ЛояльностьКлиентСервер.КопияСтруктуры(ТекущаяСтрока));
		КонецЦикла;
		Для каждого ТекущаяСтрока Из ТекущиеДанные.ИтогиКП.КГЛИнфо Цикл
			ПревьюЗаказа.ИтогиКП.КГЛИнфо.Добавить(ЛояльностьКлиентСервер.КопияСтруктуры(ТекущаяСтрока));
			ПревьюЗаказа.Итоги.КГЛИнфо.Добавить(ЛояльностьКлиентСервер.КопияСтруктуры(ТекущаяСтрока));
		КонецЦикла;
	КонецЦикла;
...
Процедура ОбновитьАкцииГостяПоПревьюЗаказа(ЗаказСсылка, ПревьюЗаказа, ВерсияДанных)
	...
	ПревьюЗаказа.ЛУЧ = ДанныеАкций.ЛУЧ;
	ПревьюЗаказа.ЛУЧМ = ДанныеАкций.ЛУЧМ;
	ПревьюЗаказа.ЛУЧО = ДанныеАкций.ЛУЧО;
	ПревьюЗаказа.ЛУЧКП = ДанныеАкций.ЛУЧКП;
	
	ДанныеАкций.Купоны.Очистить();
	Если НЕ ДанныеАкций.Свойство("Акции") Тогда
		ДанныеАкций.Вставить("Акции", Новый Массив);
	КонецЕсли;
...
Процедура УдалитьИзСПУЛа_Закрытие(ДокументСсылка, ПротоколСсылка) Экспорт
	...
	// Чистим СПУЛ по текущему протоколу
	
	НаборЗаписей = РегистрыСведений.СПУЛЛояльности.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.ДокументСсылка.Установить(ДокументСсылка);
	НаборЗаписей.Отбор.Протокол.Установить(ПротоколСсылка);	
	// Если по протоколу установлен признак финализации, тогда не удаляем, а сбрасываем признак заркытия
	НаборЗаписей.Прочитать();
	ЕстьФинализация = Ложь;
	Для каждого ТекущаяЗапись Из НаборЗаписей Цикл
	...
	// Чистим СПУЛ от финализированных и закрытых
...
Процедура ОбработатьСпулЛояльности(ЗаказСсылка=Неопределено, ПротоколСсылка=Неопределено) Экспорт
	...
	Иначе
		ОграничениеПериода = ТекущаяДата() - 10*60;
		ТаблицаЗаказов = ПолучитьСпулЗаказов(,ОграничениеПериода);
...
Функция СобратьДанныеДляФинализацииЗаказа_Протокол4(ЗаказСсылка, ПротоколСсылка)
	...
	|	ЗаказСпецифики.Количество КАК Количество,
	...
	|		ТаблицаСпецифик.Код,
	|		ТаблицаСпецифик.Количество * ТаблицаТовары.Количество,
	|		ТаблицаСпецифик.Цена,
	|		ТаблицаСпецифик.Цена * ТаблицаСпецифик.Количество * ТаблицаТовары.Количество,
	|		ТаблицаСпецифик.ЦенаРеализации,
	|		ТаблицаСпецифик.ЦенаРеализации * ТаблицаСпецифик.Количество * ТаблицаТовары.Количество,
...
Функция СобратьДанныеДляЗакрытияЗаказа_Протокол5(ЗаказСсылка) Экспорт
	...
	|		ТаблицаТовары.ИдСтроки,
	|		ТаблицаСпецифик.Код,
	|		ТаблицаТовары.Количество * ТаблицаСпецифик.Количество,
	|		ТаблицаСпецифик.Цена,
	|		ТаблицаСпецифик.Цена * ТаблицаТовары.Количество * ТаблицаСпецифик.Количество,
	|		ТаблицаСпецифик.ЦенаРеализации,
	|		ТаблицаСпецифик.ЦенаРеализации * ТаблицаТовары.Количество * ТаблицаСпецифик.Количество,
	|		ТаблицаСпецифик.Наименование,
	|		NULL,
	|		1,
	|		ТаблицаСпецифик.НомерСтроки,
...
Процедура ПовторнаяФинализацияЧека(ЗаказСсылка, ПротоколСсылка, РежимСПУЛ = Ложь, ЗаписьЛога = "", Тест=Ложь) Экспорт
	...
	ПараметрыЗапроса = СформироватьJSONДляПовторнойФинализацииЗаказа_Протокол5(Данные,, Тест);
...
Функция ПолучитьПревьюЗаказа(ЗаказСсылка, ВерсияДанных) Экспорт
	...
	ТекстИтоговГостя = Новый Соответствие;
	ТекстИтоговГостя.Вставить(1, "За каждые 100г/мл");
	ТекстИтоговГостя.Вставить(2, "За акции");
	ТекстИтоговГостя.Вставить(3, "По программе ЛУЧ");
	ТекстИтоговГостя.Вставить(4, "За персональные предложения");
	ТекстИтоговГостя.Вставить(5, "За совместные акции");
	ТекстИтоговГостя.Вставить(6, "За сумму покупки");
	
	ТекстИтоговКассира = Новый Соответствие;
	ТекстИтоговКассира.Вставить(1, "За 100г/мл");
	ТекстИтоговКассира.Вставить(2, "За акции");
	ТекстИтоговКассира.Вставить(3, "За ЛУЧ");
	ТекстИтоговКассира.Вставить(4, "За ПП");
	ТекстИтоговКассира.Вставить(5, "Совм. акции");
	ТекстИтоговКассира.Вставить(6, "Сумма пок.");
	
	НовыеИтоги = Новый Массив;
	НовыеИтогиМ = Новый Массив;
	НовыеИтогиО = Новый Массив;
	НовыеИтогиКП = Новый Массив;
	Для ТипАкции = 1 По 6 Цикл
		ТекущиеКГЛ = 0;
		Для каждого ТекущаяСтрока Из Данные.Итоги.КГЛИнфо Цикл
			Если ТипАкции <> ТекущаяСтрока.ТипАкции Тогда
				Продолжить;
			КонецЕсли;
			ТекущиеКГЛ = ТекущиеКГЛ + ТекущаяСтрока.КГЛ;
		КонецЦикла;
		НовыеИтоги.Добавить(Новый Структура("ТекстГостя,ТекстСтанции,КГЛ", ТекстИтоговГостя.Получить(ТипАкции), ТекстИтоговКассира.Получить(ТипАкции), ТекущиеКГЛ));
		
		ТекущиеКГЛ = 0;
		Для каждого ТекущаяСтрока Из Данные.ИтогиМ.КГЛИнфо Цикл
			Если ТипАкции <> ТекущаяСтрока.ТипАкции Тогда
				Продолжить;
			КонецЕсли;
			ТекущиеКГЛ = ТекущиеКГЛ + ТекущаяСтрока.КГЛ;
		КонецЦикла;
		НовыеИтогиМ.Добавить(Новый Структура("ТекстГостя,ТекстСтанции,КГЛ", ТекстИтоговГостя.Получить(ТипАкции), ТекстИтоговКассира.Получить(ТипАкции), ТекущиеКГЛ));
		
		ТекущиеКГЛ = 0;
		Для каждого ТекущаяСтрока Из Данные.ИтогиО.КГЛИнфо Цикл
			Если ТипАкции <> ТекущаяСтрока.ТипАкции Тогда
				Продолжить;
			КонецЕсли;
			ТекущиеКГЛ = ТекущиеКГЛ + ТекущаяСтрока.КГЛ;
		КонецЦикла;
		НовыеИтогиО.Добавить(Новый Структура("ТекстГостя,ТекстСтанции,КГЛ", ТекстИтоговГостя.Получить(ТипАкции), ТекстИтоговКассира.Получить(ТипАкции), ТекущиеКГЛ));
		
		Если ТипАкции <> 3 И ТипАкции <> 6 Тогда // Пропускаем ЛУЧ и сумму покупок на КП
			ТекущиеКГЛ = 0;
			Для каждого ТекущаяСтрока Из Данные.ИтогиКП.КГЛИнфо Цикл
				Если ТипАкции <> ТекущаяСтрока.ТипАкции Тогда
					Продолжить;
				КонецЕсли;
				ТекущиеКГЛ = ТекущиеКГЛ + ТекущаяСтрока.КГЛ;
			КонецЦикла;
			НовыеИтогиКП.Добавить(Новый Структура("ТекстГостя,ТекстСтанции,КГЛ", ТекстИтоговГостя.Получить(ТипАкции), ТекстИтоговКассира.Получить(ТипАкции), ТекущиеКГЛ));
		КонецЕсли;
	КонецЦикла;
	
	Данные.Итоги.КГЛИнфо.Очистить();
	Данные.Итоги.КГЛИнфо = НовыеИтоги;
	Данные.ИтогиМ.КГЛИнфо.Очистить();
	Данные.ИтогиМ.КГЛИнфо = НовыеИтогиМ;
	Данные.ИтогиО.КГЛИнфо.Очистить();
	Данные.ИтогиО.КГЛИнфо = НовыеИтогиО;
	Данные.ИтогиКП.КГЛИнфо.Очистить();
	Данные.ИтогиКП.КГЛИнфо = НовыеИтогиКП;
...
Функция ОтправитьЗапросКСервисуРасчетаЧеков(СтрокаJSON, Команда, Тест = Ложь) Экспорт
	...
	//+++
	// ВРЕМЕННАЯ ГОВНОЗАТЫЧКА
	Если ВРег(Команда) = "PRICE_LIST" Тогда
		Регион = ЛояльностьКлиентСервер.ПолучитьТекущийРегион();
		Если Регион = "77" Тогда
			ТекущийСервисРасчетаЧека.АдресСервиса = "http://p77sp.msk.local:89/check_v4/ws/check?wsdl";
		ИначеЕсли Регион = "52" Тогда
			ТекущийСервисРасчетаЧека.АдресСервиса = "http://p52sp.cd.local/check_v4/ws/check?wsdl";
		КонецЕсли;
	КонецЕсли;	
	//---
...
Функция СформироватьJSONДляВозврата_Протокол5(Данные, Тест)
	...
	Если Тест Тогда
		...
		Если ПустаяСтрока(ККМ) Тогда
			Регион = Строка(Лояльность.ПолучитьТекущийРегион());
...
Функция СформироватьJSONДляПовторнойФинализацииЗаказа_Протокол5(Данные, ДанныеЛояльностиПоЗаказу=Неопределено, Тест)
...
#КонецЕсли
	...
	МассивОпераций.Добавить("CHK_DEL");
	МассивОпераций.Добавить("CHK");
	Запрос.Вставить("operations", МассивОпераций);
