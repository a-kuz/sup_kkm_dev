...
Процедура УдалениеДанныхЛояльности() Экспорт
	...
	|	РегистрСведений.ДанныеЛояльностиПоЗаказу КАК ДанныеЛояльностиПоЗаказу
	|ГДЕ
	|	ДанныеЛояльностиПоЗаказу.Документ.Дата МЕЖДУ &НачПериода И &КонПериода";
	Запрос.УстановитьПараметр("НачПериода", НачалоДня(Период));
	Запрос.УстановитьПараметр("КонПериода", КонецДня(Период));
	РезультатЗапроса = Запрос.Выполнить().Выбрать();
...
Процедура УстановкаПараметровЗадания() Экспорт
...
Процедура ОтменитьВыполнениеЗадания(Знач ИдентификаторЗадания) Экспорт 	
	...
	Попытка
		Задание.Отменить();
	Исключение
		ЗаписьЖурналаРегистрации("Лояльность.Общие",УровеньЖурналаРегистрации.Предупреждение, , ,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;	
КонецПроцедуры
...
ПОПЫТКА
	...
	ИначеЕсли Запрос = "RUN" Тогда
		Возврат ВыполнитьКомандуПоЗапросу(ДанныеЗапроса);
	Иначе
		ВызватьИсключение "Неверный запрос";
	КонецЕсли;
...
Функция ВыполнитьКомандуПоЗапросу(ДанныеЗапроса)
