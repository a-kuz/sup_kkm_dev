...
Функция Подключиться(ServerName, UID="sa", PWD="ser09l", DB = Неопределено) Экспорт
    ...
    Исключение
		...
		ЗарегистрироватьСобытие("Подключение к базе", УровеньЖурналаРегистрации.Ошибка,,,ТекстСообщения + Символы.ПС + "ConnectString = " + ConnectString);
...
Функция ПодключитьсяКloyality_ext() Экспорт 
	...
	Если ServerName = Неопределено Тогда
		...
		ЗарегистрироватьСобытие("Подключение к базе", УровеньЖурналаРегистрации.Ошибка,,,ОшибкаТекст);
...
Функция ПодключитьсяКsup_kkm(ИнформационнаяБаза) Экспорт
	...
	DB 			= РегистрыСведений.ДополнительныеСвойства.ЗначениеСвойства(ИнформационнаяБаза, "БазаSQL");
