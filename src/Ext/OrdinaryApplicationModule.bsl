...
Функция ЗапросОперативнойРеализации(Дата, имяМестаРеализации, ВернутьОбъект = Ложь) Экспорт
	...
	Если ВернутьОбъект Тогда
		ТаблицаРеализации.Свернуть("СтавкаНДС, КодСУП, ТипОплаты, НомерККМ, Фирма, ФирмаКодТТ", "Количество, СуммаРозн, СуммаФакт, СуммаНДС");
		Для Каждого Т Из ТаблицаРеализации Цикл
			Т.ФирмаКодТТ =  Формат(Т.ФирмаКодТТ, "ЧРД=.; ЧГ=0");
		КонецЦикла;
	Иначе
		...
		Для Каждого Т Из ТаблицаРеализации Цикл
			...
			Т.Фирма =  Формат(Т.Фирма.КодТТ, "ЧРД=.; ЧГ=0");
		КонецЦикла;
	КонецЕсли;
