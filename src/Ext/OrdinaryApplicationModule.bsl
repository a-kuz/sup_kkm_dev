...
Перем глОтладкаУровень Экспорт;
...
Функция ЗаполнитьПользователейПоУмолчанию()
	...
	Нов.МестоРеализации = Справочники.МестаРеализации.НайтиПоНаименованию("Мяснов");
	Нов.НаборПрав = Справочники.НаборыПравДоступа.НайтиПоНаименованию("Администратор");
	
	Нов = Сотр.ПраваДоступа.Добавить();
	Нов.МестоРеализации = Справочники.МестаРеализации.НайтиПоНаименованию("Отдохни");
	Нов.НаборПрав = Справочники.НаборыПравДоступа.НайтиПоНаименованию("Администратор");
	Сотр.Записать(); 
...
Функция ОбработатьПараметрыЗапуска(Знач стрКоманда, Отказ = Ложь)
	...
	Если ЗначениеЗаполнено(стрКоманда) Тогда
		...
		Если Найти(стрКоманда, "ОбновитьТовары") Тогда
			...
			ЗаписьЖурналаРегистрации("Вызов команды из СУП ТТ.Ответ",УровеньЖурналаРегистрации.Информация, , , стрРезультат);
		...
		ИначеЕсли Найти(стрКоманда, "{") Тогда
			Попытка
				...
				ИначеЕсли СтруктураКоманды.Команда = "ПрогрузитьВесы" Тогда
					...
					КонецПопытки;
				КонецЕсли;
				ЗаписьЖурналаРегистрации("Вызов команды из СУП ТТ.Ответ",УровеньЖурналаРегистрации.Информация, , , Результат);
				Возврат Результат;
			Исключение
				ЗаписьЖурналаРегистрации("Вызов команды из СУП ТТ.Ответ",УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				Возврат "{""Результат"":0, ""ОписаниеОшибки"":"""+ОписаниеОшибки()+"""}";
			КонецПопытки;
		ИначеЕсли стрКоманда = "ВыполнитьОбменДанными" Тогда
			глВыполнитьОбменДаннымиРИБ();
			ЗаписьЖурналаРегистрации("Запуск обмена через параметр запуска",УровеньЖурналаРегистрации.Информация, , , Результат);
			Сообщить("Обмен закончен");
		ИначеЕсли СтрНачинаетсяС(стрКоманда, "Выполнить") Тогда
			Отказ = Ложь;
			ЗаписьЖурналаРегистрации("Выполнение кода через параметр запуска",УровеньЖурналаРегистрации.Информация, , , стрКоманда);
...
Функция ВыполнитьКомандуИзСупТТ(стрКоманда) Экспорт
	ЗаписьЖурналаРегистрации("Вызов команды из СУП ТТ.Команда",УровеньЖурналаРегистрации.Информация, , , стрКоманда);
...
Функция ЗапросОперативнойРеализации(Дата, имяМестаРеализации, ВернутьОбъект = Ложь) Экспорт
	...
	ТаблицаРеализации.Свернуть("СтавкаНДС, КодСУП, ТипОплаты, НомерККМ", "Количество, СуммаРозн, СуммаФакт, СуммаНДС");
