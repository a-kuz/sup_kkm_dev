...
Перем глВремяОтсечкиПростоя Экспорт;
////////////////////////////////////////////////////////////////////////////////
...
Процедура глУправлениеОкнами() Экспорт
	...
	ИначеЕсли ТекущееОкно.Форма.ПараметрыОкна.Свойство("Обрезать") Тогда
			...
			КонецЕсли;
			ОбрезкаОкнаШирина = 0;
			Если Не ПараметрыОкна.Свойство("ОбрезкаОкнаШирина", ОбрезкаОкнаШирина) Тогда
				ОбрезкаОкнаШирина = 0;
			КонецЕсли;
			ИнтерфейсРМ.ОбрезкаОкна(Заголовок, ОбрезкаОкнаШирина);
...
Процедура глОбработкаОчередиСобытий() Экспорт
	Если ЗначениеЗаполнено(глРабочееМесто) Тогда
		Если глСтекОкон <> Неопределено Тогда
			Если глСтекОкон.Количество() <= 1 Тогда
				ИнтерфейсРМ.ОбработкаОчередиСобытий(глРабочееМесто);	// обработка событий для текущего РМ	
			КонецЕсли;
		КонецЕсли;
	КонецЕсли; 
КонецПроцедуры
		...
		ИначеЕсли Найти(стрКоманда, "{") Тогда
			Попытка
				...
				Если СтруктураКоманды.Команда = "ОткрытьСмену" Тогда
					ЕстьКП = Справочники.ГруппыИнформационныхБаз.ГруппыПоИБ(ПараметрыСеанса.ТекущаяИБ).Найти(Справочники.ГруппыИнформационныхБаз.Рестораны) <> Неопределено;
					Если ЕстьКП И СтрНайти(НРег(СтруктураКоманды.ТТ),"Мяснов") Тогда
						// открываем смену МЪ+КП
						НачатьТранзакцию(РежимБлокировкиДанных.Разделяемый);
						
						РезультатМЪ = ОткрытьСмену(СтруктураКоманды);
						СтруктураКомандыКП = СкопироватьСтруктуру(СтруктураКоманды);
						СтруктураКомандыКП.ТТ = "Ресторан";
						
						РезультатКП = ОткрытьСмену(СтруктураКомандыКП);	
						Результат = Новый Структура;
						Результат.Вставить("Результат", РезультатКП.Результат * РезультатМЪ.Результат);
						Результат.Вставить("ОписаниеОшибки", РезультатМЪ.ОписаниеОшибки + ?(ЗначениеЗаполнено(РезультатКП.ОписаниеОшибки) И ЗначениеЗаполнено(РезультатМЪ.ОписаниеОшибки),Символы.ПС,"") + РезультатКП.ОписаниеОшибки);
				...
				ИначеЕсли СтруктураКоманды.Команда = "ПрогрузитьВесы" Тогда
					...
					Исключение
						Результат = "{""Результат"":0, ""ОписаниеОшибки"":"""+ОписаниеОшибки()+"""}";
					КонецПопытки;
				ИначеЕсли СтруктураКоманды.Команда = "ПроверкаСвязиВесы" Тогда
					ОбработкаВесы = Обработки.ПрогрузкаВесов.Создать();
					Попытка
						Ответ = ОбработкаВесы.ПроверкаСвязи(СтруктураКоманды.ИПВесов);
						Результат = "{""Результат"":""" + Ответ.Ошибка + """, ""ОписаниеОшибки"":"""+Ответ.ТекстОшибки+"""}";;
					Исключение
						Результат = "{""Результат"":0, ""ОписаниеОшибки"":"""+ОписаниеОшибки()+"""}";
					КонецПопытки;
				ИначеЕсли СтруктураКоманды.Команда = "ПечатьЦенников" Тогда
					Результат = ЛояльностьКлиентСервер.ПолучитьДанныеДляЦенников(СтруктураКоманды);
				ИначеЕсли СтруктураКоманды.Команда = "ЛояльностьРежимОффлайн" Тогда
					Результат = Лояльность.ИзменитьРежимОффлайн(СтруктураКоманды);
				//{{УК_НФ СА 20170928 
				ИначеЕсли СтруктураКоманды.Команда = "ОтчетыСУПУП" Тогда
					Результат = СУПУП.ПолучитьОтчет(стрКоманда);
				//}}УК_НФ СА 20170928
				ИначеЕсли СтруктураКоманды.Команда = "ЗакрытьСлужебныйЗаказ" Тогда
					Результат = ЗакрытьСлужебныйЗаказ(СтруктураКоманды);
				КонецЕсли;
...
// Закрывает заказ, открытый по "служебному" бейджу
//
// Параметры:
//		СтруктураКоманды - Структура - пока не используется
//
// Возвращаемое значение:
//		ТаблицаЗначений - НомерЗаказа, Товар, Количество - Таблица содержит служебные заказы за сутки
//
Функция ЗакрытьСлужебныйЗаказ(СтруктураКоманды) Экспорт
	
	Запрос = Новый Запрос("ВЫБРАТЬ
	|	ЗаказТовары.Товар.КодСУП КАК Товар,
	|	ЗаказТовары.Количество КАК Количество,
	|	ШапкаЗаказ.Номер КАК НомерЗаказа,
	|	ШапкаЗаказ.Ссылка КАК Ссылка,
	|	ЗаказДопИнф.Статус КАК Статус
	|ИЗ
	|	Документ.Заказ КАК ШапкаЗаказ
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.Заказ.Товары КАК ЗаказТовары
	|		ПО ШапкаЗаказ.Ссылка = ЗаказТовары.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КартыДоступа КАК КартыДоступа
	|		ПО ШапкаЗаказ.КартаДоступа = КартыДоступа.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ЗаказДопИнф КАК ЗаказДопИнф
	|		ПО ШапкаЗаказ.Ссылка = ЗаказДопИнф.Заказ
	|ГДЕ
	|	КартыДоступа.СлужебныйБейдж = 1
	|	И ШапкаЗаказ.Дата > &ДатаНач");	

	Запрос.УстановитьПараметр("ДатаНач", ТекущаяДата() - 86400);
	ТЗ = Запрос.Выполнить().Выгрузить();	
	СтрокиОткрытыеЗаказы = ТЗ.НайтиСтроки(Новый Структура("Статус", Перечисления.СтатусыЗаказа.Открыт));
	Для Каждого СтрокаОткрытыйЗаказ Из СтрокиОткрытыеЗаказы Цикл
		УстановитьАтрибутЗаказа(СтрокаОткрытыйЗаказ.Ссылка, "Статус", Перечисления.СтатусыЗаказа.Закрыт);
		ксТрактир.ОсвободитьКартуДоступаОтЗаказа(СтрокаОткрытыйЗаказ.Ссылка);
		Заказ = СтрокаОткрытыйЗаказ.Ссылка.ПолучитьОбъект();
		Заказ.Смена = Неопределено;
	КонецЦикла;
	
	тзРезультат = ТЗ.Скопировать(,"НомерЗаказа, Товар, Количество");
	ОписаниеОшибки = "";
	Результат = Новый Структура("Результат, ОписаниеОшибки, ТаблицаРеализации", 1, ОписаниеОшибки, тзРезультат);
...
// Параметры:
//  СтруктураКоманды - Структура
//		ТТ 	- Строка - имяМестаРеализации
//		Дата	- Строка - дата в формате "дд.мм.гггг"
// Возвращаемое значение 
//		Структура
//			Результат - Число - 1: ОК; 0: неОК
//			ОписаниеОшибки
Функция ОткрытьСмену(Знач СтруктураКоманды)
	
	Перем Док, МенеджерЗаписи, НЗ, ТТ;
	
	РезультатОткрытия = Новый Структура("Результат, ОписаниеОшибки");
