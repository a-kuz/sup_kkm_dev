...
Процедура ПриНачалеРаботыСистемы()
	...
	Если Не ПараметрыСеанса.ТекущаяИБ.Предопределенный Тогда
		...
		Иначе
			...
			ДатаМин = НачалоДня(ДобавитьДень(ТекущаяДата(),-7));
	...
	Если ЗначениеЗаполнено(стрКоманда) Тогда
		...
		ИначеЕсли Найти(стрКоманда, "{") Тогда
			Попытка
				...
				//{{УК_НФ СА 20170928 
				ИначеЕсли СтруктураКоманды.Команда = "Отчет" Тогда
				...
				Иначе
					...
					КонецПопытки;
				КонецЕсли;
				ЗаписьЖурналаРегистрации("Вызов команды из СУП ТТ.Ответ",УровеньЖурналаРегистрации.Информация, , , стрКоманда + Символы.ПС + стрРезультат);
				Возврат Результат;
			Исключение
...
Функция ЗапросОперативнойРеализации(Дата, имяМестаРеализации, ВернутьОбъект = Ложь) Экспорт
	...
	Если ВернутьОбъект Тогда
		...
		ТаблицаРеализации.Свернуть("СтавкаНДС, КодСУП, ТипОплаты, НомерККМ, Фирма", "Количество, СуммаРозн, СуммаФакт, СуммаНДС");
