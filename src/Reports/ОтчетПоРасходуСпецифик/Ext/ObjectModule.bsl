...
#Если Клиент Тогда
	...
	Процедура СформироватьОтчет(ТабличныйДокумент = "", НаПринтер = Ложь, ВарФормирования = 0, ЭлементыФормы = "") Экспорт
		...
		Заголовок = "ОТЧЕТ ПО РАСХОДУ СПЕЦИФИК" + Символы.ПС + Период;
		...
		ГруппировкаДоп = ГруппировкаРодитель.Структура.Добавить(Тип("ГруппировкаКомпоновкиДанных"));
		ПолеГруппировки                = ГруппировкаДоп.ПоляГруппировки.Элементы.Добавить(Тип("ПолеГруппировкиКомпоновкиДанных"));
		ПолеГруппировки.Использование  = Истина;
		ПолеГруппировки.ТипГруппировки = ТипГруппировкиКомпоновкиДанных.Элементы;
		Если  ВариантыФормирования = "ПоБлюдам" Тогда
			ПолеГруппировки.Поле           = Новый ПолеКомпоновкиДанных("Товар");
		Иначе
			ПолеГруппировки.Поле           = Новый ПолеКомпоновкиДанных("Специфика");
		КонецЕсли;	
		НовоеВыбранноеПоле = ГруппировкаДоп.Выбор.Элементы.Добавить(Тип("АвтоВыбранноеПолеКомпоновкиДанных"));
				
		ГруппировкаДоп2 = ГруппировкаДоп.Структура.Добавить(Тип("ГруппировкаКомпоновкиДанных")); 
		ПолеГруппировки                = ГруппировкаДоп2.ПоляГруппировки.Элементы.Добавить(Тип("ПолеГруппировкиКомпоновкиДанных"));
		ПолеГруппировки.Использование  = Истина;
		ПолеГруппировки.ТипГруппировки = ТипГруппировкиКомпоновкиДанных.Элементы;
		Если  ВариантыФормирования = "ПоБлюдам" Тогда
			ПолеГруппировки.Поле           = Новый ПолеКомпоновкиДанных("Специфика");
		Иначе
			ПолеГруппировки.Поле           = Новый ПолеКомпоновкиДанных("Товар");
		КонецЕсли;	
		НовоеВыбранноеПоле = ГруппировкаДоп2.Выбор.Элементы.Добавить(Тип("АвтоВыбранноеПолеКомпоновкиДанных")); 
		
		Если РасшифровыватьПоДокументам Тогда
			ГруппировкаПоЗаказу = ГруппировкаДоп2.Структура.Добавить(Тип("ГруппировкаКомпоновкиДанных")); 
		...
		ВыводПроцента = РасшифровыватьПоДокументам+ГруппироватьПоПериоду;
		КомпоновщикНастроек.Настройки.УсловноеОформление.Элементы[1].Использование = ВыводПроцента <> 0;
		КомпоновщикНастроек.Настройки.УсловноеОформление.Элементы[2].Использование = ВыводПроцента = 2;
		...
		Если Сортировка = 1 ИЛИ Сортировка = 2 Тогда
			 КомпоновщикНастроек.Настройки.Порядок.Элементы[Сортировка+2].Использование = Истина;
        КонецЕсли;
        КомпоновщикНастроек.Настройки.Порядок.Элементы[5].Использование = Истина;
	...
	Процедура СформироватьДиаграмму() Экспорт
		...
		Поле.Поле = Новый ПолеКомпоновкиДанных(?(ГруппироватьПоПериоду,ВидПериода, "Специфика" ));
	...
	Процедура ПечататьНаПринтере(Смена) 
		...
		Запрос.УстановитьПараметр("ПустаяСсылка", Справочники.Специфики.ПустаяСсылка());
		...
		Если ВариантыФормирования = "ОбщийРасход" Тогда   
		...
		Иначе
			Для Каждого Строка Из Выгрузка.Строки Цикл
				...
				НоваяСтрока.Данные    = Врег(Строка[ВариантыФормирования]);
		...
		ФормированиеОтчетов.ПечатьОтчетаРМ(Задание, "Расход специфик", "", Шапка);
