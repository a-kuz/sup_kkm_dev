		...
		//Если ЗначениеЗаполнено(Ссылка.НомерКартыЛояльности) Тогда
			Если Ссылка.НомерКартыЛояльности <> НомерКартыЛояльности Тогда
				Если Ссылка.Товары.Найти(1, "СтатусОплаты") <> Неопределено Тогда
					...
					Если глОтладкаУровень = Истина И Не РаботаСокнами = Неопределено Тогда
						//:РаботаСокнами = Обработки.РаботаСокнами.Создать();
						РаботаСокнами.ПоказатьПлашку("Ошибка", "Попытка изменить НомерКартыЛояльности в заказе с оплаченными строками");
					КонецЕсли;
					ЗаписьЖурналаРегистрации("Попытка изменить НомерКартыЛояльности в заказе с оплатой",УровеньЖурналаРегистрации.Ошибка,,Ссылка,"" + Ссылка.НомерКартыЛояльности + " -> " + НомерКартыЛояльности);
				КонецЕсли;
			КонецЕсли;
		//КонецЕсли;
	КонецЕсли;
