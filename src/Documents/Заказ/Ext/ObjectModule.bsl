...
Функция идСтроки(Префикс = "") Экспорт
	стрУИД = СтрЗаменить(Строка(Новый УникальныйИдентификатор), "-", "");
	ДД = ПолучитьДвоичныеДанныеИзHexСтроки(стрУИД);	
	B64 = Base64Строка(ДД);
	B64 = СтрЗаменить(B64, "==","");
	//Получается 22 символа (но это не еще точно, могут быть вариации). идСтроки - 30 символов
	Возврат Прав(Строка(Префикс) + B64, 30);
КонецФункции
...
Функция ЗаписатьЗаказТоварыДопИнф() Экспорт
	...
	Для Каждого Т Из Товары Цикл
		...
		Иначе
			...
			Если ЗначениеЗаполнено(ЗаписьРегистра.ВремяЗаказано) Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ЗаписьРегистра.Статус) Тогда
		ИначеЕсли ЗначениеЗаполнено(Т.Статус) Тогда
...
// Возвращает значение ресурса/реквизита из РС ЗаказДопИнф
// Обертка АтрибутыЗаказа()
//
// Параметры:
...
//
Функция ДопИнф(Атрибут) Экспорт
	Возврат АтрибутыЗаказа(Ссылка, Атрибут)[Атрибут];
КонецФункции
...
// 	Произвольный - значение атрибута
//
Функция ПолучитьАтрибут(Атрибут) Экспорт
	Возврат АтрибутыЗаказа(Ссылка, Атрибут)[Атрибут];
КонецФункции
...
// 	Неопределено
//
Функция УстановитьАтрибут(Атрибут, Значение) Экспорт
	УстановитьАтрибутЗаказа(Ссылка, Атрибут, Значение);
КонецФункции
