...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Если не Отказ Тогда//и не ПроцедурыОбменаДанными.ОпределитьЭтаИнформационнаяБазаФайловая() Тогда
		Если глПараметрыРМ = Неопределено или НЕ глПараметрыРМ.ККМЕсть Тогда
			...
			Исключение
				ЗарегистрироватьСобытие("Ошибка записи файла закрытия смены кассы",УровеньЖурналаРегистрации.Ошибка,ЭтотОбъект,,"Ошибка доступа до " + Профиль);
			КонецПопытки;
			ИмяФайла = ПолучитьИмяВременногоФайла("txt");
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		Если Не ПисьмоОтправлено И фРучноеЗакрытие Тогда
			...
			Исключение
				ЗарегистрироватьСобытие("Ручное закрытие кассовой смены",УровеньЖурналаРегистрации.Ошибка,Метаданные.Документы.Касса_ОткрытиеСмены,СменаКассы,"Ошибка при отправке оповещения на почту
				|" + ОписаниеОшибки());
			КонецПопытки;
		КонецЕсли;                          		
	КонецЕсли;
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение и не ПроцедурыОбменаДанными.ОпределитьЭтаИнформационнаяБазаФайловая() Тогда
		...
		Если Коннект = Неопределено Тогда
			ЗарегистрироватьСобытие("Подключение к базе", УровеньЖурналаРегистрации.Ошибка,,?(ЭтоНовый(),СменаКассы , Ссылка),"ошибка подключения к серверу при записи открытия смены");
		Иначе
			ОтказЗаписи = ложь;
			ТекстОшибки = "";
			...
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"&КодТТ",Формат(СменаКассы.Касса.Фирма.КодТТ,"ЧН=0; ЧГ=0"));
			...
			Если ОтказЗаписи Тогда 
				ЗарегистрироватьСобытие("Запись закрытия смены", УровеньЖурналаРегистрации.Ошибка,,?(ЭтоНовый(),СменаКассы , Ссылка),ТекстОшибки + "" + ТекстЗапроса);
			КонецЕсли;
			...
			иначе
				...
				Если ТаблицаРезультат.Количество() <> 0 Тогда 
					ИдЗаписи = Формат(ТаблицаРезультат[0].ИдСтроки,"ЧН=0; ЧГ=0");
				Иначе 
					ЗарегистрироватьСобытие("Запись закрытия смены", УровеньЖурналаРегистрации.Ошибка,,?(ЭтоНовый(),СменаКассы , Ссылка),"не смог найти записть смены в кассовых показателях после записи" + "" + ТекстЗапроса);
					Возврат;
				КонецЕсли;
			КонецЕсли;
