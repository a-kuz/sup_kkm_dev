...
Функция ПрименитьПланМеню() Экспорт
	...
	Если НЕ ЗначениеЗаполнено(КаталогТоваров) Тогда
		...
		Если Выборка.Количество() = 1 Тогда
			...
			ЗаписьЖурналаРегистрации("План меню. Предупреждение", УровеньЖурналаРегистрации.Предупреждение, , , "В реквизите ""каталог товаров"" заменено пустое значение на " + КаталогТоваров);
		КонецЕсли;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(КатегорияЦен) И глВерсия > 1 И ЗначениеЗаполнено(Склад) И ЗначениеЗаполнено(Склад.ТипЦен) Тогда
		...
		ЗаписьЖурналаРегистрации("План меню. Предупреждение", УровеньЖурналаРегистрации.Предупреждение, , , "В реквизите ""тип цены"" заменено пустое значение на " + КатегорияЦен);
	КонецЕсли;
...
Процедура ОшибкаПрименения(СтрокаОписания)
	...
	ЗаписьЖурналаРегистрации("ПланМеню: Ошибка применения", УровеньЖурналаРегистрации.Ошибка, , , Строка(ЭтотОбъект) + Символы.ПС + СтрокаОписания);
