...
Процедура СписокРМПриАктивизацииСтроки(Элемент)
	...
	Если ПустаяСтрока(Хост) Тогда
		Хост = ИмяКомпьютераИзПрофиля();
	КонецЕсли;
	Модифицированность = Ложь;
//	РазблокироватьДанныеДляРедактирования(СправочникОбъект);
КонецПроцедуры
...
Функция ИмяКомпьютераИзПрофиля() Экспорт
	
	ирПлатформа = ирПлатформа;
	Если ирПлатформа = Неопределено Тогда
		ирПлатформа = ирКэш.Получить();
	КонецЕсли;
	V8RegEx = ирПлатформа.ПолучитьОбъектВнешнейКомпонентыИзМакета("V8Rex","V8RegEx","V8RegEx");
	V8RegEx.Шаблон = "\\\\([^\\]+)";
	Совпадения = V8RegEx.Выполнить(ПрофильВхода);
	Если Совпадения.Количество Тогда
		локХост = Совпадения[0].Подгруппы[0];
	КонецЕсли;
	Возврат локХост;
КонецФункции

Процедура ПрофильВходаПриИзменении(Элемент)
	Хост = ИмяКомпьютераИзПрофиля();
КонецПроцедуры

Процедура СписокРМПередУдалением(Элемент, Отказ)
	СправочникОбъект = Неопределено;
КонецПроцедуры

Процедура СписокРМПередУстановкойПометкиУдаления(Элемент, Отказ)
	СправочникОбъект.Разблокировать();
	РазблокироватьДанныеДляРедактирования(ТекущееРМ);
	СправочникОбъект = Неопределено;
