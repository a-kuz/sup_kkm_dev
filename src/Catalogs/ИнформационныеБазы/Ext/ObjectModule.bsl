...
Функция СоздатьРМ(ИмяРМ, ИмяХоста, ИмяПользователя, МестоРеализации, КодКассы, Знач ЗаводскойНомерККМ, РабочееМесто = Неопределено, ПараметрыПилот = Неопределено) Экспорт
	...
	Если ПараметрыПилот = Неопределено Тогда
		ПараметрыПилот = Новый Структура("ЭтоВива, ПортФР, ПортСканера, ПортДП");
	КонецЕсли;
	...
	РМ.Фирма = Фирма(МестоРеализации);
	РМ.МестоРеализации = МестоРеализации;   
	РМ.Тип = Перечисления.ТипыРМ.СтанцияОплаты;
	РМ.Станция = ?(МестоРеализации = Справочники.МестаРеализации.Мяснов, Справочники.Станции.НайтиПоНаименованию("Мясо"), Справочники.Станции.НайтиПоНаименованию("Отдохни"));
	РМ.Записать();
	...
	ДП = ДП;
	Выб = Справочники.ТорговоеОборудование.Выбрать(Справочники.ТорговоеОборудование.ЭталонныйНабор);
	Пока Выб.Следующий() Цикл
		...
		Если Выб.Ссылка = Справочники.ТорговоеОборудование.ККТ Тогда
		...
		КонецЕсли;
		Если Выб.Ссылка = Справочники.ТорговоеОборудование.ПР410 Тогда
			Если ЗаводскойНомерККМ <> КодКассы Тогда
				Продолжить;
			КонецЕсли;
			обТО.КодСУП = КодКассы;
		...
		КонецЕсли;
		обТО.Записать();
		Если Выб.Ссылка = Справочники.ТорговоеОборудование.СканерШК Тогда
			...
			Если ЗначениеЗаполнено(ПараметрыПилот.ПортСканера) Тогда
				...
				обТО.Записать();
			КонецЕсли;
		ИначеЕсли Выб.Ссылка = Справочники.ТорговоеОборудование.РидерКарт Тогда
			Если ПараметрыПилот.ЭтоВива = Ложь Тогда
				обТО.Наименование = ИмяХоста + " | Ридер (+autohotkey)";
				обТО.Записать();
			КонецЕсли;
			СписокСУ.Добавить(обТО.Ссылка);
		ИначеЕсли Выб.Ссылка = Справочники.ТорговоеОборудование.ККТ Или Выб.Ссылка = Справочники.ТорговоеОборудование.ПР410 Тогда
			ККМ = обТО.Ссылка;
			Если ЗначениеЗаполнено(ПараметрыПилот.ПортФР) Тогда
				...
				обТО.Записать();
			КонецЕсли;
		ИначеЕсли Выб.Ссылка = Справочники.ТорговоеОборудование.ПлатежнаяСистема Тогда
	...
	ПараметрыРМ.Вставить("ККМЕсть", Истина);
	ПараметрыРМ.Вставить("ДПЕсть", 	Истина);
	...
	ПараметрыРМ.Вставить("БлокировкаАвтоВремя", 45);
...
Процедура ЗаполнитьПоНомеруТТ(НомерТТ, СтандартныеПорты = Ложь) Экспорт
	...
	Фирмы.Свернуть("Фирма_Адрес, Фирма_Наименование, Фирма_НаименованиеЮридическогоЛица, Фирма_ИНН, Фирма_КПП, МестоРеализации, КодТТ");
	...
	Для Каждого ДанныеФирмы Из Фирмы Цикл
		...
		Для НомерККМвТТ = 1 По МС.Количество()+1 Цикл
			...
			РабочееМесто 	= ОбщегоНазначенияПовтИсп.НайтиПоРеквизиту("Справочник.РабочиеМеста", "ПрофильВхода", Профиль, Ложь, , ГруппаРМ());
