...
Функция СоздатьРМ(ИмяРМ, ИмяХоста, ИмяПользователя, МестоРеализации, КодКассы, Знач ЗаводскойНомерККМ, РабочееМесто = Неопределено, ПараметрыПилот = Неопределено) Экспорт
	...
	Если СтрНайти(ИмяХоста,"ss") Тогда
		ВариантыТиповРМ = Массив(Перечисления.ТипыРМ.СтанцияСканирования);
	ИначеЕсли СтрНайти(ИмяХоста,"ak") Тогда
		ВариантыТиповРМ = Массив(Перечисления.ТипыРМ.Автокасса);
	ИначеЕсли СтрНайти(ИмяХоста,"sc") Тогда
		ВариантыТиповРМ = Массив(Перечисления.ТипыРМ.СтанцияПовараМОКП);
	ИначеЕсли СтрНайти(ИмяХоста,"k") Тогда
		Если ТипТТ = Справочники.ТипыТТ.КМ Тогда
			ВариантыТиповРМ = Массив(Перечисления.ТипыРМ.СтанцияОплатыКМ);
		ИначеЕсли ТипТТ = Справочники.ТипыТТ.МОКП Тогда
			ВариантыТиповРМ = Массив(Перечисления.ТипыРМ.СтанцияОплатыМОКП);
		Иначе
	...
	Если ВариантыТиповРМ.Количество() > 1 Тогда
		...
		Выб = СЗ.ВыбратьЭлемент("Тип РМ " + ИмяРМ, ВариантыТиповРМ[0]);
		Если Выб = Неопределено Тогда
			Возврат Неопределено;
		КонецЕсли;
		ТипРМ = Выб.Значение;
		Станция = Справочники.Станции.ПустаяСсылка();
		Если ТипРМ = Перечисления.ТипыРМ.СтанцияОплаты ИЛИ ТипРМ = Перечисления.ТипыРМ.СтанцияОплатыКМ Или ТипРМ = Перечисления.ТипыРМ.СтанцияОплатыМОКП Тогда
			Станция = ?(СтрНайти(ИмяХоста, "m"), Справочники.Станции.НайтиПоНаименованию("Мясо"), Справочники.Станции.НайтиПоНаименованию("Отдохни"));
		ИначеЕсли ТипРМ = Перечисления.ТипыРМ.СтанцияСканирования Тогда
	...
	Пока Выб.Следующий() Цикл
		...
		ИначеЕсли Выб.КодМодели = "РидерМК" Тогда
			...
			КонецЕсли;
			СписокСУ.Добавить(обТО.Ссылка);
		ИначеЕсли СтрНайти(Выб.КодМодели, "Z2") Тогда
			СписокСУ.Добавить(обТО.Ссылка);
		...
		ИначеЕсли Выб.КодВида = "ЭЛВЕСЫ" Тогда
			Весы = обТО.Ссылка; 			
		Иначе
			обТО.Записать();
		КонецЕсли;
	...
	ПараметрыРМ.Вставить("ВесыЕсть", ЗначениеЗаполнено(Весы));
	ПараметрыРМ.Вставить("Весы", Весы);
