...
Процедура МастерСозданияРМ(МестоРеализации, ИмяПользователя)
	...
	ИмяРМ = Наименование + " | " + НРег(стрИмяХоста);
	РМ = Справочники.РабочиеМеста.НайтиПоНаименованию(ИмяРМ,Ложь);
	Если РМ.Пустая() Тогда
		СоздатьРМ(ИмяРМ,стрИмяХоста,ИмяПользователя,МестоРеализации,"","");	
	Иначе
		Если Вопрос("РМ " + ИмяРМ + " уже есть в справочнике!", РежимДиалогаВопрос.ДаНет) = КодВозвратаДиалога.Да Тогда
			ККМ = РМ.ПолучитьОбъект().ОсновнаяККМ();
			//:ККМ = Справочники.ТорговоеОборудование.ПустаяСсылка();
			СоздатьРМ(ИмяРМ,стрИмяХоста,ИмяПользователя,МестоРеализации,ККМ.КодСУП,ККМ.ЗаводскойНомер,РМ);	
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ДействияФормыЗаполнитьПоДаннымСУПцентра(Кнопка)
	...
	ЗаполнитьПоНомеруТТ(ДанныеДляЗаполнения.НомерТТ, ДанныеДляЗаполнения.СтандартныеПорты, ДанныеДляЗаполнения.СоздатьРабочиеМеста );
...
Процедура КоманднаяПанель1Ресторан(Кнопка)
	МР = Справочники.МестаРеализации.Ресторан;
	МастерСозданияРМ(МР, "myasnov-pos");
КонецПроцедуры

Процедура ДействияФормыДействие4(Кнопка)
	// Вставить содержимое обработчика.
КонецПроцедуры

Процедура ДействияФормыОткрытьУправляемуюФорму(Кнопка)
	//ОткрытьФорму("Справочник.ИнформационныеБазы.Форма.ФормаЭлементаУправляемая", Новый Структура("ключ", Ссылка));
	COMСеанс = ирКэш.ПолучитьСеансТонкогоКлиентаЛкс("СлужебныйКиоск");
	ВнешняяСсылка = COMСеанс.NewObject("СправочникСсылка.ИнформационныеБазы", COMСеанс.NewObject("УникальныйИдентификатор", "" + ссылка.УникальныйИдентификатор()));
	ВнешняяСтруктура = COMСеанс.NewObject("Структура");
	ВнешняяСтруктура.Вставить("Ключ", ВнешняяСсылка);
	
	COMСеанс.ОткрытьФорму("Справочник.ИнформационныеБазы.Форма.ФормаЭлементаУправляемая", ВнешняяСтруктура);
