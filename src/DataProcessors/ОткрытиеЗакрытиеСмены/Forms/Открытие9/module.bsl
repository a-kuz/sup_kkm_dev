...
Процедура ПередОткрытием(Отказ, СтандартнаяОбработка)
	...
	СписокМестРеализации.Добавить(глПараметрыРМ.ККМ.МестоРеализации);
	
	Если глПараметрыРМ.ККМЕстьДоп тогда
		Попытка
			РазмерСпискаККМ = глПараметрыРМ.ККМСписокДоп.Количество() + 1;
			Для каждого эл из глПараметрыРМ.ККМСписокДоп Цикл
				СписокМестРеализации.Добавить(эл.Значение.МестоРеализации);
			КонецЦикла;
		Исключение
			РазмерСпискаККМ = 1;
		КонецПопытки;
	...
	Для каждого элСписка из СписокМестРеализации Цикл
		
		ТекСмена = ИнтерфейсРМ.ТекущаяСменаЗапрос(элСписка.Значение);
		...
		Иначе
			...
			Запрос.УстановитьПараметр("МестоРеализации", элСписка.Значение);
			...
			ДокОткрытияСмены.МестоРеализации = элСписка.Значение;
	...
	ЭлементыФормы.тМестоРеализации	.Заголовок = элСписка.Значение.Наименование;
