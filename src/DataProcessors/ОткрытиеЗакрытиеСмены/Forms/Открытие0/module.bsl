...
Процедура ПередОткрытием(Отказ, СтандартнаяОбработка)
	...
	//Если НЕ глПараметрыРМ.ККМЕсть И НЕ глПараметрыРМ.ЭтоCallЦентр Тогда
	//	Текст1="Нет доступа!";
	...
	Иначе
		...
		ДокОткрытияСмены.МестоРеализации = глПараметрыРМ.МестоРеализации;
	...
	ЗаполнитьМассивПейджеров();
	Если НЕ ЗначениеЗаполнено(МассивПейджеров) Тогда
		ЭлементыФормы.КнопкаПейджер.Видимость = Ложь;
		ЭлементыФормы.ТаблицаСотрудников.Колонки.Пейджер.Видимость = Ложь;
	КонецЕсли; 
...
Процедура ТаблицаСотрудниковВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Если Колонка.Имя = "Пейджер" Тогда
		ПривязкаПейджера();
	КонецЕсли; 
...
Процедура КнопкаУдалитьСотрудникаНажатие(Элемент)
	
	Если ЗначениеЗаполнено( ЭлементыФормы.ТаблицаСотрудников.ТекущаяСтрока ) Тогда
		ОбработкаВыбораСотрудника( ЭлементыФормы.ТаблицаСотрудников.ТекущаяСтрока.Сотрудник );
	КонецЕсли; 
...
Процедура ОткрытьСмену()
	...
	Если глПараметрыРМ.РежимВыхода <> 0 Тогда     
		ЕстьПравоВыходРМ = Ложь;
		Для Каждого СтрокаТС Из ТаблицаСотрудников Цикл
			Если ИнтерфейсРМ.ПроверкаПраваДоступа("ВыходОС",Ложь,СтрокаТС.Сотрудник) Тогда
				ЕстьПравоВыходРМ = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		
		Если НЕ ЕстьПравоВыходРМ Тогда
			Текст1 = "Открытие смены";
			Текст2 ="Нет ни одного сотрудника 
			|с правом выхода из рабочего места!";
...
Процедура ПривязкаПейджера()

	ТекСтрока = ЭлементыФормы.ТаблицаСотрудников.ТекущаяСтрока;
	Если ТекСтрока = Неопределено Тогда
		Возврат;
	КонецЕсли; 
	
	ВыбранныйПейджер = ИнтерфейсРМ.ВыборИзСписка(МассивПейджеров);
	Если ВыбранныйПейджер = Неопределено Тогда
		Возврат;
	КонецЕсли;
		
	Объект = ТекСтрока.Сотрудник.ПолучитьОбъект();
	Объект.Пейджер = ВыбранныйПейджер;
	
	Если НЕ ИнтерфейсРМ.ПопыткаДействияСОбъектом( Объект, "Объект.Записать()" ) Тогда
		Возврат;
	КонецЕсли; 
	
	ТекСтрока.Пейджер = ВыбранныйПейджер;
