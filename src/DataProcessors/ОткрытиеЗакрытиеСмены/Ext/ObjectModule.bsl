...
Функция ПроверкаНезакрытыхМарок(Молча = Ложь, МассивОшибок = Неопределено) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
	|	Марки.Заказ КАК Заказ
	|ИЗ
	|	РегистрСведений.Марки КАК Марки
	|ГДЕ
	|	Марки.Заказ.Смена = &Смена");

	Запрос.УстановитьПараметр("Смена", ДокОткрытияСмены.Ссылка);
	
	ЕстьМарки = Не Запрос.Выполнить().Пустой();
	Если ЕстьМарки Тогда
		Текст1="Ошибка закрытия смены";
		Текст2="В смене остались заказы с незакрытими марками." + "
		|Закрытие смены не разрешено!";
		Если Не Молча Тогда
			ИнтерфейсРМ.ВопросПредупреждение("Предупреждение",Текст1,Текст2,"","ОК","");
		КонецЕсли;
...
Функция ПроверкаНесданныхПейджеров(Молча = Ложь, МассивОшибок = Неопределено) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ
	|	ВыданныеПейджеры.Заказ КАК Заказ
	|ИЗ
	|	РегистрСведений.ВыданныеПейджеры КАК ВыданныеПейджеры
	|ГДЕ
	|	ВыданныеПейджеры.Заказ.Смена = &Смена");

	Запрос.УстановитьПараметр("Смена", ДокОткрытияСмены.Ссылка);
	
	ЕстьПейджеры = Не Запрос.Выполнить().Пустой();
	Если ЕстьПейджеры Тогда
		Текст1="Ошибка закрытия смены";
		Текст2="В смене остались заказы, по которым не сданы пейджеры!" + "
		|Закрытие смены не разрешено!";
		Если Не Молча Тогда
			ИнтерфейсРМ.ВопросПредупреждение("Предупреждение",Текст1,Текст2,"","ОК","");
		КонецЕсли;
...
Функция ЗакрытьСмену(МассивОшибок = Неопределено) Экспорт
	...
	Если ДокОткрытияСмены.МестоРеализации = Справочники.МестаРеализации.Ресторан Тогда
		ПроверкаНезакрытыхМарок(Молча, МассивОшибок);
		ПроверкаНесданныхПейджеров(Молча, МассивОшибок);
	...
	Если МассивОшибок.Количество() Тогда
		...
		Возврат Ложь;
	КонецЕсли;
	Если Не Молча и не ТранзакцияАктивна() Тогда
		НачатьТранзакцию();
	КонецЕсли;
	ДокЗакрытияСмены = Документы.ЗакрытиеСмены.СоздатьДокумент();
	ДокЗакрытияСмены.Дата = КонецДня(ДокОткрытияСмены.Дата);
	...
	Иначе
		...
		РегистрыСведений.ОткрытыеЗаказыПоКартам.ОчиститьПривязкиДоДатыВключительно(ДокЗакрытияСмены.Дата);
