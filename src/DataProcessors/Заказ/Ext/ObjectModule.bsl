...
//Z-
	...
	Функция ВыходИзЗаказа() Экспорт
		...
		Если Модифицирован Тогда
			...
			ИначеЕсли Ответ = "Нет" Тогда  
				ИнтерфейсРМ.ЗаписьСобытия(Справочники.События.ЗаказОтказОтЗаписи, Заказ.Ссылка, Заказ.Номер);
	...
	Функция СоздатьНовый(ОбъектВвода=Неопределено, ПриРазделении=Ложь,ЗакрытьФормы=Ложь) Экспорт
		...
		ИнтерфейсРМ.ЗаписьСобытия(Справочники.События.ЗаказНовый, Заказ.Ссылка, Заказ.Номер, НаимПосадочногоМеста(Заказ.ПосадочноеМесто) );
...
//Z-				
		...
		Если НЕ ФоновыйРежим Тогда
			...
			ИнтерфейсРМ.ЗаписьСобытия(Справочники.События.ЗаказВход, Заказ.Ссылка, Заказ.Номер );
		КонецЕсли;
...
КонецФункции
	...
	Функция ИзменитьКартуЛояльности() экспорт
...
КонецПроцедуры
	...
	//
	Функция ВводПоКоду(КодТовара=Неопределено) Экспорт
	...
	Процедура УдалитьВсеЗаказанное() Экспорт
		...
		ИнтерфейсРМ.ЗаписьСобытия(Справочники.События.ЗаказУдалениеНеПустого, Заказ.Ссылка, Заказ.Номер, НаимПричиныУдаления);
		...
		Если глВерсия>1 И ЗначениеЗаполнено(Заказ.Бронь) Тогда
			...
			ИнтерфейсРМ.ПопыткаДействияСОбъектом( БроньОбъект, "Объект.Записать()" );
		КонецЕсли; 
