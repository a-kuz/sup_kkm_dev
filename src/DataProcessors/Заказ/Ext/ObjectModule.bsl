...
//Z+
		...
		ТаблицаЗаказа.Колонки.Добавить("РабочееМесто");
//Z-
	...
	Функция ОткрытьРабочуюФормуЗаказа(ВладелецФормы = Неопределено) Экспорт
		Если Не БезАвторизации ИЛИ Не ЗначениеЗаполнено(глПользователь) Тогда
...
//Z-				
	...
	Функция ПечатьМарокВыполнить()
		...
		ВремяСекунд = Формат(ТекущаяДатаНаСервере()-НачалоДня(ТекущаяДатаНаСервере()),"ЧГ=");
	...
	Процедура СоздатьДокументыУдаления(ГруппыПечатиОтказы)
		...
		Пока н<=ТаблицаУдалений.Строки.Количество()-1 Цикл
			...
			ДокУдаление.Дата			= ТекущаяДатаНаСервере();
	...
	Процедура ПрименитьЛояльность(Заказ, ЕстьДеньРожденияВЗаказе)
		...
		Если НЕ ОшибкаЛояльности Тогда
			...
			Если ОтветСервера.Ошибка Тогда
				...
				ЗарегистрироватьСобытие("Лояльность.Ошибка", УровеньЖурналаРегистрации.Ошибка, , Заказ.Ссылка, ОписаниеОшибки);
		...
		Если ОшибкаЛояльности Тогда
			...
			Если Ответ = "НАЧИСЛИТЬ ПОЗЖЕ" Тогда //Если ответ = "НЕТ" тогда
				...
				ЗарегистрироватьСобытие("Лояльность.Обмен", УровеньЖурналаРегистрации.Информация, , Заказ.Ссылка, "Выбрана оплата без лояльности");
			иначе
		...
		Если ответ= "СПИСАТЬ" ИЛИ ответ= "НАЧИСЛИТЬ" Тогда
			...
			Для каждого ТекущаяСтрока Из СтрокиЗаказа Цикл
				...
				Иначе
					...
					ЗарегистрироватьСобытие("Лояльность.Ошибка", УровеньЖурналаРегистрации.Ошибка, , Заказ.Ссылка, "Некорректный ответ лояльности");
	...
	//КонецЕсли;
		...
		//иначе
		//	ЗарегистрироватьСобытие("Лояльность.Ошибка", УровеньЖурналаРегистрации.Ошибка, , Заказ.Ссылка, "Не нашли файл ответа: "+имяФайлаОтвета);
		//	ответ = ИнтерфейсРМ.ВопросПредупреждение("Вопрос","Повторить запрос ?",
	...
	Функция ИзменитьКартуЛояльности(Знач пНомерКартыЛояльности = Неопределено) Экспорт
		...
		Иначе
			...
			Иначе
				Заказ.НомерКартыЛояльности = "";
			КонецЕсли;
			ЗаказДопИнф.ДатаАвторизации = ТекущаяДатаНаСервере();
...
КонецФункции
	...
	Попытка
		ЗарегистрироватьСобытие("Изменение статуса заказа", УровеньЖурналаРегистрации.Информация,,Заказ.Ссылка, Статус);	
	Исключение
	КонецПопытки;  
...
//			Защита.СозданиеСпискаМониторыМарокДляУведомления(Выборка.ГруппаПечати, МониторыМарок); 	
	...
	Процедура ОбновитьОстатокСтопЛиста(СтрокиДозаказа)
		...
		СтопЛист.Дата	= ТекущаяДатаНаСервере();
	...
	Функция ЗаписатьТарификацию(СтрокаДок, СтрокаТовара)
		...
		Если ДокРасчетыПоТарифу = Неопределено Тогда
			...
			ДокРасчетыПоТарифу.Дата = ТекущаяДатаНаСервере();
		КонецЕсли;
		...
		Если СтрокаТовара.Статус=СтатусЗаказано И СтрокаТовара.Количество=0 Тогда
			...
			Если ВремяКон > ТекущаяДатаНаСервере() Тогда
				// время конца больше текущего времени - надо остановить сейчас
				ВремяКон = НачалоМинуты(ТекущаяДатаНаСервере());
			КонецЕсли;
		КонецЕсли;
	...
	Функция ПересчетПоТарифам(ФлагОстановить=Ложь)
		...
		ЕстьИзменения = Ложь;
		ТекВремя = НачалоМинуты(ТекущаяДатаНаСервере());
	...
	Процедура ПолучитьПоследнийСтатусДоставки(ТекЗаказ, ОбновитьВФорме = Истина, ТекСтатус = Неопределено) Экспорт
		...
		ПоследняяДата = ?(ТабСрезПоследних.Количество()=0, ТекущаяДатаНаСервере(), ТабСрезПоследних[0].Период);
	...
	Процедура РегистрЗаказТоварыДопИнфДобавить(СтрокаТовара, РежимПодтверждения, ТекДата, ВремяСекунд = 0, ЭтоТовар = Истина, ИдСтрокиВладельца = неопределено, КоличествоВладельца = 0)
		...
		Если МенеджерЗаписи.Выбран() Тогда
			...
			БылНомерМарки			= МенеджерЗаписи.НомерМарки;
...
//				Защита.СозданиеСпискаМониторыМарокДляУведомления(МенеджерЗаписи.ГруппаПечати, МониторыМарок); 	
			...
			ИначеЕсли ПозицияУжеЗаказывалась Тогда
				...
				БылНомерМарки = БылНомерМарки;
				Если ЗначениеЗаполнено(БылНомерМарки) Тогда
					
					МЗ = РегистрыСведений.Марки.СоздатьМенеджерЗаписи();
					МЗ.Заказ = Заказ.Ссылка;
					МЗ.НомерМарки = БылНомерМарки;
					МЗ.Удалить();
				КонецЕсли;
			Иначе 
				Попытка
	...
	Функция НовыйИДСтрокиЗаказа(Префикс = "t", ТекНомерСтроки = 0) Экспорт
		...
		//Ид = Префикс + НомерКлюча +"_"+ Формат(ТекущаяДатаНаСервере() - Дата(1970,1,1), "ЧГ=") + Формат(ТекНомерСтроки, "ЧГ=") + ГСЧ.СлучайноеЧисло(1,999);
