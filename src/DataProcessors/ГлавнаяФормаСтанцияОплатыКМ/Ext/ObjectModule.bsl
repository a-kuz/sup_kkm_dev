	...
	Если ТипЗнч(_Знач) = Тип("Строка") Тогда
		...
		Если ТипЗнч(_Объект) = Тип("Структура") Тогда
			...
			Если _Объект.Тип = Справочники.ТипыШаблоновШК.КартаЛояльности Тогда
				...
				ТекущийДокумент.НомерКартыЛояльности = НКЛ;
		...
		ИначеЕсли НЕ ЗначениеЗаполнено(_Объект) Тогда
			Если МастерТестирования = Неопределено Тогда
				...
				КонецЕсли;
				ЗаписьЖурналаРегистрации("Неопознанный штрихкод", УровеньЖурналаРегистрации.Предупреждение, , СсылкаДок, """"+ШК+"""");
				Возврат;
			Иначе
				Если _Знач = "1" Тогда
...
Процедура СброситьТекущийДокумент(ЗапроситьАвторизациюГостя = Истина) Экспорт
...
Процедура ЗаписатьЗаказ(Статус, Оплачен = Ложь) Экспорт
...
Процедура ЗаписатьВозврат(Статус)
	...
	Если докВозврат.Проведен Тогда
		Попытка
			ЛояльностьКлиент.ОбработатьВозврат(докВозврат);
		Исключение
...
Процедура Оплата() Экспорт
	Попытка
		...
		Если Обработки.Оплата.ПолучитьФорму("Форма" + глПараметрыРМ.ИнтерфейсТип).Открыта() Тогда
			ГлавнаяФорма.УстановитьДоступностьКнопки("Оплата", 1, 1);
			Возврат;
		КонецЕсли;
		...
		Если фВводТоваровДоступен Тогда 
			...
			Попытка
				...
				Заказ.Товары.ЗагрузитьКолонку(Заказ.Товары.ВыгрузитьКолонку("Сумма"),"СуммаРеализации");
				Если Не Заказ.НомерЧека Тогда
					НомерЧека = НомерЧека;
...
Функция ПрочитатьТекущийДокумент() Экспорт
	...
	ИначеЕсли ТекущийРежим = "ПРОДАЖА" Тогда
		...
		Иначе
			//:Заказ = Документы.Заказ.СоздатьДокумент();
			Для Каждого Т Из Заказ.Товары Цикл
				Если Т.СтатусОплаты = 0 и Т.Фирма = глПараметрыРМ.Фирма Тогда
					Т.СтатусОплаты = -1;
...
Процедура МенюЛояльности() Экспорт
	...
	ФормаПоиска = Рез.Выгрузить()[0][0].ПолучитьФорму();
	...
	Если ЗначениеЗаполнено(РезультатПоиска) Тогда
		...
		ИначеЕсли РезультатПоиска = Справочники.ДействияЭлементовМеню.ОчиститьНомерКартыЛояльности Тогда
			ПрочитатьТекущийДокумент();
			ТекущийДокумент.НомерКартыЛояльности = "";					
			ЗаписатьЗаказ(Перечисления.СтатусыЗаказа.Открыт);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Функция СтруктураПлашки() Экспорт
	СтруктураПлашки = Новый Структура;
	СтруктураПлашки.Вставить("Текст");
	СтруктураПлашки.Вставить("Обновлено");
	СтруктураПлашки.Вставить("Тип");
КонецФункции
