...
Процедура ОбработкаВнешнегоСобытия(Источник, Событие, Данные) Экспорт
	...
	Если ЗначениеЗаполнено(глПользователь) Тогда
		...
		Если НЕ Шаблон.Пустая() И НЕ Шаблон.ПометкаУдаления И Шаблон.Тип = Справочники.ТипыШаблоновШК.Товар Тогда
			...
			ВыбТов = Неопределено;
			Попытка
				Каталог = глПараметрыРМ.МестоРеализации.ОсновнойКаталогТоваров;
			Исключение
				Каталог = Неопределено;
			КонецПопытки;
			Справочники.ШаблоныШК.ПолучитьКодКоличествоИзШК(Префикс, ШК, КодТовара, Колво, ВыбТов, Каталог);
...
//
Процедура СоздатьНовыйЗаказ(КартаДоступа) Экспорт
...
Функция ПоказатьЗаказНаМонитореГостя() Экспорт
	...
	ГлавнаяФорма.ПодключитьОбработчикОжидания("ПоказатьЗаказНаМонитореГостя_ОбработчикОжидания",0.5,Истина);	
КонецФункции
	...
	Если ЗапроситьАвторизациюГостя Тогда
		...
		Если ЗначениеЗаполнено(КартаДоступа) Тогда // поищем заказы по карте клиента
			...
			Если ЗначениеЗаполнено(ЗаказСсылка) Тогда
				ОткрытьЗаказ(ЗаказСсылка);
			Иначе
				СоздатьНовыйЗаказ(КартаДоступа);	
			КонецЕсли;
...
Процедура ЗаписатьЗаказ(Статус = "Открыт", Оплачен = Ложь) Экспорт
	Если Статус = "Открыт" Тогда
		Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаказа.Открыт");
	КонецЕсли;
	ГлавнаяФорма.ЭлементыФормы.тпТовары.ВыделенныеСтроки.Очистить();
	Р = Заказ.ЗаблокироватьРедактированиеПоФирме(глПараметрыРМ.Фирма, УИДформы);	
	Если Не Р.Успех Тогда
...
Процедура ЗаписатьВозврат(Статус)
	...
	Если докВозврат.Проведен Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации("ГлавнаяФорма.ФинализацияВозврата.Ошибка выполнения", УровеньЖурналаРегистрации.Ошибка, , докВозврат.Ссылка, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
	КонецЕсли;
...
Функция ПроверитьПДФ()
	Для каждого Т Из ТекущийДокумент.Товары Цикл
		Если Т.Количество И Т.фАлкоголь = 1 И Т.СтатусОплаты = -1 Тогда
			РезультатПроверки = Алкоголь.ПроверитьПДФ(Т.ПДФ, Новый Массив, Т.Товар, ТекущийРежим = "ВОЗВРАТ");
	...
	Попытка
		...
		Если фВводТоваровДоступен Тогда 
			...
			Попытка
				...
				ЗаписьЖурналаРегистрации("Оплата.РасчетЧека.До вызова",УровеньЖурналаРегистрации.Информация,,Заказ.Ссылка);
				...
				ЗаписатьЗаказ(Перечисления.СтатусыЗаказа.Открыт);
				ЗаписьЖурналаРегистрации("Оплата.РасчетЧека.После вызова",УровеньЖурналаРегистрации.Информация,,Заказ.Ссылка, "Товары: " + ЗначениеВСтрокуВнутр(Заказ.Товары.Выгрузить()));
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации("Заказ.Ошибка лояльности",УровеньЖурналаРегистрации.Предупреждение,,Заказ.Ссылка, "Товары: " + Символы.ПС + ЗначениеВСтрокуВнутр(Заказ.Товары.Выгрузить()));
				ПрочитатьТекущийДокумент();
			...
			Если фВозврат = 1 Тогда
				ЗаписьЖурналаРегистрации("Заказ.Востановление состояния заказа",УровеньЖурналаРегистрации.Предупреждение,,Заказ.Ссылка, "Товары: " + Символы.ПС + ЗначениеВСтрокуВнутр(Заказ.Товары.Выгрузить()));
				Заказ = СериализаторXDTO.ПрочитатьXDTO(ЗаказXDTO);
				ПрочитатьТекущийДокумент();
				Заказ = СериализаторXDTO.ПрочитатьXDTO(ЗаказXDTO);
				Заказ.ОбработанСистемойЛояльности = 2;
		...
		КонецЕсли;
	Исключение
		ЗаписьЖурналаРегистрации("Ошибка при оплате", УровеньЖурналаРегистрации.Ошибка, Метаданные.Документы.Заказ, Заказ.Ссылка, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
КонецПроцедуры
...
Процедура ВозвратОплаты() Экспорт
	...
	Исключение
		РезультатОплаты = Неопределено;
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации("Возврат.Ошибка в процессе оплаты", УровеньЖурналаРегистрации.Ошибка, , докВозврат.Ссылка, ПодробноеПредставлениеОшибки);
	КонецПопытки;
	ЗаписьЖурналаРегистрации("Возврат.Получен результат оплаты", УровеньЖурналаРегистрации.Информация, Метаданные.Документы.Возврат, докВозврат.Ссылка, "РезультатОплаты = " + ЗначениеВСтрокуВнутр(РезультатОплаты));
	Если РезультатОплаты = Неопределено Тогда // Закрыли по Esc или крестик в окне оплаты
		ТекущийДокумент = докВозврат;
		ЗаписатьДокумент(Перечисления.СтатусыЗаказа.Открыт);
		ГлавнаяФорма.УстановитьВидимостьКнопки("Оплата", 1, 1);
...
Функция СторноВсе() Экспорт
	...
	Попытка
		ЗаписьЖурналаРегистрации("Сторно все", УровеньЖурналаРегистрации.Информация,,ТекущийДокумент.Ссылка);	
	Исключение
	КонецПопытки;
...
Функция ПрочитатьТекущийДокумент() Экспорт
	...
	ИначеЕсли ТекущийРежим = "ПРОДАЖА" Тогда
		...
		Если Не РезультатБлокировки.Успех Тогда
			Ответ = ИнтерфейсРМ.ВопросПредупреждение("Ошибка", "Не удалось заблокировать заказ", "Заказ редактируется на РМ " + РезультатБлокировки.КемЗаблокирован,"","Esc=ОК","Забрать заказ");
			Если Ответ = "Забрать заказ" Тогда
				РабочееМестоАК = Справочники.РабочиеМеста.НайтиПоРеквизиту("ПрофильВхода", РезультатБлокировки.КемЗаблокирован);
				Если ЗначениеЗаполнено(РабочееМестоАК) Тогда
					...
					//Оповестить("ГлавнаяФормаАК_СбросЗаказаСоСтандартнойКассы", РезультатБлокировки.КемЗаблокирован, ГлавнаяФорма);
					глОжидание.Начало("Заказ не открыт", "Производится попытка закрыть заказ на автокассе.");
					//ПараметрыФЗ = Новый Массив;
					//ПараметрыФЗ.Добавить(10000);
					//ФЗ = ФоновыеЗадания.Выполнить("ОбщегоНазначенияКлиентСервер.ОбождатьМиллисекунд", ПараметрыФЗ);
					//ФЗ.ОжидатьЗавершения(11);
					ОбщегоНазначенияКлиент.ОбождатьМиллисекунд(10000);
					глОжидание.Конец();
					Возврат ПрочитатьТекущийДокумент();
				КонецЕсли;
			КонецЕсли; 
...
Процедура ВыборТовараИзМеню() Экспорт 
	...
	Если ГлавнаяФорма.ЭлементыФормы.Найти("тпТовары") <> Неопределено Тогда
		ФормаПодбораТоваров.ВерхняяГраницаПоложения	= ГлавнаяФорма.ЭлементыФормы.тпТовары.Высота + 5;
	КонецЕсли; 
...
Функция ЗаказНеВыбран() Экспорт 
	...
	Результат = ТекущийРежим = "";
	
	Если Результат Тогда
		ИнтерфейсРМ.ВопросПредупреждение("Предупреждение", "Заказ не выбран!", "Редактирование без заказа не возможно.", "", "ОК", "");
