...
&НаКлиенте
Перем мКомпонентаKeyBoardHook,ahk;
&НаКлиенте
...
Процедура ПодключитьВнешнююКомпонентуKeyboardHook()
	...
	мКомпонентаKeyBoardHook.СобытиеПриНажатии		= Ложь;
...
//	мПерехвата.Добавить(32);//Пробел
	...
	ahk.ahkassign("CheckingKey", "Ctrl");
	ctrlKey = ahk.ahkfunction("GKS") = "D";
	Если Данные=32 И (ctrlKey Или shiftKey) И НЕ altKey Тогда //Подпорка для работы Ctrl-Пробел
...
мСчетчикВыполнений = 0;
СисИнфо = Новый СистемнаяИнформация;
мАктуальнаяПлатформа	 = Истина;//Лев(СисИнфо.ВерсияПриложения,10)>="8.3.5.1385";
