...
Процедура ЗаполнитьПараметрыПоЗапросу(ТекстЗапроса)
	...
	Для каждого ПараметрЗапроса Из ПараметрыЗапросаНайденные Цикл
		...
		Если  СтрокаПараметров = Неопределено Тогда
			флТаблица = ПараметрЗапроса.ТипЗначения.СодержитТип(Тип("ТаблицаЗначений"));
			
			СтрокаПараметров = ПараметрыЗапроса.Добавить();
			СтрокаПараметров.ИмяПараметра = ИмяПараметра;
			СтрокаПараметров.ДопустимыеТипы = ?(флТаблица,Новый ОписаниеТипов,ПараметрЗапроса.ТипЗначения);
		    СтрокаПараметров.ЗначениеПараметра = ?(флТаблица,Неопределено,ПараметрЗапроса.ТипЗначения.ПривестиЗначение(СтрокаПараметров.ЗначениеПараметра));
		КонецЕсли; 
	КонецЦикла;
