...
Функция ОткрытьСмену() Экспорт
	...
	Для каждого эл из СписокФирм Цикл
		СменаТТПоМестуреализации = ИнтерфейсРМ.ТекущаяСмена(эл.Значение.МестоРеализации);
		Если СменаТТПоМестуреализации.Пустая() Или
			НачалоДня(СменаТТПоМестуреализации.Дата) <> НачалоДня(ТекущаяДатаНаСервере()) Тогда
			СменыОткрыты = ложь;
		КонецЕсли;
	КонецЦикла;
	
	Если не СменыОткрыты Тогда
		ИнтерфейсРМ.ВопросПредупреждение("Ошибка открытия смены","Не все сменые открыты!","","","ОК","");
		Возврат ложь;
	КонецЕсли;
