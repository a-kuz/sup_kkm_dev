...
Функция ЗакрытьСмену() Экспорт 
	...
	Если ИнтерфейсРМ.ВопросПредупреждение("Вопрос", "Закрытие смены ККТ", "Выполнить закрытие смены?", "Да","","Esc=Нет") <> "Да" Тогда
		Возврат ложь;
	КонецЕсли;
	...
	Если НомерСмены < НомерСмены1 и не ОтветЗ Тогда // смена в ФН закрылась и нет параметров по смене
		ВхПараметры1 = новый Структура;
		ВыхПараметры1 = новый Структура;
		Попытка 
		...
		КонецПопытки;
		Если ответ Тогда
			ЗакрытиеСмены = Обработки.ОткрытиЗакрытиеСменыКасса.ПолучитьДокументЗакрытияСмены();//Документы.Касса_ЗакрытиеСмены.СоздатьДокумент();
			ЗакрытиеСмены.Автор = глПользователь;
			ЗакрытиеСмены.Дата = ТекущаяДатаНаСервере();
			ЗакрытиеСмены.СменаТТ =  СменаТТ;
			ЗакрытиеСмены.СменаКассы = СменаКасса;
...
Функция ПроверитьСуммы(ПараметрыВх, РабочееМесто,СменаКасса)
	...
	Если не Сравнилось Тогда // отправим почту
		...
		ЭлектроннаяПочка.ОтправитьОповещениеНаПочту("Ошибка закрытия ККМ",ТекстПисьма,Получатели);
	КонецЕсли;
	Возврат Сравнилось;
КонецФункции
