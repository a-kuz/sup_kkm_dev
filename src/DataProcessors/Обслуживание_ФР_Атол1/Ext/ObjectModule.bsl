...
Процедура УстановитьСвойстваДрайвера() Экспорт
	...
	DRV.CurrentDeviceNumber	= ПараметрыТО.ИдУстройства;
	
	// Параметры связи
	//DRV.Model				= КодыМоделей[ТО.КодМодели];
	DRV.PortNumber			= ПараметрыТО.PortNumber;
...
Функция ФисказизацияЧека(ВходныеПараметры,ВыходныеПараметры)
	...
	// получим из доп свойств нужно ли печатать чек
	Электронно = ?(РегистрыСведений.ДополнительныеСвойства.ЗначениеСвойства(глРабочееМесто,"ПечататьЧек") = Неопределено,Электронно,РегистрыСведений.ДополнительныеСвойства.ЗначениеСвойства(глРабочееМесто,"ПечататьЧек"));
	...
	Для инд = 0 по МассивСтрокЧека.Количество() - 1 Цикл
		...
		КонецЕсли;
	КонецЦикла;
	Если МассивСтрокОплаты.Количество() = 0 и СуммаПоСтрокам <> 0 Тогда
		Если не ВыходныеПараметры.Свойство("ОписаниеОшибки") ТОгда
			ош = Новый Массив;
			ВыходныеПараметры.Вставить("ОписаниеОшибки", ош);
		Иначе
	...
	Если Ответ Тогда
		...
		Если Ответ Тогда
			...
			Исключение
				ЧислоБиты = 0;
			КонецПопытки;
			Если ЧислоБиты >= 2 Тогда
			//Если Цел(ЧислоБиты/4) >= 1 Тогда
				ЕНВД = Истина;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	DRV.НомерРегистра = 52;
	DRV.ПолучитьРегистр();
	ФорматФФД = ?(Лев(DRV.ROMVersion,1) = "1",ФорматФФД,"1.05");
	...
	Если МассивСтрокЧека.Количество() <> 0 Тогда
		...
		Для инд = 0 по МассивСтрокЧека.Количество() - 1 Цикл
			...
			Если ПозицияЧека.Свойство("ФискальнаяСтрока") Тогда
				...
				Если ЕНВД Тогда
					DRV.TaxTypeNumber  = ПолучитьИндексНалога("без НДС",ФорматФФД);//4;
				Иначе
					DRV.TaxTypeNumber = ПолучитьИндексНалога(ПозицияЧека.СтавкаНДС,ФорматФФД);
				КонецЕсли;
				// тестово
				DRV.RecFont = 2;
				// тестово
...
Функция ПечатьТекста(ВходныеПараметры,ВыходныеПараметры)
	...
	ПечататьШКЧека = Ложь;
	Если ВходныеПараметры.Свойство("ПечататьШКЧека") ТОгда
		ПечататьШКЧека = ВходныеПараметры.ПечататьШКЧека;
	КонецЕсли;
	
	Если ПечататьШКЧека Тогда
		МассивШКЧеков = Неопределено;
		Если ВходныеПараметры.Свойство("МассивШКЧеков") ТОгда
			МассивШКЧеков = ВходныеПараметры.МассивШКЧеков;
		КонецЕсли;
	КонецЕсли;
	...
	Если МассивСтрок.Количество() <> 0 ТОгда
		...
		Если ПечататьШКЧека и МассивШКЧеков <> Неопределено Тогда
			Для инд = 0 по МассивШКЧеков.Количество() - 1 Цикл
				СтрокаШКЧека = МассивШКЧеков[инд];
				Попытка
					СтрШК = "t=" + Формат(СтрокаШКЧека.ДатаЧека,"ДФ=yyyyMMddTHHmm");
					СтрШК = СтрШК + "&s=" + Формат(СтрокаШКЧека.СуммаЧека,"ЧДЦ=2; ЧРД=.; ЧН=0.00; ЧГ=0");
					СтрШК = СтрШК + "&fn=" + СтрЗаменить(СтрЗаменить(СтрокаШКЧека.НомерФН," ",""),Символ(160),"");
					СтрШК = СтрШК + "&i=" + Формат(СтрокаШКЧека.НомерФД,"ЧДЦ=2; ЧРД=.; ЧН=0.00; ЧГ=0");
					СтрШК = СтрШК + "&pf=" + СтрЗаменить(СтрЗаменить(СтрокаШКЧека.НомерФПД," ",""),Символ(160),"");
					СтрШК = СтрШК + "&n=" + СтрокаШКЧека.СНО; // число 0 - общая, 3 - ЕНВД
				Исключение
					Продолжить;
				КонецПопытки;
				DRV.ШтрихКод = СтрШК;
...
Функция ОткрытьСмену(ВходныеПараметры,ВыходныеПараметры)
	...
	Если ВходныеПараметры.Свойство("Кассир") Тогда
		Кассир = Лев(ВходныеПараметры.Кассир,63);
	КонецЕсли;
	ККМСсылка = Неопределено;
	Если ВходныеПараметры.Свойство("ККМСсылка") Тогда
		ККМСсылка = ВходныеПараметры.ККМСсылка;
	КонецЕсли;

	МестоРеализации = ?(ККМСсылка = Неопределено, Неопределено,ККМСсылка.Фирма.МестоРеализации);
	...
	// добавим проверику на то что смена уже открыта
	...
	Если DRV.СменаОткрыта Тогда
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |ГДЕ
		               |	Касса_ОткрытиеСмены.НомерСмены = &НомерСмены
		               |	И Касса_ОткрытиеСмены.КассаЗаводскойФН = &КассаЗаводскойФН
		               |	И Касса_ОткрытиеСмены.КассаЗаводскойНомер = &КассаЗаводскойНомер
		...
		Если КассаЗаводскойНомер <> "" Тогда
			Запрос.УстановитьПараметр("КассаЗаводскойНомер",КассаЗаводскойНомер);
		Иначе
			Запрос.Текст = СтрЗаменить(Запрос.Текст,"И Касса_ОткрытиеСмены.КассаЗаводскойНомер = &КассаЗаводскойНомер","И Истина");
		КонецЕсли;
		Запрос.УстановитьПараметр("КассаЗаводскойФН",НомерФН);
		Запрос.УстановитьПараметр("НомерСмены",DRV.Смена);
		Рез = Запрос.Выполнить();
		Если не Рез.Пустой() ТОгда
			Логирование(0,"Открытие смены","Смена уже открыта");
			Возврат Истина;
		КонецЕсли;
	Иначе
	...
	Если Ответ Тогда
		...
		ОткрытиеСмены.Касса = ККМСсылка;//глПараметрыРМ.ККМ;   
		ОткрытиеСмены.КассаНомер = ККМСсылка.КодСуп;//глПараметрыРМ.ККМ.КодСуп;
		ОткрытиеСмены.КассаЗаводскойНомер = ?(ЗначениеЗаполнено(КассаЗаводскойНомер),КассаЗаводскойНомер,ККМСсылка.ЗаводскойНомер);
		...
		ОткрытиеСмены.КассаЗаводскойФН = НомерФН;
...
//10 – чек коррекции возврата расхода
	...
	инд = ПолучитьИндексНалога("10",ФорматФФД);
	...
	инд = ПолучитьИндексНалога("18",ФорматФФД);
	...
	инд = ПолучитьИндексНалога("без НДС",ФорматФФД);
...
Функция ПроверитьИОткрытьСмену(ВходныеПараметры,ВыходныеПараметры)
	...
	КонецЕсли;
	ККМСсылка = Неопределено;
	Если ТипЗнч(ВходныеПараметры) = Тип("Структура") и ВходныеПараметры.Свойство("ККМСсылка") Тогда
		ККМСсылка = ВходныеПараметры.ККМСсылка;
	КонецЕсли;

	МестоРеализации = ?(ККМСсылка = Неопределено, МестоРеализации, ККМСсылка.Фирма.МестоРеализации);
	...
	Если Ответ Тогда
		...
		Если ПолучитьСменуКассы().Пустая() и DRV.СменаОткрыта = 0 Тогда
			...
			ОткрытиеСмены.Касса = ККМСсылка;//глПараметрыРМ.ККМ;
			ОткрытиеСмены.КассаНомер = ККМСсылка.КодСуп;//глПараметрыРМ.ККМ.КодСуп;
			ОткрытиеСмены.КассаЗаводскойНомер = ККМСсылка.ЗаводскойНомер;//глПараметрыРМ.ККМ.ЗаводскойНомер;
			ОткрытиеСмены.НомерСмены = НомерСмены;
			ОткрытиеСмены.НомерДокумента = НомерЧека;
			ОткрытиеСмены.РабочееМесто = глРабочееМесто;
			ОткрытиеСмены.СменаТТ = СменаТТ; 
...
Процедура Логирование(Уровень,Место,Текст)
	Если УровеньЛогирования <= Уровень Тогда
		...
		Иначе
			 ЗарегистрироватьСобытие("Ошибка логирования",УровеньЖурналаРегистрации.Ошибка,,,"Путь логирования не существует");
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьИндексНалога(НДСТекст,Формат = "1.0")
	...
	СоответНалогов = Новый Соответствие;
	Если Формат = "1.0" Тогда
		СоответНалогов.Вставить("18",6);
		СоответНалогов.Вставить("10",5);
		СоответНалогов.Вставить("без НДС",4);
	Иначе
		СоответНалогов.Вставить("18",3);
		СоответНалогов.Вставить("10",2);
		СоответНалогов.Вставить("без НДС",6);
	КонецЕсли;
