#Если Клиент Тогда
	...
	//   HTTPСоединение
	//
	Функция СоздатьСосединениеЧерезПрокси(ОписаниеОшибкиСоединения = "")
	...
	Функция ЗапроситьФайлыССервера(СерверИсточникПараметр, СтрокаПараметраПолученияПараметр, ИмяВходящегоФайлаПараметр, HTTP = Неопределено) Экспорт
		...
		Исключение
			...
			ОписаниеОшибкиСоединения = СформироватьТекстСообщения(ОписаниеОшибки());
			...
			HTTP = СоздатьСосединениеЧерезПрокси(ОписаниеОшибкиСоединения);
