...
Процедура ЗаполнитьТовары(Товары) Экспорт
	...
	ЗаполнитьТаблицуТоваровДляПриготовления();
КонецПроцедуры

Процедура ЗаполнитьТаблицуТоваровДляПриготовления() Экспорт
...
Процедура ДействияПередОткрытиемФормы(ТекущаяФорма, Отказ) Экспорт
	...
	ТаблицаПоиска = ОбщегоНазначенияПовтИсп.СформироватьТаблицуПоиска(Константы.ДатаОбновленияПризнакаНаличияВПродаже.Получить());
	ТекущаяФорма.Товары.Очистить();
КонецПроцедуры
...
Функция ВыполнитьПоиск() Экспорт
	Если ФормаПоиска = Неопределено Тогда
		ФормаПоиска = ПолучитьФорму("Форма" + глПараметрыРМ.ИнтерфейсТип);
	ИначеЕсли ФормаПоиска.Открыта() Тогда
		ФормаПоиска.Закрыть();			
	КонецЕсли;
	
	Возврат ФормаПоиска.ОткрытьМодально();
КонецФункции
