...
Процедура Инициализация() Экспорт
	...
	Если НЕ глТорговоеОборудование.Свойство("InfoDisplay",DRV) Тогда
		Если РазмерОкнаИнфоДисплея = 0 Тогда
			...
			ИначеЕсли ЗначениеЗаполнено(глРабочееМесто) И глПараметрыРМ.ТипТТ = ПредопределенноеЗначение("Справочник.ТипыТТ.КМ") Тогда
				ИмяФормы = "ИнфоДисплей_МОКП";
			КонецЕсли;
			//Попытка
	...
	ПрочитатьПараметр("ИзображениеДляМонитораГостя0", "" );
...
Процедура ВыполнитьДействие(Действие, ПараметрыДействия=Неопределено) Экспорт
	...
	ИначеЕсли Действие = "ПоказатьЗаказ" Тогда
		...
		ТабДокЛояльность = ИнтерфейсРМЛояльность.ПолучитьТабДокЛояльность_МониторГостя(Заказ, ЛояльностьКлиентСервер.ПолучитьВерсиюДанныхЛояльностиПоЗаказу(Заказ), ЛояльностьДанныеЗаказа, ФирмаФильтр);
		...
		//////	Иначе
		//////		Если НЕ ПолеКартинкиГостя.Высота = 664 Тогда
		//////			ПолеКартинкиГостя.Высота = 664;
		//////		КонецЕсли;
		//////		Попытка
...
Процедура ПоказатьСледующуюКартинку() Экспорт
	Если НЕ ЗначениеЗаполнено(СписокКартинок[0]) Тогда
		Возврат;
	КонецЕсли;
