...
Процедура ЗаполнитьТаблицуСпецифик()
	...
	ОбластьМакетаТовар			= Макет.ПолучитьОбласть("СтрокаТовара|ВидимаяОбласть");
	ОбластьМакетаТовар.Параметры.Заполнить(ТаблицаЗаказа.Строки[0]);
	ОбластьМакетаТовар.Параметры.Состав = СтрСоединить(ТаблицаСпецифик.ВыгрузитьКолонку("Специфика"), "; ");
	ТабличныйДокумент.Вывести(ОбластьМакетаТовар);
