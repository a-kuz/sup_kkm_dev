...
Процедура ЗаполнитьТаблицуСпецифик()
	...
	Если НЕ ЗначениеЗаполнено(Товар) ИЛИ ТаблицаЗаказа.Строки.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	Макет = ЭтотОбъект.ПолучитьМакет("Заказ1280х1024");
	ОбластьМакета = Макет.ПолучитьОбласть("Заголовок|ВидимаяОбласть");
	ТабличныйДокумент.Вывести(ОбластьМакета);
	
	ОбластьМакета = Макет.ПолучитьОбласть("СтрокаТовара|ВидимаяОбласть");
	ОбластьМакета.Параметры.Заполнить(ТаблицаЗаказа.Строки[0]);
	ВыбранныеСпецифики = ТаблицаСпецифик.Скопировать(Новый Структура("Выбрана", 1), "Специфика");
	ОбластьМакета.Параметры.Состав = СтрСоединить(ВыбранныеСпецифики.ВыгрузитьКолонку("Специфика"), "; ");
	ТабличныйДокумент.Вывести(ОбластьМакета);
	
	//Возврат;
	
	
	Макет = ЭтотОбъект.ПолучитьМакет("Специфика1280х1024");
	ОбластьМакета = Макет.ПолучитьОбласть("ГруппаСпецифик");
	...
	СтрокиГруппы = ТаблицаСпецифик.Скопировать(Новый Структура("ГруппаСпецифик", ГруппаСпецифик));
	...
	КонецЕсли;
	ОбластьМакета = Макет.ПолучитьОбласть(СтрШаблон("СтрокаСпецифик|ВидимаяОбласть_%1х%2", чКолонок, чСтрок));
	...
	Для Яч = 0 По Вграница Цикл
		...
		Если ТекСпецифика.Выбрана = 0 Тогда
