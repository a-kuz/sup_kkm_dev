...
Процедура ЗаполнитьТаблицуЗаказа() 
	...
	МКТ = ПолучитьМакет("Специфика1280х1024").ПолучитьОбласть("СтрокаСпецифик|ВидимаяОбласть");
	...
	ОбластьГруппаСпецифик.Параметры.Наименование = "Очень длинное наименование группы специфик для одного из товаров Кухни Полли";
	...
	Для Яч = 0 По 7 Цикл
		...
		Если Яч % 2 Тогда
			МКТ.Область("R1C2").Картинка = Неопределено;
		Иначе
			МКТ.Область("R1C2").Картинка = БиблиотекаКартинок.CheckMark48;
		КонецЕсли;
