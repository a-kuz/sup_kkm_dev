...
//	
//	Если РазметкаСтрок.Количество() = 1 Тогда
//		
//		Запрос = Новый Запрос;
//		Запрос.Текст = 
...
//				МКТ.Параметры.Цена = Формат(123.45, "ЧДЦ=0; ЧН==; ЧГ=3,0") + " " + Символ(8381);
//			Исключение
...
//				МКТ.Область("R3C1:R4C2").ЦветФона = WebЦвета.Белый;
//			Иначе
//				МКТ.Область("R4C1:R4C2").Картинка = БиблиотекаКартинок.CheckMark48;
//				МКТ.Область("R3C1:R4C2").ЦветФона = WebЦвета.Роса;
//			КонецЕсли;
//			
...
Процедура ЗаполнитьТаблицуСпецифик()
	...
	Если НЕ ЗначениеЗаполнено(Товар) ИЛИ СтрокаЗаказа.Строки.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	ОбластьМакета.Параметры.Заполнить(СтрокаЗаказа.Строки[0]);
