...
Процедура ЗаполнитьТаблицуСпецифик()
	...
	Для каждого ТекСтрока Из ВыбранныеСпецифики Цикл
		мПодстрок.Добавить(СтрШаблон("%1 (%2 р.)", ТекСтрока.Специфика, Формат(ТекСтрока.Сумма, "ЧДЦ=; ЧГ=0")));
	КонецЦикла;
	...
	Для Яч = 0 По Вграница Цикл
		...
		Иначе
			...
			ОбластьМакета.Параметры.Цена = Формат(ТекСпецифика.Цена, "ЧДЦ=0; ЧН==; ЧГ=3,0") + " " + Символ(8381);
