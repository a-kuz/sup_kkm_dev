...
//Z-			
	...
	Если ПоказатьЗаказ И ТипЗнч(ЛояльностьДанныеЗаказа) = Тип("Структура") Тогда
		ЭтаФорма.ПодключитьОбработчикОжидания("ВывестиАлерты_Ожидание", 1, Истина);
	КонецЕсли;
...
Процедура ВывестиАлерты_Ожидание() Экспорт 
	...
	Если ЛояльностьДанныеЗаказа.Свойство("Алерты", Алерты) Тогда
		Для Сч = 0  По Алерты.ВГраница() Цикл
			...
			РаботаСокнами.ПоказатьПлашку(ТекАлерт.Наименование, ТекАлерт.ИнфоГостя, Сч, "3000", 1024 + 490, 230);
		КонецЦикла;
	КонецЕсли;
