...
//Z-			
	...
	Если ПоказатьЗаказ И ТипЗнч(ЛояльностьДанныеЗаказа) = Тип("Структура") Тогда
...
Процедура ЗаполнитьСпецификиПоСтроке(ОбластьМакетаСтрока, НомерСтрокиТовара)
	...
	мПодстрок = Новый Массив;
	Для каждого ТекСтрока Из ВыбранныеСпецифики Цикл
		мПодстрок.Добавить(СтрШаблон("%1 ( %2 %3)", ТекСтрока.Специфика, Формат(ТекСтрока.ЦенаРеализации, "ЧДЦ=; ЧН==; ЧГ=0"), ?(ТекСтрока.ЦенаРеализации, Символ(8381), "")));
	КонецЦикла;
...
Процедура ВывестиАлерты_Ожидание() Экспорт 
	...
	Если ЛояльностьДанныеЗаказа.Свойство("Алерты", Алерты) Тогда
