...
Перем ВысотаТабДокЛояльностьДоЛУЧа; // высота, которую мы закрываем уменьшенной картинкой, чтобы подвал лояльности и ЛУЧ были видны гостю.
...
//~ТОВАРГЕТИНФО	
//ЛУЧ	
	ЭлементыФормы.ТабДокЛУЧ.Видимость = Ложь;
//~ЛУЧ	
	...
	Если НЕ Изображение.Пустая() Тогда
		Если ЗначениеЗаполнено(СписокКартинок[НомерТекущейКартинки]) Тогда
	...
	ВысотаТабДокЛояльностьДоЛУЧа = 644;
...
Процедура ЗаполнитьТаблицуЗаказа() 
	...
	Если НЕ ЭлементыФормы.ПолеТабличногоДокумента.Высота = 845 Тогда
		ЭлементыФормы.ПолеТабличногоДокумента.Высота = 845;
	КонецЕсли;
	...
	СтрокиСторно = СтрокиТоваров.НайтиСтроки(Новый Структура("Количество", 0));
	Для каждого тсСторно Из СтрокиСторно Цикл
		СтрокиТоваров.Удалить(тсСторно);
	КонецЦикла;
	
	Если ФильтрПоСтанции Тогда
		СтрокиДляОтображения = СтрокиТоваров.НайтиСтроки(новый Структура("Станция", ТекСтанция));
	ИначеЕсли ФильтрПоФирме Тогда	
		СтрокиДляОтображения = СтрокиТоваров.НайтиСтроки(новый Структура("Фирма", глПараметрыРМ.Фирма));
	Иначе
		СтрокиДляОтображения = СтрокиТоваров;
	КонецЕсли;
	
	Для Каждого СтрокаЗаказа Из СтрокиДляОтображения Цикл
		...
		НомерСтроки_тпТовары = СтрокаЗаказа.Ном;
		
		Кл = Кл + 1;
		
		Если НомерСтроки_тпТовары = НомерСтрокиЗаказа Тогда
		...
		Если ОбластьСтрока.Рисунки.Количество() Тогда 
			...
			КонецЕсли;
		КонецЕсли;
		ЗаполнитьСпецификиПоСтроке(ОбластьСтрока, НомерСтроки_тпТовары);
...
Процедура _ЗаполнитьТаблицуЗаказа() 
...
Процедура ЗаполнитьТаблицуСпецифик()
	...
	Если НЕ ЗначениеЗаполнено(Товар) ИЛИ ДеревоСтрокаЗаказа.Строки.Количество() = 0 Тогда
		ЭлементыФормы.ПолеТабличногоДокумента.Высота = 845;
		Возврат;
	КонецЕсли;
	
	Если Товар.СпецификиТипЭкранаГостя = 0 Тогда
		ЗаполнитьТаблицуСпецифик_Стандарт(ТабличныйДокумент);
	ИначеЕсли Товар.СпецификиТипЭкранаГостя = 1 Тогда
		ЗаполнитьТаблицуСпецифик_Inspired(ТабличныйДокумент);
	КонецЕсли;
	
	ИнтерфейсРМ.ЗаполнитьПодвалГостя(ЭлементыФормы.ТабДокПодвал, Заказ, МакетПодвал, ЛояльностьДанныеЗаказа, КПКратко);
	
КонецПроцедуры
//Z-
Процедура ЗаполнитьТаблицуСпецифик_Inspired(ТабличныйДокумент)
	
	Макет1 = ЭтотОбъект.ПолучитьМакет("Заказ1280х1024");
	ОбластьМакета = Макет1.ПолучитьОбласть("Заголовок|ВидимаяОбласть");
	ТабличныйДокумент.Вывести(ОбластьМакета);
	
	ОбластьМакета = Макет1.ПолучитьОбласть("СтрокаТовараНов|ВидимаяОбласть");
	//ОбластьМакета = Макет1.ПолучитьОбласть("СтрокаТовара|ВидимаяОбласть");
	ОбластьМакета.Параметры.Заполнить(ДеревоСтрокаЗаказа.Строки[0]);
	ВыбранныеСпецифики = ТаблицаСпецифик.Скопировать(Новый Структура("Выбрана", 1), "Специфика,Сумма");
	мПодстрок = Новый Массив;
	Для каждого ТекСтрока Из ВыбранныеСпецифики Цикл
		мПодстрок.Добавить(СтрШаблон("%1 ( %2 %3)", ТекСтрока.Специфика, Формат(ТекСтрока.Сумма, "ЧДЦ=; ЧН==; ЧГ=0"), ?(ТекСтрока.Сумма, Символ(8381), "")));
	КонецЦикла;
	ОбластьМакета.Область(2,,2,).Видимость = Ложь;
	ОбластьМакета.Параметры.Состав = СтрСоединить(мПодстрок, "; ");
	ТабличныйДокумент.Вывести(ОбластьМакета);
	
	
	Макет1 = ЭтотОбъект.ПолучитьМакет("Специфика1280х1024");
	ОбластьМакета = Макет1.ПолучитьОбласть("ГруппаСпецифик");
	ОбластьМакета.Параметры.Наименование = ГруппаСпецифик;
	ТабличныйДокумент.Вывести(ОбластьМакета);
	
	ТаблицаСКартинками = ХранилищеДополнительнойИнформации.ПолучитьКартинкиПодгруппыСпецифик(ГруппаСпецифик);
	СтрокиГруппы = ТаблицаСпецифик.Скопировать(Новый Структура("Группа", ГруппаСпецифик));
	Если СтрокиГруппы.Количество() = 0 Тогда
		СтрокиГруппы = ТаблицаСпецифик.Скопировать(Новый Структура("ПодГруппа", ГруппаСпецифик));
	КонецЕсли;
	СтрокиГруппы.Сортировать("Порядок");
	
	КоличествоСпецифик = СтрокиГруппы.Количество(); 
	Если КоличествоСпецифик > 8 Тогда
		чСтрок = 3;
		чКолонок = 5;
		Вграница = 14;
		ЭлементыФормы.ПолеТабличногоДокумента.Высота = 845 + 75;
		КПКратко = Истина;
		КартинкаИмяРек = "КартинкаСпецифика";
	Иначе	
		чСтрок = 2;
		чКолонок = 4;
		Вграница = 7;
		ЭлементыФормы.ПолеТабличногоДокумента.Высота = 845;
		КартинкаИмяРек = "КартинкаСпецификаБольшая";
	КонецЕсли;
	ОбластьМакета = Макет1.ПолучитьОбласть(СтрШаблон("СтрокаСпецифик|ВидимаяОбласть_%1х%2", чКолонок, чСтрок));
	
	Для Яч = 0 По Вграница Цикл
		Если Яч > КоличествоСпецифик - 1 Тогда
			ОбластьМакета.Параметры.Наименование = "";
			ОбластьМакета.Параметры.Цена = "";
			ТекКартинка = Неопределено;
			ТекСпецификаВыбрана = 0;
		Иначе
			ТекСпецифика = СтрокиГруппы[Яч];
			ТекСпецификаВыбрана = ТекСпецифика.Выбрана;
			ОбластьМакета.Параметры.Наименование = ТекСпецифика.Специфика.Наименование;
			ОбластьМакета.Параметры.Цена = Формат(ТекСпецифика.Цена, "ЧДЦ=0; ЧН==; ЧГ=3,0") + " " + ?(ТекСпецифика.Цена, Символ(8381), "");
			СтрокаТабКартинок = ТаблицаСКартинками.Найти(ТекСпецифика.Специфика, "Специфика");
			Если НЕ СтрокаТабКартинок = Неопределено Тогда
				СсылкаНаКартинку = СтрокаТабКартинок[КартинкаИмяРек];
				Если Не СсылкаНаКартинку = Null Тогда
					ТекКартинка = СсылкаНаКартинку.Хранилище.Получить();
				КонецЕсли;                                              				
			КонецЕсли;
		КонецЕсли;
		
		ОбластьМакета.Область("R1C1:R2C1").Картинка = ТекКартинка;
		Если ТекСпецификаВыбрана = 0 Тогда
			ОбластьМакета.Область("R4C1:R4C2").Картинка = Неопределено;
			ОбластьМакета.Область("R3C1:R4C2").ЦветФона = ЦветаСтиля.ЦветФонаФормы;
		Иначе
			ОбластьМакета.Область("R4C1:R4C2").Картинка = БиблиотекаКартинок.CheckMark48;
			ОбластьМакета.Область("R3C1:R4C2").ЦветФона = WebЦвета.НейтральноЗеленый;
		КонецЕсли;
...
КонецПроцедуры
Процедура ЗаполнитьТаблицуСпецифик_Стандарт(ТабличныйДокумент)
	
	Макет1 = ЭтотОбъект.ПолучитьМакет("Специфика1280х1024");
	
	ОбластьМакетаЗаголовок		= Макет1.ПолучитьОбласть("Заголовок|ВидимаяОбласть");
	// Товар
	ОбластьМакетаТовар1			= Макет1.ПолучитьОбласть("СтрокаТовара1|ВидимаяОбласть");
	ОбластьМакетаТовар2			= Макет1.ПолучитьОбласть("СтрокаТовараНов2|ВидимаяОбласть");
	ОбластьМакетаТоварРед 		= Макет1.ПолучитьОбласть("СтрокаТовараРед|ВидимаяОбласть");
	// Специфики
	ОбластьМакетаСпецифика1		= Макет1.ПолучитьОбласть("СтрокаСпецифики1|ВидимаяОбласть");
	ОбластьМакетаСпецифика2		= Макет1.ПолучитьОбласть("СтрокаСпецификиНов2|ВидимаяОбласть");
    ОбластьМакетаСпецификаРед	= Макет1.ПолучитьОбласть("СтрокаСпецификиРед|ВидимаяОбласть");
	...
	КонецЕсли;
	ИтогиПоТовару = ДеревоСтрокаЗаказа.Строки[0];
	ОбластьМакетаТовар2.Параметры.Наименование = пНаименование;
	ОбластьМакетаТовар2.Параметры.Количество = ИтогиПоТовару.Количество;
	ОбластьМакетаТовар2.Параметры.Цена = ИтогиПоТовару.ЦенаРеализации;
	ОбластьМакетаТовар2.Параметры.Сумма = ИтогиПоТовару.СуммаРеализации;
	...
	//ОбластьТоварЦена = ТабличныйДокумент.Область("R2C5");
	//ОбластьТоварСумма = ТабличныйДокумент.Область("R2C7");
...
Процедура ОбновитьТаймер()
	...
	тВремя	= Формат(ТекВремя,"ДФ='HH:mm
	|ddd, dd MMMM'");	
КонецПроцедуры
...
//Z+			
		ИначеЕсли РежимВывода = "Специфика" И НЕ Товар.СпецификиТипЭкранаГостя = 2 Тогда
...
//Z-			
	...
	Если ФирмаФильтр = Неопределено Тогда
		тЗаказ = "Ваш ЗАКАЗ";
	Иначе
		тЗаказ = "Ваш ЗАКАЗ " + ФирмаФильтр.МестоРеализации.Наименование;
	КонецЕсли;
	Попытка
	...
	// отображение ПД на МГ
	Если ПоказатьЗаказ И НЕ Заказ.Пустая() Тогда
		Если ЛояльностьКлиентСервер.РежимОффлайн() Тогда
			Если НЕ ЭлементыФормы.ПолеКартинкиГостя.Высота = 1024 Тогда
				ЭлементыФормы.ПолеКартинкиГостя.Высота = 1024;
			КонецЕсли;
			ЭлементыФормы.ПолеКартинкиГостя.Картинка = ИзображениеДляМонитораГостя.Хранилище.Получить();
			ТекСкрыть = Истина;
		ИначеЕсли ПустаяСтрока(Заказ.НомерКартыЛояльности) Тогда
			Если НЕ ЭлементыФормы.ПолеКартинкиГостя.Высота = ВысотаТабДокЛояльностьДоЛУЧа Тогда
				ЭлементыФормы.ПолеКартинкиГостя.Высота = ВысотаТабДокЛояльностьДоЛУЧа;
			КонецЕсли;
			Попытка
				ЭлементыФормы.ПолеКартинкиГостя.Картинка = ЭтотОбъект["ИзображениеДляМонитораГостя" + Заказ.ОтказОтКартыЛояльности].Хранилище.Получить();
			Исключение
				ЭлементыФормы.ПолеКартинкиГостя.Высота = 1024;
				ЭлементыФормы.ПолеКартинкиГостя.Картинка = ИзображениеДляМонитораГостя.Хранилище.Получить();
			КонецПопытки;
		...
		Иначе
			...
			ЭлементыФормы.ПолеКартинкиГостя.Картинка = ИзображениеДляМонитораГостя.Хранилище.Получить();
			Попытка
				ДЛПЗ = РегистрыСведений.ДанныеЛояльностиПоЗаказу.Получить(Новый Структура("Документ,ТипДанных", Заказ, "СкрытьПерсДанные"));
				ТекСкрыть = ДЛПЗ.Данные = "1";
			Исключение
			    ТекСкрыть = Ложь;
			КонецПопытки;
		КонецЕсли;
		ЭлементыФормы.ПолеКартинкиГостя.Видимость = ТекСкрыть;
	КонецЕсли;
	
	Если ПоказатьЗаказ Тогда
		ЭтаФорма.ПодключитьОбработчикОжидания("ВывестиАлерты_Ожидание", 2, Истина);
	КонецЕсли;
...
Процедура ТоварГетИнфоДокументСформирован(Элемент)
	...
	ПоказатьХТМЛ = СтрНайти(Элемент.Документ.body.innerText, "HTTP 404") = 0 И НЕ ПустаяСтрока(Элемент.Документ.url) И НЕ Элемент.Документ.url = "about:blank";
	
	Если НЕ ПоказатьХТМЛ Тогда
		Элемент.Видимость = Ложь;
	КонецЕсли;
	//Элемент.Видимость = ПоказатьХТМЛ;
	
КонецПроцедуры

Процедура ЗаполнитьСпецификиПоСтроке(ОбластьМакетаСтрока, НомерСтрокиТовара)
	
	ВыбранныеСпецифики = ТаблицаСпецифик.Скопировать(Новый Структура("НомерСтрокиТовара", НомерСтрокиТовара), "Специфика,ЦенаРеализации,Количество");
	мПодстрок = Новый Массив;
	Для каждого ТекСтрока Из ВыбранныеСпецифики Цикл
		Если ТекСтрока.ЦенаРеализации Тогда
			мПодстрок.Добавить(СтрШаблон("%1 ( %2 %3 )", ТекСтрока.Специфика, Формат(ТекСтрока.ЦенаРеализации * ТекСтрока.Количество, "ЧДЦ=; ЧГ=0"), Символ(8381)));
		Иначе	
			мПодстрок.Добавить("" + ТекСтрока.Специфика);
		КонецЕсли;
	КонецЦикла;
	СпецификиСтрокой = СтрСоединить(мПодстрок, "; ");
	
	Если ПустаяСтрока(СпецификиСтрокой) Тогда
		ОбластьМакетаСтрока.Область(2,,2,).Видимость = Ложь;
	Иначе
		ОбластьМакетаСтрока.Область(2,,2,).Видимость = Истина;
		ОбластьМакетаСтрока.Параметры.Специфики = СпецификиСтрокой;
	КонецЕсли;
	
КонецПроцедуры

Процедура ВывестиАлерты_Ожидание() Экспорт 
	
	ЭтаФорма.ОтключитьОбработчикОжидания("ВывестиАлерты_Ожидание");
	
	Если ЗначениеЗаполнено(ФирмаФильтр) Тогда
	...
	Если ТипЗнч(ЛояльностьДанныеЗаказа) = Тип("Структура") И ЛояльностьДанныеЗаказа.Свойство("Алерты", Алерты) Тогда
		Для Сч = 0 По Алерты.ВГраница() Цикл
			ТекАлерт = Алерты[Сч];
			ТекПроект = ТекАлерт.Проект;
			Если НЕ ПроектФильтр = 0 И НЕ ТекПроект = 0 И НЕ ТекПроект = ПроектФильтр Тогда
				Продолжить;
			КонецЕсли;
			
			РаботаСокнами.ПоказатьПлашкуПрост(ТекАлерт.Наименование, ТекАлерт.ИнфоГостя, Сч + 1, "5000", 1024 + 490, 230);
		КонецЦикла;
	КонецЕсли;
