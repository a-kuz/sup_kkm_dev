...
Функция КассовыйОтчет() Экспорт
	...
	СуммаСтрокой = Формат(СуммаИтогоПриход,"ЧГ=3,0");
	ТекстСтроки = "Приход ";
	...
	СуммаСтрокой = Формат(СуммаПриходНал,"ЧГ=3,0");
	...
	СуммаСтрокой = Формат(СуммаПриходБНал,"ЧГ=3,0");
	...
	СуммаСтрокой = Формат(СуммаИтогоВозврат,"ЧГ=3,0");
	ТекстСтроки = "Возврат ";
	Кол = 41 - СтрДлина(ТекстСтроки) - СтрДлина(СуммаСтрокой);
	Для к = 1 по Кол Цикл
		ТекстСтроки = ТекстСтроки + " ";
	...
	СуммаСтрокой = Формат(СуммаВозвратНал,"ЧГ=3,0");
	ТекстСтроки = "    Наличными ";
	Кол = 41 - СтрДлина(ТекстСтроки) - СтрДлина(СуммаСтрокой);
	Для к = 1 по Кол Цикл
		ТекстСтроки = ТекстСтроки + " ";
	...
	СуммаСтрокой = Формат(СуммаВозвратБНал,"ЧГ=3,0");
	ТекстСтроки = "    Электронными ";
	Кол = 41 - СтрДлина(ТекстСтроки) - СтрДлина(СуммаСтрокой);
	Для к = 1 по Кол Цикл
		ТекстСтроки = ТекстСтроки + " ";
	...
	Пока ВыборкаВозврат.Следующий() Цикл
		...
		СуммаСтрокой = Формат(ВыборкаВозврат.СуммаФакт,"ЧГ=3,0");
		ТекстСтроки = "    №" + Формат(ВыборкаВозврат.ЗаказНомерЧека,"ЧГ=0") + "," + Формат(ВыборкаВозврат.ЗаказДата,"ДФ=ЧЧ:мм") + "," + ?(ВыборкаВозврат.ВариантОплаты = Перечисления.ТипыОплаты.Нал,"нал","б/нал");
		Кол = 41 - СтрДлина(ТекстСтроки) - СтрДлина(СуммаСтрокой);
		Если Кол < 0 Тогда
			Отчет.Добавить(ТекстСтроки);
			ТекстСтроки = "    ";
	...
	СуммаСтрокой = Формат(СуммаИтогоВнесение,"ЧГ=3,0");
	...
	для каждого СтрВнес из ТаблВнесение Цикл
		...
		Отчет.Добавить(ТекстСтроки);
		СуммаСтрокой = Формат(СтрВнес.СуммаВнесения,"ЧГ=3,0");
	...
	СуммаСтрокой = Формат(СуммаИтогоИзъятие,"ЧГ=3,0");
	ТекстСтроки = "Изъятие ";
	Кол = 41 - СтрДлина(ТекстСтроки) - СтрДлина(СуммаСтрокой);
	Для к = 1 по Кол Цикл
		ТекстСтроки = ТекстСтроки + " ";
	...
	для каждого СтрИзъя из ТаблИзъятие Цикл
		Сч = Сч + 1;
		ТекстСтроки = "    Внесение " + Формат(Сч,"ЧГ=0") + " - " + СокрЛП(СтрИзъя.Автор);
		Отчет.Добавить(ТекстСтроки);
		СуммаСтрокой = Формат(СтрИзъя.СуммаВнесения,"ЧГ=3,0");
	...
	СуммаСтрокой = Формат(СуммаИтогоНаличность,"ЧГ=3,0");
	...
	СуммаСтрокой = Формат(СуммаЧистаяВыручка,"ЧГ=3,0");
