...
Функция КассовыйОтчет() Экспорт
	...
	СуммаСтрокой = Формат(СуммаИтогоПриход,"ЧДЦ=2; ЧГ=3,0");
	ТекстСтроки = "Приход ";
	...
	СуммаСтрокой = Формат(СуммаПриходНал,"ЧДЦ=2; ЧГ=3,0");
	...
	СуммаСтрокой = Формат(СуммаПриходБНал,"ЧДЦ=2; ЧГ=3,0");
	...
	СуммаСтрокой = Формат(СуммаИтогоВозврат,"ЧДЦ=2; ЧГ=3,0");
	ТекстСтроки = "Возврат ";
	Кол = 41 - СтрДлина(ТекстСтроки) - СтрДлина(СуммаСтрокой);
	Для к = 1 по Кол Цикл
		ТекстСтроки = ТекстСтроки + " ";
	...
	СуммаСтрокой = Формат(-1*СуммаВозвратНал,"ЧДЦ=2; ЧГ=3,0");
	ТекстСтроки = "    Наличными ";
	Кол = 41 - СтрДлина(ТекстСтроки) - СтрДлина(СуммаСтрокой);
	Для к = 1 по Кол Цикл
		ТекстСтроки = ТекстСтроки + " ";
	...
	СуммаСтрокой = Формат(-1*СуммаВозвратБНал,"ЧДЦ=2; ЧГ=3,0");
	ТекстСтроки = "    Электронными ";
	Кол = 41 - СтрДлина(ТекстСтроки) - СтрДлина(СуммаСтрокой);
	Для к = 1 по Кол Цикл
		ТекстСтроки = ТекстСтроки + " ";
	...
	Пока ВыборкаВозврат.Следующий() Цикл
		...
		СуммаСтрокой = Формат(ВыборкаВозврат.СуммаФакт,"ЧДЦ=2; ЧГ=3,0");
		ТекстСтроки = "    №" + Формат(-1*ВыборкаВозврат.ЗаказНомерЧека,"ЧГ=0") + "," + Формат(ВыборкаВозврат.ЗаказДата,"ДФ=ЧЧ:мм") + "," + ?(ВыборкаВозврат.ВариантОплаты = Справочники.ВариантыОплаты.Наличные,"нал","б/нал");//?(ВыборкаВозврат.ВариантОплаты = Перечисления.ТипыОплаты.Нал,"нал","б/нал");
		Кол = 41 - СтрДлина(ТекстСтроки) - СтрДлина(СуммаСтрокой);
		Если Кол < 0 Тогда
			Отчет.Добавить(ТекстСтроки);
			ТекстСтроки = "    ";
	...
	СуммаСтрокой = Формат(СуммаИтогоВнесение,"ЧДЦ=2; ЧГ=3,0");
	...
	для каждого СтрВнес из ТаблВнесение Цикл
		...
		Отчет.Добавить(ТекстСтроки);
		СуммаСтрокой = Формат(СтрВнес.СуммаВнесения,"ЧДЦ=2; ЧГ=3,0");
	...
	СуммаСтрокой = Формат(СуммаИтогоИзъятие,"ЧДЦ=2; ЧГ=3,0");
	ТекстСтроки = "Изъятие ";
	Кол = 41 - СтрДлина(ТекстСтроки) - СтрДлина(СуммаСтрокой);
	Для к = 1 по Кол Цикл
		ТекстСтроки = ТекстСтроки + " ";
	...
	для каждого СтрИзъя из ТаблИзъятие Цикл
		...
		Отчет.Добавить(ТекстСтроки);
		СуммаСтрокой = Формат(СтрИзъя.СуммаВнесения,"ЧДЦ=2; ЧГ=3,0");
	...
	СуммаСтрокой = Формат(СуммаИтогоНаличность,"ЧДЦ=2; ЧГ=3,0");
	ТекстСтроки = "Наличность ";
	Кол = 41 - СтрДлина(ТекстСтроки) - СтрДлина(СуммаСтрокой);
	Для к = 1 по Кол Цикл
		ТекстСтроки = ТекстСтроки + " ";
	КонецЦикла;
	...
	СуммаСтрокой = Формат(СуммаЧистаяВыручка,"ЧДЦ=2; ЧГ=3,0");
