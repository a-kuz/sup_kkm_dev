...
Функция НаличиеВКассе() экспорт
	...
	//НовСтрока.РабочееМесто = глПараметрыРМ.ККМ.Фирма.РабочееМесто;
	НовСтрока.ККМ = глПараметрыРМ.ККМ;
	Если глПараметрыРМ.ККМЕстьДоп Тогда
		Попытка
			НовСтрока = СписокРабочихМест.Добавить();
			//НовСтрока.РабочееМесто = глПараметрыРМ.ККМСписокДоп[0].Значение.РабочееМесто;
			НовСтрока.ККМ = глПараметрыРМ.ККМСписокДоп[0].Значение;
		Исключение
		КонецПопытки;
	КонецЕсли;
...
Функция СуммаПоБНТекущая(Сумма,ПротоколРасчетов = Неопределено) Экспорт
	...
	Иначе			
		...
		Если ПротоколРасчетов = Неопределено Тогда
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"&КодТТ",формат(глРабочееМесто.Фирма.КодТТ,"ЧН=0; ЧГ=0"));
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"&НомерККМ",Формат(глПараметрыРМ.ККМ.КодСуп,"ЧН=0; ЧГ=0"));
		Иначе
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"&КодТТ",формат(ПротоколРасчетов.Фирма.КодТТ,"ЧН=0; ЧГ=0"));
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"&НомерККМ",Формат(ПротоколРасчетов.ККМ.КодСуп,"ЧН=0; ЧГ=0"));
		КонецЕсли;
		Отказ = Ложь;
		ТекстОшибки = "";
		...
		Для каждого стр из ТЗСлип Цикл
			...
			Для инд = 1 по СтрЧислоСтрок(Строки) Цикл
				...
				Если СтрНайти(СтрокаТекста,"Отдел     :") Тогда
					НомерОтдела = НомерОтдела + 1;
					СтрТабл = ТаблицаОтделов.Найти(НомерОтдела,"Отдел");
					Если СтрТабл = Неопределено Тогда
						СтрТабл = ТаблицаОтделов.Добавить();
					КонецЕсли;
					СтрТабл.Отдел = НомерОтдела;
					СтрТабл.Сумма = СтрТабл.Сумма + СуммаИтого;
					СуммаИтого = 0;
					ТипБлока = 0;
				КонецЕсли;
		...
		Если ТаблицаОтделов.Количество() <> 0 Тогда
			...
			Если глПараметрыРМ.ККМЕстьДоп Тогда
				МассивККМ.Добавить(глПараметрыРМ.ККМСписокДоп[0].Значение);
			КонецЕсли;
			СтрТабл = ТаблицаОтделов.Найти(ПротоколРасчетов.Фирма.Мерчант,"Отдел");
			Если СтрТабл = Неопределено Тогда
				СуммаИтого = 0;
			Иначе
				СуммаИтого = СтрТабл.Сумма;
			КонецЕсли;
		КонецЕсли;
