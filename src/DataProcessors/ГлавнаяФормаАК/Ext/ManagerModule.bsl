...
Функция ОткрытьКассовуюСмену() Экспорт
	...
	Исключение
		...
		//Возврат СтруктураОшибки(Истина, "Ошибка ККМ", ОписаниеОшибки(), "ЗакрытьПриложение");
	КонецПопытки;
	Если НЕ Ответ Тогда
		...
		//Возврат СтруктураОшибки(Истина, "Текущее состояние ККТ", ТекстОшибки, "ЗакрытьПриложение");
	...
	Если ДокСмены.Пустая() Тогда
		...
		Если НЕ Ответ Тогда
			...
			//Возврат СтруктураОшибки(Истина, "Ошибка Открытия смены", ТекстОшибки, "ЗакрытьПриложение");
			
		КонецЕсли;
		
	КонецЕсли;
	
	Возврат СтруктураОшибки(, "ОК", "Смена ККМ открыта");
...
Функция ЗакрытьКассовуюСмену() Экспорт 
	...
	Исключение
		...
		//Возврат СтруктураОшибки(Истина, "Ошибка платежного терминала", "", "ЗакрытьПриложение");
	КонецПопытки;
	ВходныеПараметры = новый Структура;
	...
	Попытка
		...
		Если НЕ ОтветПС Тогда
			...
			//Возврат СтруктураОшибки(Истина, "Ошибка платежного терминала", ВыходныеПараметры[0], "ЗакрытьПриложение");
		Иначе
				...
				Если не ОтветККТ Тогда
					Если ВыходныеПараметры.Свойство("Описаниеошибки") Тогда
						...
						Иначе
							...
							//Возврат СтруктураОшибки(Истина, "Ошибка платежного терминала", ТекстОшибки, "ЗакрытьПриложение");	
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
...
//DeBuG		Возврат СтруктураОшибки(Истина, "Ошибка платежного терминала", "", "ЗакрытьПриложение");
	...
	Если НЕ ОтветЗ Тогда
		...
		//Возврат СтруктураОшибки(Ложь, "Ошибка Закрытия смены", ТекстОшибки, "ЗакрытьПриложение");
	...
	Возврат СтруктураОшибки(, "ОК", "Смена ККМ закрыта");
...
Функция ЗакрытиеДняСбербанк() Экспорт
	...
	Возврат СтруктураОшибки(, "ОК", "Закрыта смена по СБ");
...
Процедура ПечатьПодвалаЛояльности(Фирма = Неопределено, ККМСсылка, ЗаказСсылка, ПротоколРасчетовСсылка = Неопределено) Экспорт 
	...
	ЗаписьЖурналаРегистрации("Оплата.ПечатьПодвалаЛояльности.До вызова", УровеньЖурналаРегистрации.Информация, , Заказ.Ссылка);
