...
//DeBuG	КонецЕсли;
	...
	Если ЛояльностьДоступна
//ТЧЛ1		И ЗначениеЗаполнено(НомерКартыЛояльности)
		Тогда
	...
	ПараметрыФормы.Вставить("ТаблицаТовары", СформироватьТоварыДляПередачи(, "Товар,Количество,фАлкоголь,МестоРеализации,КОплате,action_group,Цена,Наименование,Фирма"));
...
Процедура ОбрабокаЗакрытияФормыВыборСпособаОплаты(Результат, ДополнительныеПараметры) Экспорт 
	...
	ИначеЕсли Результат.ВариантОплаты = 0 Тогда
		...
		Для Инд = 0 По МестаРеализации.Количество() - 1 Цикл
			Если МестаРеализации[Инд].КОплате > 0 Тогда
		...
		//отработка лояльности
//ТЧЛ1		Если ЗначениеЗаполнено(НомерКартыЛояльности) Тогда
...
//ТЧЛ1		КонецЕсли;
		...
		ПараметрыФормы.Вставить("СписаниеОплата", СписаниеОплата);
		ПараметрыФормы.Вставить("СписаниеСписание", СписаниеСписание);
		ПараметрыФормы.Вставить("ТаблицаТовары", СформироватьТоварыДляПередачи(Фирма, "Товар,Количество,фАлкоголь,МестоРеализации,КОплате,action_group,Цена,Наименование,Фирма"));
		ПараметрыФормы.Вставить("ПоказатьТаблицуТовары", Истина);
		ПараметрыФормы.Вставить("ИдентификаторЛояльности", ИдентификаторЛояльности);
...
&НаКлиенте
Процедура тестМенюОпераций(Команда)
	ОткрытьМенюОпераций();
КонецПроцедуры

&НаКлиенте
Процедура тестНомерКартыЗаказа(Команда)
...
&НаКлиенте
Процедура ВывестиСообщениеОбОшибке(СтруктураОшибки)
	...
	КонецПопытки;
	ОткрытьФорму("Обработка.ГлавнаяФормаАК.Форма.ФормаОшибка", СтруктураОшибки, ЭтотОбъект, , , , Новый ОписаниеОповещения("ВывестиСообщениеОбОшибкеЗавершение", ЭтотОбъект), );
...
Функция ПроверкаНаличияОткрытыхСмен(МестоРеализации)
	...
	Иначе
		...
		СтруктураОшибки.Вставить("РезультатКнопкиОК", "ЗакрытьПриложение");
...
Функция ОбновитьРеквизитыПоЗаказу(ЗаказСсылка, Тест)
	...
	Если ЗначениеЗаполнено(ЗаказСсылка) Тогда
		...
		//|	И ЗаказТовары.КоличествоУдалено = 0
...
Процедура ОбновитьОтображениеЭлементовДляМестаРеализации(МестоРеализации)
	...
	Если МестоРеализации = ПредопределенноеЗначение("Справочник.МестаРеализации.Ресторан") Тогда
		...
		Элементы.НадписьЗавершение_7_2.Заголовок = "Не забудьте сдать бэйдж гостя";
