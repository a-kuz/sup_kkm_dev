...
//ТЧЛ1		И ЗначениеЗаполнено(НомерКартыЛояльности)
	...
	ПараметрыФормы.Вставить("ТаблицаТовары", СформироватьТоварыДляПередачи(, "Товар,Количество,фАлкоголь,МестоРеализации,КОплате,action_group,ЦенаРеализации,Наименование,Фирма,ККМ,ЛояльностьГруппаАкции"));
...
//ТЧЛ1		КонецЕсли;
		...
		ПараметрыФормы.Вставить("СписаниеОплата", СписаниеОплата);
		ПараметрыФормы.Вставить("СписаниеСписание", СписаниеСписание);
		ПараметрыФормы.Вставить("ТаблицаТовары", СформироватьТоварыДляПередачи(Фирма, "Товар,Количество,фАлкоголь,МестоРеализации,КОплате,action_group,ЦенаРеализации,Наименование,Фирма,ККМ,ЛояльностьГруппаАкции"));
		ПараметрыФормы.Вставить("ПоказатьТаблицуТовары", Истина);
		ПараметрыФормы.Вставить("ИдентификаторЛояльности", ИдентификаторЛояльности);
...
Процедура ВывестиСообщениеОбОшибке(СтруктураОшибки)
	...
	КонецПопытки;
	ОткрытьФорму("Обработка.ГлавнаяФормаАК.Форма.ФормаОшибка", СтруктураОшибки, ЭтотОбъект, Истина, , , Новый ОписаниеОповещения("ВывестиСообщениеОбОшибкеЗавершение", ЭтотОбъект), );
