...
Функция ЗаполнитьПараметрыФормыПредупреждения(Фирма)
	...
	ПараметрыФормыПредупреждения = Новый Структура("НетВозможностиОплатить,ОплатаМР1,ОплатаМР2", "", "", "");
	...
	Если СтрокаМестаРеализации.Количество() Тогда
		...
		ПараметрыФормыПредупреждения.Вставить("НетВозможностиОплатить", "Для оплаты " + СтрСоединить(МассивПодстрок) + " Вам необходимо обратиться в соответствующий отдел магазина");
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(Фирма) Тогда
		...
		Для каждого СтрокаМестаРеализации Из МестаРеализации Цикл
			Если НЕ СтрокаМестаРеализации.ККМ.Пустая() И НЕ СтрокаМестаРеализации.Фискализирован Тогда
				...
				ПараметрыФормыПредупреждения.Вставить("ОплатаМР" + Сч, СтрШаблон("Оплата %1 на сумму %2 %3", ФирмаКратко, СтрокаМестаРеализации.КОплате, Символ(8381)));
				Сч = Сч + 1;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
