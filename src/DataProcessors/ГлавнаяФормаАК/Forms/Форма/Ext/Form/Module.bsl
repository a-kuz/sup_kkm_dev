...
Процедура ВнешнееСобытие(Источник, Событие, Данные)
	...
	ЗаписьЖурналаРегистрации("Автокасса.Внешнее событие", УровеньЖурналаРегистрации.Информация, , , СтрШаблон("Прочитан ID %1", _Знач));
	...
	ЗаписьЖурналаРегистрации("Автокасса.Поиск заказа.Начало", УровеньЖурналаРегистрации.Информация, , , СтрШаблон("ID=%1", _Знач));
	НачатьОжидание();
	...
	КонецЕсли;
	ЗаписьЖурналаРегистрации("Автокасса.Поиск заказа", УровеньЖурналаРегистрации.Информация, , _КартаДоступаКлиента, СтрШаблон("ID=%1", _Знач));
	...
	Если НЕ ЗначениеЗаполнено(ЗаказСсылка) Тогда
		...
		Возврат;
	КонецЕсли;
	ЗаписьЖурналаРегистрации("Автокасса.Поиск заказа.Окончание", УровеньЖурналаРегистрации.Информация, , ЗаказСсылка, СтрШаблон("ID=%1", _Знач));
...
Процедура ЗаписатьЗаказ(Статус, Заказ)
	...
	Если Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаказа.Закрыт") Тогда
		Если глРабочееМесто.МестоРеализации = ПредопределенноеЗначение("Справочник.МестаРеализации.КМ") Тогда
			ЗаписьЖурналаРегистрации("Лояльность.ЗакрытиеЗаказа.До вызова", УровеньЖурналаРегистрации.Информация, , Заказ.Ссылка);
			ЛояльностьКлиент.ЗакрытьЗаказ(Заказ, ПротоколРасчетов);
			Заказ.Записать();
			ЗаписьЖурналаРегистрации("Лояльность.ЗакрытиеЗаказа.После вызова", УровеньЖурналаРегистрации.Информация, , Заказ.Ссылка);
		КонецЕсли;
