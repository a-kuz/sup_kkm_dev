...
Процедура ПриОткрытии(Отказ)
	...
	КонецЕсли;
	РезультатИнициализации.Свойство("ФискальныйРежим", ФискальныйРежим);
	РезультатИнициализации.Свойство("ВариантОтправкиЭлектронногоЧека", ВариантОтправкиЭлектронногоЧека);
...
//УДАЛИТЬ	РезультатПроверки = ПроверкаНаличияОткрытыхСмен(глПараметрыРМ.МестоРеализации);
	...
	Если глПараметрыРМ.ККМЕстьДоп Тогда
		Для каждого ТО Из глПараметрыРМ.ККМСписокДоп Цикл
			РезультатПроверки = ПроверкаКассовойСмены(ТО.Значение);
			Если РезультатПроверки.Ошибка Тогда
				Отказ = Истина;
				ВывестиСообщениеОбОшибке(РезультатПроверки);
			КонецЕсли;
...
Процедура ОтобразитьЗаказВФорме(ЗаказСсылка, ЛояльностьДоступна = Истина)
	...
	ИначеЕсли НЕ ВозможнаДальнейшаяОплата(ЗаказСсылка, глПараметрыРМ.ФирмыРМ) Тогда
		
		ВывестиСообщениеОбОшибке(СтруктураОшибки(Истина, "У Вас не оплачен заказ ОТДОХНИ. Не забудьте пройти в ОТДОХНИ для его оплаты!", "На этой кассе дальнейшая оплата невозможна.", "НачальноеОкно"));
		Возврат;
	...
	МестаРеализации.Сортировать("Фискализирован Убыв,ДопПорядок Убыв,КОплате Убыв");
...
//DeBuG	ПараметрыФормы.Вставить("ПИН_Хэш", "81dc9bdb52d04dc20036dbd8313ed055"); 
	...
	ПараметрыФормы.Вставить("ВариантОтправкиЭлектронногоЧека", ВариантОтправкиЭлектронногоЧека);
	ПараметрыФормы.Вставить("ФискальныйРежим", ФискальныйРежим);
...
Процедура ОбрабокаЗакрытияФормыВыборСпособаОплаты(Результат, ДополнительныеПараметры) Экспорт 
	...
	Иначе
		...
		ПараметрыФормыПредупреждения = ЗаполнитьПараметрыФормыПредупреждения(Фирма);
		ПараметрыФормыПредупреждения.Вставить("ПодвалПредупреждение", Результат.ПодвалПредупреждение);
...
//УДАЛИТЬ		ПараметрыФормы = Новый Структура("НомерТелефона,Email,РабочееМесто,ДекорацияКеглиРубли,ПодвалКеглиРубли", НомерТелефона, Email, глРабочееМесто, Результат.ДекорацияКеглиРубли, Результат.ПодвалКеглиРубли); 		
		...
		ПараметрыФормы.Вставить("ФискальныйРежим", ФискальныйРежим);
		ПараметрыФормы.Вставить("ВариантОтправкиЭлектронногоЧека", ВариантОтправкиЭлектронногоЧека);
		ДополнительныеПараметры.Вставить("ПараметрыФормы_ЗапросВариантаПолученияЧека", ПараметрыФормы);
		
		Оповещение = Новый ОписаниеОповещения("ОбрабокаЗакрытияФормыПредупреждение", ЭтотОбъект, ДополнительныеПараметры);
		
		ЕстьДвойнаяОплата = НЕ ПустаяСтрока(ПараметрыФормыПредупреждения.ОплатаМР1) И НЕ ПустаяСтрока(ПараметрыФормыПредупреждения.ОплатаМР2);
		ЕстьМР_БезОплаты = НЕ ПустаяСтрока(ПараметрыФормыПредупреждения.НетВозможностиОплатить);
		НеОткрывать = НЕ ЕстьМР_БезОплаты И НЕ ЕстьДвойнаяОплата;
	
		Если НеОткрывать Тогда
			ВыполнитьОбработкуОповещения(Оповещение, 1);
		Иначе
			ОткрытьФорму("Обработка.ГлавнаяФормаАК.Форма.Предупреждение", ПараметрыФормыПредупреждения, ЭтаФорма, , , , Оповещение);
		КонецЕсли;
...
&НаКлиенте
Процедура ОбрабокаЗакрытияФормыПредупреждение(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Результат = 1 Тогда
		...
		ПараметрыФормы = ДополнительныеПараметры.ПараметрыФормы_ЗапросВариантаПолученияЧека;
...
Процедура ОбрабокаЗакрытияФормыОплаты(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Результат.Ошибка = 0 Тогда
		...
		Элементы.ПродолжитьПокупки1.Видимость = Ложь;
		Элементы.ЗавершитьОбслуживание.Видимость = Истина;
		Элементы.ПродолжитьПокупки.Видимость = Истина;
		...
		Если ЗаказОплаченПолностью(Заказ.Ссылка) Тогда
			...
			глОтключитьОбработчикОжидания("глАвтоблокировка");
			флЗавершениеПоТаймауту = Истина;
			ПодключитьОбработчикОжидания("ЗавершитьОбслуживание_Ожидание", ?(глПараметрыРМ.БлокировкаАвтоВремя, глПараметрыРМ.БлокировкаАвтоВремя, 15), Истина);
		ИначеЕсли ВозможнаДальнейшаяОплата(Заказ.Ссылка, глПараметрыРМ.ФирмыРМ) Тогда	
			Элементы.ЗавершитьОбслуживание.Доступность = Ложь;
			ПеречитатьЗаказПоКартеКлиента();
		Иначе
			Элементы.ЗавершитьОбслуживание.Видимость = Ложь;
			Элементы.ПродолжитьПокупки.Видимость = Ложь;
			Элементы.ПродолжитьПокупки1.Видимость = Истина;
			ДопТекстПроБейдж = Символы.ПС + "Не забудьте забрать БЕЙДЖ гостя!";
		КонецЕсли;
		
		// все оплачено успешно
		Если СпособПолученияЧека = 1 Тогда
			Заголовок_6_1 = 
				"Чек/Чеки отправлен/ы Вам в СМС" + ДопТекстПроБейдж;
		ИначеЕсли СпособПолученияЧека = 2 Тогда
			Заголовок_6_1 = 
				"Чек/Чеки отправлен/ы Вам на e-mail" + ДопТекстПроБейдж;
		Иначе
			Заголовок_6_1 = 
				"Не забудьте забрать распечатанные чеки." + ДопТекстПроБейдж;
		КонецЕсли;
...
Функция ПроверкаНаличияОткрытыхСмен(МестоРеализации)
	...
	|	ЕСТЬNULL(ТекущиеСмены.Смена.Дата, &ТекДата) КАК ПараметрОшибки,
	|	ВЫБОР
	...
	|ГДЕ
	|	ТекущиеСмены.МестоРеализации В(&МестоРеализации)
	|	И (ТекущиеСмены.Смена.Ссылка ЕСТЬ NULL
...
Функция ПроверкаКассовойСмены(Касса)
	...
	ККМСтрокой = СтрШаблон("Касса %1 (%2):", ККМ.КодСУП, ККМ.Фирма);
	...
	Исключение
		Возврат СтруктураОшибки(Истина, "Ошибка ККМ", ККМСтрокой + Символы.ПС + ОписаниеОшибки(), "ОшибкаЗапуска");
	КонецПопытки;
	Если НЕ Ответ Тогда
		ТекстОшибки = "";
		...
		КонецЕсли;
		Возврат СтруктураОшибки(Истина, "Ошибка ККТ", ККМСтрокой + Символы.ПС + ТекстОшибки, "ОшибкаЗапуска");
	ИначеЕсли НЕ ВыходныеПараметры.СменаОткрыта Тогда
		Возврат СтруктураОшибки(Истина, "Ошибка ККТ", ККМСтрокой + Символы.ПС + "Не открыта смена ККТ.", "ОшибкаЗапуска");
	ИначеЕсли НачалоДня(ВыходныеПараметры.ДатаСменыККТ) <> НачалоДня(ТекущаяДата()) 
		...
		Возврат СтруктураОшибки(Истина, "Ошибка ККТ", ККМСтрокой + Символы.ПС + "Не закрыта смена прошлого дня.", "ОшибкаЗапуска");
	КонецЕсли;
	
	ДокСмены = Обработки.ГлавнаяФормаАК.ПолучитьСменуКассы(Касса);
	Если ДокСмены.Пустая() Тогда
		Возврат СтруктураОшибки(Истина, "Ошибка ККМ", ККМСтрокой + Символы.ПС + "Смена ККМ не открыта.", "ОшибкаЗапуска");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ЗаказСсылка) Тогда
		...
		Если ИтогиПоЗаказу.Следующий() Тогда
			...
			Пока ИтогиПоМестуРеализации.Следующий() Цикл
				...
				СтрокаМестаРеализации.НачислениеОплата = СтрокаМестаРеализации.КОплате;
...
&НаСервере
Функция ВозможнаДальнейшаяОплата(ЗаказСсылка, ФирмыРМ)
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ЗаказСсылка", ЗаказСсылка);
	Запрос.УстановитьПараметр("ФирмыРМ", ФирмыРМ);
	Запрос.Текст = 
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ЗаказТовары.НомерСтроки КАК НомерСтроки
	|ИЗ
...
&НаСервере
Функция ЗаполнитьПараметрыФормыПредупреждения(Фирма)
	
	ПараметрыФормыПредупреждения = Новый Структура("НетВозможностиОплатить,ОплатаМР1,ОплатаМР2,ПодвалКеглиРубли", "", "", "");
	СтрокаМестаРеализации = МестаРеализации.НайтиСтроки(Новый Структура("ККМ,Фискализирован", ПредопределенноеЗначение("Справочник.ТорговоеОборудование.ПустаяСсылка"), Ложь));
	Если СтрокаМестаРеализации.Количество() Тогда
		МассивПодстрок = МестаРеализации.Выгрузить(СтрокаМестаРеализации, "МестоРеализации").ВыгрузитьКолонку("МестоРеализации");
		ПараметрыФормыПредупреждения.Вставить("НетВозможностиОплатить", 
		СтрШаблон("У Вас не оплачен заказ %1. НЕ забудьте пройти в %1 для его оплаты!", ВРег(СтрСоединить(МассивПодстрок))));
		
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(Фирма) Тогда
		Сч = 1;
		Для каждого СтрокаМестаРеализации Из МестаРеализации Цикл
			Если НЕ СтрокаМестаРеализации.ККМ.Пустая() И НЕ СтрокаМестаРеализации.Фискализирован И СтрокаМестаРеализации.КОплате > 0 Тогда
				ФирмаКратко = ?(СтрокаМестаРеализации.МестоРеализации = ПредопределенноеЗначение("Справочник.МестаРеализации.Ресторан"), "Кухни Полли", СтрокаМестаРеализации.МестоРеализации);
				ПараметрыФормыПредупреждения.Вставить("ОплатаМР" + Сч, СтрШаблон("при оплате заказа %1%2" , ФирмаКратко, ?(Сч = 1, ",", "!")));
				Сч = Сч + 1;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	
	Возврат ПараметрыФормыПредупреждения;
