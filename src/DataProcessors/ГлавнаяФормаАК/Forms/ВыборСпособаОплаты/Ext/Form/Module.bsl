...
Функция ПолучитьТабдокПредупреждение(Кнопка = "Рубли")
	...
	Если ВРег(Кнопка) = "КЕГЛИ" Тогда
		Для Сч = 0 По МестаРеализацииКОплате.Количество() - 1 Цикл
			...
			ОблМакета.Параметры["Сумма" + Сч] = Формат(ТекСтрока.СписаниеОплата, "ЧДЦ=2; ЧН=-; ЧГ=3,0") + " " + Символ(8381);
		КонецЦикла;
		ОблМакета.Параметры.Сумма = Формат(СписаниеОплата, "ЧДЦ=2; ЧН=-; ЧГ=3,0") + " " + Символ(8381);
	КонецЕсли;
	...
	Если ВРег(Кнопка) = "РУБЛИ" Тогда
		Для Сч = 0 По МестаРеализацииКОплате.Количество() - 1 Цикл
			...
			ОблМакета.Параметры["МР" + Сч]    = ФирмаКратко;
			ОблМакета.Параметры["Сумма" + Сч] = Формат(ТекСтрока.НачислениеОплата, "ЧДЦ=2; ЧН=-; ЧГ=3,0") + " " + Символ(8381);
		КонецЦикла;
		ОблМакета.Параметры.Сумма = Формат(НачислениеОплата, "ЧДЦ=2; ЧН=-; ЧГ=3,0") + " " + Символ(8381);
	КонецЕсли;
