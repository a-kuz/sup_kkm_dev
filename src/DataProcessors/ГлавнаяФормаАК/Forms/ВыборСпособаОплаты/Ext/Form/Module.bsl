...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, Параметры, "КОплате,НачислениеНачисление,НачислениеОплата,СписаниеНачисление,СписаниеОплата,СписаниеСписание,НадписьКартаЛОЛ,ПИН_Хэш,НомерКартыЛояльности,НомерТелефона,НомерЗаказа,РабочееМесто,Баланс,ДисплейИнфо");
...
//УДАЛИТЬ		Элементы.НачислитьКегли.ЦветТекста = ЦветаСтиля.ЦветФонаФормы;
		...
		ИначеЕсли НЕ СписаниеВозможно Тогда
			...
			Если ТипЗнч(ДисплейИнфо) = Тип("Массив") Тогда
				Для каждого ТекИнфо Из ДисплейИнфо Цикл
			...
			ИначеЕсли ТипЗнч(ДисплейИнфо) = Тип("Строка") Тогда 
				мЗаголовокКегли.Добавить(Новый ФорматированнаяСтрока(ДисплейИнфо, ШрифтПомельче));
			КонецЕсли; 
		Иначе
