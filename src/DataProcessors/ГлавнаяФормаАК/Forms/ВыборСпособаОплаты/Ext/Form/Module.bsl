...
//УДАЛИТЬ		ПодвалКеглиРубли = ПолучитьТабДокПодписьКнопки("Рубли");
		...
		ПодвалПредупреждение = ПолучитьТабдокПредупреждение("Рубли");
...
//УДАЛИТЬ		ПодвалКеглиРубли = ПолучитьТабДокПодписьКнопки("КеГЛи");
		...
		ПодвалПредупреждение = ПолучитьТабдокПредупреждение("КеГЛи");
	Иначе
//УДАЛИТЬ		ДекорацияКеглиРубли = "";
		ПодвалКеглиРубли = Новый ТабличныйДокумент;
		ПодвалПредупреждение = Новый ТабличныйДокумент;
	КонецЕсли;
//УДАЛИТЬ	Возврат Новый Структура("ВариантОплаты,ТипЧекаЛояльность,НомерТелефона,КОплате,ДекорацияКеглиРубли,ПодвалКеглиРубли", ВыбранныйВариант, ТипЧекаЛояльность, НомерТелефона, ?(ВыбранныйВариант = 2, СписаниеОплата, КОплате), ДекорацияКеглиРубли, ПодвалКеглиРубли);
	Возврат Новый Структура("ВариантОплаты,ТипЧекаЛояльность,НомерТелефона,КОплате,ПодвалКеглиРубли,ПодвалПредупреждение", ВыбранныйВариант, ТипЧекаЛояльность, НомерТелефона, ?(ВыбранныйВариант = 2, СписаниеОплата, КОплате), ПодвалКеглиРубли, ПодвалПредупреждение);
...
Функция ПолучитьРасширенныйТабдокПодвалКеглиРубли(Кнопка = "Рубли")
	...
	Если ВРег(Кнопка) = "КЕГЛИ" Тогда
		...
		Если МестаРеализацииКОплате.Количество() = 2 Тогда
			...
			ОблМакета.Параметры.Текст = "ВНИМАНИЕ! Вам нужно применить банковскую карту 2 раза: при оплате заказа МясновЪ, при оплате заказа Кухни Полли!";
			ОблМакета.Область("R1C1:R1C3").ЦветТекста = ЦветаСтиля.ЦветОсобогоТекста;
			ТабДок.Вывести(ОблМакета);
		КонецЕсли;
...
&НаСервере
Функция ПолучитьТабдокПредупреждение(Кнопка = "Рубли")
	
	Макет = Обработки.ГлавнаяФормаАК.ПолучитьМакет("Надписи");
	ОблМакета = Макет.ПолучитьОбласть("ПодвалПредупреждение");
	ТабДок = Новый ТабличныйДокумент;
	
	Если ВРег(Кнопка) = "КЕГЛИ" Тогда
		Для Сч = 0 По МестаРеализацииКОплате.Количество() - 1 Цикл
			ТекСтрока = МестаРеализацииКОплате[Сч];
			Если ТекСтрока.Фирма.МестоРеализации = ПредопределенноеЗначение("Справочник.МестаРеализации.Ресторан") Тогда
				ФирмаКратко = "Кухне Полли";
			Иначе
				ФирмаКратко = СокрЛП(ТекСтрока.Фирма.МестоРеализации);
			...
			ОблМакета.Параметры["МР" + Сч]    = ФирмаКратко;
			ОблМакета.Параметры["Сумма" + Сч] = Формат(Цел(ТекСтрока.СписаниеОплата), "ЧДЦ=; ЧГ=3,0");
			ОблМакета.Параметры["денег" + Сч] = Формат(ТекСтрока.СписаниеОплата * 100 % 100, "ЧЦ=2; ЧН=00; ЧВН=") + " " + Символ(8381);
		КонецЦикла;
		ОблМакета.Параметры.Сумма = Формат(Цел(СписаниеОплата), "ЧДЦ=; ЧГ=3,0");
		ОблМакета.Параметры.денег = "," + Формат(СписаниеОплата * 100 % 100, "ЧЦ=2; ЧН=00; ЧВН=") + " " + Символ(8381);
	КонецЕсли;
	...
	Если ВРег(Кнопка) = "РУБЛИ" Тогда
		Для Сч = 0 По МестаРеализацииКОплате.Количество() - 1 Цикл
			...
			ОблМакета.Параметры["МР" + Сч]    = ФирмаКратко;
			ОблМакета.Параметры["Сумма" + Сч] = Формат(Цел(ТекСтрока.НачислениеОплата), "ЧДЦ=; ЧГ=3,0");
			ОблМакета.Параметры["денег" + Сч] = Формат(ТекСтрока.НачислениеОплата * 100 % 100, "ЧЦ=2; ЧН=00; ЧВН=") + " " + Символ(8381);
		КонецЦикла;
		ОблМакета.Параметры.Сумма = Формат(Цел(НачислениеОплата), "ЧДЦ=; ЧГ=3,0");
		ОблМакета.Параметры.денег = "," + Формат(НачислениеОплата * 100 % 100, "ЧЦ=2; ЧН=00; ЧВН=") + " " + Символ(8381);
	КонецЕсли;
