...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НадписьБаланс = Новый ФорматированнаяСтрока(Формат(Параметры.Баланс, "ЧДЦ=") + " КеГЛей");
...
Процедура ПоказатьБаланс(Команда)
	глОтсечкаПростоя();
	Если Элементы.ПоказатьБаланс.Заголовок = "Показать баланс" Тогда
		Элементы.ПоказатьБаланс.Заголовок = НадписьБаланс;
	Иначе
		Элементы.ПоказатьБаланс.Заголовок = "Показать баланс"
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбновитьОтображениеЭлементовДляМестаРеализации(МестоРеализации, СписаниеВозможно)
	...
	Если МестоРеализации = ПредопределенноеЗначение("Справочник.МестаРеализации.КМ") Тогда
		...
		МестаРеализации = ТаблицаТовары.Выгрузить(, "Фирма, КОплате");
		МестаРеализации.Свернуть("Фирма", "КОплате");
		Для каждого ТекСтрока Из МестаРеализации Цикл
			ФирмаКратко = Лев(СокрЛП(ТекСтрока.Фирма), 7);
			Если ТекСтрока.КОплате = 0 Тогда
				НадписьСписокКОплате.Добавить("Оплачено в " + ФирмаКратко);
			Иначе
				НадписьСписокКОплате.Добавить("К оплате в " + ФирмаКратко + ": " + Формат(ТекСтрока.КОплате, "ЧДЦ=2; ЧГ=3,0") + Символ(8381));
			КонецЕсли;
		КонецЦикла;
		Элементы.НадписьСписокКОплате.Заголовок = СтрСоединить(НадписьСписокКОплате, Символы.ПС);
