	...
	Если МестоРеализации = ПредопределенноеЗначение("Справочник.МестаРеализации.КМ") Тогда
		...
		Для каждого ТекСтрока Из МестаРеализации Цикл
			...
			ИначеЕсли ТекСтрока.ККМ.Пустая() Тогда 
				НадписьСписокКОплате.Добавить(Новый ФорматированнаяСтрока("К оплате в " + ФирмаКратко + ": " + Формат(ТекСтрока.КОплате, "ЧДЦ=2; ЧГ=3,0") + Символ(8381), БиблиотекаКартинок.Внимание36));
			Иначе
