...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	Если флТаймаут Тогда
		// по таймауту или по кнопке Закрыть
		Отказ = Истина;
...
Процедура ОткрытиеКассовойСмены(Команда)
	...
	Если Результат.Ошибка ИЛИ Результат.РезультатКнопкиОК = "СервисноеМеню" Тогда
...
Процедура ЗакрытиеПоТаймауту() Экспорт 
	...
	флТаймаут = Ложь;
	Результат =  Новый Структура("Ошибка,ТекстОшибки1,ТекстОшибки2,РезультатКнопкиОК", Ложь, "Выход из сервисного меню.", "Будет произведена попытка запуска автокассы.", "ОткрытьГлавнуюФорму");
