...
&НаСервере
Процедура ОбновитьОтображениеСостоянияОплаты(МестоРеализации, ТипТТ = Неопределено, ТекстТекущейОплаты = "")
	...
	Если флОплатаВПроцессе Тогда
		...
		ОбновитьТабДокСписокОплаты(ТипТТ, ТекстТекущейОплаты);
...
Процедура ОжиданиеНачалоОплаты()
	...
	Если Открыта() Тогда
		...
		Для каждого ТекСтрока Из МестаРеализации Цикл
			...
			ОбновитьОтображениеСостоянияОплаты(РабочееМесто.МестоРеализации, РабочееМесто.Фирма.ТипТТ, ТекстТекущейОплаты);
			Если НЕ ПустаяСтрока(ТекстТекущейОплаты) Тогда
				РаботаСокнами.ПоказатьПлашку("Оплата картой", ТекстТекущейОплаты, 2, "2500", 0, 255);
			КонецЕсли;
...
&НаСервере
Процедура ОбновитьТабДокСписокОплаты(ТипТТ, ТекстТекущейОплаты = "")
