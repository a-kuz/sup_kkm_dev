	...
	Если ТипТТ = ПредопределенноеЗначение("Справочник.ТипыТТ.МОКП") Тогда
		...
		Для каждого ТекСтрока Из МестаРеализации Цикл
			...
			Если ТекСтрока.Фискализирован Тогда
				...
				ОблМакета.Этап = ?(ОтображатьЭтапы И ТекСтрока.ЭтапОплаты, "Этап " + ТекСтрока.ЭтапОплаты, "");
				ОблМакета.Текст = СтрШаблон("Оплата покупок в %1 завершена", ФирмаКратко);
				ОблМакета.Сумма = ?(ТекСтрока.КОплате = 0, СуммаРеализацииСтрокой, СуммаСтрокой);
				СостояниеОплаты.Вывести(ОблМакета);	
			ИначеЕсли ТекСтрока.ККМ.Пустая() Тогда	
				ОблМакета = Макет.ПолучитьОбласть("Оплата_НеОплачено");
				ОблМакета.Этап = "";
				ОблМакета.Текст = СтрШаблон("Не забудьте оплатить покупки в  %1", ФирмаКратко);
				ОблМакета.Сумма = СуммаСтрокой;
				СостояниеОплаты.Вывести(ОблМакета);	
			ИначеЕсли ПервоеНеоплаченное Тогда
				ПервоеНеоплаченное = Ложь;
				ОблМакета = Макет.ПолучитьОбласть("Оплата_ВПроцессе");
				ОблМакета.Этап = ?(ОтображатьЭтапы И ТекСтрока.ЭтапОплаты, "Этап " + ТекСтрока.ЭтапОплаты, "");
				ОблМакета.Текст = СтрШаблон("Ожидается оплата покупок в %1", ФирмаКратко);
				ОблМакета.Сумма = СуммаСтрокой;
				СостояниеОплаты.Вывести(ОблМакета);	
			Иначе
