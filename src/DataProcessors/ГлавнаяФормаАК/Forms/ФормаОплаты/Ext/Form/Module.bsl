...
Процедура ОбновитьОтображениеСостоянияОплаты(МестоРеализации, ТипТТ = Неопределено, Форма)
	...
	ИначеЕсли ТипТТ = ПредопределенноеЗначение("Справочник.ТипыТТ.Ресторан") Тогда
		Элементы.СостояниеОплаты.Видимость = Ложь;
	КонецЕсли;
	...
	Если ТипТТ = ПредопределенноеЗначение("Справочник.ТипыТТ.МОКП") Тогда
		...
		Для каждого ТекСтрока Из МестаРеализации Цикл
			...
			Если ТекСтрока.Фискализирован Тогда
				...
				ОблМакета.Параметры.Текст = СтрШаблон("Оплата покупок в %1 завершена", ФирмаКратко);
			...
			ИначеЕсли ТекСтрока.ККМ.Пустая() Тогда	
				...
				ОблМакета.Параметры.Текст = СтрШаблон("Пройтиде в %1 для оплаты заказа", ФирмаКратко);
			...
			ИначеЕсли ПервоеНеоплаченное Тогда
				...
				ОблМакета.Параметры.Этап = ?(ОтображатьЭтапы И ТекСтрока.ЭтапОплаты, "Этап " + ТекСтрока.ЭтапОплаты, "");
				ОблМакета.Параметры.Текст = СтрШаблон("Ожидается оплата покупок в %1", ФирмаКратко);
				ОблМакета.Параметры.Сумма = СуммаСтрокой;
				СостояниеОплаты.Вывести(ОблМакета);	
			Иначе
