...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для каждого ТекСтрока Из Параметры.МестаРеализации Цикл
		НовСтрока = МестаРеализации.Добавить();
		ЗаполнитьЗначенияСвойств(НовСтрока, ТекСтрока);
		Если НЕ НовСтрока.ККМ.Пустая() И НЕ НовСтрока.Фискализирован Тогда
			НовСтрока.ЭтапОплаты = ТекЭтап;
			ТекЭтап = ТекЭтап + 1;
		КонецЕсли;
	КонецЦикла;
	...
	ОбновитьОтображениеЭлементовДляМестаРеализации(РабочееМесто.МестоРеализации, РабочееМесто.Фирма.ТипТТ, ТекЭтап = 3);
	
	ОбновитьОтображениеСостоянияОплаты(РабочееМесто.МестоРеализации, РабочееМесто.Фирма.ТипТТ);
...
&НаСервере
Процедура ОбновитьОтображениеЭлементовДляМестаРеализации(МестоРеализации, ТипТТ = Неопределено, ОплатаВДваЭтапа = Ложь)
...
//УДАЛИТЬ		Элементы.НадписьСостояниеОплаты.ВертикальноеПоложениеВГруппе = ВертикальноеПоложениеЭлемента.Низ;
//УДАЛИТЬ		Элементы.НадписьСостояниеОплаты.ВертикальноеПоложение = ВертикальноеПоложениеЭлемента.Низ;
	    Элементы.ГруппаИнструкции2.Видимость = Ложь;
...
//УДАЛИТЬ		Элементы.НадписьСостояниеОплаты.ВертикальноеПоложение = ВертикальноеПоложениеЭлемента.Низ;
	...
	Если ТипТТ = ПредопределенноеЗначение("Справочник.ТипыТТ.МОКП") Тогда
		...
		Элементы.ГруппаИнструкции.Видимость = НЕ ОплатаВДваЭтапа;
	    Элементы.ГруппаИнструкции2.Видимость = ОплатаВДваЭтапа;
		
		мФС = Новый Массив;
		мФС.Добавить(Новый ФорматированнаяСтрока("Пожалуйста, следуйте инструкциям на терминале оплаты", Новый Шрифт("Segoe UI", 20, Истина), ЦветаСтиля.НеактивныйЭлемент));
		мФС.Добавить(Символы.ПС);
		мФС.Добавить(Новый ФорматированнаяСтрока("(Внимание! Будет распечатано 2 чека)", Новый Шрифт("Segoe UI", 18, Ложь), ЦветаСтиля.НеактивныйЭлемент));
		НадписьСостояниеОплаты = Новый ФорматированнаяСтрока(мФС);
		
		Элементы.НадписьСостояниеОплаты.Видимость = ОплатаВДваЭтапа;
		Элементы.СостояниеОплаты.Видимость = НЕ ОплатаВДваЭтапа;
...
&НаСервере
Процедура ОбновитьОтображениеСостоянияОплаты(МестоРеализации, ТипТТ = Неопределено)
	...
	Если флОплатаВПроцессе Тогда
		...
		Для каждого ТекСтрока Из МестаРеализации Цикл
			...
			Если ТипТТ = ПредопределенноеЗначение("Справочник.ТипыТТ.Ресторан") Тогда
				...
				Если ТекСтрока.Фискализирован Тогда                      // ТекСтрока.КОплате = 0 ИЛИ 
					...
					МассивСтрок.Добавить(СтрШаблон("Оплачен заказ %1 (%2)", ТекСтрока.Фирма, СуммаСтрокой) + Символы.ПС);
				...
				Иначе	
					...
					МассивСтрок.Добавить(СтрШаблон("Ожидается оплата заказа %1      %2.", ТекСтрока.Фирма, СуммаСтрокой) + Символы.ПС);
			...
			Если ТипТТ = ПредопределенноеЗначение("Справочник.ТипыТТ.КМ") 
				ИЛИ ТипТТ = ПредопределенноеЗначение("Справочник.ТипыТТ.МОКП")
				Тогда
				...
				СуммаСтрокой = Формат(ТекСтрока.КОплате, "ЧДЦ=2; ЧГ=3,0") + " " + Символ(8381);
				СуммаРеализацииСтрокой = Формат(ТекСтрока.СуммаРеализации, "ЧДЦ=2; ЧГ=3,0") + " " + Символ(8381);
				Если ТекСтрока.Фискализирован Тогда                      // ТекСтрока.КОплате = 0 ИЛИ 
					МассивСтрок.Добавить(БиблиотекаКартинок.CheckMark48);
					МассивСтрок.Добавить(СтрШаблон("Оплачен заказ %1:                (%2)", ФирмаКратко, ?(ТекСтрока.КОплате = 0, СуммаРеализацииСтрокой, СуммаСтрокой)) + Символы.ПС);
				ИначеЕсли ТекСтрока.ККМ.Пустая() Тогда
					МассивСтрок.Добавить(БиблиотекаКартинок.Внимание48);
					МассивСтрок.Добавить(СтрШаблон("Пройтиде в %1 для оплаты заказа:  %2", ФирмаКратко, СуммаСтрокой) + Символы.ПС);
				Иначе
					МассивСтрок.Добавить(БиблиотекаКартинок.ДлительнаяОперация48);
		...
		Если НЕ ТипТТ = ПредопределенноеЗначение("Справочник.ТипыТТ.МОКП") Тогда
			НадписьСостояниеОплаты = Новый ФорматированнаяСтрока(МассивСтрок);
		КонецЕсли;
		
		ОбновитьТабДокСписокОплаты(ТипТТ);
		
	КонецЕсли;
	
	Элементы.ГруппаИнструкции.Видимость = флОплатаВПроцессе И НЕ ТипТТ = ПредопределенноеЗначение("Справочник.ТипыТТ.МОКП");
	...
	Если Открыта() Тогда
		...
		Для каждого ТекСтрока Из МестаРеализации Цикл
			...
			Если НЕ ТекСтрока.Фискализирован 
				...
				//Если НЕ ПустаяСтрока(ТекстТекущейОплаты) Тогда
				//	РаботаСокнами.ПоказатьПлашку(" ", ТекстТекущейОплаты, 2, "10000", 1, 255, 300, 200, 16, 20);
				//КонецЕсли;
				ФирмаКратко = ЛояльностьКлиентСерверПовтИсп.МестоРеализацииПоПроекту(ЛояльностьКлиентСерверПовтИсп.ПроектПоМестуРеализации(ТекСтрока.МестоРеализации), Истина, 3);
				ПараметрыПлашки = Новый Структура;
				ПараметрыПлашки.Вставить("Заголовок", "Ожидается оплата");
				ПараметрыПлашки.Вставить("Текст", "заказа в " + ФирмаКратко);
				ПараметрыПлашки.Вставить("Таймаут", 10000);
				ПараметрыПлашки.Вставить("НомерПлашки", 2);
				ПараметрыПлашки.Вставить("Лево", ?(Лево = 1, 814, 1));
				ПараметрыПлашки.Вставить("Верх", 412);
				ПараметрыПлашки.Вставить("Ширина", 210);
				ПараметрыПлашки.Вставить("Высота", 200);
				ПараметрыПлашки.Вставить("РазмерЗаголовка", 16);
				ПараметрыПлашки.Вставить("РазмерТекста", 20);
				ПараметрыПлашки.Вставить("Прозрачность", 255);
				ПараметрыПлашки.Вставить("ЦветТекста", "FFFFFF");
				ПараметрыПлашки.Вставить("ЦветФона", "068A3F");
				РаботаСокнами.ПоказатьПлашку(ПараметрыПлашки);
			...
			ОбновитьОтображениеСостоянияОплаты(РабочееМесто.МестоРеализации, РабочееМесто.Фирма.ТипТТ);
			//ОбождатьМиллисекунд(1500);
			
		КонецЦикла;
		
		Если НЕ флОплатаВПроцессе Тогда
			СекундОжидать = 2;
			Если РабочееМесто.Фирма.ТипТТ = ПредопределенноеЗначение("Справочник.ТипыТТ.КМ") 
				ИЛИ РабочееМесто.Фирма.ТипТТ = ПредопределенноеЗначение("Справочник.ТипыТТ.МОКП")
				Тогда
				СекундОжидать = 0.1;
			КонецЕсли;
...
Функция ОплатаПоМестуРеализации(Фирма, КОплате)
	...
	Попытка
		...
		Если Не ЗначениеЗаполнено(ИнтерфейсРМ.ТекущаяСмена(Фирма.МестоРеализации)) Тогда
			Возврат СтруктураРезультата(-1, "Обратитесь к продавцу-консультанту.", "Смена не открыта", "НачальноеОкно");
		КонецЕсли;
		...
		ИначеЕсли РезультатОплаты.Неудача Тогда
			Заказ = СериализаторXDTO.ПрочитатьXDTO(ЗаказXDTO);
			ЗаписатьЗаказ(ПредопределенноеЗначение("Перечисление.СтатусыЗаказа.Открыт"));
			Возврат СтруктураРезультата(-1, "Обратитесь к продавцу-консультанту", "Оплата не удалась", "НачальноеОкно");
		...
		ИначеЕсли РезультатОплаты.СделанаОтменаОплатыКартой Тогда
			Заказ = СериализаторXDTO.ПрочитатьXDTO(ЗаказXDTO);
			ЗаписатьЗаказ(ПредопределенноеЗначение("Перечисление.СтатусыЗаказа.Открыт"));
			Возврат СтруктураРезультата(-1, "Обратитесь к продавцу-консультанту", "Отмена оплаты картой.", "НачальноеОкно");
		Иначе // НЕ Успех	
			Заказ = СериализаторXDTO.ПрочитатьXDTO(ЗаказXDTO);
			ЗаписатьЗаказ(ПредопределенноеЗначение("Перечисление.СтатусыЗаказа.Открыт"));
			Возврат СтруктураРезультата(-1, "Отмена оплаты", "", "НачальноеОкно");
	...
	Исключение
		...
		Возврат СтруктураРезультата(-1, "Обратитесь к продавцу-консультанту", "Ошибка при оплате", "НачальноеОкно");
	КонецПопытки;
	
	Возврат СтруктураРезультата(-1, "Обратитесь к продавцу-консультанту", "Произошла ошибка", "НачальноеОкно");
...
Функция ОбработкаОплаты_Оплата(Фирма, КОплате) Экспорт
	...
	Если РезультатФискализации Тогда
		...
		ПротоколРасчетов.КассоваяСмена = Обработки.ОткрытиЗакрытиеСменыКасса.ПолучитьСменуКассы(ПротоколРасчетов.ККМ.КодСУП);
...
Функция ОбработкаОплаты_Фискализация(РезультатОплаты, Фирма) Экспорт
	...
	Для Каждого Т Из Товары Цикл
		...
		Для каждого С Из СтрокиСпецифик Цикл
			...
			СтрокаЧека.Вставить("Количество",?(С.Количество, С.Количество * Т.Количество, Т.Количество));
...
//2мерчанта		Обработка_ПС.ВыполнитьКоманду("ОплатитьПлатежнойКартой",Новый Структура("Сумма", СтрокаБезнал.Сумма), ВыходныеПараметры);
	Исключение
...
Процедура ЗаписатьПротоколВЗаказ(Фирма)
	Для каждого ТекСтрока Из Заказ.Товары Цикл
		Если ТекСтрока.Фирма = Фирма И ТекСтрока.ДокументОплаты.Пустая() И ТекСтрока.Количество > 0 Тогда  // И ТекСтрока.СуммаРеализации <> 0
...
Функция СтруктураРезультата(Ошибка = 0, ТекстОшибки1 = "", ТекстОшибки2 = "", РезультатКнопкиОК = "")
	...
	Если глПараметрыРМ.ТипТТ = ПредопределенноеЗначение("Справочник.ТипыТТ.МОКП") Тогда
		Для Сч = 1 По СостояниеОплаты.ВысотаТаблицы Цикл
			СостояниеОплаты.Область(Сч, 1).Текст = "";
		КонецЦикла;
		Результат.Вставить("СостояниеОплаты", СостояниеОплаты);
	Иначе	
...
&НаСервере
Процедура ОбновитьТабДокСписокОплаты(ТипТТ)
	...
	Если ТипТТ = ПредопределенноеЗначение("Справочник.ТипыТТ.МОКП") Тогда
		...
		ПервоеНеоплаченное = Истина;
		ОтображатьЭтапы = МестаРеализации.Итог("ЭтапОплаты") > 1;
		...
		Для каждого ТекСтрока Из МестаРеализации Цикл
			...
			Если ТекСтрока.Фискализирован Тогда
				ОблМакета = Макет.ПолучитьОбласть("Оплата_Оплачено");
				ОблМакета.Параметры.Этап = ?(ОтображатьЭтапы И ТекСтрока.ЭтапОплаты, "Этап " + ТекСтрока.ЭтапОплаты, "");
				ОблМакета.Параметры.Текст = СтрШаблон("Оплата заказа в %1 завершена", ФирмаКратко);
				ОблМакета.Параметры.Сумма = ?(ТекСтрока.КОплате = 0, СуммаРеализацииСтрокой, СуммаСтрокой);
				СостояниеОплаты.Вывести(ОблМакета);	
			ИначеЕсли ТекСтрока.ККМ.Пустая() Тогда	
				ОблМакета = Макет.ПолучитьОбласть("Оплата_НеОплачено");
				ОблМакета.Параметры.Текст = СтрШаблон("У Вас не оплачен заказ %1. Не забудьте пройти в %1 для его оплаты!", ВРег(ФирмаКратко));
				ОблМакета.Параметры.Сумма = СуммаСтрокой;
				СостояниеОплаты.Вывести(ОблМакета);	
			ИначеЕсли ПервоеНеоплаченное Тогда
				ПервоеНеоплаченное = Ложь;
				ОблМакета = Макет.ПолучитьОбласть("Оплата_ВПроцессе");
				ОблМакета.Параметры.Этап = ?(ОтображатьЭтапы И ТекСтрока.ЭтапОплаты, "Этап " + ТекСтрока.ЭтапОплаты, "");
				ТекстТекущейОплаты = СтрШаблон("Ожидается оплата заказа в %1", ФирмаКратко);
				ОблМакета.Параметры.Текст = ТекстТекущейОплаты;
				ОблМакета.Параметры.Сумма = СуммаСтрокой;
				СостояниеОплаты.Вывести(ОблМакета);
			Иначе
				ОблМакета = Макет.ПолучитьОбласть("Оплата_КОплате");
				ОблМакета.Параметры.Этап = ?(ОтображатьЭтапы И ТекСтрока.ЭтапОплаты, "Этап " + ТекСтрока.ЭтапОплаты, "");
				ОблМакета.Параметры.Текст = СтрШаблон("Ожидается оплата заказа в %1", ФирмаКратко);
				ОблМакета.Параметры.Сумма = СуммаСтрокой;
				СостояниеОплаты.Вывести(ОблМакета);	
			КонецЕсли;
