...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	////Если РабочееМесто.Тип = Перечисления.ТипыРМ.Автокасса
	////	и РабочееМесто.МестоРеализации = ПредопределенноеЗначение("Справочник.МестаРеализации.Ресторан") Тогда
...
Процедура ОбновитьОтображениеЭлементовДляМестаРеализации(МестоРеализации)
	...
	Если МестоРеализации = ПредопределенноеЗначение("Справочник.МестаРеализации.КМ") Тогда
		...
		Элементы.НадписьКартаЛОЛ.Видимость = Ложь;
...
Процедура ОбновитьОтображениеСостоянияОплаты(МестоРеализации)
		...
		Для каждого ТекСтрока Из МестаРеализации Цикл
			...
			Если МестоРеализации = ПредопределенноеЗначение("Справочник.МестаРеализации.КМ") Тогда
				...
				Если ТекСтрока.Фискализирован Тогда                      // ТекСтрока.КОплате = 0 ИЛИ 
					...
					МассивСтрок.Добавить("Оплачен заказ " + ФирмаКратко + "." + Символы.ПС);
				ИначеЕсли ТекСтрока.ККМ.Пустая() Тогда
					МассивСтрок.Добавить(БиблиотекаКартинок.Внимание48);
					МассивСтрок.Добавить("Не забудьте оплатить заказ " + ФирмаКратко + "!" + Символы.ПС);
				Иначе
					МассивСтрок.Добавить(БиблиотекаКартинок.ДлительнаяОперация48);
					МассивСтрок.Добавить("Ожидается оплата заказа " + ФирмаКратко + "..." + Символы.ПС);
					флОплатаВПроцессе = Истина;
					//Прервать;
				КонецЕсли;
			КонецЕсли;
	...
	Элементы.НадписьИнструкции.Видимость = флОплатаВПроцессе;
...
Процедура ОжиданиеНачалоОплаты()
	...
	Если Открыта() Тогда
		...
		Для каждого ТекСтрока Из МестаРеализации Цикл
			...
			Если НЕ ТекСтрока.Фискализирован Тогда
		...
		Если НЕ флОплатаВПроцессе Тогда
			ПодключитьОбработчикОжидания("ЗавершитьОплату", 2, Истина);
		КонецЕсли;
...
Функция ОплатаПоМестуРеализации(Фирма, КОплате)
	...
	Попытка
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации("Заказ.Ошибка в процессе оплаты", УровеньЖурналаРегистрации.Ошибка, , ЗаказСсылка, ПодробноеПредставлениеОшибки);
		КонецПопытки;
		ЗаписьЖурналаРегистрации("Получен результат оплаты", УровеньЖурналаРегистрации.Информация, Метаданные.Документы.Заказ, ЗаказСсылка, "РезультатОплаты = " + ЗначениеВСтрокуВнутр(РезультатОплаты));
		...
		ИначеЕсли РезультатОплаты.Отмена Тогда
			ЗаписьЖурналаРегистрации("Заказ.Востановление состояния заказа при отмене оплат",УровеньЖурналаРегистрации.Предупреждение,,ЗаказСсылка, "Товары: " + Символы.ПС + ЗначениеВСтрокуВнутр(Заказ.Товары.Выгрузить()));
			Заказ = СериализаторXDTO.ПрочитатьXDTO(ЗаказXDTO);
			ЗаписатьЗаказ(ПредопределенноеЗначение("Перечисление.СтатусыЗаказа.Открыт"));
			Возврат СтруктураРезультата(-1, "Отмена оплаты", , "НачальноеОкно");
		...
		КонецЕсли;
	Исключение
		ЗаписьЖурналаРегистрации("Ошибка при оплате", УровеньЖурналаРегистрации.Ошибка, Метаданные.Документы.Заказ, ЗаказСсылка, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ОбработкаОплаты_Оплата(Фирма, КОплате) Экспорт
	...
	ЗаписьЖурналаРегистрации("Оплата.Вход в процедуру оплаты", УровеньЖурналаРегистрации.Информация, , Заказ.Ссылка);
	
	Если Не Товары.Итог("Количество") Тогда
		ИнтерфейсРМ.ВопросПредупреждение("Ошибка","Заказ сторнирован","","","ОК","",,6);
		ЗаписьЖурналаРегистрации("Оплата.Заказ сторнирован",УровеньЖурналаРегистрации.Ошибка,,Заказ.Ссылка);
	...
	ЗаписьЖурналаРегистрации("Оплата.Фискализация.До вызова", УровеньЖурналаРегистрации.Информация, , Заказ.Ссылка);
	РезультатФискализации = ОбработкаОплаты_Фискализация(РезультатОплаты, Фирма);
	ЗаписьЖурналаРегистрации("Оплата.Фискализация.После вызова", УровеньЖурналаРегистрации.Информация, , Заказ.Ссылка, СтрШаблон("РезультатФискализации = %1",РезультатФискализации));
	...
	Если РезультатФискализации Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации("Ошибка проведения протокола расчетов", УровеньЖурналаРегистрации.Ошибка, , Заказ.Ссылка, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
				
		//{ Закрываем заказ, чтобы при вылете на финализации попадали в новый заказ
		ЗаписьЖурналаРегистрации("Оплата.Закрытие заказа перед финализацией", УровеньЖурналаРегистрации.Информация, , Заказ.Ссылка);
		...
		ЗаписьЖурналаРегистрации("Оплата.ФинализацияЧека.До вызова", УровеньЖурналаРегистрации.Информация, , Заказ.Ссылка);
		Попытка
		...
		Исключение
			ЗаписьЖурналаРегистрации("Оплата.ФинализацияЧека.Ошибка выполнения", УровеньЖурналаРегистрации.Ошибка, , Заказ.Ссылка, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
		ЗаписьЖурналаРегистрации("Оплата.ФинализацияЧека.После вызова", УровеньЖурналаРегистрации.Информация, , Заказ.Ссылка);
		
		//DeBuG
		Обработки.ГлавнаяФормаАК.ПечатьПодвалаЛояльности(Фирма, ККМ.Ссылка, Заказ.Ссылка, ПротоколРасчетов.Ссылка);
		//Если Фирма.МестоРеализации = ПредопределенноеЗначение("Справочник.МестаРеализации.Ресторан") Тогда
		//	РезультатПечатиЧека = Заказ_ПечатьЧека_ПечатьЧека(ПротоколРасчетов.ИтогСуммаФакт);
		//Иначе
		//	Попытка ОбработкаОплаты_ПечатьПодвалаЛояльности() Исключение КонецПопытки;
		//КонецЕсли;
		
		ЗаписьЖурналаРегистрации("Оплата.ПечатьПодвалаЛояльности.После вызова", УровеньЖурналаРегистрации.Информация, , Заказ.Ссылка);
...
Функция ОбработкаОплаты_Фискализация(РезультатОплаты, Фирма) Экспорт
	...
	Если СуммаБезнал Тогда
		ЗаписьЖурналаРегистрации("Оплата.Оплата картой.До вызова",УровеньЖурналаРегистрации.Информация,,Заказ.Ссылка);
		Если Не ОбработкаОплаты_ОплатаКартой(РезультатОплаты) Тогда
			ЗаписьЖурналаРегистрации("Оплата.Оплата картой.После вызова.Ошибка",УровеньЖурналаРегистрации.Информация,,Заказ.Ссылка, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ИнтерфейсРМ.ВопросПредупреждение("Ошибка", "Оплата картой не удалась", "Ошибка при работе с платежным терминалом","","ОК","");
			Возврат Ложь;
		КонецЕсли;
		ЗаписьЖурналаРегистрации("Оплата.Оплата картой.После вызова",УровеньЖурналаРегистрации.Информация,,Заказ.Ссылка);
	КонецЕсли;
	
	СтрокиЧека = Новый Массив;
	СуммаСкидкиНаЧек = 0;
	ЗаписьЖурналаРегистрации("Оплата.Фискализация.Расчет параметров для ФН.Начало",УровеньЖурналаРегистрации.Информация,,Заказ.Ссылка);
	...
	ЗаписьЖурналаРегистрации("Оплата.Фискализация.Расчет параметров для ФН.Конец цикла по товарам",УровеньЖурналаРегистрации.Информация,,Заказ.Ссылка);
...
//326	ПараметрыВходные.Вставить("МестоРеализации", ПротоколРасчетов.МестоРеализации);//!
	...
	ЗаписьЖурналаРегистрации("Оплата.Фискализация.Расчет параметров для ФН.Конец",УровеньЖурналаРегистрации.Информация,,Заказ.Ссылка);
	ПараметрыВыходные = ПараметрыВыходные;
	Если СтрокиЧека.Количество() Тогда
		Попытка
			ЗаписьЖурналаРегистрации("Оплата.Фискализация.Входные параметры",УровеньЖурналаРегистрации.Информация,,Заказ.Ссылка);
			РезультатКомандыФискализация = Обработка_ККМ.ВыполнитьКоманду("ФискализацияЧека",ПараметрыВходные,ПараметрыВыходные);
			ЗаписьЖурналаРегистрации("Оплата.Фискализация.Выходные параметры",УровеньЖурналаРегистрации.Информация,,Заказ.Ссылка);
			фОшибка = Ложь;
		Исключение
	...
	Если ЗначениеЗаполнено(ПротоколРасчетов.ОтветФН) Тогда
		ЗаписьЖурналаРегистрации("Фискализация.Изменение ответа ФН", УровеньЖурналаРегистрации.Предупреждение, , Заказ.Ссылка, ПротоколРасчетов.ОтветФН);
	КонецЕсли;
...
//326	ПараметрыВходные.Вставить("МестоРеализации", ПротоколРасчетов.МестоРеализации);
	...
	Исключение
		ЗаписьЖурналаРегистрации("Ошибка выполнения", УровеньЖурналаРегистрации.Ошибка,, Заказ.Ссылка, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат Ложь;
	КонецПопытки;
...
Процедура ОбработкаОплаты_ОтменаТранзакцииПлатежнойСистемы(РезультатОплаты, КОплате) Экспорт
	...
	ЗаписьЖурналаРегистрации("Оплата.ОтменаТранзакцииПлатежнойСистемы.ВходВпроцедуру", УровеньЖурналаРегистрации.Информация, , Заказ.Ссылка);
	ОбработкаОплаты_ЗаполнитьПротокол(КОплате);
	...
	Исключение
		ЗаписьЖурналаРегистрации("Платежная система", УровеньЖурналаРегистрации.Ошибка, Метаданные.Документы.Заказ, Заказ.Ссылка, "Не удалось отменить транзакцию " + ПротоколРасчетов.идТранзакцииПлатежнойСистемы + " " + ОписаниеОшибки());
		ИнтерфейсРМ.ВопросПредупреждение("Ошибка", "Ошибка платежной системы", ОписаниеОшибки(), "","ОК", "");
		фОшибка = Истина;
	КонецПопытки;
	РезультатОплаты.Неудача = Истина;
...
Процедура ЗаписатьПротоколВЗаказ(Фирма)
	Для каждого ТекСтрока Из Заказ.Товары Цикл
		Если ТекСтрока.Фирма = Фирма И ТекСтрока.ДокументОплаты.Пустая() И ТекСтрока.Количество > 0 И ТекСтрока.СуммаРеализации <> 0 Тогда  //TODO
...
// для не КП
&НаКлиенте
Процедура ОбработкаОплаты_ПечатьПодвалаЛояльности()
	
	Если КассаАвтономная() Тогда
		Возврат;
	КонецЕсли;
	
	СтрокиПодвала = СтрРазделить(ПротоколРасчетов.ПодвалЧека, Символы.ПС);
	ПараметрыВходные = Новый Структура;
...
// для КП
&НаКлиенте
Функция Заказ_ПечатьЧека_ПечатьЧека(Сумма)
	...
	ОбработкаПечати.ККМ = ККМ.Ссылка;
	ОбработкаПечати.Заказ = Заказ.Ссылка;
	ОбработкаПечати.ЗаказДопИнф = ЗаказДопИнф;
	ОбработкаПечати.ФоновыйРежим = Истина;
	ОбработкаПечати.Текст1 = ПротоколРасчетов.ПодвалЧека;
	
	ЧекПрошел = ОбработкаПечати.ПечатьЧека(ПротоколРасчетов.Протокол, Сумма, ГруппаОплаты);
...
#Область ПроцеддурыИнициализации
...
Функция СтруктураРезультата(Ошибка = 0, ТекстОшибки1 = "", ТекстОшибки2 = "", РезультатКнопкиОК = "")
	...
	Возврат Новый Структура("Ошибка,ТекстОшибки1,ТекстОшибки2,РезультатКнопкиОК,ВариантОплаты,СпособПолученияЧека,ТипЧекаЛояльность", Ошибка, ТекстОшибки1, ТекстОшибки2, РезультатКнопкиОК, ВариантОплаты, СпособПолученияЧека, ТипЧекаЛояльность);
