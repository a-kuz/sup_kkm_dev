...
//УДАЛИТЬ		Элементы.НадписьСостояниеОплаты.ВертикальноеПоложение = ВертикальноеПоложениеЭлемента.Низ;
	...
	Если ТипТТ = ПредопределенноеЗначение("Справочник.ТипыТТ.МОКП") Тогда
		...
		мФС = Новый Массив;
		мФС.Добавить(Новый ФорматированнаяСтрока("Пожалуйста, следуйте инструкциям на терминале оплаты", Новый Шрифт("Segoe UI", 20, Истина), ЦветаСтиля.НеактивнаяКнопка));
		мФС.Добавить(Новый ФорматированнаяСтрока("(Внимание! Будет распечатано 2 чека)", Новый Шрифт("Segoe UI", 18), ЦветаСтиля.НеактивнаяКнопка));
