...
Процедура тпТоварыПриАктивизацииСтроки(Элемент)
	...
	Если глПараметрыРМ.ТипТТ = Справочники.ТипыТТ.МОКП Тогда
		Если ТекущийТоварМожноПриготовить() Тогда
			ЭлементыФормы.КнопкаПриготовить.ЦветТекста = ЦветаСтиля.Акцент;
		Иначе
			ЭлементыФормы.КнопкаПриготовить.ЦветТекста = ЦветаСтиля.ЦветТекстаКнопкиАппБара;
		КонецЕсли;
	КонецЕсли;
...
тпТовары.Колонки.Добавить("НомерСтроки",Новый ОписаниеТипов("Число"),"N",1);
тпТовары.Колонки.Добавить("Иконка",Новый ОписаниеТипов("Строка")," ",1);
