...
Процедура ВводКартыЛояльности(ВопросПриОтмене = Ложь) Экспорт 
...
Процедура Заблокировать() Экспорт
	...
	//Если ЕстьЗаказыПоПродавцуКарте(глПользователь) Тогда
	//	ОткрытьЗаказПоПродавцуКарте(глПользователь);
	//Иначе // запросим приложить карту гостя
	//	Обработка = ИнтерфейсРМ.ПолучитьОбъектОбработки("ВводКартыКлиента");
	//	Форма = Обработка.ПолучитьФорму("Форма9");
	//	РезОткрытия = Форма.ОткрытьМодально();
	//	Если ЗначениеЗаполнено(РезОткрытия) Тогда // поищем заказы по карте клиента
	//		Если ЕстьЗаказыПоПродавцуКарте(,РезОткрытия) Тогда // есть заказ
	//			ОткрытьЗаказПоПродавцуКарте(,РезОткрытия);
	//		Иначе
	//			СоздатьЗаказ(глПользователь,РезОткрытия); // Создаем заказ по карте и продавцу
	//		КонецЕсли;
	...
	Если не ЗначениеЗаполнено(Заказ.КартаДоступа) ТОгда
		ВводКартыКлиента();		
	КонецЕсли;
	Если ПустаяСтрока(Заказ.НомерКартыЛояльности) И Заказ.ОтказОтКартыЛояльности <> 1 Тогда //и Заказ.НеСправшиватьКарту
		ВводКартыЛояльности(Истина);
	КонецЕсли;
	ВводТоваровДоступен();
...
Процедура СоздатьЗаказ(Автор,КартаДоступа)
	...
	Заказ1.Дата = ТекущаяДатаНаСервере();
	Заказ1.Автор = Автор;
	...
	Заказ1.Записать();
	Заказ = Заказ1.Ссылка;
	ЗаписатьЗаказ(Заказ1,Перечисления.СтатусыЗаказа.Открыт);
...
Функция ПоказатьЗаказНаМонитореГостя() Экспорт
	...
	Для Каждого стТовары Из Заказ.Товары Цикл
		...
		НовСтр.Наименование = Строка(стТовары.Товар);
...
Процедура ЗаписатьЗаказ(Заказ1,Статус = Неопределено) Экспорт 
	...
	//ГлавнаяФорма.ОтключитьОбработчикОжидания("ОбновлениеДанныхЛояльности");
	...
	//Лояльность.ОбновитьПредварительныйРасчетЗаказа(Заказ, глПараметрыРМ.Тест);       
	//ГлавнаяФорма.ПодключитьОбработчикОжидания("ОбновлениеДанныхЛояльности", 0.5, Истина);
...
Функция ВводНовойСтроки(ВыбТов, Количество = 0, ШК = "") Экспорт
	...
	ТекКоличество = 1;
//	док1.Записать();
	ЗаписатьЗаказ(док1,Перечисления.СтатусыЗаказа.Открыт);
