...
//рф1	ГлавнаяФорма.ОтключитьОбработчикОжидания("ОбновитьЗаказНаМонитореГостя");
	Если Не ВводТоваровДоступен() Тогда
		Возврат;
	КонецЕсли; 
	...
	КонецЕсли;
	глОтсечкаПростоя();
//рф1	ГлавнаяФорма.ПодключитьОбработчикОжидания("ОбновитьЗаказНаМонитореГостя", 0.5, Истина);
...
//рф1	ГлавнаяФорма.ОтключитьОбработчикОжидания("ПоказатьЗаказНаМонитореГостя");
...
Функция ИнициализацияРабочегоМеста(ТекущееРМ) Экспорт
	...
	Попытка
		...
		ИначеЕсли ДатаЛокальнойКопии > НачалоДня(ТекущаяДатаНаСервере()) Тогда 	
			ЗаписьЖурналаРегистрации("Резервирование.Ошибка создания локальной базы", УровеньЖурналаРегистрации.Ошибка, , глРабочееМесто, "Дата локальной копии базы больше текущей.");
		КонецЕсли; 
	Исключение
		ЗаписьЖурналаРегистрации("Резервирование.Ошибка создания локальной базы", УровеньЖурналаРегистрации.Ошибка, , глРабочееМесто, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
//рф1	ПоказатьЗаказНаМонитореГостя();
	//ДГ-
...
Функция ПоказатьЗаказНаМонитореГостя() Экспорт
	...
	ДопПараметрыИнфо.ТабДокЛояльность = ИнтерфейсРМЛояльность.ПолучитьТабДокЛояльность_МониторГостя(ГлавнаяФорма, "", ЛояльностьДанныеЗаказа);
	КонецЕсли;
...
Процедура УДАЛИТЬ_ИнициализацияЗаказа(ПриОткрытии = Ложь, КартаДоступа = Неопределено) Экспорт
	докВозврат = Неопределено;
	ТекущийДокумент = Неопределено;
КонецПроцедуры 

Процедура УДАЛИТЬ_ИнициализацияВозврата() Экспорт
	Заказ = Неопределено;
	ТекущийДокумент = Неопределено;
...
Процедура УДАЛИТЬ_ПроинициализироватьЗаказЕслиНеПроинициализирован() Экспорт
	Попытка
	...
	Исключение
		...
		//Иначе
			УДАЛИТЬ_ИнициализацияЗаказа();	
		//КонецЕсли;
	КонецПопытки;
КонецПроцедуры

Функция УДАЛИТЬ_ПроверитьЗаказНаВылетВоВремяФискализации()
	////:Заказ = Документы.Заказ.СоздатьДокумент();
	////:ПротоколРасчетов = Документы.ПротоколРасчетов.ПустаяСсылка();
...
Функция УДАЛИТЬ_РасчетнаяВыручкаЗаКассовуюСмену()
...
Функция УДАЛИТЬ_ВыручкаПоБанкуЗаСмену()
	ТЗ = Обработки.ФормированиеОтчетовФО.ТаблицаОперативнойРеализации(ИнтерфейсРМ.ТекущаяСмена(),1);
...
Процедура УДАЛИТЬ_Оплата() Экспорт

КонецПроцедуры

Процедура УДАЛИТЬ_ВозвратОплаты() Экспорт

КонецПроцедуры

Процедура УДАЛИТЬ_ОткрытьЧек() Экспорт
...
Функция УДАЛИТЬ_НомерТекущейСмены() Экспорт
...
Функция УДАЛИТЬ_НомерТекущегоЧека() Экспорт
	НомерТекущегоЧека = 0;
...
//
Процедура УДАЛИТЬ_ПроверитьОбработкаСобытий()
...
//
Процедура УДАЛИТЬ_ОткрытьРежимАдминистратора() Экспорт
...
// < КС_ВДВ -------------------------------------------------------------------- 
Процедура УДАЛИТЬ_ВыдатьКартуЛояльности() Экспорт
...
// < КС_ВДВ -------------------------------------------------------------------- 
Процедура УДАЛИТЬ_ЗаменаКартыЛояльности() Экспорт
...
// < КС_ВДВ ------------------------------------------------------------ 
Процедура УДАЛИТЬ_ОткрытьМониторЗаказов() Экспорт 
...
//
Процедура УДАЛИТЬ_СоздатьНовыйЗаказ(ОбъектВвода=Неопределено, ФлагПоКарте=Ложь,ЗакрытьФормы=Ложь) Экспорт
...
//
Процедура УДАЛИТЬ_ОткрытьСписокЗаказов() Экспорт
...
// Открытие режима администратора
//
Процедура УДАЛИТЬ_ОткрытьСтопЛист() Экспорт
...
//
Процедура УДАЛИТЬ_ОбработкаВыбораПосадочногоМеста(ВыбМесто, ФлагПоКарте=Ложь)
...
Процедура УДАЛИТЬ_ВыводНаДПТекущейСтроки(СтрокаТовар) Экспорт
...
ЛояльностьДанныеЗаказа = Неопределено;
//ДГ+
ДопПараметрыИнфо	= Новый Структура("НомерСтрокиЗаказа, СуммаПоСтанцииНач, СброситьНастройки, ТабДокЛояльность");
//ДГ-
