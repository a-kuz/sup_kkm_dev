...
Перем ИнтервалОбновленияИнтерфейса Экспорт; // устанавливается в форме
	...
	Если ЗначениеЗаполнено(РезультатПоиска) Тогда
		...
		ИначеЕсли РезультатПоиска = Справочники.ДействияЭлементовМеню.ОчиститьКупоны Тогда
			...
			Если НЕ Док1.ЗаблокированПоФирме(глРабочееМесто.Фирма) Тогда
				...
				ЗаписатьЗаказ(Док1,,Истина);
			КонецЕсли;
		ИначеЕсли РезультатПоиска = Справочники.ДействияЭлементовМеню.ОчиститьНомерКартыЛояльности Тогда
			Док1 = ТекущийДокумент.ПолучитьОбъект();
			Если НЕ Док1.ЗаблокированПоФирме(глРабочееМесто.Фирма) Тогда
				Док1.НомерКартыЛояльности = "";
				Док1.СпособАвторизацииГостя = 0;
				ЗаписатьЗаказ(Док1,,Истина);
			КонецЕсли;
			ГлавнаяФорма.ОбновитьДанныеЛояльности(ИнтервалОбновленияИнтерфейса);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры

Процедура Сторно(НомерСтроки = 0) Экспорт 
	...
	Для Каждого Т Из ТекущийДокумент.Товары Цикл 
		Если Т.Количество И НЕ ЗначениеЗаполнено(Т.ДокументОплаты) И глПараметрыРМ.ФирмыРМ.Найти(Т.Фирма) <> Неопределено Тогда
			ПоследняяСтрока = ТекущийДокумент.Товары.Индекс(Т);
			Если Т.НомерСтроки = НомерСтроки Тогда
				НужнаяСтрока = ТекущийДокумент.Товары.Индекс(Т);
			КонецЕсли;
			КоличествоСтрок = КоличествоСтрок + 1;
		КонецЕсли;
	КонецЦикла;
	Если НужнаяСтрока = Неопределено Тогда
		НужнаяСтрока = ПоследняяСтрока;
	КонецЕсли;
	
	Если КоличествоСтрок Тогда
		Док1 = Заказ.ПолучитьОбъект();
		СтрДок = Док1.Товары[НужнаяСтрока];
		СтрДок.КоличествоУдалено = СтрДок.КоличествоНачальное;
		СтрДок.Количество = 0;
		СтрДок.Сумма = 0;
		СтрДок.СуммаРеализации = 0;
		СтрДок.Статус = ПредопределенноеЗначение("Перечисление.СтатусыПозицийЗаказа.Удалено");
		
		МЗ = РегистрыСведений.ЗаказТоварыДопИнф.СоздатьМенеджерЗаписи();
		МЗ.Заказ = Заказ;
		МЗ.ИдСтроки = СтрДок.ИдСтроки;
		МЗ.Прочитать();
		МЗ.Статус = СтрДок.Статус;
		МЗ.Количество = 0;
		МЗ.КоличествоУдалено = СтрДок.КоличествоУдалено;
		МЗ.Записать();
		
		ЗаписатьЗаказ(Док1, ПредопределенноеЗначение("Перечисление.СтатусыЗаказа.Открыт"), Истина);
	Иначе
...
Процедура ВводКартыКлиента(флЗаказОткрыт = Ложь) Экспорт 
	...
	Если ТипЗнч(РезОткрытия) = Тип("Структура") Тогда
		КартаДоступа = РезОткрытия.КартаДоступа;
		Если ЗначениеЗаполнено(КартаДоступа) Тогда
			Если ЕстьЗаказыПоПродавцуКарте(,КартаДоступа) Тогда // есть заказ
				флЗаказОткрыт = ОткрытьЗаказПоПродавцуКарте(,КартаДоступа);
			Иначе
				СоздатьЗаказ(глПользователь, КартаДоступа, РезОткрытия.СпособРегистрацииБейджа); // Создаем заказ по карте и продавцу
...
Процедура ВводКартыЛояльности() Экспорт 
	...
	Попытка
		Если НЕ ПустаяСтрока(ЗаказОбъект.НомерКартыЛояльности) Тогда
			...
			ЗаписатьЗаказ(ЗаказОбъект, ПредопределенноеЗначение("Перечисление.СтатусыЗаказа.Открыт"), Истина);
		КонецЕсли;
	Исключение
		Возврат;		
	КонецПопытки;	
...
Процедура Заблокировать() Экспорт
	...
	Если Авторизация() Тогда
		...
		Иначе
			ГлавнаяФорма.ОтключитьОбработчикОжидания("СброситьЗаказВБлокировке");
		КонецЕсли;
	КонецЕсли;
	Если НЕ флЗаказОткрыт Тогда
...
Процедура ОбработкаВнешнегоСобытия(Источник, Событие, Данные) Экспорт
	...
	Если глСтекОкон.Количество() > 2 Тогда
		Возврат;
	ИначеЕсли глСтекОкон.Количество() = 2 Тогда
		Попытка
			Если НЕ глПараметрыРМ.ИнфоДисплей_РазрешитьАвторизациюГостяВБлокировке Тогда
				Возврат;
			КонецЕсли;
			Если ТипЗнч(глСтекОкон[0].Форма.ОбработкаОбъект) = Тип("ОбработкаОбъект.Авторизация") Тогда
				
				ОбработкаВводКарты = ИнтерфейсРМ.ПолучитьОбъектОбработки("ВводКартыКлиента");
				КартаДоступаКлиента = ОбработкаВводКарты.ПолучитьКартуДоступаПоИдентификатору(_Знач);
				Если ЗначениеЗаполнено(КартаДоступаКлиента) 
					И ЕстьЗаказыПоПродавцуКарте(, КартаДоступаКлиента) Тогда 
					флЗаказОткрыт = ОткрытьЗаказПоПродавцуКарте(, КартаДоступаКлиента);
					Если флЗаказОткрыт Тогда
						ГлавнаяФорма.ПодключитьОбработчикОжидания("ОтобразитьЗаказВБлокировке", 0.1, Истина);
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		Исключение
	...
	Если ТипЗнч(_Знач) = Тип("Строка") Тогда
		...
		ИначеЕсли НЕ ЗначениеЗаполнено(_Объект) Тогда
			Если МастерТестирования = Неопределено Тогда
				...
				ИнтерфейсРМ.ВопросПредупреждение("Ошибка",Текст1,Текст2,"","ОК","");			
				ЗарегистрироватьСобытие("Неопознанный штрихкод", УровеньЖурналаРегистрации.Предупреждение, , ТекущийДокумент.Ссылка, """"+ШК+"""");
				Возврат;
			Иначе
				Если _Знач = "1" Тогда
...
Функция ИнициализацияРабочегоМеста(ТекущееРМ) Экспорт
	...
	Если НЕ РежимТестирования И НЕ глПараметрыРМ.Тест Тогда
		
		Попытка
			ФайлЛокальнойКопии = Новый Файл("c:\sup_kkm\1cv8.1CD");
			ДатаЛокальнойКопии = '00010101';
			Если ФайлЛокальнойКопии.Существует() Тогда
				ДатаЛокальнойКопии = НачалоДня(ФайлЛокальнойКопии.ПолучитьВремяИзменения());
			КонецЕсли;
			Если ДатаЛокальнойКопии < НачалоДня(ТекущаяДатаНаСервере()) Тогда
				УправлениеСерверамиПриложений.СоздатьЛокальнуюФайловуюБазу();	
			ИначеЕсли ДатаЛокальнойКопии > НачалоДня(ТекущаяДатаНаСервере()) Тогда 	
				ЗарегистрироватьСобытие("Резервирование.Ошибка создания локальной базы", УровеньЖурналаРегистрации.Ошибка, , глРабочееМесто, "Дата локальной копии базы больше текущей.");
			КонецЕсли; 
		Исключение
			ЗарегистрироватьСобытие("Резервирование.Ошибка создания локальной базы", УровеньЖурналаРегистрации.Ошибка, , глРабочееМесто, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
	...
	глПараметрыРМ.Вставить("ФирмыРМ", ОбщегоНазначенияПовтИсп.ФирмыРабочегоМеста());
	...
	Если Не РезультатБлокировки.Успех Тогда
		...
		Если НЕ ПустаяСтрока(ПрофильБлок) Тогда
			...
			Если ЗначениеЗаполнено(РМ_Блок) 
				...
				И НЕ РМ_Блок.Тип = ПредопределенноеЗначение("Перечисление.ТипыРМ.СтанцияОплатыМОКП")
				Тогда
...
Функция ОткрытьЗаказ(ЗаказСсылка) Экспорт
	...
	фСлужебныйБейдж = ЗаказСсылка.КартаДоступа.СлужебныйБейдж;
	Заказ = ЗаказСсылка;
	ТекущийДокумент = Заказ;
	...
	ПоказатьЗаказНаМонитореГостя(ИнтервалОбновленияИнтерфейса, "ОткрытьЗаказ()");
	//ГлавнаяФорма.ПодключитьОбработчикОжидания("ОбновитьЗаказНаМонитореГостя", 0.1, Истина);
	//ДГ-
...
Функция ВводНовойСтроки(ВыбТов, Количество = 0, ШК = "") Экспорт
	...
	Если фСлужебныйБейдж Тогда
		Если Не ОбщегоНазначенияПовтИсп.ЭтоТоварМяснов(ВыбТов) Тогда
			Возврат Неопределено;
		КонецЕсли;
	КонецЕсли;
	...
	//:ТекущийДокумент = Документы.Заказ.ПустаяСсылка();
	Док1 = ТекущийДокумент.ПолучитьОбъект();
	...
	Нов.Статус = Перечисления.СтатусыПозицийЗаказа.Выдано;
		
	ТекКоличество = 1;
	ЗаписатьЗаказ(док1,, Истина);
	ЗарегистрироватьСобытие("Заказ.Добавление строки",,,Заказ.Ссылка,"" + ВыбТов + ", кол. " + Количество);
...
// заказ1 - объект
Процедура ЗаписатьЗаказ(Заказ1, Статус = Неопределено, ОбновитьДанныеЛояльности = Ложь) Экспорт 
	...
	КонецЕсли;
	Если Заказ1.Автор.Пустая() Тогда
		Заказ1.Автор = глПользователь;
	КонецЕсли;
	Если Заказ1.РабочееМесто.Пустая() Тогда
	...
	Если Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаказа.Удален") Тогда
		Для каждого Т Из Заказ1.Товары Цикл
			...
			Т.СуммаРеализации = 0;
			Т.Сумма = 0;
			Т.Статус = ПредопределенноеЗначение("Перечисление.СтатусыПозицийЗаказа.Удалено");
		КонецЦикла;
	КонецЕсли;
	...
	Заказ1.Записать();
	Заказ1.ЗаписатьЗаказТоварыДопИнф();
	...
	Оповестить("ИзменениеЗаказаНаСтанцииСканирования", Заказ, ГлавнаяФорма);
	...
	Иначе
		...
		Если ЗначениеЗаполнено(Заказ.КартаДоступа) Тогда
			ксТрактир.РегистрацияЗаказаПоКартеДоступа(Заказ.КартаДоступа, Заказ);
		КонецЕсли;
	...
	ПоказатьЗаказНаМонитореГостя(ИнтервалОбновленияИнтерфейса_ВводНовойСтроки,"ЗаписатьЗаказ()");
...
Функция ПоказатьЗаказНаМонитореГостя_ОбработчикОжидания() Экспорт
	...
	Для Каждого стТовары Из ГлавнаяФорма.тпТовары Цикл
...
Процедура НапечататьЦенник(ВыбТов, ШК, Вес) Экспорт 
	...
	Если не ВнешняяШК.ГрафикаУстановлена ТОгда
		ЗарегистрироватьСобытие("Ошибка выполнения.Ошибка формирования ШК", УровеньЖурналаРегистрации.Ошибка,,,);
		Возврат;
	КонецЕсли;
...
Процедура СоздатьЗаказ(Автор, КартаДоступа, СпособРегистрацииБейджа = 0)
	
	ДатаОткрытия = ТекущаяДатаНаСервере();
	
	Заказ1 = Документы.Заказ.СоздатьДокумент();
	Заказ1.КартаДоступа = КартаДоступа;
	Заказ1.СпособРегистрацииБейджа = СпособРегистрацииБейджа;
	...
	МЗ = РегистрыСведений.ОткрытыеЗаказыПоКартам.СоздатьМенеджерЗаписи();
	МЗ.Заказ = Заказ;
	МЗ.КартаДоступа = КартаДоступа;
	МЗ.ВремяПоследнейАктивности = ТекущаяДатаНаСервере();
	МЗ.МестоПоследнейАктивности = глРабочееМесто;
	МЗ.ТекущийАвтор = Автор;
	МЗ.Записать(Истина);
...
//	ЗаписатьЗаказ(Заказ1);
	...
	ДопПараметрыИнфо.СуммаПоСтанцииНач = СуммаРеализации();
	ДопПараметрыИнфо.СброситьНастройки = Ложь;
	ПоказатьЗаказНаМонитореГостя(ИнтервалОбновленияИнтерфейса, "СоздатьЗаказ()");
	//ГлавнаяФорма.ПодключитьОбработчикОжидания("ОбновитьЗаказНаМонитореГостя", 0.1, Истина);
	//ДГ-
...
#Область ПРИГОТОВЛЕНИЕ

Функция ТекущийТоварМожноПриготовить() Экспорт
	ТД = ЭлементыФормы.тпТовары.ТекущиеДанные;
	Если ТД = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	Если ТД.Статус = Перечисления.СтатусыПозицийЗаказа.ДляПриготовления Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Возврат ТоварыДляПриготовления.Найти(Товар) <> Неопределено И Не ЗначениеЗаполнено(ТД.ИдСвязаннойСтроки);
КонецФункции

Функция МожноОтменитьПроготовлениеТекущегоТовара() Экспорт
	ТД = ЭлементыФормы.тпТовары.ТекущиеДанные;
	Если ТД = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	МЗ.ИдСтроки = ТД.ИдСтроки;
	МЗ.Прочитать();
	
	
	Если ЗначениеЗаполнено(МЗ.ИдСвязаннойСтроки) Тогда
		// повар уже начал готовить
		ТД.ИдСвязаннойСтроки = МЗ.ИдСвязаннойСтроки;
		Возврат Ложь;
	КонецЕсли;
...
Функция Приготовить() Экспорт
	Если ТекущийТоварМожноПриготовить() Тогда
		ИдСтроки = ЭлементыФормы.тпТовары.ТекущиеДанные.ИдСтроки;
		
		Если ПроверитьБлокировкуЗаказа(Заказ) Тогда
			Док1 = Заказ.ПолучитьОбъект();
			СтрДок = Док1.Товары.Найти(ИдСтроки, "ИдСтроки");
			СтрДок.Статус = Перечисления.СтатусыПозицийЗаказа.ДляПриготовления;
			Док1.Записать();
			Оповестить("ИзменениеЗаказаНаСтанцииСканирования", Заказ, ГлавнаяФорма);
		Иначе
			Возврат Ложь;
		...
		//Если Не Док1.Модифицированность() Тогда
		МЗ = РегистрыСведений.ЗаказТоварыДопИнф.СоздатьМенеджерЗаписи();
		МЗ.Заказ = Заказ;
		МЗ.ИдСтроки = ИдСтроки;
		МЗ.Прочитать();
		Если Не ЗначениеЗаполнено(МЗ.Товар) Тогда
			ЗаполнитьЗначенияСвойств(МЗ, ЭлементыФормы.тпТовары.ТекущиеДанные);
		КонецЕсли;
		МЗ.Заказ = Заказ;
		МЗ.ИдСтроки = ИдСтроки;
		МЗ.Статус = Перечисления.СтатусыПозицийЗаказа.ДляПриготовления;
		МЗ.Станция = глПараметрыРМ.Станция;
		МЗ.РабочееМесто = глРабочееМесто;
		МЗ.Записать();
		
		ЭлементыФормы.тпТовары.ТекущиеДанные.Статус = МЗ.Статус;
		//КонецЕсли;
		
		
		НапечататьЭтикеткуПриготовления(СтрДок.Товар, СтрДок.ШК, СтрДок.Количество);
		РаботаСокнами.ПоказатьПлашкуПрост("Приготовление запланировано", "Гость может пройти к повару");
		ЭлементыФормы.КнопкаПриготовить.ЦветТекстаКнопки = ЦветаСтиля.ЦветТемы;
		
	ИначеЕсли МожноОтменитьПроготовлениеТекущегоТовара() Тогда
		ИдСтроки = ЭлементыФормы.тпТовары.ТекущиеДанные.ИдСтроки;
		
		Если ПроверитьБлокировкуЗаказа(Заказ) Тогда
			Док1 = Заказ.ПолучитьОбъект();
			СтрДок = Док1.Товары.Найти(ИдСтроки, "ИдСтроки");
			СтрДок.Статус = Перечисления.СтатусыПозицийЗаказа.Дозаказ;
			Док1.Записать();
			Оповестить("ИзменениеЗаказаНаСтанцииСканирования", Заказ, ГлавнаяФорма);
		Иначе
			Возврат Ложь;
		...
		МЗ.Заказ = Заказ;
		МЗ.ИдСтроки = ИдСтроки;
		МЗ.Статус = Перечисления.СтатусыПозицийЗаказа.Дозаказ;
		МЗ.Станция = глПараметрыРМ.Станция;
		МЗ.РабочееМесто = глРабочееМесто;
		МЗ.Записать();
		...
		РаботаСокнами.ПоказатьПлашкуПрост("Приготовление отменено", "");
		ЭлементыФормы.КнопкаПриготовить.ЦветТекстаКнопки = ЦветаСтиля.Акцент;
		
	КонецЕсли;
КонецФункции

Процедура НапечататьЭтикеткуПриготовления(ВыбТов, ШК, Вес) Экспорт 
	Таб = Новый ТабличныйДокумент;
	Макет = ПолучитьМакет("ЭтикеткаПриготовление");
	ОбластьЭтикетка = Макет.ПолучитьОбласть("Строка|Колонка");
	ОбластьЭтикетка.Параметры.Масса = Вес;
	Цена = ЦенаТовара(ВыбТов).Цена;
	ОбластьЭтикетка.Параметры.Цена = Цена;
	...
	ОбластьЭтикетка.Параметры.ДатаВремя = ТекущаяДата();
	ОбластьЭтикетка.Параметры.наименование = ВыбТов.наименование;
	ОбластьЭтикетка.Параметры.НомерЗаказа = УбратьВсеБуквы(Заказ.Номер);
	ОбластьЭтикетка.Параметры.НомерБейджа = Заказ.КартаДоступа.Идентификатор2;
	ПодключитьВнешнююКомпоненту("ОбщийМакет.КомпонентаПечатиШтрихкодов","КартинкаШтрихкода",ТипВнешнейКомпоненты.Native);
	ВнешняяШК = Новый("AddIn.КартинкаШтрихкода.Barcode");
	Если не ВнешняяШК.ГрафикаУстановлена ТОгда
		ЗарегистрироватьСобытие("Ошибка выполнения.Ошибка формирования ШК", УровеньЖурналаРегистрации.Ошибка,,,);
