...
Процедура ЗаполнитьТекущийУровеньПоСправочнику(Каталог, Родитель=Неопределено, ДобавитьВыходВверх=Истина)
	...
	Если ПараметрыНастройки.КнопкиПоказЦены Тогда
		Запрос.Текст = "
			...
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.Цены.СрезПоследних(&Период, ТипЦен = &ТипЦен) КАК ЦеныСрезПоследних
			|		ПО Товары.Ссылка = ЦеныСрезПоследних.Товар
			|";
	Иначе
		Запрос.Текст = "
			|ВЫБРАТЬ
...
Процедура ОбработкаНажатияКнопки(Элемент, Вес = Неопределено) Экспорт
...
Функция ПолучитьПараметрыОкна() Экспорт
	...
	ИначеЕсли ПараметрыНастройки.ПоложениеОкна = "Низ" Тогда
		...
		//ПараметрыОкна.Вставить("Обрезать", Истина);
