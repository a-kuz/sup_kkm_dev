...
Процедура КнопкиДействияПередОткрытием(ТекущаяФорма) Экспорт
	...
	КонецЕсли;
	ТипРМ = Перечисления.ТипыРМ.СтанцияОплатыМОКП;	
	
	Если Не ПараметрыНастройки.Свойство("ТипРМ", ТипРМ) Тогда
	
	Если Не глПараметрыРМ.Свойство("Тип", ТипРМ) Тогда
		ТипРМ = Перечисления.ТипыРМ.СтанцияОплатыМОКП;
	КонецЕсли;
	КонецЕсли;
	
	
	Если ТипРМ = Перечисления.ТипыРМ.СтанцияОплатыМОКП Или ПараметрыНастройки.МестоРеализации = Справочники.МестаРеализации.Ресторан Тогда
...
Процедура ЗаполнитьНаименование(СтрокаКаталога)
	...
	Иначе
		...
		ТоварСвойства.Вставить("Доступность", Товар.ЕстьВПродаже);	
...
Процедура КнопкиИнициализацияНастройки() Экспорт
	...
	Исключение
		...
		КнопкаДляОпределенияРазмера.Ширина = 1000;
		КнопкаДляОпределенияРазмера.Высота = ФормаПодбора.Высота;//-45;  ПараметрыНастройки
	КонецПопытки;
	
	ШиринаВнутреннейОбласти = мин(КнопкаДляОпределенияРазмера.Ширина,ПараметрыНастройки.ШиринаОкна-10);
	ВысотаВнутреннейОбласти = мин(КнопкаДляОпределенияРазмера.Высота,ПараметрыНастройки.ВысотаОкна-25);
	ЭлементыФормы.Очистить();	
	...
	КонецЕсли; 
	ШиринаВнутреннейОбласти = Мин(Ширина-10,ШиринаВнутреннейОбласти);
...
Функция СоздатьПанельКнопок(ИмяКнопок, ПанельВерх, ПанельЛево, ПанельВысота, ПанельШирина, ОтступПанели=0)
	...
	Если ОбрезкаОкнаШирина <> Неопределено Тогда
		...
		Если КоличествоКнопокПоГоризонтали Тогда
...
Процедура ПоказатьКнопку(ИмяКнопки, НомерКнопки, Каталог)
	...
	Иначе
		...
		Если СтрокаКаталога.ТипКнопки=3 ИЛИ не ПараметрыНастройки.СвоиНастройкиДляГруппы Тогда
		...
		Иначе
			Кнопка.ЦветФонаКнопки = ?(СтрокаКаталога.ЦветФона.Вид = ВидЦвета.АвтоЦвет,ПараметрыНастройки.ЦветФонаКнопкиГруппы, СтрокаКаталога.ЦветФона);	
		КонецЕсли;
		...
		Если ПараметрыНастройки.КнопкиВыводТекста Тогда
			...
			Иначе
				Кнопка.ЦветТекстаКнопки = ?(СтрокаКаталога.ЦветТекста.Вид = ВидЦвета.АвтоЦвет,	ПараметрыНастройки.ЦветТекстаКнопкиГруппы,	СтрокаКаталога.ЦветТекста);
				Кнопка.Шрифт				= ?(СтрокаКаталога.Шрифт.Вид = ВидШрифта.АвтоШрифт,		ПараметрыНастройки.ШрифтКнопкиГРуппы,			СтрокаКаталога.Шрифт);
			КонецЕсли;
		КонецЕсли; 
...
Процедура НастройкаКнопки(ИмяКнопки, НомерКнопки, Каталог, СтрокаКаталога)
	...
	ФормаНастройки.Товар		= ВнешниеИсточникиДанных.СУП_Центр.Таблицы.Спр_Товары.НайтиПоПолю("КодГрПгрПрд", СтрокаКаталога.КодСУП);
	...
	Иначе // рабочий режим
		...
		ИначеЕсли СтрокаКаталога.ТипКнопки=3 Тогда // Товар
			...
			Иначе
				...
				Если ТекущаяНастройка.МестоРеализации <> Справочники.МестаРеализации.Ресторан И ФормаПодбора.модальныйРежим Тогда
...
Функция ПолучитьРабочуюФорму(ОсновноеМеню=Неопределено) Экспорт
	...
	Если ПараметрыНастройки.ТипРМ = Перечисления.ТипыРМ.СтанцияПовараМОКП Тогда
		ИмяФормы = "КнопкиСвободноеОкноРесторанПовар"; 
	ИначеЕсли  ПараметрыНастройки.ТипРМ = Перечисления.ТипыРМ.СтанцияОплатыМОКП Тогда
...
Функция ПолучитьПараметрыНастройки() Экспорт
	...
	Попытка
		Если ПараметрыНастройки.ТипРМ = Перечисления.ТипыРМ.СтанцияПовараМОКП Тогда
			ПараметрыНастройки.ШиринаОкна = Мин(ПараметрыНастройки.ШиринаОкна,780);
			ПараметрыНастройки.ВысотаОкна = Мин(ПараметрыНастройки.ВысотаОкна,330);
		КонецЕсли;
	Исключение
	КонецПопытки;
	...
	Если Не ПараметрыНастройки.Свойство("ТипРМ", ТипРМ) Тогда
		Если Не глПараметрыРМ.Свойство("Тип", ТипРМ) Тогда
			ТипРМ = Перечисления.ТипыРМ.СтанцияОплатыМОКП;
		Иначе
			ПараметрыНастройки.Вставить("ТипРМ", ТипРМ);
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьПараметрыОкна() Экспорт
	...
	Если ОкноШирина = Неопределено Тогда
	...
	Попытка
		Если ПараметрыНастройки.ТипРМ = Перечисления.ТипыРМ.СтанцияПовараМОКП Тогда
			ПараметрыОкна.Ширина = Мин(ПараметрыНастройки.ШиринаОкна,780);
			//ПараметрыОкна.Высота = Мин(ПараметрыНастройки.ВысотаОкна,323);
...
Функция ВыборМеню(ЗакрыватьФорму = Истина) Экспорт
	...
	Если ФормаПодбора.ВладелецФормы <> Неопределено Тогда
	...
	ВыбНастройка = ИнтерфейсРМ.ВыборИзСписка(СписокНастроек,,,"МЕНЮ", "ДОСТУПНЫЕ МЕНЮ");
	...
	Иначе
		...
		Если ФормаПодбора.открыта() Тогда
			УдалитьФормуИзСтекаОкон(ФормаПодбора);
			ИнтерфейсРМ.ПриОткрытииОкна(ФормаПодбора);
		КонецЕсли;
...
Процедура УдалитьФормуИзСтекаОкон(Форма) Экспорт
	Индексы = Новый Массив;
	//:глСтекОкон=новый массив;

	Для Каждого т Из глСтекОкон Цикл
		Если Т.форма = Форма Тогда
			Индексы.Добавить(глСтекОкон.Найти(т));
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого Индекс Из Индексы Цикл
		глСтекОкон.Удалить(Индекс);
	КонецЦикла;
КонецПроцедуры
...
Процедура ВставитьИзБуфера(ИмяКнопки, НомерКнопки, Каталог, СтрокаКаталога) Экспорт
	...
	ирОбщийКлиент.ОткрытьФормуПроизвольногоЗначенияЛкс(Текст);
	...
	Для Каждого Т Из Массив Цикл
		...
		КонецПопытки;
		КодСУП = СокрЛП(Т);
		Товар = ВнешниеИсточникиДанных.СУП_Центр.Таблицы.Спр_Товары.НайтиПоПолю("КодГрПгрПрд", КодСУП);
