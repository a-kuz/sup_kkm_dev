...
Процедура ПриИзмененииСуммы()
	...
	//		УстановитьДоступностьКнопки("ОплатаБезнал", Ложь);
	//	КонецЕсли; 
	...
	Если Получено Тогда
		ЭлементыФормы.кнВыбратьВариант.Видимость = 0;
	Иначе
		ЭлементыФормы.кнВыбратьВариант.Видимость = 1;
	КонецЕсли;
...
Процедура ПриОткрытии()
	...
	НадписьКоплате = Формат(Коплате, "ЧДЦ=2; ЧРД=.; ЧН=") + " " + глСимволРубля;
	НадписьСуммаЧека = ""+Формат(СуммаЧека, "ЧДЦ=2; ЧРД=.; ЧН=") + " " + глСимволРубля;
	ИнтерфейсРМ.ПриОткрытииОкна(ЭтаФорма);
	ПриИзмененииСуммы(); 
...
Процедура кнВыбратьВариантНажатие(Элемент)
	...
	ЗапрещенныеТипыОплат.Добавить(Перечисления.ТипыОплаты.Нал);
	ЗапрещенныеТипыОплат.Добавить(Перечисления.ТипыОплаты.Карта);
	...
	ЗарегистрироватьСобытие("ИнтерактивноеДействие.Выбор варианта оплаты",УровеньЖурналаРегистрации.Предупреждение,,Заказ.Ссылка,Строка(ВыбВариант));
