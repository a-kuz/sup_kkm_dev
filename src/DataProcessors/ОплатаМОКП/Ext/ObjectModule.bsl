...
Процедура ИнициализироватьПротоколПоВозврату() Экспорт
	...
	ПротоколРасчетов.КассоваяСмена = Обработки.ОткрытиЗакрытиеСменыКасса.ПолучитьСменуКассы(ККМ.КодСуп);
...
Функция Фискализация(СуммаЮрлицо) Экспорт
	...
	Для Каждого Т Из Товары Цикл
		...
		Для Каждого СтрокаСпецифика Из СпецификиСтроки Цикл
			...
			СтрокаЧека.Вставить("Количество",?(СтрокаСпецифика.Количество,СтрокаСпецифика.Количество * Т.Количество,Т.Количество));
...
Функция ПроверитьОткрытиеСмен() Экспорт
	ОшибкиЕсть = ложь;
	Если Обработка_ККМ <> Неопределено Тогда
		ВыходныеПараметры = ВыходныеПараметры;
		Ответ = Обработка_ККМ.ВыполнитьКоманду("ПолучитьТекущееСостояние",Новый Структура(),ВыходныеПараметры);
		Если Не Ответ Тогда
			ОшибкиЕсть = Истина;
		Иначе 
			Если ВыходныеПараметры.Свойство("СменаОткрыта") и не ВыходныеПараметры.СменаОткрыта Тогда
				ОшибкиЕсть = Истина;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	Если Обработка_ККМ1 <> Неопределено Тогда
		ВыходныеПараметры = ВыходныеПараметры;
		Ответ = Обработка_ККМ1.ВыполнитьКоманду("ПолучитьТекущееСостояние",Новый Структура(),ВыходныеПараметры);
		Если Не Ответ Тогда
			ОшибкиЕсть = Истина;
		Иначе 
			Если ВыходныеПараметры.Свойство("СменаОткрыта") и не ВыходныеПараметры.СменаОткрыта Тогда
	...
	Если ОшибкиЕсть Тогда
		ИнтерфейсРМ.ВопросПредупреждение("Ошибка","Есть неоткрытые смены на ККТ","","","ОК","");
	КонецЕсли;
	Возврат ОшибкиЕсть;
КонецФункции
...
Функция ФискализацияВозврата() Экспорт
	...
	Если РезультатКомандыФискализация Тогда
		...
		ПротоколРасчетов.НеУдалосьОтправитьОтменуВегаис = 0;
		Если ТипЗнч(ПараметрыВыходные) = Тип("Структура") Тогда
			ПараметрыВыходные.Свойство("НомерЧека", докВозврат.НомерЧека);
			ПараметрыВыходные.Свойство("НомерСмены", докВозврат.НомерСмены);
			ПараметрыВыходные.Свойство("НомерЧека", ПротоколРасчетов.НомерЧека);
			ПараметрыВыходные.Свойство("НомерСмены", ПротоколРасчетов.НомерСмены);
		КонецЕсли;
		// допишем параметры для печати QR на чеке лояльности {
...
Функция Оплата() Экспорт
	...
	Для Инд = 1 по ТаблицаЮрЛиц.Количество() Цикл
		...
		Заказ.ПересчетНДС(); 
		ОтветЗаписи = ИнтерфейсРМ.ПопыткаДействияСОбъектом(Заказ, "Объект.Записать()");
		Если не ОтветЗаписи Тогда
			Заказ = ПротоколРасчетов.Заказ.ПолучитьОбъект();
		...
		Если РезультатФискализации Тогда
			...
			ПротоколРасчетов.Смена = ПротоколРасчетов.КассоваяСмена.СменаТТ;
	...
	КонецЦикла;
	фИдетОплата = Ложь;
	ПечататьЧекНастройка = РегистрыСведений.ДополнительныеСвойства.ЗначениеСвойства(глРабочееМесто,"ПечататьЧек");
	Если ПечататьЧекНастройка <> Неопределено и ПечататьЧекНастройка = 0 Тогда
...
Функция ВозвратОплаты(Картой = Ложь) Экспорт 
	...
	Если не Картой или получено > 0 Тогда
		ОткрытьДенежныйЯщик();
	КонецЕсли;
	...
	Для каждого стр из товары Цикл
		...
		Если СтрТабл = Неопределено Тогда
			СтрТабл = ТаблицаЮрЛиц.Добавить();
			СтрТабл.Фирма = Стр.Фирма;
			СтрТабл.Сумма = Стр.Сумма;
		Иначе
			СтрТабл.Сумма = СтрТабл.Сумма + стр.Сумма;
		КонецЕсли;
	КонецЦикла;
