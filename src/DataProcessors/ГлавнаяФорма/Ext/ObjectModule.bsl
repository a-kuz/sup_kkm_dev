...
Функция ВводНовойСтроки(ВыбТов, Количество = 0, ШК = "") Экспорт
	...
	Если ТекущийРежим = "ВОЗВРАТ" Тогда
		...
		СуммаРеализации = Нов.Сумма;
	Иначе
		Нов.ЦенаРеализации = Цена;
		Нов.СуммаРеализации = Цена * Количество;
		ЦенаТовара = Цена;
		СуммаРеализации = Нов.СуммаРеализации;
	КонецЕсли;
	...
	ЗарегистрироватьСобытие("Добавление строки", ,, ТекущийДокумент.Ссылка, 
							СтрШаблон("%1 %2х%3=%4
							|идСтроки: %5", Нов.Товар, Нов.Цена, Нов.Количество,СуммаРеализации,Нов.идСтроки));
	ЗаписатьДокумент(Перечисления.СтатусыЗаказа.Открыт);
...
Функция ВводТоваровДоступен()
	Попытка
		Если Не фВводТоваровДоступен Тогда
			...
			Иначе
				ГлавнаяФорма.ПодключитьОбработчикОжидания("ВозвратОплаты", 0.2, 1);
			КонецЕсли;
