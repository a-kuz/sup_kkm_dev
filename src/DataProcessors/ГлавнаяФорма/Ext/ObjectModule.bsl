...
Функция ПроверитьЗаказНаВылетВоВремяФискализации()
	...
	ЗаписатьЗаказ(Перечисления.СтатусыЗаказа.Закрыт);
	ЛояльностьКлиент.ФинализацияЧека(Заказ,обПротокол.Ссылка);
...
Процедура ЗаписатьДокумент(Статус)
	Если ТекущийРежим = "ВОЗВРАТ" Тогда
...
Процедура Оплата() Экспорт
	Попытка
		...
		Если Обработки.Оплата.ПолучитьФорму("Форма" + глПараметрыРМ.ИнтерфейсТип).Открыта() Тогда
			ГлавнаяФорма.УстановитьДоступностьКнопки("Оплата", 1, 1, "Видимость");
			Возврат;
		КонецЕсли;
		...
		Если фВводТоваровДоступен Тогда 
			...
			Попытка
				...
				РезультатЛояльности = ЛояльностьКлиент.РасчетЧека(Заказ);
...
Процедура Сторно() Экспорт 
	...
	ИначеЕсли КоличествоСтрок Тогда
		...
		ПоследняяСтрока.КоличествоУдалено = ПоследняяСтрока.КоличествоНачальное;
		ПоследняяСтрока.Количество = 0;
		ПоследняяСтрока.Сумма = 0;
		Если ТекущийРежим = "ПРОДАЖА" Тогда
			ПоследняяСтрока.СуммаРеализации = 0;
		КонецЕсли;
...
Процедура ОткрытьМенюОпераций() Экспорт
	Если Не ТекущийДокумент = Неопределено Тогда
		Если ТекущийДокумент.Товары.Итог("Количество") Тогда
			ИнтерфейсРМ.ВопросПредупреждение("Ошибка", "Заказ не закрыт", "Текущий заказ не закрыт","","ОК","");
			Возврат;
		КонецЕсли;
