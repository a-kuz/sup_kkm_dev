...
//
Функция Авторизация(ФлагПоКнопке=Ложь, ЗапретБыстрогоВыбора = Ложь) Экспорт
	...
	КонецЕсли;
 	...
 	Если ФлагПоКнопке ИЛИ НЕ ЗначениеЗаполнено(глПользователь) Тогда
		ИнтерфейсРМ.СменаПользователя(,ЗапретБыстрогоВыбора);
		ГлавнаяФорма.ОбновитьВремя();
		ГлавнаяФорма.ОбновитьОстальное();
	КонецЕсли; 
	
	Если Не ЗначениеЗаполнено(глПользователь) Тогда
		Авторизация(,ЗапретБыстрогоВыбора);
		...
		Если Т.Количество И Т.фАлкоголь = 1 Тогда
			Если ТекущийРежим = "ПРОДАЖА" Тогда
				Попытка
					...
					Если ПротоколРасчетов.ДобавленыЗаписиВexcise Тогда
						Продолжить;
					КонецЕсли;
...
Процедура ВозвратОплаты() Экспорт
	...
	Иначе
		ОбработкаОплаты.ПротоколРасчетов = ПротоколРасчетов.ПолучитьОбъект();
		ФормаОплаты = ОбработкаОплаты.ПолучитьФорму("ПроблемныйВозврат"+глПараметрыРМ.ИнтерфейсТип);
	КонецЕсли;
