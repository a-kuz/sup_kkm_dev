...
#Если Клиент Тогда
	...
	Процедура ЗаполнитьЧеки() Экспорт
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |	И ПротоколРасчетовПротокол.Ссылка.Фирма = &Фирма
		               |	И ПротоколРасчетовПротокол.Ссылка.Дата МЕЖДУ НАЧАЛОПЕРИОДА(&ДатаНач, ДЕНЬ) И НАЧАЛОПЕРИОДА(&ДатаКон, ДЕНЬ)
		               |
		               |СГРУППИРОВАТЬ ПО
	...
	Процедура ОК() Экспорт
		...
		ВопросОтвет = ИнтерфейсРМ.ВопросПредупреждение("Печать копии чека","Копию какого чека расчатать?","","Фискального","Лояльности","Esc=Отмена");
