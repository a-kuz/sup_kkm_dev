...
//
Функция ОД_РазложитьСтрокуВМассивПодстрок(Знач Строка, Знач Разделитель = ",", Знач ПропускатьПустыеСтроки = Неопределено) Экспорт
	...
	Пока ПравилаОбмена.Прочитать() Цикл
		...
		ИначеЕсли ИмяУзла = "ПоискПоТабличнымЧастям" Тогда
			...
			Для Номер = 1 По СтрЧислоСтрок(Значение) Цикл
				...
				СтрокаТаблицы.ПоляПоискаТЧ = ОД_РазложитьСтрокуВМассивПодстрок(СтрокаПоиска);
