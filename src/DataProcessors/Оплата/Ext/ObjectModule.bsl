	...
	Если СуммаБезнал Тогда
		...
		Пока Истина Цикл
			...
			Если Не ОплатаКартой() Тогда
				...
				Если ИнтерфейсРМ.ВопросПредупреждение("Ошибка", "Оплата картой не удалась", "Ошибка при работе с платежным терминалом","Повтор","","Отмена") = "Отмена" Тогда
					Если ИнтерфейсРМ.ВопросПредупреждение("Предложение", "Оплатить наличными?", "","Да","","Нет") = "Да" Тогда
						Получено = Коплате;
						СуммаНал = Коплате;
						СуммаБезнал = 0;
	...
	Если СтрокиЧека.Количество() Тогда
		Попытка
			...
			Пока Истина Цикл
				...
				ЗаписьЖурналаРегистрации("Оплата.Фискализация.Выходные параметры",УровеньЖурналаРегистрации.Информация,,Заказ.Ссылка,ЗначениеВСтрокуВнутр(ПараметрыВыходные));
				Если РезультатКомандыФискализация = Истина Тогда
					Прервать;
				Иначе
					Если ИнтерфейсРМ.ВопросПредупреждение("Ошибка", "Фискализация не удалась", "Ошибка при работе с принтером чеков","Повтор","","Отмена") = "Отмена" Тогда
						Прервать;
					Иначе
						Продолжить;
					КонецЕсли;
