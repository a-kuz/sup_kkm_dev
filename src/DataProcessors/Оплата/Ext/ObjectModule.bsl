...
Функция ВозвратНаКарту() Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации("Ошибка выполнения", УровеньЖурналаРегистрации.Ошибка,, докВозврат.Ссылка, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Функция Фискализация() Экспорт
	...
	Для Каждого Т Из Товары Цикл
		...
		Если Т.Количество Тогда
			//Если Pow(Т.ЦенаРеализации * Т.Количество - Т.СуммаРеализации, 2) > 0.0001 Тогда
...
Процедура ОтменаТранзакцииПлатежнойСистемыПоВозврату(Картой) Экспорт
	ЗаписьЖурналаРегистрации("Оплата.ОтменаТранзакцииПлатежнойСистемыПоВозврату.ВходВпроцедуру", УровеньЖурналаРегистрации.Информация, , докВозврат.Ссылка);
	...
	Попытка
		...
		Обработка_ПС.ВыполнитьКоманду("ОтменитьПлатежПоПлатежнойКарте",Новый Структура("Сумма, СсылочныйНомер",СуммаБезнал, ПротоколРасчетов.идТранзакцииПлатежнойСистемы), ВыходныеПараметры);
	Исключение
		ЗаписьЖурналаРегистрации("Платежная система", УровеньЖурналаРегистрации.Ошибка, Метаданные.Документы.Возврат, докВозврат.Ссылка, "Не удалось отменить транзакцию " + ПротоколРасчетов.идТранзакцииПлатежнойСистемы + " " + ОписаниеОшибки());
		ИнтерфейсРМ.ВопросПредупреждение("Ошибка", "Ошибка платежной системы", ОписаниеОшибки(), "","ОК", "");
		фОшибка = Истина;
	КонецПопытки;
	РезультатОплаты.Неудача = Истина;
...
Функция Оплата() Экспорт
	...
	Если фИдетОплата Тогда
		...
		ЗаписьЖурналаРегистрации("Оплата.ПовторныйВходВОплату",УровеньЖурналаРегистрации.Ошибка,,Заказ.Ссылка);
		Возврат Ложь;
	КонецЕсли;
	...
	Если Товары.Итог("фАлкоголь") Тогда	
		Если Не ЗначениеЗаполнено(ПротоколРасчетов.ИдентификаторЗапросаЕГАИС) Или ЗначениеЗаполнено(ПротоколРасчетов.ИдентификаторЗапросаЕГАИС_Отмена) Тогда
			РезультатЕГАИС = Алкоголь.ОтправитьЗаказВЕГАИС(Заказ);
		...
		Попытка
			Если Товары.Итог("фАлкоголь") Тогда
				Если Алкоголь.ЗаписатьВExcise(Заказ) Тогда
					...
					ИнтерфейсРМ.ПопыткаДействияСОбъектом(ПротоколРасчетов, "Объект.Записать()");
				Иначе
					ЗаписьЖурналаРегистрации("Excise", УровеньЖурналаРегистрации.Ошибка, Метаданные.Документы.Заказ, Заказ.Ссылка, "Не удалось записать в Excise");
				КонецЕсли;
			КонецЕсли;
		Исключение
			ЗаписьЖурналаРегистрации("Excise", УровеньЖурналаРегистрации.Ошибка, Метаданные.Документы.Заказ, Заказ.Ссылка, "Не удалось записать в Excise. " + ОписаниеОшибки());
		КонецПопытки;
...
Функция ВозвратОплаты(Картой = Ложь) Экспорт 
	ЗаписьЖурналаРегистрации("Возврат оплаты.Вход в процедуру", УровеньЖурналаРегистрации.Информация, , докВозврат.Ссылка);
	Если фИдетОплата Тогда
		ИнтерфейсРМ.ВопросПредупреждение("Ошибка","Идет оплата","Процесс оплаты уже запущен","","ОК","",,6);
		ЗаписьЖурналаРегистрации("Оплата.ПовторныйВходВОплату",УровеньЖурналаРегистрации.Ошибка,,докВозврат.Ссылка);
		Возврат Ложь;
	КонецЕсли;
	фИдетОплата = Истина;
	...
	Если РезультатФискализации Тогда
		...
		Попытка
			ПротоколРасчетов.Записать(РежимЗаписиДокумента.Проведение);
		Исключение
			ЗаписьЖурналаРегистрации("Ошибка проведения протокола расчетов", УровеньЖурналаРегистрации.Ошибка, , докВозврат.Ссылка, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
		...
		Попытка
			Если Товары.Итог("фАлкоголь") Тогда
				Если Алкоголь.ЗаписатьВExcise(докВозврат, Истина) Тогда
					...
					ИнтерфейсРМ.ПопыткаДействияСОбъектом(ПротоколРасчетов, "Объект.Записать()");
				Иначе
					ЗаписьЖурналаРегистрации("Excise", УровеньЖурналаРегистрации.Ошибка, 	, докВозврат.Ссылка, "Не удалось записать в Excise");
				КонецЕсли;
			КонецЕсли;
		Исключение
			ЗаписьЖурналаРегистрации("Excise", УровеньЖурналаРегистрации.Ошибка, , докВозврат.Ссылка, "Не удалось записать в Excise");
...
Процедура ОтменаВегаис()
	...
	Если ЗначениеЗаполнено(ПротоколРасчетов.ИдентификаторЗапросаЕГАИС) И Не ЗначениеЗаполнено(ПротоколРасчетов.ИдентификаторЗапросаЕГАИС_Отмена) Тогда
		...
		ИнтерфейсРМ.ПопыткаДействияСОбъектом(ПротоколРасчетов, "Объект.Записать()");
		ЗаписьЖурналаРегистрации("Оплата.ОтменаВегаис.До вызова", УровеньЖурналаРегистрации.Информация, , Заказ.Ссылка, "ПротоколРасчетов.ИдентификаторЗапросаЕГАИС = " + ПротоколРасчетов.ИдентификаторЗапросаЕГАИС + "
		...
		ЗаписьЖурналаРегистрации("Оплата.ОтменаВегаис.После вызова", УровеньЖурналаРегистрации.Информация, , Заказ.Ссылка, "ПротоколРасчетов.ИдентификаторЗапросаЕГАИС = " + ПротоколРасчетов.ИдентификаторЗапросаЕГАИС + "
...
Функция ПечатьЧекаЕГАИС() Экспорт
	...
	Если КассаАвтономная() Тогда
		Возврат Истина;
	КонецЕсли;
	ЗаписьЖурналаРегистрации("Оплата.Печать чека ЕГАИС.До вызова", УровеньЖурналаРегистрации.Информация, , Заказ.Ссылка);
	Строки = Новый Массив;
	...
	Параметры.Вставить("Строки", Строки);
	ПараметрыВыходные = ПараметрыВыходные;
	ЗаписьЖурналаРегистрации("Оплата.Печать чека ЕГАИС.После вызова", УровеньЖурналаРегистрации.Информация, , Заказ.Ссылка);
	Попытка
