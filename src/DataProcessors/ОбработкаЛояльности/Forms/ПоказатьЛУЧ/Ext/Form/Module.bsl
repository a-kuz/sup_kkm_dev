...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для каждого ТекСтрока Из Параметры.ЛУЧ Цикл
		...
		КонецЕсли;
	КонецЦикла;
	Для ТекПроект = 1 По 3 Цикл
		СтрокиПоПроекту = ЛУЧ.НайтиСтроки(Новый Структура("Проект", ТекПроект));
		Для Ном = 1 По СтрокиПоПроекту.Количество() Цикл
			СтрокиПоПроекту[Ном - 1].Ном = Ном;
		КонецЦикла;
	КонецЦикла;
...
Функция СформироватьТабДокЛуч()
	...
	Для каждого ТекСтрока Из ЛУЧ Цикл
		...
		КонецЕсли;
		ОблМакета.Параметры.Сч = ТекСтрока.Ном;
