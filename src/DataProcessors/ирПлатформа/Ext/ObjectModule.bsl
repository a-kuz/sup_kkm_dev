...
Процедура ИнициализацияОписанияПараметровИТипов() Экспорт
	...
	Если ТипЗнч(ТаблицаОбщихТипов) = Тип("ТаблицаЗначений") Тогда
		Возврат;
	КонецЕсли;
	ИмяФайлаТаблицаИменЭлементовКоллекций = КаталогВременныхФайлов() + "ТаблицаИменЭлементовКоллекций.txt";
	...
	Если Файл.Существует() И Файл.Размер() Тогда
		Попытка
			стрТаблицаИменЭлементовКоллекций = РаботаСФайлами.ИзвлечьТекст(ИмяФайлаТаблицаИменЭлементовКоллекций, "utf8");
		Исключение
		КонецПопытки;
	КонецЕсли;
	...
	Исключение
		...
		Попытка
			МассивТаблиц = ЗначениеИзСтрокиВнутр(стрТаблицаИменЭлементовКоллекций);	
		Исключение
			ЗаписьЖурналаРегистрации("Ошибка выполнения",УровеньЖурналаРегистрации.Ошибка,,,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			Возврат;
		КонецПопытки;
	...
	ИмяФайлаТаблицаПараметровИТипов = КаталогВременныхФайлов() + "ТаблицаПараметровИТипов.txt";
	Файл = Новый Файл(ИмяФайлаТаблицаПараметровИТипов);
	стрТаблицаПараметровИТипов = "";
	Если Файл.Существует() И Файл.Размер() Тогда
		Попытка
			стрТаблицаПараметровИТипов = РаботаСФайлами.ИзвлечьТекст(ИмяФайлаТаблицаПараметровИТипов,"utf8");
		Исключение
		КонецПопытки;
	КонецЕсли;
	...
	Для Каждого СтрокаОбщегоТипа Из МассивОбщихТипов Цикл
		...
		Для Каждого СтрокаСлова Из НайденныеСтроки Цикл
			...
			Если ВычислятьТипы Тогда
				...
				КонецЕсли;
			КонецЕсли; 
			Тс = ВнутренняяТаблицаСлов;
			ТСТ = ТаблицаСтруктурТипов;
			ДобавитьВТаблицуСлов(Тс, СтрокаСлова.Слово, СтрокаСлова.ТипСлова, ТСТ, СтрокаСлова.ТипЗначения);
		КонецЦикла;
	КонецЦикла;
	Возврат ВнутренняяТаблицаСлов;
