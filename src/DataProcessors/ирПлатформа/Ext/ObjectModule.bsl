...
Процедура ИнициализацияОписанияПараметровИТипов() Экспорт
	...
	Если ТипЗнч(ТаблицаОбщихТипов) = Тип("ТаблицаЗначений") Тогда
		Возврат;
	КонецЕсли;
	ИмяФайлаТаблицаИменЭлементовКоллекций = КаталогВременныхФайлов() + "ТаблицаИменЭлементовКоллекций";
	Файл = Новый Файл(ИмяФайлаТаблицаИменЭлементовКоллекций);
	стрТаблицаИменЭлементовКоллекций = "";
	Если Файл.Существует() И Файл.Размер() Тогда
		Попытка
			стрТаблицаИменЭлементовКоллекций = РаботаСФайлами.ИзвлечьТекст(ИмяФайлаТаблицаИменЭлементовКоллекций,КодировкаТекста.UTF8);
		Исключение
		КонецПопытки;
	КонецЕсли;
	Если Не ЗначениеЗаполнено(стрТаблицаИменЭлементовКоллекций) Тогда
		стрТаблицаИменЭлементовКоллекций = ПолучитьМакет("ТаблицаИменЭлементовКоллекций").ПолучитьТекст();
		Если Файл.Существует() Тогда
			Попытка
				УдалитьФайлы(ИмяФайлаТаблицаИменЭлементовКоллекций);
			Исключение
			КонецПопытки;
		КонецЕсли;
		ЗТ = Новый ЗаписьТекста(ИмяФайлаТаблицаИменЭлементовКоллекций,,,Ложь);
		ЗТ.Записать(стрТаблицаИменЭлементовКоллекций);
		ЗТ.Закрыть();
	КонецЕсли;
	Попытка
		МассивТаблиц = ЗначениеИзСтрокиВнутр(стрТаблицаИменЭлементовКоллекций);	
	Исключение
		стрТаблицаИменЭлементовКоллекций = ПолучитьМакет("ТаблицаИменЭлементовКоллекций").ПолучитьТекст();	
		Попытка
			МассивТаблиц = ЗначениеИзСтрокиВнутр(стрТаблицаИменЭлементовКоллекций);	
		Исключение
			ЗарегистрироватьСобытие("Ошибка выполнения",УровеньЖурналаРегистрации.Ошибка,,,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			Возврат;
		КонецПопытки;
	...
	ИмяФайлаТаблицаПараметровИТипов = КаталогВременныхФайлов() + "ТаблицаПараметровИТипов";
	Файл = Новый Файл(ИмяФайлаТаблицаПараметровИТипов);
	стрТаблицаПараметровИТипов = "";
	Если Файл.Существует() И Файл.Размер() Тогда
		Попытка
			стрТаблицаПараметровИТипов = РаботаСФайлами.ИзвлечьТекст(ИмяФайлаТаблицаПараметровИТипов,КодировкаТекста.UTF8);
		Исключение
		КонецПопытки;
	КонецЕсли;
	Если Не ЗначениеЗаполнено(стрТаблицаПараметровИТипов) Тогда
		стрТаблицаПараметровИТипов = ПолучитьМакет("ТаблицаПараметровИТипов").ПолучитьТекст();
		Если Файл.Существует() Тогда
			Попытка
				УдалитьФайлы(ИмяФайлаТаблицаПараметровИТипов);
			Исключение
			КонецПопытки;
		КонецЕсли;
		ЗТ = Новый ЗаписьТекста(ИмяФайлаТаблицаПараметровИТипов,,,Ложь);
		ЗТ.Записать(стрТаблицаПараметровИТипов);
		ЗТ.Закрыть();
	КонецЕсли;
	Попытка
		МассивТаблиц = ЗначениеИзСтрокиВнутр(стрТаблицаПараметровИТипов);	
	Исключение
		стрТаблицаПараметровИТипов = ПолучитьМакет("ТаблицаПараметровИТипов").ПолучитьТекст();	
		Попытка
			МассивТаблиц = ЗначениеИзСтрокиВнутр(стрТаблицаПараметровИТипов);	
...
Процедура ОткрытьАлгоритмВОтладчике(АлгоритмОбъект, НомерСтрокиВМодуле = 0, ИдентификаторПроцесса = 0, Переоткрыть = Истина) Экспорт 
	...
	Если ФайлВнешнейОбработки <> Неопределено Тогда
		...
		КонецЕсли;
		WshShell_.Run(СтрокаЗапуска, 0, Ложь);
	КонецЕсли;
...
Процедура ОткрытьФайлВКонфигураторе(ПолноеИмя, ЭлементОбработки = "", ИдентификаторПроцесса = 0) Экспорт 
	...
	Если ФайлВнешнейОбработки.Существует() Тогда
		//WshShell_ = Новый COMОбъект("WScript.Shell");
	...
	Исключение
		...
		Если МаскироватьФайловыйКэш Тогда
			ОписаниеОшибки = ОписаниеОшибки + ": " + ИнформацияОбОшибке.Описание
				+ Символы.ПС + ИнформацияОбОшибке.ИсходнаяСтрока;
		КонецЕсли; 
		ЗарегистрироватьСобытие(СобытиеОшибки, УровеньЖурналаРегистрации.Ошибка, Метаданные.НайтиПоТипу(ТипЗнч(АлгоритмОбъект.Ссылка)),
...
Функция УпаковатьВнешнююОбработку(ПутьКШаблонуВнешнейОбработки, ИмяВыходногоФайла, СоздатьФайлыУпаковщика = Ложь) Экспорт
	...
	Исключение
		...
		//ЗарегистрироватьСобытие(Событие, УровеньЖурналаРегистрации.Ошибка);
		//Возврат Неопределено;
			...
			Если ФайлВнешнейОбработки <> Неопределено Тогда
				...
				Если ВнешняяОбработка <> Неопределено Тогда
					...
					Если ИмяАлгоритмаВнешнейОбработки <> ДескрипторСервиса.Наименование Тогда
						ЗарегистрироватьСобытие("Несоответствие внешней обработки и сервиса", УровеньЖурналаРегистрации.Ошибка, ,
...
Процедура ЗапуститьСкрытоеПриложениеИДождатьсяЗавершения(Знач СтрокаЗапуска, Знач ТекущийКаталог = "", Знач ИспользоватьWSH = Истина,
	...
	Если ИспользоватьWSH Тогда
		...
		СтарыйТекущийКаталог = WshShell_.CurrentDirectory;
		WshShell_.CurrentDirectory = ТекущийКаталог;
		Попытка
			WshShell_.Run(ИмяКомандногоФайла, 0, ОжидатьЗавершения);
		Исключение
			// Для x64 ОС
			СтрокаЗапуска = "%windir%\Sysnative\" + СтрокаЗапуска;
			WshShell_.Run(СтрокаЗапуска, 0, ОжидатьЗавершения);
		КонецПопытки;
		WshShell_.CurrentDirectory = СтарыйТекущийКаталог;
	Иначе
