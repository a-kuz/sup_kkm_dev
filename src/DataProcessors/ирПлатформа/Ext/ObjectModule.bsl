...
Процедура ИнициализацияОписанияПараметровИТипов() Экспорт
	...
	Если Файл.Существует() И Файл.Размер() Тогда
		Попытка
			стрТаблицаИменЭлементовКоллекций = РаботаСФайлами.ИзвлечьТекст(ИмяФайлаТаблицаИменЭлементовКоллекций);
		Исключение
		КонецПопытки;
	КонецЕсли;
	...
	Если Файл.Существует() И Файл.Размер() Тогда
		Попытка
			стрТаблицаПараметровИТипов = РаботаСФайлами.ИзвлечьТекст(ИмяФайлаТаблицаПараметровИТипов);
		Исключение
		КонецПопытки;
	КонецЕсли;
