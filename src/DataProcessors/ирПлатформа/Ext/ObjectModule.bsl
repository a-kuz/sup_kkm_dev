...
//}}MRG[ <-> ]
//{{MRG[ <-> ]
//		_WshShell.Run(СтрокаЗапуска, 0, Ложь);
//}}MRG[ <-> ]
	КонецЕсли;
...
//}}MRG[ <-> ]
//{{MRG[ <-> ]
//		//_WshShell = Новый COMОбъект("WScript.Shell");
//}}MRG[ <-> ]
...
//}}MRG[ <-> ]
//{{MRG[ <-> ]
//	ВК.Выполнить(ИмяКомандногоФайла, "", ТекущийКаталог, ОжидатьЗавершения, Элевация);
//}}MRG[ <-> ]
