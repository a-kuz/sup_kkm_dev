...
Процедура ПриОткрытии()
	...
	Если глПараметрыРМ.ЭтоОтдохни или глПараметрыРМ.ТипТТ = Справочники.ТипыТТ.КМ Тогда
...
Процедура тпТоварыПриАктивизацииСтроки(Элемент)
	...
	Если ТекИдСтрокиТовара <> ТекИдСтрокиТовара() Тогда
		...
		БылаСтрока = тпТовары.Строки.Найти(ТекИдСтрокиТовара);
		МассивСтрок = Новый Массив;
		Если БылаСтрока <> Неопределено Тогда
			МассивСтрок = Массив(БылаСтрока);
			ДополнитьМассив(МассивСтрок, БылаСтрока.Строки);
		КонецЕсли;		
		ТекИдСтрокиТовара = ТекИдСтрокиТовара();
		ТеперьСтрока = тпТовары.Строки.Найти(ТекИдСтрокиТовара);
		МассивСтрок.Добавить(ТеперьСтрока);
		ДополнитьМассив(МассивСтрок,ТеперьСтрока.Строки);
		ЭлементыФормы.тпТовары.ОбновитьСтроки(МассивСтрок); // это нужно, чтобы при смене текущей строки товара снять выделение с предыдущей строки товара и подчиненных строк
		ПоказатьЗаказНаМонитореГостя(0.1,"Форма9.тпТоварыПриАктивизацииСтроки()");//,0.1);
		
	Иначе
		
		Если ЭлементыФормы.тпТовары.ТекущиеДанные.Уровень() Тогда
			МассивСтрок = Массив(ЭлементыФормы.тпТовары.ТекущиеДанные.Родитель);
			ДополнитьМассив(МассивСтрок, ЭлементыФормы.тпТовары.ТекущиеДанные.Родитель.Строки);
			
			
		Иначе
			МассивСтрок = Массив(ЭлементыФормы.тпТовары.ТекущиеДанные.Родитель);
			ДополнитьМассив(МассивСтрок, ЭлементыФормы.тпТовары.ТекущиеДанные.Строки);
			
		КонецЕсли;
		ЭлементыФормы.тпТовары.ОбновитьСтроки(МассивСтрок);
	КонецЕсли;
...
Процедура УстановитьТекСтроку(Задержка = 0) Экспорт
	...
	Попытка
		ТекСтрока = тпТовары.Строки.Найти(ТекИдСтрокиТовара);
		Если НЕ ТекСтрока = Неопределено И НЕ ЭлементыФормы.тпТовары.ТекущаяСтрока = ТекСтрока Тогда
...
Процедура ЗаполнитьТПтовары_Ожидание() Экспорт
	...
	Если ТекущийДокумент = Неопределено Тогда
		...
		Возврат;
	ИначеЕсли тпТовары.Строки.Количество() > ТекущийДокумент.Товары.Количество() Тогда
		тпТовары.Строки.Очистить();
	КонецЕсли;
	
	Если ТекущийРежим = "ВОЗВРАТ" Тогда
		ГлавнаяФорма.ОтключитьОбработчикОжидания("ЗаполнитьТПтовары_Ожидание");
		ЗаполнитьТПтовары_Возврат();	
		Возврат;
	КонецЕсли;
	
	НаборЗаписей = РегистрыСведений.ЗаказТоварыДопИнф.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.Заказ.Установить(Заказ.Ссылка);
	НаборЗаписей.Прочитать();
	тзНаборЗаписей = НаборЗаписей.Выгрузить(); //:тзНаборЗаписей = Новый ТаблицаЗначений
	
	Для каждого СтрокаДокумента из ТекущийДокумент.Товары Цикл
		
		СтрокаДопИнф = тзНаборЗаписей.Найти(СтрокаДокумента.ИдСтроки, "ИдСтроки");
		фСтрокаНеНужна = Ложь;
		Если ОтборФирма И глПараметрыРМ.ФирмыРМ.Найти(СтрокаДокумента.Фирма) = Неопределено Тогда
			фСтрокаНеНужна = Истина;

		КонецЕсли;
		
		СтрокаТпТовары = тпТовары.Строки.Найти(СтрокаДокумента.ИдСтроки, "ИдСтроки", Ложь);
		фСтрокаЕсть = СтрокаТпТовары <> Неопределено;
		Если фСтрокаНеНужна И фСтрокаЕсть Тогда
			// удалить строку
			тпТовары.Строки.Удалить(СтрокаТпТовары);
			Продолжить;
		ИначеЕсли НЕ фСтрокаНеНужна И Не фСтрокаЕсть Тогда
			// добавить строку
			СтрокаТпТовары = тпТовары.Строки.Добавить();
		ИначеЕсли фСтрокаНеНужна Тогда
			Продолжить;
		КонецЕсли;
		
		ЗаполнитьЗначенияСвойств(СтрокаТпТовары,СтрокаДокумента,,"Статус");
		Если СтрокаДопИнф <> Неопределено Тогда
			ЗаполнитьЗначенияСвойств(СтрокаТпТовары,СтрокаДопИнф);
		КонецЕсли;
		
		МС = ТекущийДокумент.Специфики.НайтиСтроки(Новый Структура("НомерСтрокиТовара",СтрокаДокумента.НомерСтроки));
		Для Каждого СтрокаСпецифика ИЗ МС Цикл 
			//:СтрокаСпецифика = Заказ.Специфики[0]
			НовСтрокаСпецифика =	СтрокаТпТовары.Строки.Найти(СтрокаСпецифика.ИдСтроки, "ИдСтроки");
			Если НовСтрокаСпецифика = Неопределено Тогда
				НовСтрокаСпецифика = СтрокаТпТовары.Строки.Добавить();
			КонецЕсли;                                               			
		...
		УдаляемыеСпецифики = Новый Массив;
		Для Каждого СтрокаСпецификаТпТовары Из СтрокаТпТовары.Строки Цикл
			СтрокаСпецифика = ТекущийДокумент.Специфики.Найти(СтрокаСпецификаТпТовары.ИдСтроки, "ИдСтроки");
			Если СтрокаСпецифика = Неопределено Тогда
				СтрокаСпецифика = тзНаборЗаписей.Найти(СтрокаСпецификаТпТовары.ИдСтроки, "ИдСтроки");
				Если СтрокаСпецифика = Неопределено Тогда
					УдаляемыеСпецифики.Добавить(СтрокаСпецификаТпТовары);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		
		Для Каждого УдаляемаяСтрокаСпецифики Из УдаляемыеСпецифики Цикл
			СтрокаТпТовары.Строки.Удалить(УдаляемаяСтрокаСпецифики);
		КонецЦикла;
...
Процедура ЗаполнитьТПтовары_Ожидание_() Экспорт 
	...
	Если фУстанавливатьТекСтроку = Истина Тогда
		ТекИдСтрокиТовара = ТекИдСтрокиТовара();
	КонецЕсли;
	...
	Для каждого Стр из ТекущийДокумент.Товары Цикл
		...
		//////////КонецЕсли;
	КонецЦикла;
	Если ТекИдСтрокиТовара = Неопределено И тпТовары.Строки.Количество() Тогда
		ТекИдСтрокиТовара = НовСтрока.ИдСтроки;
	КонецЕсли;
