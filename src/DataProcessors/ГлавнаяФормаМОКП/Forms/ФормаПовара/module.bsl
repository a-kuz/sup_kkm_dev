...
Процедура ЗаполнитьТПтовары() Экспорт
	...
	Для каждого стр из ТекущийДокумент.Товары Цикл
		...
		Если ОтборСтрок = 2 Тогда // Станция
			Если стр.РабочееМесто.Станция <> глПараметрыРМ.Станция Тогда
				Продолжить;				
			КонецЕсли;
...
Процедура ЗаблокироватьНажатие(Элемент)
	...
	Если ТекущийРежим = "ПРОДАЖА" Тогда
		...
		Если Не ЗначениеЗаполнено(глПользователь) И ТекущийРежим = "ПРОДАЖА" Тогда
			Авторизация();
		КонецЕсли;
	Иначе
...
Процедура тпТоварыПриАктивизацииСтроки(Элемент)
	...
	КонецЕсли;
	Если ТекСтрокаТП <> ЭлементыФормы.тпТовары.ТекущаяСтрока Тогда
