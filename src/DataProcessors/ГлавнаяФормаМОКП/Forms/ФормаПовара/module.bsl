...
Процедура ОбновитьВремя() Экспорт
	...
	ИначеЕсли Не глФлагБлокировка Тогда
		...
		?(Не ОтборФирма, "Весь заказ", "По станции");
	КонецЕсли;
...
Процедура ЗаблокироватьВформе() Экспорт
	...
	ПодключитьОбработчикОжидания("ПоказатьМенюПовара",1,1);
КонецПроцедуры

Процедура ПоказатьМенюПовара() Экспорт
	Если ВысотаТаблицыЗаказа = Неопределено Тогда
		ВысотаТаблицыЗаказа	= ЭлементыФормы.тпТовары.Высота;
		ВысотаПодвала		= Высота - (ЭлементыФормы.тпТовары.Верх + ВысотаТаблицыЗаказа+ЭлементыФормы.КнопкаКоличество.Высота);
	КонецЕсли;
