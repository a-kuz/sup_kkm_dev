...
Процедура ПриОткрытии()
	...
	ИнтервалОбновленияИнтерфейса = 0;
	ИнтервалОбновленияИнтерфейса_ВводНовойСтроки = 0;
...
Процедура тпТоварыПриАктивизацииСтроки(Элемент)
	...
	Если ТекИдСтрокиТовара <> ТекИдСтрокиТовара() Тогда
		Попытка
			...
			Если Элемент.ТекущаяСтрока.Уровень() Тогда
				НовСтрокаТовар = Элемент.ТекущаяСтрока.Родитель;
			Иначе
				НовСтрокаТовар = Элемент.ТекущаяСтрока;
			КонецЕсли;
			ДополнитьМассив(МассивСтрок, НовСтрокаТовар.Строки);
			МассивСтрок.Добавить(НовСтрокаТовар);
...
Процедура ОбновитьНадписьИтого() Экспорт
	...
	Иначе
		...
		Попытка
			//ТекИдСтрокиТовара = ТекИдСтрокиТовара();
			УстановитьТекСтроку();
//реф			ПодключитьОбработчикОжидания("УстановитьТекСтроку", 0.1, 1);
		Исключение
		КонецПопытки;
...
Процедура ЗаполнитьТПтовары() Экспорт
	...
	Для каждого СтрокаДокумента из ТекущийДокумент.Товары Цикл
		...
		Если ОтборСтрок = 1 Тогда // Место реализации
			Если СтрокаДокумента.Фирма.МестоРеализации <> глПараметрыРМ.МестоРеализации Тогда
		...
		Если ОтборСтрок = 2 Тогда // Станция
			...
			Иначе
				Станция = СтрокаДопИнф.Станция;
			КонецЕсли;
			
			Если Станция <> глПараметрыРМ.Станция И ЗначениеЗаполнено(Станция) Тогда
				фСтрокаНеНужна = Истина;	
			КонецЕсли;
		КонецЕсли;
