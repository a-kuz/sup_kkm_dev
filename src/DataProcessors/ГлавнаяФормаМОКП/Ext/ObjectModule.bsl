...
Функция ПоказатьЗаказНаМонитореГостя(Задержка = 0.5, ВызывающийКонтекст = "") Экспорт
	ЗарегистрироватьСобытие("ПоказатьЗаказНаМонитореГостя."+ВызывающийКонтекст);
...
Функция ВводНовойСтроки(ВыбТов, Количество = 0, ШК = "", НельзяОтказатьсяОтВводаПозиции = Ложь, ИдСвязаннойСтроки = "", ОбязательнаяСпецифика = Неопределено, МинКоличествоСпецифики = 0) Экспорт
	...
	ЗаписатьДокумент(Перечисления.СтатусыЗаказа.Открыт,,,Истина);
	ЗаполнитьНадписиЛояльности();
	ПоказатьЗаказНаМонитореГостя(,"ВводНовойСтроки()");
	Попытка
		ГлавнаяФорма.ЭлементыФормы.тптовары.ТекущаяСтрока = ГлавнаяФорма.тптовары.Строки.найти(Нов.ИдСтроки, "ИдСтроки");	
	Исключение
	КонецПопытки;     
