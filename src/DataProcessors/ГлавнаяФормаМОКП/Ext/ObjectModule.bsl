...
Процедура ОткрытьЗаказ(ЗаказСсылка) Экспорт
	...
	Если ксТрактир.ЭтоРМСтанция() Тогда
		ТоварыДляПриготовления = ОбщегоНазначенияПовтИсп.СформироватьТаблицуТоваровДляПриготовления(Константы.ДатаОбновленияПризнакаНаличияВПродаже.Получить());
		Если БылОтборСтрок <> ГлавнаяФорма.ОтборСтрок Тогда
			ГлавнаяФорма.ОтборСтрок = БылОтборСтрок;
...
Функция ВводНовойСтроки(ВыбТов, Количество = 0, ШК = "", НельзяОтказатьсяОтВводаПозиции = Ложь, ИдСвязаннойСтроки = "") Экспорт
	...
	Если НЕ ПроверитьДобавитьСпецифику(Нов,Неопределено,Истина,НельзяОтказатьсяОтВводаПозиции) Тогда
...
Функция ПроверитьДобавитьСпецифику(СтрокаДокЗаказ, ВыбСпецифика=Неопределено, ОбязательноОткрыватьФорму=Ложь,АвтоПозиция=Ложь)
		...
		РезультатВыбора = ОбработкаВыборСпецифик.ВыполнитьВыборСпецифик(СтрокаТпТовары,Ложь,АвтоПозиция);
	...
	Для Каждого Т Из ДанныеСтрок.Строки Цикл
		Если Т.Статус = Перечисления.СтатусыПозицийЗаказа.ДляПриготовления И не ЗначениеЗаполнено(Т.ИдСвязаннойСтроки) Тогда
			...
			Если ИнтерфейсРМ.ВопросПредупреждение("" + Услуга, "Вопрос гостю", ТекстВопроса,  "Оформить", , "Отказаться") = "Оформить" Тогда
				идСтроки = ВводНовойСтроки(Услуга, 1,,Истина,Строкат.идСтроки);
				СтрокаТ = ГлавнаяФорма.тпТовары.Строки.Найти(Т.идСтроки);
