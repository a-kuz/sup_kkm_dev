...
Процедура ИзменитьКоличество() Экспорт
		...
		СтрокаЗаказа = Заказ.Товары.Найти(ГлавнаяФорма.ЭлементыФормы.тпТовары.ТекущаяСтрока.ИдСтроки);		
...
Процедура ЗаписатьЗаказ(Статус = "Открыт", Оплачен = Ложь) Экспорт
	...
	НЗ.Записать(Истина);	
	Заказ.ЗаписатьЗаказТоварыДопИнф();
	//Лояльность.ОбновитьПредварительныйРасчетЗаказа(Заказ.Ссылка, глПараметрыРМ.Тест);
КонецПроцедуры
...
Функция ПроверитьДобавитьСпецифику(СтрокаТовара, ВыбСпецифика=Неопределено, ОбязательноОткрыватьФорму=Ложь,АвтоПозиция=Ложь)
		...
		РезультатВыбора = ОбработкаВыборСпецифик.ВыполнитьВыборСпецифик(ГлавнаяФорма.тпТовары.Строки.Найти(СтрокаТовара.НомерСТроки, "НомерСтроки"));
		...
		Если РезультатВыбора.Контроль Тогда
			...
			Если ГлавнаяФорма <> Неопределено Тогда
				...
				ТабПолеТЗ = ГлавнаяФорма.ЭлементыФормы.тпТовары;
...
Процедура СменаМенюПовара() Экспорт
	...
	ФормаПодбораТоваров.ОбрезкаОкнаШирина = ГлавнаяФорма.ЭлементыФормы.тпТовары.Ширина+10;
	ФормаПодбораТоваров.Открыть();
	// видоизменение формы заказа в зависимости от настройки меню
	глПараметрыРМ.Вставить("Окно1СИзменено", Ложь);
	ЭлементФормыТаблицаЗаказа		= ГлавнаяФорма.ЭлементыФормы.тпТовары;
...
Функция ОбновитьСпецификиПоСтрокеТовара(СтрокаЗаказа, ДеревоСпецифик)
	
	СтрокаДокЗаказа = Заказ.Товары.Найти(СтрокаЗаказа.ИдСтроки, "ИдСтроки");
	МсСпецифики = Заказ.Специфики.НайтиСтроки(Новый Структура("НомерСтрокиТовара", СтрокаЗаказа.НомерСтроки));
	Для Каждого Т Из МсСпецифики Цикл
	...
	Для Каждого СтрокаСпецифика из ДеревоСпецифик.Строки Цикл
		...
		НовСтрокаСпецифика.НомерСтрокиТовара = СтрокаЗаказа.НомерСтроки;
...
Функция ДобавитьНовыеСпецифики(СтрокаЗаказа, ДеревоНовыхСпецифик)
	...
	Если ТипЗнч(ДеревоНовыхСпецифик) = Тип("Массив") Тогда
		Для Каждого Специфика из ДеревоНовыхСпецифик Цикл
			Если СтрокаЗаказа.Строки.Найти(Специфика) = Неопределено Тогда
				Нов = Заказ.Специфики.Добавить();
				Нов.Специфика = Специфика;
				Нов.НомерСтрокиТовара = СтрокаЗаказа.НомерСтроки;
