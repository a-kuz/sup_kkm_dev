...
Процедура ПроверитьБлюдаДляПриготовления() Экспорт
	...
	Для Каждого Т Из ГлавнаяФорма.тпТовары.Строки Цикл
		Если Т.Статус = Перечисления.СтатусыПозицийЗаказа.ДляПриготовления Тогда
			...
			ТекстВопроса = СтрШаблон("У гостя есть товар для приготовления: %1 x%2", Т.Товар, ""+Т.Количество + " " + Т.Товар.ЕдиницаИзмерения);	
			Если ИнтерфейсРМ.ВопросПредупреждение("Вопрос", "Приготовление МЪ", ТекстВопроса,  "Оформить",ТекстВопроса, "Отказаться") = "Оформить" Тогда
