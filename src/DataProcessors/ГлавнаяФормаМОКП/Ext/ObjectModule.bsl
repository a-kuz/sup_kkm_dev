...
Процедура ИзменитьКоличество() Экспорт
		...
		СтрокаЗаказа = ГлавнаяФорма.ЭлементыФормы.тпТовары.ТекущиеДанные;		
		...
		Если ЗначениеЗаполнено(СтрокаЗаказа.НомерМарки) Тогда
			Возврат;
		КонецЕсли;
		
		Если СтрокаЗаказа.Статус <> СтатусДозаказ И СтрокаЗаказа.Статус <> СтатусЗаказано Тогда
			Возврат;
		КонецЕсли;
		...
		ИдСтроки = СтрокаЗаказа.ИдСтроки;
		...
		Если СтрокаЗаказа.Статус = СтатусДозаказ Или СтрокаЗаказа.Статус = СтатусЗаказано Тогда
			...
			Если Колво=0 Тогда
				...
				Сторно(СтрокаЗаказа.НомерСтроки);
