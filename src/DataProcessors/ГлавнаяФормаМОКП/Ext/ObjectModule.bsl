...
Функция ВыполнитьПроверкуПоСтанции(СтрокаЗаказа, ВыводПредупреждения=Истина) Экспорт
	...
	Если ксТрактир.ЭтоРМСтанция() Тогда
		...
		Результат = СтрокаЗаказа.Станция = глПараметрыРМ.Станция;
		Текст1="Нет доступа!";
		...
		Если ОбработкаОплаты.ПротоколРасчетов.Протокол.Итог("СуммаФакт") = 0 Тогда
			...
			Если фВводТоваровДоступен Тогда 
				...
				Попытка
					...
					// передаем истину в оплачен специально, чтобы не перетирались данные лояльности
					// если выбрано списание, то при записи все пересчитывается на начисление
					// внутри замой процедцры параметр Оплачен больше нигде не используется
					ЗаписатьЗаказ(Перечисления.СтатусыЗаказа.Открыт);
...
Функция Сторно(НомерСтроки = 0) Экспорт 
	...
	Если КоличествоСтрок Тогда
		...
		МЗ.КоличествоУдалено = МЗ.КоличествоУдалено;
		МЗ.Записать();
