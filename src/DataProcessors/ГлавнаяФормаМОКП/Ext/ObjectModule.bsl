...
Процедура ОткрытьЗаказ(ЗаказСсылка) Экспорт
	...
	КонецЕсли;
	МассивПейджеров = ксТрактир.ВыданныеПейджеры(ЗаказСсылка, ?(ксТрактир.ЭтоРМСтанция(), глПараметрыРМ.Станция, Неопределено));
	...
	Если ксТрактир.ЭтоРМСтанция() и ЗначениеЗаполнено(глПользователь) Тогда
		Если ФормаПодбораТоваров = Неопределено Тогда
			//СменаМенюПовара();
		Иначе
			Если Не ФормаПодбораТоваров.Открыта() Тогда
...
Функция ИзменитьКоличество(пКолво = Неопределено) Экспорт
	...
	Если ксТрактир.ЭтоРМСтанция() И СтрокаТабПоля.Станция <> глПараметрыРМ.Станция Тогда
		Возврат ложь;
	КонецЕсли;	
...
функция ОбновитьСтатусыСтрок(НеОткрыватьМенюПовара = Ложь) Экспорт
	...
	Для Каждого Т Из ТЗ Цикл
		...
		КонецЕсли;
	КонецЦикла;
	Если ксТрактир.ЭтоРМСтанция() И Не НеОткрыватьМенюПовара Тогда
		Если ФормаПодбораТоваров = Неопределено Тогда
			//СменаМенюПовара();
		Иначе
			Если Не ФормаПодбораТоваров.Открыта() Тогда
