...
Процедура ДействияПередЗакрытиемФормы(ТекущаяФорма, Отказ) Экспорт
	...
	Если КолвоОкон > 1 Тогда
		Для н=1 По КолвоОкон Цикл
			//глСтекОкон[0].Закрыть();
			Попытка
				глСтекОкон[н-1].Форма.Закрыть();
			Исключение
			КонецПопытки;
		КонецЦикла;	
...
Процедура ИзменитьКоличество() Экспорт
		...
		Если СтрокаТабПоля.Статус <> СтатусДозаказ И СтрокаТабПоля.Статус <> СтатусЗаказано Тогда
			Возврат;
		КонецЕсли;
		...
		Если СтрокаТабПоля.Статус = СтатусДозаказ Или СтрокаТабПоля.Статус = СтатусЗаказано Тогда
		...
		Иначе
			...
			Иначе
				Если СтрокаТабПоля.НомерМарки и ксТрактир.ЭтоРМСтанция() Тогда
					Текст1="Нет доступа!";
...
Процедура ЗаписатьЗаказ(Статус =	 "Открыт", Оплачен = Ложь) Экспорт
	...
	//Для Каждого Т Из ГлавнаяФорма.тпТовары Цикл
	//	
	//	Если Т.Модифицирована Тогда
	//		Ст = Заказ.Товары.Найти(Т.идСтроки, "идСтроки");
	//		Если Ст = Неопределено Тогда
	//			Ст = Заказ.Товары.Добавить();
	//		КонецЕсли;	
	//		ЗаполнитьЗначенияСвойств(Модифицирована);
	//		
	//	КонецЕсли;
	//КонецЦикла;
	...
	Для Каждого Т Из Заказ.Товары Цикл
		...
		//	
		//	Если не ксТрактир.ЭтоРМСтанция() И Т.РабочееМесто = глРабочееМесто Тогда
		//		Т.Статус = СтатусВыдано;			
		//	ИначеЕсли ксТрактир.ЭтоРМСтанция() И Т.РабочееМесто = глРабочееМесто Тогда
		//		Если Т.Статус = СтатусДозаказ Тогда
		//			Т.Статус = СтатусЗаказано					
		//		КонецЕсли;
		//	КонецЕсли;		
		//Иначе
		//	Т.Статус = СтатусУдалено;
		//КонецЕсли;
...
Процедура ОбновитьСтатусыСтрок() Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ
...
Функция ЗаписатьЗаказТоварыДопИнф(СоздатьМарки = Ложь) Экспорт
	...
	Для Каждого Т Из ГлавнаяФорма.тпТовары.Строки Цикл
		...
		Если НЗ.Количество() = 0 Тогда
			...
			ЗаписьРегистра.ВремяЗаказано = ТекущаяДатаНаСервере();
			ЗаписьРегистра.Станция = глРабочееМесто.Станция;
			ЗаписьРегистра.АвторЗаказано = глПользователь;
			ЗаписьРегистра.Статус = ?(ксТрактир.ЭтоРМСтанция(), СтатусЗаказано, СтатусВыдано);
			Записать = Истина;
		...
		Если СоздатьМарки 	
			...
			и (ЗаписьРегистра.Статус = Перечисления.СтатусыПозицийЗаказа.Заказано или ЗаписьРегистра.Статус = СтатусДозаказ)
			и ксТрактир.ЭтоРМСтанция() Тогда
...
Функция ВыполнитьРегистрациюМарок() Экспорт 
	...
	КонецЕсли; 
		...
		СледующийНомерПодачиНач = 1; //TODO
		...
		Для каждого СтрокаОчередность Из ТаблицаОчередность Цикл
			...
			ТекущаяПодача = НайтиДобавитьПодачу(Курс, СтатусЗаказано);
...
Функция ВыполнитьВыборСпецификВФорме(ФормаЗаказа, ФлагОбщая, СтрокаЗаказа)
	...
	Если ЗначениеЗаполнено(НайтиСтатусТовара) Тогда
		...
		Если (Статус <> СтатусЗаказано и Статус <> СтатусДозаказ) Или ЗначениеЗаполнено(СтрокаТовара.НомерМарки) Тогда
...
Функция ОбновитьСпецификиПоСтрокеТовара(Знач СтрокаДокЗаказ, ДеревоСпецифик, Перечитывать = Истина)
	...
	МсСпецифики = Заказ.Специфики.НайтиСтроки(Новый Структура("НомерСтрокиТовара", СтрокаДокЗаказа.НомерСтроки));
	Для Каждого Т Из МсСпецифики Цикл
	...
	Для Каждого СтрокаСпецифика из ДеревоСпецифик.Строки Цикл
		...
		НовСтрокаСпецифика.НомерСтрокиТовара = СтрокаДокЗаказа.НомерСтроки;
	...
	КонецЦикла;
	РасчетПоСтроке(СтрокаДокЗаказа);
	ЗаписатьЗаказ();
	ПоказатьЗаказНаМонитореГостя();
	ГлавнаяФорма.ОбновитьНадписьИтого();
	Возврат Истина;
...
Процедура ОформлениеСтрокиЗаказа(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	Если ДанныеСтроки.Статус = СтатусДозаказ ИЛИ ДанныеСтроки.Статус = СтатусЗаказано Тогда
		...
		ПоказатьПодачу = глВерсия>1 И ИспользоватьЗаказТоварыДопИнф И ИспользоватьКурсы И ДанныеСтроки.Уровень()=0;
