...
Функция ОсвободитьКартуДоступа(Интерактивно = Истина) Экспорт
	...
	Если Интерактивно Тогда
		...
		Если Ответ = "НЕТ" Тогда
			...
			Возврат Ложь;
		Иначе
			Если ксТрактир.ПоЗаказуЕстьМарки(Массив(Заказ.Ссылка)) Тогда
				ИнтерфейсРМ.ВопросПредупреждение("Ошибка","Есть невыданные блюда","Сначала нужно отметить выдачу блюд", "","ОК", "");	
				СброситьТекущийДокумент(Ложь);
				Заблокировать(Истина);
