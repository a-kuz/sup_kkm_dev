...
Перем ТекСтрокаТП Экспорт;
Перем ТекИдСтрокиТовара Экспорт;
Перем фНеУстанавливатьТекСтроку Экспорт;
...
Функция ТекИдСтрокиТовара() Экспорт
	...
	Попытка
		...
		Если ЭлементыФормы.тпТовары.ТекущаяСтрока = Неопределено Тогда
			//ПС = ГлавнаяФорма.тпТовары.Строки.Количество() - 1;
			//Если ПС >= 0 Тогда
...
//реф	ГлавнаяФорма.ОбновитьНадписьИтого();
	...
	Если ЕстьМарки Тогда
		...
		Если БлюдоВыдали = "Да" Тогда
			Для Каждого СтрокаТовара Из ТаблицаЗаказа.Строки Цикл
				Если СтрокаТовара.НомерМарки ИЛИ СтрокаТовара.Статус = СтатусГотово ИЛИ СтрокаТовара.Статус = СтатусЗаказано Тогда
...
Процедура Оплата() Экспорт
	Попытка
		...
		Если Не ПроверкаМарок(Отказ) Тогда
			Если Отказ Тогда
				Возврат;
			Иначе
				ПроверкаПейджера();
...
//реф	ГлавнаяФорма.ЗаполнитьТПтовары(0.2);
	ПоказатьЗаказНаМонитореГостя(0.5,"ВводНовойСтроки()");
//реф	Попытка
//реф		ГлавнаяФорма.ЭлементыФормы.тптовары.ТекущаяСтрока = ГлавнаяФорма.тптовары.Строки.найти(Нов.ИдСтроки, "ИдСтроки");	
//реф	Исключение
//реф	КонецПопытки;
...
//реф			Попытка
//реф				ГлавнаяФорма.ЭлементыФормы.тптовары.ТекущаяСтрока = ГлавнаяФорма.тптовары.Строки[ ГлавнаяФорма.тптовары.Строки.Количество()-1];
//реф			Исключение
//реф			КонецПопытки;
			ОбновитьСтатусыСтрок();
			...
			ИначеЕсли Ответ = "Номерок" Тогда
				...
				Если ЗначениеЗаполнено(РезультатВвода) Тогда
					...
					Если ТекПейджер.Пустая() Тогда
						...
						ИспользоватьНомерок = Неопределено;
					Иначе
						ЗарегистрироватьСобытие("Выбран номерок №" + РезультатВвода);
					КонецЕсли;
				Иначе
					ИспользоватьНомерок = Неопределено;
				КонецЕсли;
...
Функция ЕстьСтрокиДляПриготовления() Экспорт
	Если ТекущийРежим = "ПРОДАЖА" Тогда
		Возврат Заказ.Товары.НайтиСтроки(Новый Структура("Статус", Перечисления.СтатусыПозицийЗаказа.ДляПриготовления)).Количество();
	Иначе
		Возврат Ложь;
	КонецЕсли;
КонецФункции
