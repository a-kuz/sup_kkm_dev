...
Перем ЦветДозаказ;
Перем ЦветЗаказано;						// Цвета выделения ячеек в колонке "Статус" (А")
Перем ЦветГотово;
Перем ЦветВыдано;
Перем ЦветДругое;
...
Перем Показатель_ДобавлениеСтроки Экспорт;
Перем Показатель_ПечатьЧеков Экспорт;
Перем Показатель_УТМ Экспорт;
Перем Показатель_ОбновлениеМонитораГостя Экспорт;
Перем Показатель_ОбсчетЛОЛ Экспорт;

Перем ТекИдСтрокиТовара Экспорт;
Перем фУстанавливатьТекСтроку Экспорт;
Перем ТоварыДляПриготовления Экспорт;
Перем ЭтоРМСтанция Экспорт;

Перем Обработка_ПДФ Экспорт ;
Перем ФормаПДФ Экспорт ;

Перем ИнтервалОбновленияИнтерфейса Экспорт; //
Перем ИнтервалОбновленияИнтерфейса_ВводНовойСтроки Экспорт; //устанавливается в форме

////////////////////////////////////////////////////////////////////////////////
Функция ТекИдСтрокиТовара() Экспорт
	
	Попытка
		
		Если ЭлементыФормы.тпТовары.ТекущаяСтрока = Неопределено Тогда
			//Если ПС >= 0 Тогда
			//	Возврат ГлавнаяФорма.тпТовары.Строки[ПС].идСтроки;
			//КонецЕсли;
			//
			//Возврат Неопределено;
		ИначеЕсли ЭлементыФормы.тпТовары.ТекущаяСтрока.Уровень() > 0 Тогда
			Возврат ГлавнаяФорма.ЭлементыФормы.тпТовары.ТекущаяСтрока.Родитель.идСтроки;
		Иначе
			Возврат ГлавнаяФорма.ЭлементыФормы.тпТовары.ТекущаяСтрока.идСтроки;
		КонецЕсли;
	Исключение
	КонецПопытки;
	
	Возврат ТекИдСтрокиТовара;
...
Функция ИнициализацияРабочегоМеста(ТекущееРМ) Экспорт
	...
	ТоварыДляПриготовления = ОбщегоНазначенияПовтИсп.СформироватьТаблицуТоваровДляПриготовления(Константы.ДатаОбновленияПризнакаНаличияВПродаже.Получить());
...
Процедура ДействияПередЗакрытиемФормы(ТекущаяФорма, Отказ) Экспорт
	...
	Если КолвоОкон > 1 Тогда
		Для н=1 По КолвоОкон-1 Цикл
			...
			Попытка
				Если глСтекОкон[0].Форма.Открыта() Тогда
	...
	Если ПроцессВыходаЗапущен Тогда
		...
		КонецПопытки;	
		Попытка
			Для Каждого Фирма Из глПараметрыРМ.ФирмыИБ Цикл
				Заказ.РазблокироватьРедактированиеПоФирме(Фирма, УИДформы);
			КонецЦикла;
...
Процедура ОбработкаВнешнегоСобытия(Источник, Событие, Данные) Экспорт
	...
	Если найти(Источник,"Штрих АС POS") <> 0 Тогда //"Штрих АС POS" Тогда
		...
		ГлавнаяФорма.ПодключитьОбработчикОжидания("ОбновитьВес", 0.1, Истина);
		Возврат;
	КонецЕсли;
	
	Если глСтекОкон.Количество() > 1 Тогда
		Если глСтекОкон.Количество() = 2 Тогда
			Попытка
				МенюПовараОткрыто = ФормаПодбораТоваров.открыта();
			Исключение
				МенюПовараОткрыто = Ложь;
			КонецПопытки;
			Если Не МенюПовараОткрыто Тогда
				Возврат;
			КонецЕсли;
		Иначе
			Возврат;
		...
		Если ТипЗнч(_Объект) = Тип("Структура") Тогда
			...
			Если _Объект.Тип = Справочники.ТипыШаблоновШК.КартаЛояльности Тогда
				Если ТекущийРежим <> "ВОЗВРАТ" Тогда
					...
					Иначе
						РаботаСокнами.ПоказатьПлашку(Новый Структура("Заголовок, Текст", "Карта лояльности", "применена"));
					КонецЕсли;
					ЗаписатьЗаказ(ПредопределенноеЗначение("Перечисление.СтатусыЗаказа.Открыт"),,,Истина);
			...
			ИначеЕсли _Объект.Тип = Справочники.ТипыШаблоновШК.Купон Тогда
				...
				Если ТекущийРежим = "ПРОДАЖА" Тогда
					...
					Если глОтладкаУровень()<>0 Тогда
						РаботаСокнами.ПоказатьПлашкуПрост("Добавлен купон", _Знач);
					КонецЕсли;
...
Процедура Заблокировать(ЗапроситьАвторизациюГостя = Ложь) Экспорт
	...
	КонецЕсли;
	ВводТоваровДоступен();
	Если ЭтоРМСтанция Тогда
		Если ТекущийРежим = "ПРОДАЖА" Тогда
			ПроверитьБлюдаДляПриготовления();	
		КонецЕсли;
...
Процедура СоздатьНовыйЗаказ(КартаДоступа, СпособРегистрацииБейджа = 0) Экспорт
	...
	ЗаписатьЗаказ(Перечисления.СтатусыЗаказа.Открыт,,,Истина);
	ГлавнаяФорма.ОбновитьВремя(0.1);
	
	ДопПараметрыИнфо.СуммаПоСтанцииНач = СуммаРеализации();
	ДопПараметрыИнфо.СброситьНастройки = Ложь;
	
	ПоказатьЗаказНаМонитореГостя(,"ОткрытьЗаказ()");
	ЗаполнитьНадписиЛояльности(Истина);	
КонецПроцедуры

Функция ПоказатьЗаказНаМонитореГостя_ОбработчикОжидания() Экспорт
	Если фВыборСпецифик и НЕ глПараметрыРМ.ЭтоОтдохни Тогда
		ЗарегистрироватьСобытие("ПоказатьЗаказНаМонитореГостя_ОбработчикОжидания.Выход:фВыборСпецифик = Истина");
		Возврат Ложь;
	КонецЕсли;
	...
	КонецЕсли;
	Если ТекущийДокумент = Неопределено Тогда
		ЗарегистрироватьСобытие("ПоказатьЗаказНаМонитореГостя_ОбработчикОжидания.Выход:ТекущийДокумент = Неопределено");
		Возврат Ложь;
	КонецЕсли;
	КонтрольныеПоказатели.НачатьЗамер(Показатель_ОбновлениеМонитораГостя);
	ТипКолво = ПолучитьОписаниеТиповЧисла(10,3);
	ТипСумма = ПолучитьОписаниеТиповЧисла(12,2);
	
	Попытка
		НомерСтроки = ГлавнаяФорма.тпТовары.Строки.Найти(ТекИдСтрокиТовара(), "идСтроки").НомерСтроки;
	Исключение
	...
	Для Каждого стТовары Из ГлавнаяФорма.тпТовары.Строки
		Цикл
		НовСтр = ТаблицаЗаказа.Строки.Добавить();
		НовСтр.Наименование = СокрЛП(Строка(стТовары.Товар));
		Если ЗначениеЗаполнено(стТовары.ИдСвязаннойСтроки) И стТовары.Строки.Количество() Тогда
			СвязаннаяСтрока = Заказ.Товары.Найти(стТовары.ИдСвязаннойСтроки, "ИдСтроки");
			Если Не СвязаннаяСтрока = Неопределено Тогда
				НовСтр.Наименование = ("Готовим " + СвязаннаяСтрока.Товар);
			КонецЕсли;
			
		КонецЕсли;  
		ЗаполнитьЗначенияСвойств(НовСтр, стТовары);
		НовСтр.Ном = стТовары.НомерСтроки;
		ИнтерфейсРМЛояльность.ЗаполнитьДанныеЛояльностиВСтроке_МониторГостя(НовСтр, ЛояльностьДанныеЗаказа);
	...
	КонтрольныеПоказатели.ЗакончитьЗамер(Показатель_ОбновлениеМонитораГостя);
КонецФункции

Функция ПоказатьЗаказНаМонитореГостя(Задержка = 0.5, ВызывающийКонтекст = "", Сразу = Ложь) Экспорт
	Если Сразу Тогда
		ПоказатьЗаказНаМонитореГостя_ОбработчикОжидания();	
		Возврат Неопределено;
	КонецЕсли;
	ЗарегистрироватьСобытие("ПоказатьЗаказНаМонитореГостя."+ВызывающийКонтекст+?(Сразу,".Сразу",""),,,,"Задержка="+Задержка + "; СтрокДокумента="+заказ.Товары.Количество());
...
Функция СуммаРеализации() Экспорт
	...
	Для Каждого Т Из Товары Цикл
		Если Т.СтатусОплаты=-1 И глПараметрыРМ.ФирмыРМ.Найти(Т.Фирма) <> Неопределено Тогда
...
Процедура ОткрытьЗаказ(ЗаказСсылка) Экспорт
	...
	//БылОтборСтрок = ГлавнаяФорма.ОтборСтрок;
	//ГлавнаяФорма.ДанныеСтрок = 1;
	КонецЕсли;
	...
	ТекущийДокумент = Заказ;
	ТекущийРежим = "ПРОДАЖА";
	Если Не ПрочитатьТекущийДокумент(Истина, Истина) Тогда
	...
	Исключение
		ЗарегистрироватьСобытиеОшибки(ИнформацияОбОшибке(), ЗаказСсылка);
	КонецПопытки;
	ЗаполнитьНадписиЛояльности(Истина);
	...
	КонецЕсли;
	МассивПейджеров = ксТрактир.ВыданныеПейджеры(ЗаказСсылка, ?(ЭтоРМСтанция, глПараметрыРМ.Станция, Неопределено));
	ПейджерКВыдаче = ?(МассивПейджеров.Количество(), МассивПейджеров[0], Неопределено);
	ОбновитьИнформациюОВыданныхПейджерах();
	
	Если ЭтоРМСтанция Тогда
	КонецЕсли;

	Если ЭтоРМСтанция и ЗначениеЗаполнено(глПользователь) Тогда
		Если ФормаПодбораТоваров = Неопределено Тогда
			//СменаМенюПовара();
		Иначе
			Если Не ФормаПодбораТоваров.Открыта() Тогда
...
Процедура СброситьТекущийДокумент(ЗапроситьАвторизациюГостя = Истина, ПоказатьДиалогВводаБэйджа = Истина, НеОткрыватьМеню = Ложь, НеРегистрироватьМарки = Ложь) Экспорт
	
	Если ЗначениеЗаполнено(Заказ.Ссылка) Тогда
		Если Не НеРегистрироватьМарки Тогда
	...
	фВводТоваровДоступен = Ложь;
	ТекИдСтрокиТовара = Неопределено;
	ГлавнаяФорма.ОбновитьВремя(0.1);
...
#Область ВылетВоВремяФискализации
Функция ПроверитьЗаказНаВылетВоВремяФискализации()
	//:Заказ = Документы.Заказ.СоздатьДокумент();
	//:ПротоколРасчетов = Документы.ПротоколРасчетов.ПустаяСсылка();
	
	// РегистрацияФН = 1 - фискализация началась но не завершилась
	//Запрос = Новый Запрос;
	...
	//               |ГДЕ
	//               |	ПротоколРасчетов.РабочееМесто = &РабочееМесто
	//               |	И ПротоколРасчетов.РегистрацияФН = 1";
	//Запрос.УстановитьПараметр("РабочееМесто",глРабочееМесто);
	//Выборка = Запрос.Выполнить().Выбрать();
	...
	Иначе
		...
		Если СтрокаБезнал <> Неопределено Тогда
			Если Не СтрокаБезнал.СуммаФакт Тогда
				...
				Если ОтчетыККМ.СуммаПоБНТекущая(СуммаПоБанку, ПротоколРасчетов) Тогда
...
Функция ВыполнитьПроверкуПоСтанции(СтрокаЗаказа, ВыводПредупреждения=Истина) Экспорт
	...
	Если глПараметрыРМ.ФирмыРМ.Найти(СтрокаЗаказа.Фирма) = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если ЭтоРМСтанция Тогда
		
		Результат = (СтрокаЗаказа.Станция = глПараметрыРМ.Станция) ИЛИ (СтрокаЗаказа.Статус = СтатусДозаказ И СтрокаЗаказа.РабочееМесто = глРабочееМесто);
...
// Изменение количества в текущей строке заказа
	//
Функция ИзменитьКоличество(пКолво = Неопределено) Экспорт
	...
	Если СтрокаТабПоля.Уровень() Тогда
		СтрокаТабПоля = СтрокаТабПоля.Родитель;
	КонецЕсли;
	...
	Если СтрокаЗаказа.СтатусОплаты = 1 Тогда
		ИнтерфейсРМ.ВопросПредупреждение("Ошибка","Строка уже оплачена","Нельзя редактировать оплаченные строрки", "","ОК", "");	
		Возврат Ложь;
	КонецЕсли;
	...
	Если пКолво = Неопределено Тогда
	...
	БылоКолво = СтрокаЗаказа.Количество;
	...
	Если СтрокаТабПоля.Статус = СтатусДозаказ  Тогда
		...
		Иначе
			...
			СтрокаТабПоля.КоличествоНачальное = Колво;
			СтрокаТабПоля.Модифицирована = 1;
	...
	Иначе
		...
		Если Колво > СтрокаЗаказа.Количество  Тогда
			...
			Возврат Истина;
		Иначе
			Если СтрокаТабПоля.НомерМарки Тогда
				Текст1="Удаление позиции с марками";
				Текст2="По позиции есть марки. Удалить позицию?";
				Если ИнтерфейсРМ.ВопросПредупреждение("Вопрос",Текст1,Текст2,"Да","Нет","Отмена") <> "Да" Тогда
					Возврат Ложь;
				КонецЕсли;
			...
			СтрокаЗаказа.Количество = Колво;
			Если Колво = 0 Тогда
				Для Каждого Т Из СтрокаТабПоля.СТроки Цикл
					Т.СуммаРеализации = 0;
					Т.ЦенаРеализации = 0;
					Т.Модифицирована = 1;
				КонецЦикла;
	...
	Попытка
			ЗарегистрироватьСобытие("Изменение количества ", УровеньЖурналаРегистрации.Информация,,ТекущийДокумент.Ссылка, "" +СтрокаЗаказа.Товар + ": " + БылоКолво + " -> " + СтрокаЗаказа.Количество);	
	Исключение
	КонецПопытки;
	
		
	СтрокаТабПоля.СуммаРеализации =  СтрокаЗаказа.СуммаРеализации;
	СтрокаТабПоля.Количество =  СтрокаЗаказа.Количество;
	
	ПоказатьЗаказНаМонитореГостя(,"ИзменитьКоличество()");
	
	Возврат Истина;
КонецФункции
	
Процедура ЗаписатьЗаказ(Статус =	 "Открыт", Оплачен = Ложь, СоздатьМарки = Ложь, ОбновитьДанныеЛояльности = Ложь) Экспорт
	Если Статус = "Открыт" Тогда
		Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаказа.Открыт");
	КонецЕсли;
	
	Для Каждого Фирма Из глПараметрыРМ.ФирмыИБ Цикл
		РезультатБлокировки = Заказ.ЗаблокироватьРедактированиеПоФирме(Фирма, УИДформы);
	...
	Заказ.Записать();
	Если ОбновитьДанныеЛояльности Тогда
		ГлавнаяФорма.ПодключитьОбработчикОжидания("ОбновитьДанныеЛояльностиПослеЗаписи_ОбработчикОжидания", 1.5, Истина);
		//ЛояльностьДанныеЗаказа = ЛояльностьКлиент.ПолучитьДанныеЛояльностиПоЗаказу(Заказ, "");
		//Заказ.Записать();
	КонецЕсли;
	
//реф	ГлавнаяФорма.ЗаполнитьТПтовары();
...
Процедура ОбновитьДанныеЛояльностиПослеЗаписи_ОбработчикОжидания() Экспорт 
	
	ГлавнаяФорма.ОтключитьОбработчикОжидания("ОбновитьДанныеЛояльностиПослеЗаписи_ОбработчикОжидания");
...
Процедура ЗаписатьДокумент(Статус, Оплачен = Ложь, СоздатьМарки = Ложь, ОбновитьДанныеЛояльности = Ложь)
	Если ТекущийРежим = "ВОЗВРАТ" Тогда
		ЗаписатьВозврат(Статус);
	Иначе
		ЗаписатьЗаказ(Статус, Оплачен, СоздатьМарки, ОбновитьДанныеЛояльности);
	КонецЕсли;
КонецПроцедуры
...
Функция ЕстьНеоплаченныеСтроки() Экспорт
	Если ТекущийРежим="ПРОДАЖА" Тогда
		Для Каждого Т Из ТекущийДокумент.Товары Цикл
			Если (Т.СтатусОплаты = -1 Или Т.СтатусОплаты = 0) И Т.Количество Тогда
				Возврат Истина;
			КонецЕсли;
		КонецЦикла;
		Возврат Ложь;
...
Функция ЕстьОплаченныеСтроки() Экспорт
	Если ТекущийРежим="ПРОДАЖА" Тогда
		Для Каждого Т Из ТекущийДокумент.Товары Цикл
			Если (Т.ДокументОплаты.Фискализирован = Истина) Тогда
				Возврат Истина;
			КонецЕсли;
		КонецЦикла;
		Возврат Ложь;
...
Функция НефискализированныйПротоколПоЗаказу() Экспорт
	...
	|	И ПротоколРасчетовПротокол.Ссылка.Фирма В (&Фирмы)
	|	И ПротоколРасчетовПротокол.СуммаФакт > 0
	|	И ПротоколРасчетовПротокол.Ссылка.Заказ = &Заказ");

	Запрос.УстановитьПараметр("Фирмы", глПараметрыРМ.ФирмыРМ);
...
Функция ПроверкаМарок(Отказ = Ложь) Экспорт
	
	Отказ = Ложь;
	Если глПараметрыРМ.ЭтоОтдохни Тогда
		Возврат Истина;
	КонецЕсли;
	ТаблицаЗаказа = ГлавнаяФорма.тпТовары;
	ЕстьМарки = ксТрактир.ПоЗаказуЕстьМарки(Заказ.Ссылка,,Истина);
	Если ЕстьМарки Тогда
		БлюдоВыдали = ИнтерфейсРМ.ВопросПредупреждение("Вопрос гостю","Есть невыданные блюда","Блюда приготовили и выдали?", "Да","Нет", "Отмена");	
		Если БлюдоВыдали = "Да" Тогда
			ПроверкаПейджера();
			ксТрактир.СнятьМаркиСУчета("Заказ", Массив(Заказ.Ссылка));
						
		ИначеЕсли БлюдоВыдали = "Отмена" Тогда
			Отказ = Истина;
			Возврат Ложь;
		Иначе
			Отказ = Ложь;
...
Функция ПроверкаПейджера() Экспорт
	...
	Текст2 = "Гость сдал пейджер" + ?(ТаблицаВыданных.Количество()>1,"ы "," ") + "
	|" + СтрПейджеры + "?";
	
	Если ИнтерфейсРМ.ВопросПредупреждение("Вопрос",Текст1,Текст2,"Да","","Нет",,,,,Заказ.Ссылка) = "Да" Тогда
		ксТрактир.ВозвратПейджеровПоЗаказам(МассивЗаказов,,Ложь);
		ПейджерКВыдаче = Неопределено;
		ОбновитьИнформациюОВыданныхПейджерах();
		Возврат Истина;
	Иначе
...
Процедура Оплата() Экспорт
	Попытка
		...
		Если ЕстьСтрокиДляПриготовления() Тогда
			ИнтерфейсРМ.ВопросПредупреждение("Ошибка", "Есть строки для приготовления", "Нужно оформить приготовление на станции повара 
			|или отменить его на станции сканирования","","ОК","");
			Возврат;
		КонецЕсли;
		...
		Если НЕ ЕстьСтрокиКоплате() И не ЕстьНеоплаченныеСтроки() Тогда
			Если Не ксТрактир.ПоЗаказуЕстьМарки(Заказ.Ссылка,,Истина) Тогда
		...
		ОбработкаОплаты = ИнтерфейсРМ.ПолучитьОбъектОбработки("ОплатаМОКП"); //:ОбработкаОплаты = Обработки.Оплата.Создать();
		Заказ.ПодвалЧека = "";
		...
		ИначеЕсли РезультатОплаты.Успех Тогда
			КонтрольныеПоказатели.ЗакончитьЗамер(Показатель_ПечатьЧеков);
			ТекущийДокумент.КассоваяСмена = Обработки.ОткрытиЗакрытиеСменыКасса.ПолучитьСменуКассы();
			ПротоколРасчетов = ОбработкаОплаты.ПротоколРасчетов.Ссылка;
			Заказ = Заказ.Ссылка.ПолучитьОбъект();
			...
			Если ЗначениеЗаполнено(НадписьСдача) Тогда
				//Если глОтладкаУровень() Тогда
					РаботаСокнами.ПоказатьПлашкуПрост("Не забудьте сдачу", НадписьСдача,5,0);
				//КонецЕсли;
			КонецЕсли;
			ГлавнаяФорма.ОбновитьВремя(0);
			Если Заказ.Товары.Итог("СтатусОплаты") = Заказ.Товары.Количество() Тогда
				Если Не ксТрактир.ПоЗаказуЕстьМарки(Заказ.Ссылка) Тогда
					ОсвободитьКартуДоступа();
				КонецЕсли;
...
Процедура ВозвратОплаты() Экспорт
	...
	ОбработкаОплаты = ИнтерфейсРМ.ПолучитьОбъектОбработки("ОплатаМОКП");
...
Функция Сторно(НомерСтроки = 0) Экспорт 
	...
	ИдСтроки = ТекИдСтрокиТовара();	
	СтрокаТп = ГлавнаяФорма.тпТовары.Строки.Найти(ИдСтроки, "ИдСтроки");
	Если СтрокаТп = Неопределено Тогда
		ИнтерфейсРМ.ВопросПредупреждение("Сторно - ошибка", "", "Не выбрана строка для сторнирования!","","Ок","");
		Возврат Ложь;
	КонецЕсли;
	ЗарегистрироватьИнтерактивноеДействие("Сторно", Строка(СтрокаТп.Товар));
	...
	Если ТекущийРежим = "ПРОДАЖА" Тогда
		...
		Для Каждого Т Из ТекущийДокумент.Товары Цикл 
			Если Т.Количество И Т.СтатусОплаты = -1 Тогда
				...
				Если Т.ИдСтроки = ИдСтроки Тогда
					НужнаяСтрока = Т;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	...
	Если КоличествоСтрок Тогда
		...
		Попытка
			ЗарегистрироватьСобытие("Сторно", УровеньЖурналаРегистрации.Информация,,ТекущийДокумент.Ссылка, НужнаяСтрока.Товар);	
		Исключение
		КонецПопытки;
		...
		МЗ.ИдСтроки = НужнаяСтрока.ИдСтроки;
		МЗ.Прочитать();
		МЗ.Статус = СтатусУдалено;
		МЗ.Количество = НужнаяСтрока.Количество;
		МЗ.КоличествоУдалено = НужнаяСтрока.КоличествоУдалено;
		МЗ.Записать();
		
		Если ТекущийРежим = "ПРОДАЖА" Тогда
			НужнаяСтрока.СуммаРеализации = 0;
		КонецЕсли;
		Если ЕстьНеоплаченныеСтроки() Тогда
			Статус = Перечисления.СтатусыЗаказа.Открыт;
		ИначеЕсли ЕстьОплаченныеСтроки() Тогда
			Статус = Перечисления.СтатусыЗаказа.Закрыт;
		Иначе
			Статус = Перечисления.СтатусыЗаказа.Удален;
		КонецЕсли;
		Если Статус <> Перечисления.СтатусыЗаказа.Открыт Тогда
			Если ОсвободитьКартуДоступа() = Ложь Тогда
				//ЗаписатьЗаказ(Перечисления.СтатусыЗаказа.Открыт,,,Истина);
			Иначе
				//ЗаписатьЗаказ(Статус);
			КонецЕсли;
		Иначе
			ЗаписатьЗаказ(Статус,,,Истина);
		КонецЕсли;
	КонецЕсли;
	
	ТекИдСтрокиТовара = ПоследняяСтрока.ИдСтроки;
	
	глОтсечкаПростоя();
	ПоказатьЗаказНаМонитореГостя(,"Сторно()");
КонецФункции
...
Процедура ПечатьПодвалаЛояльности()
	...
	Обработка_ККМ=Обработка_ККМ;
	ИнициализацияТО(глПараметрыРМ.ККМ, Обработка_ККМ,глПараметрыРМ);
  
	Обработка_ККМ.ВыполнитьКоманду("ПечатьТекста",Параметры,ПараметрыВыходные);
	ПоказатьОшибкуККМ(ПараметрыВыходные);
Исключение
	ЗарегистрироватьСобытиеОшибки(ИнформацияОбОшибке(), заказ.Ссылка);
КонецПопытки;
КонецПроцедуры

Функция ПоказатьИнформациюОтоваре(РежимПредпросмотра = Ложь, Знач ВыбранноеДействие = "") Экспорт
	
	ГлавнаяФорма.ОтключитьОбработчикОжидания("ПоказатьЗаказНаМонитореГостя_ОбработчикОжидания");	
	
	Уровень = ЭлементыФормы.тпТовары.ТекущаяСтрока.Уровень();
	Если Уровень Тогда
		ТекущиеДанные = ЭлементыФормы.тпТовары.ТекущиеДанные.Родитель;
	Иначе
		ТекущиеДанные = ЭлементыФормы.тпТовары.ТекущиеДанные;
	КонецЕсли;
	Товар = ТекущиеДанные.Товар;
	идСтроки = ТекущиеДанные.ИдСтроки;
	
	ИнтерфейсРМЛояльность.ПоказатьИнформациюОТоваре(Товар, ЛояльностьДанныеЗаказа, идСтроки, РежимПредпросмотра, ВыбранноеДействие);
	
	Возврат ВыбранноеДействие;
...
Функция ВводПДФ(Товар, ИсключатьПДФчека = Истина) Экспорт
	...
	Обработка_ПДФ.ЭтоВозврат = ТекущийРежим = "ВОЗВРАТ";
	ФормаПДФ.Товар = Товар;
	ФормаПДФ.ПДФЧека = МассивПДФчека;
	Возврат ФормаПДФ.ОткрытьМодально();
	
КонецФункции

Функция ВводШК() Экспорт

	Обработка = ИнтерфейсРМ.ПолучитьОбъектОбработки("ВводШК");
	Обработка.ЗаголовокФормы = "Просканируйте ШК бутылки";
	Форма = Обработка.ПолучитьФорму("Форма"+глПараметрыРМ.ИнтерфейсТип);
	Возврат Форма.ОткрытьМодально();
КонецФункции
...
Функция ПрочитатьТекущийДокумент(ОбновитьСтатусыСтрок = Истина, ЗаполнитьТПтовары = Истина) Экспорт
	...
	ИначеЕсли ТекущийРежим = "ПРОДАЖА" Тогда
		...
		МЗ.Заказ = Заказ.Ссылка;
		МЗ.Прочитать();
		Если МЗ.Статус = Перечисления.СтатусыЗаказа.Закрыт ИЛИ МЗ.Статус = Перечисления.СтатусыЗаказа.Удален Тогда
			ТекущийРежим = "";

			Текст1="Заказ " + УбратьВсеБуквы(Заказ.Номер) + МЗ.Статус + "!";
			Текст2="Возможно, по заказу остались несданные пейджеры или незакрытые марки";
...
//реф					ГлавнаяФорма.ЗаполнитьТПтовары();
			...
			УстановитьФорматКоличестваИсуммы();
			
			//:Заказ = Документы.Заказ.СоздатьДокумент();
			Для Каждого Т Из Заказ.Товары Цикл
				Если ЗначениеЗаполнено(Т.ДокументОплаты) И Т.СтатусОплаты = -1 И Т.ДокументОплаты.Фискализирован  Тогда
					Т.СтатусОплаты = 1;
				ИначеЕсли Т.СтатусОплаты = 0 и глПараметрыРМ.ФирмыРМ.Найти(Т.Фирма) <> Неопределено Тогда
					Т.СтатусОплаты = -1;
				ИначеЕсли Т.СтатусОплаты = -1 И глПараметрыРМ.ФирмыРМ.Найти(Т.Фирма) = Неопределено Тогда
					т.СтатусОплаты = 0;
				КонецЕсли;
			КонецЦикла;		
		КонецЕсли;
...
Процедура УстановитьФорматКоличестваИсуммы()
	
	Перем ИтогКоличество, ИтогЦена, ТочностьСуммы;
	
	ИтогКоличество = Заказ.Товары.Итог("Количество");
	ИтогСумма 	   = Заказ.Товары.Итог("СуммаРеализации");
	ТочностьКоличества = ИтогКоличество = Цел(ИтогКоличество);
	ТочностьСуммы 	   = ИтогСумма 		= Цел(ИтогСумма);
	ГлавнаяФорма.ЭлементыФормы.тпТовары.Колонки.СуммаРеализации.Формат 	= ("ЧДЦ="+?(ТочностьСуммы, 0, 2));
	ГлавнаяФорма.ЭлементыФормы.тпТовары.Колонки.ЦенаРеализации.Формат 			= ("ЧДЦ="+?(ТочностьСуммы, 0, 2));
	ГлавнаяФорма.ЭлементыФормы.тпТовары.Колонки.Количество.Формат 		= ("ЧДЦ="+?(ТочностьКоличества, 0, 3));

КонецПроцедуры

функция ОбновитьСтатусыСтрок(НеОткрыватьМенюПовара = Ложь) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ *
	|ИЗ                 
	...
	Для Каждого Т Из ТЗ Цикл
		...
		ЗаполнитьЗначенияСвойств(СтрокаЗаказа, Т,,"Количество");
		...
		КонецЕсли;
	КонецЦикла;
	Если ЭтоРМСтанция И Не НеОткрыватьМенюПовара Тогда
		Если ФормаПодбораТоваров = Неопределено Тогда
			//СменаМенюПовара();
		Иначе
			Если Не ФормаПодбораТоваров.Открыта() Тогда
...
Процедура ИзменитьКоличествоПриВыборе() Экспорт
	Если ЗаказНеВыбран() Тогда
		Возврат;
	КонецЕсли;

	ВведенноеЧисло = ИнтерфейсРМ.ВводЧисла("ВВЕДИТЕ КОЛИЧЕСТВО", "Число", 10, 3, 0,,,Истина);
...
Функция ПроверитьБлокировкуЗаказа() Экспорт 
...
Функция ПроверитьПробковыйСбор(Товар) Экспорт
	Номенклатура = Товар.Номенклатура;
	Если ЗначениеЗаполнено(Номенклатура) Тогда
		Если Номенклатура = Справочники.Номенклатура.ПробковыйСборКрепкийАлкоголь Или Номенклатура = Справочники.Номенклатура.ПробковыйСборПиво Тогда
			ШК = ВводШК();
			Если Не ЗначениеЗаполнено(ШК) Тогда
				Возврат Ложь;
			Иначе
				Если Заказ.Товары.Найти(ШК) = Неопределено Тогда
					Если ИнтерфейсРМ.ВопросПредупреждение("Ошибка", "Товар не найден в заказе", "ШК " + ШК + " не найден в заказе.
...
Функция ВводНовойСтроки(ВыбТов, Количество = 0, ШК = "", НельзяОтказатьсяОтВводаПозиции = Ложь, ИдСвязаннойСтроки = "", ОбязательнаяСпецифика = Неопределено, МинКоличествоСпецифики = 0) Экспорт
	КонтрольныеПоказатели.НачатьЗамер(Показатель_ДобавлениеСтроки);
	Если Не ПрочитатьТекущийДокумент(Ложь, Ложь) Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	РезультатПроверкиПробковыйСбор = ПроверитьПробковыйСбор(ВыбТов);
	Если не РезультатПроверкиПробковыйСбор Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	Если ТекущийРежим = "ПРОДАЖА" Тогда
		...
		Нов.ИдСвязаннойСтроки = ИдСвязаннойСтроки;
		фВыборСпецифик = 1;
		
	ГлавнаяФорма.ОбновитьВремя(ИнтервалОбновленияИнтерфейса_ВводНовойСтроки);
	ПоказатьЗаказНаМонитореГостя(0.5,"ВводНовойСтроки()");
	ОбновитьСтатусыСтрок();
	
	Попытка
		Если ГлавнаяФорма.РежимПредпросмотра И НЕ ПоказатьИнформациюОтоваре(ГлавнаяФорма.РежимПредпросмотра, "ДОБАВИТЬ") = "ДОБАВИТЬ" Тогда
			
			ТекущийДокумент.Товары.Удалить(Нов);
			фВыборСпецифик = 0;
			ГлавнаяФорма.ОбновитьВремя(ИнтервалОбновленияИнтерфейса);
//реф			ГлавнаяФорма.ЗаполнитьТПтовары();
			
			ОбновитьСтатусыСтрок();
			КонтрольныеПоказатели.ПрерватьЗамер(Показатель_ДобавлениеСтроки);
			Возврат Ложь;
		КонецЕсли;
	Исключение
		// значит режим не предусмотрен формой
	КонецПопытки;
	
	ГлавнаяФорма.ЗаполнитьТПтовары();
	Если НЕ ПроверитьДобавитьСпецифику(Нов,Неопределено,Истина,НельзяОтказатьсяОтВводаПозиции, ОбязательнаяСпецифика, МинКоличествоСпецифики) Тогда
		ТекущийДокумент.Товары.Удалить(Нов);
		фВыборСпецифик = 0;
		ГлавнаяФорма.ОбновитьВремя(ИнтервалОбновленияИнтерфейса);
//реф		ГлавнаяФорма.ЗаполнитьТПтовары();
	...
	КонецЕсли; 
	фВыборСпецифик = 0;
	ТекИдСтрокиТовара = Нов.ИдСтроки;
	фУстанавливатьТекСтроку = Ложь;
	ГлавнаяФорма.ЗаполнитьТПтовары();
	фУстанавливатьТекСтроку = Ложь;
	Попытка
	...
	ЗаписатьДокумент(Перечисления.СтатусыЗаказа.Открыт,,,Истина);
	ЗарегистрироватьСобытие("Заказ.Добавление строки",,,,"" + ВыбТов + ", кол. " + Количество);
	ЗаполнитьНадписиЛояльности();
...
// Описание процедуры
//
Функция ПроверитьДобавитьСпецифику(СтрокаДокЗаказ, ВыбСпецифика=Неопределено, ОбязательноОткрыватьФорму=Ложь,АвтоПозиция=Ложь, ОбязательнаяСпецифика = Неопределено, МинКоличествоСпецифики = 0)
		...
		Если НЕ ОбязательноОткрыватьФорму Тогда
			...
			Если РезультатВыбора.Контроль ИЛИ АвтоПозиция Тогда
		...
		//ВыполнитьВыборСпецифик																			 (СтрокаТовара  , ТолькоПроверка,АвтоПозиция, СпецификаВыбор, ОбязательнаяСпецифика, МинКоличествоОбязательнойСпецифики = 0)
		РезультатВыбора = ОбработкаВыборСпецифик.ВыполнитьВыборСпецифик(СтрокаТпТовары, Ложь,          АвтоПозиция,               , ОбязательнаяСпецифика,	МинКоличествоСпецифики);
		
		Если (РезультатВыбора.Контроль и РезультатВыбора.ВыборСделан) Или АвтоПозиция Тогда
			ОбновитьСпецификиПоСтрокеТовара(СтрокаДокЗаказ, РезультатВыбора.Специфики, Ложь);
		...
		Возврат (РезультатВыбора.Контроль и РезультатВыбора.ВыборСделан) Или АвтоПозиция;
КонецФункции
...
Функция КонтрольВозвраста(ГруппаКонтроля, фАлкоголь)
	...
	Если ДобавитьМесяц(ДатаРождения, 12 * МинимальныйВозраст) > ТекущаяДатаНаСервере() Тогда 
		...
		Заказ.ДатаРождения = '00010101';
		Возврат Ложь;
	ИначеЕсли ГруппаКонтроля = Справочники.ГруппыКонтроляПродажи.Алкоголь и фАлкоголь = 1 И ПервыйВводДаты Тогда
		Заказ.ДатаРождения = ДатаРождения;
		ЗаписатьЗаказ();
		Возврат Не ПервыйВводДаты;
	//ИначеЕсли НЕ Заказ.ДатаРождения = ДатаРождения Тогда
		//Заказ.ДатаРождения = ДатаРождения;
		//ЗаписатьЗаказ();
		//Возврат Истина;
	//Иначе
	...
	Если НЕ Заказ.ДатаРождения = ДатаРождения Тогда
		Заказ.ДатаРождения = ДатаРождения;
		ЗаписатьЗаказ();
	КонецЕсли;
...
Процедура ПодборТоваров(ВыводПредупреждения=Истина, ОсновноеМеню=Неопределено) Экспорт
	...
	Если Не ЭтоРМСтанция Тогда
		ФормаПодбораТоваров.ЗакрыватьПриВыборе = Истина;
	КонецЕсли;
	...
	Если ФормаПодбораТоваров.ПараметрыОкна.Свойство("Высота") Тогда
...
Функция ВыполнитьРегистрациюМарок() Экспорт 
	...
	Если ЕстьДозаказ() Тогда
		Номерок = Заказ.ДопИнф("Номерок");
		Если ЗначениеЗаполнено(Номерок) Тогда
			ТекПейджер = Номерок;
		Иначе
		...
		ИспользоватьНомерок = Неопределено;
		
		Если ТипЗнч(ТекПейджер) = Тип("СправочникСсылка.Пейджеры") Тогда
			ИспользоватьНомерок = Ложь;	
		ИначеЕсли ТипЗнч(ТекПейджер) = Тип("СправочникСсылка.Номерки") Тогда
			ВсеПейджеры = ксТрактир.ВыданныеПейджеры(Заказ.Ссылка);
			Если ВсеПейджеры.Количество() Тогда
				ИспользоватьНомерок = Ложь;
			ИначеЕсли ксТрактир.ПоЗаказуЕстьМарки(Массив(заказ.Ссылка),,Истина) Тогда
				ИспользоватьНомерок = Истина;
			Иначе
				ИспользоватьНомерок = Неопределено;			
			КонецЕсли;
		КонецЕсли;
		
		Пока (ИспользоватьНомерок = Неопределено И ЕстьДлинныеПозиции()) Цикл
			Ответ = ИнтерфейсРМ.ВопросПредупреждение("Вопрос гостю","Вариант обслуживания", "Пейджер или номерок?","Пейджер","Номерок","Отмена");
			Если Ответ = "Отмена" Тогда
				Возврат Ложь;
			ИначеЕсли Ответ = "Номерок" Тогда
				ИспользоватьНомерок = Истина;
				РезультатВвода = ИнтерфейсРМ.ВводЧисла("Номерок", "Число", 3, 0);
				Если ЗначениеЗаполнено(РезультатВвода) Тогда
					
					ТекПейджер = Справочники.Номерки.НайтиПоКоду(РезультатВвода);
					Если ТекПейджер.Пустая() Тогда
						ИнтерфейсРМ.ВопросПредупреждение("ошибка","Номерок не найден","Номерок с кодом " + РезультатВвода + " 
						|не найден в справочнике","","ОК","");
						ИспользоватьНомерок = Неопределено;
					Иначе
						ЗарегистрироватьСобытие("Выбран номерок ",,,Заказ.Ссылка, РезультатВвода);
					КонецЕсли;
				Иначе
					ИспользоватьНомерок = Неопределено;
				КонецЕсли;
			ИначеЕсли Ответ = "Пейджер" Тогда
				ТекПейджер = Справочники.Пейджеры.ПустаяСсылка();
				ИспользоватьНомерок = Ложь;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		...
		Если СледующийНомерПодачиНач = Неопределено Тогда
			СледующийНомерПодачиНач = ТекущийМаксНомерПодачи;
		КонецЕсли;
		РезультатВыбора				= ОбработкаВыборОчередности.ВыполнитьВыборОчередности(ГлавнаяФорма, Заказ.Ссылка, ГлавнаяФорма.тпТовары, ТекСтанция, ТекПейджер, мин(СледующийНомерПодачиНач,1), СтанцияОтборСуммаНач);
		...
		Для каждого СтрокаОчередность Из ТаблицаОчередность Цикл
			...
			СтрокаТПтовары = ГлавнаяФорма.тпТовары.Строки.Найти(СтрокаТовара.ИдСтроки);
			Если СтрокаТПтовары = Неопределено Тогда
				Продолжить;
			КонецЕсли; 
			
			Если СтрокаОчередность.Курс <> ТекущийМаксНомерПодачи Тогда
			...
			СтрокаТПтовары.Подача = Курс;
			СтрокаТПтовары.Модифицирована = Истина;
		КонецЦикла; 
		
		// Пейджер
		Если РезультатВыбора.Пейджер <> Неопределено Тогда
			Если ТипЗнч(РезультатВыбора.Пейджер) = Тип("СправочникСсылка.Пейджеры") Тогда
				...
				Заказ.УстановитьАтрибут("Номерок", Справочники.Номерки.ПустаяСсылка());
			ИначеЕсли ТипЗнч(РезультатВыбора.Пейджер) = Тип("СправочникСсылка.Номерки") Тогда
				Заказ.УстановитьАтрибут("Номерок", РезультатВыбора.Пейджер);
				ПейджерКВыдаче = РезультатВыбора.Пейджер;
			КонецЕсли;
			ОбновитьИнформациюОВыданныхПейджерах();
		КонецЕсли; 
...
Функция ЗаписатьЗаказТоварыДопИнф(СоздатьМарки = Ложь) Экспорт
	...
	Для Каждого Т Из ГлавнаяФорма.тпТовары.Строки Цикл
		...
		Если НЗ.Количество() = 0 Тогда
			...
			ЗаписьРегистра.Статус = СтатусДозаказ;//;?(ЭтоРМСтанция,СтатусДозаказ, СтатусВыдано);
			ЗаписьРегистра.ИдСвязаннойСтроки = Т.ИдСвязаннойСтроки;
		...
		Иначе
			...
			иначеЕсли Т.Количество-Т.КоличествоУдалено> 0 и не ЭтоРМСтанция Тогда
		...
		ЗаписьРегистра.Подача = Т.Подача; 
		ЗаписьРегистра.Станция = ОбщегоНазначенияПовтИсп.СтанцияТовара(ЗаписьРегистра.Товар);
		...
		Если СоздатьМарки 	
			...
			Если НомерМарки = Неопределено ИЛИ Станции.Найти("" + Т.Станция + Т.Подача) = Неопределено Тогда
				НомерМарки = ксТрактир.ПолучитьНовыйНомерМарки();	
				Станции.Добавить("" + Т.Станция + Т.Подача);
			КонецЕсли;
			ЗаписьРегистра.НомерМарки = НомерМарки;
			ЗаписьРегистра.КодПодтверждения = ЗаписьРегистра.ИдСтроки;
			ЗаписьРегистра.Статус = СтатусЗаказано;
			ЗаписьРегистра.Станция = Т.Станция;
			ЗаписьРегистра.РабочееМесто = глРабочееМесто;
		...
		Если Записать Тогда
			...
			для Каждого тСпециф из т.строки Цикл
				...
				ЗаполнитьЗначенияСвойств(ЗаписьСпецифики, ЗаписьРегистра,,"ИдСтроки,ИдСтрокиВладельца,Товар,Количество,КоличествоУдалено");
				ЗаписьСпецифики.Количество = тСпециф.Количество;
				ЗаписьСпецифики.КоличествоУдалено = тСпециф.КоличествоУдалено;
				Если Не ЗаписьРегистра.Количество Тогда
					ЗаписьСпецифики.Количество = 0;
				КонецЕсли;
...
Процедура ОбработатьОповещение(ИмяСобытия, Параметр, Источник) Экспорт
	Если ИмяСобытия = "ОбновитьСтатусыПозицийЗаказа" Тогда
		ОбновитьСтатусыСтрок(Истина);
	ИначеЕсли ИмяСобытия = "СброситьЗаказ" Тогда
		Если ТекущийРежим = "ПРОДАЖА" Тогда
			Если Заказ.Номер = Параметр Тогда  
				РаботаСокнами.ПоказатьПлашкуПрост("Гость завершил покупки");
				Заказ = Документы.Заказ.СоздатьДокумент();
				СброситьТекущийДокумент(,,Истина);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Функция ЕстьДозаказ() Экспорт
	Если ТекущийРежим = "ПРОДАЖА" Тогда
		Возврат Заказ.Товары.НайтиСтроки(Новый Структура("Статус, РабочееМесто", СтатусДозаказ, глРабочееМесто)).Количество();
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
Функция ЕстьСтрокиДляПриготовления() Экспорт
	Если ТекущийРежим = "ПРОДАЖА" Тогда
		Возврат Заказ.Товары.НайтиСтроки(Новый Структура("Статус,ИдСвязаннойСтроки", Перечисления.СтатусыПозицийЗаказа.ДляПриготовления, "")).Количество();
	Иначе
		Возврат Ложь;
	КонецЕсли;
КонецФункции

Функция ЕстьДлинныеПозиции() Экспорт
	Если ТекущийРежим = "ПРОДАЖА" Тогда
		Строки = ГлавнаяФорма.тпТовары.Строки.НайтиСтроки(Новый Структура("Статус, РабочееМесто", СтатусДозаказ, глРабочееМесто));
		Для Каждого Т Из Строки Цикл
			Товар =  Т.Товар; //:Товар=Справочники.Товары.ПустаяСсылка();
			Если Товар.ВремяПриготовления Тогда
				Возврат Истина;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура СменаМенюПовара() Экспорт
	...
	ФормаПодбораТоваров.ОбрезкаОкнаШирина = Мин(ГлавнаяФорма.ЭлементыФормы.тпТовары.Ширина+5,778);
	Если Не ФормаПодбораТоваров.Открыта() Тогда
		ФормаПодбораТоваров.Открыть();
	КонецЕсли;
	...
	Попытка
		ЭлементФормыТаблицаЗаказа.Высота		= ГлавнаяФорма.Высота - (ЭлементФормыТаблицаЗаказа.Верх + ФормаПодбораТоваров.ПараметрыОкна.Высота + ВысотаПодвала) + 15;
	Исключение
	КонецПопытки;
	
	ЭлементФормыТаблицаВозврат.Высота		= ЭлементФормыТаблицаЗаказа.Высота;
	Кнопки = Массив("КнопкаКоличество","КнопкаОбщиеСпецифики", "КнопкаСпецифики","КнопкаВыдача","КнопкаИнфо", "КнопкаРецептыГостя");
	Для Каждого Кнопка  Из Кнопки Цикл
		//:ГлавнаяФорма = ПолучитьФорму();
		элКнопка = ГлавнаяФорма.ЭлементыФормы.Найти(Кнопка);
		Если Неопределено <> элКнопка Тогда
			элКнопка.Верх = ЭлементФормыТаблицаЗаказа.Верх+ЭлементФормыТаблицаЗаказа.Высота - 2;
		КонецЕсли;
	КонецЦикла;	
	Попытка
		ЭлементФормыТаблицаЗаказа.Высота		= ГлавнаяФорма.Высота - (ЭлементФормыТаблицаЗаказа.Верх + ФормаПодбораТоваров.ПараметрыОкна.Высота + ВысотаПодвала) + 15;
...
Процедура СкрытьМенюПовара() Экспорт
	//РаботаСокнами.ПоказатьПлашку("СкрытьМенюПовара", ""+ЭлементыФормы.тпТовары.Ширина);

	ГлавнаяФорма.ОтключитьОбработчикОжидания("ПоказатьМенюПовараФорма");
	Попытка
...
Процедура ПоказатьМенюПовара() Экспорт
	//РаботаСокнами.ПоказатьПлашку("ПоказатьМенюПовара", ""+ЭлементыФормы.тпТовары.Ширина);
	Если ЭтоРМСтанция и ЗначениеЗаполнено(глПользователь) и ТекущийРежим = "ПРОДАЖА" и не фВыборСпецифик Тогда
		Если глСтекОкон.Количество() = 1 Тогда
			//ГлавнаяФорма.ПодключитьОбработчикОжидания("ПоказатьМенюПовараФорма",0.2,1);
			ГлавнаяФорма.ПоказатьМенюПовараФорма();
		КонецЕсли;
...
Функция ПредупреждениеЕстьДозаказ() Экспорт
	ТекстОшибки = "Есть строки, по которым не оформлены марки (зеленый цвет).
	|Создайте по ним марки или удалите строки.";
	
	Заголовок = "Завершите работу с новыми строками";
...
Процедура ЗаполнитьНадписиЛояльности(Сразу = Ложь) Экспорт
	Если Сразу ИЛИ ИнтервалОбновленияИнтерфейса = 0 Тогда
		ЗаполнитьНадписиЛояльности_ОбработчикОжидания();
	Иначе
		ГлавнаяФорма.ОтключитьОбработчикОжидания("ЗаполнитьНадписиЛояльности_ОбработчикОжидания");		
		ГлавнаяФорма.ПодключитьОбработчикОжидания("ЗаполнитьНадписиЛояльности_ОбработчикОжидания", ИнтервалОбновленияИнтерфейса, Истина);		
	КонецЕсли;                                                                                                  	
КонецПроцедуры
...
Процедура ЗаполнитьНадписиЛояльности_ОбработчикОжидания() Экспорт
	...
	ИнтерфейсРМЛояльность.ОбновитьЭлементыФормы(ГлавнаяФорма, Неопределено, ЛояльностьДанныеЗаказа);
	КонтрольныеПоказатели.ЗакончитьЗамер(Показатель_ОбсчетЛОЛ);
	//ПоказатьЗаказНаМонитореГостя(,"ЗаполнитьНадписиЛояльности_ОбработчикОжидания()");
КонецПроцедуры
...
// Выбор специфики
//
Процедура ВыборСпецифики(ФлагОбщая=Ложь, пСтрокаЗаказа=Неопределено) Экспорт
	
	Если глВерсия=1 Тогда   
		Возврат;
	КонецЕсли;
	
	Если пСтрокаЗаказа = Неопределено Тогда
		пСтрокаЗаказа = ЭлементыФормы.тпТовары.ТекущаяСтрока;
	КонецЕсли;
	СтрокаЗаказа = ?(пСтрокаЗаказа.Уровень(), пСтрокаЗаказа.Родитель, пСтрокаЗаказа);
	...
	КонецЕсли; 
	фВыборСпецифик = 0;
	ПоказатьЗаказНаМонитореГостя(,"ВыборСпецифик()");
...
Функция ВыполнитьВыборСпецификВФорме(ФормаЗаказа, ФлагОбщая, СтрокаЗаказа)
	...
	Если ФлагОбщая Тогда
		...
		Модиф=Ложь;
		Если (ТипЗнч(РезультатВыбора)=Тип("СправочникСсылка.Специфики") и ЗначениеЗаполнено(РезультатВыбора)) или (ТипЗнч(РезультатВыбора)=Тип("Массив")) Тогда
			...
			МодифУдалитьОбщиеСпецифики = УдалитьОбщиеСпецифики(СтрокаТовара, РезультатВыбора);
			
			Если Модиф Или МодифУдалитьОбщиеСпецифики Тогда
				СтрокаТовара.Модифицирована = Истина;
				ЗаписатьЗаказТоварыДопИнф();
				ЗаписатьЗаказ(,,,Истина);
				ПрочитатьТекущийДокумент(Истина,Истина);
				ПоказатьЗаказНаМонитореГостя(,"ВыполнитьВыборСпецификВФорме()");
				ГлавнаяФорма.ОбновитьВремя(0.1);
		...
		Если ФормаЗаказа <> Неопределено Тогда
			...
			СтрокаТП = ГлавнаяФорма.тпТовары.Строки.Найти(ИдСтроки, "ИдСтроки");
			ТабПолеТЗ.ТекущаяСтрока = СтрокаТП;
			ТабПолеТЗ.Развернуть(СтрокаТП, Истина);
...
Функция ОбновитьСпецификиПоСтрокеТовара(Знач СтрокаДокЗаказ, ДеревоСпецифик, Перечитывать = Истина)
	...
	Для Каждого Т Из НЗспецифики Цикл
		Если Т.ИдСтрокиВладельца = СтрокаДокЗаказ.ИдСтроки Тогда
			Если ксТрактир.ОбщаяСпецифика(Т.Товар) Тогда
				Продолжить;
			КонецЕсли;
			МЗ = РегистрыСведений.ЗаказТоварыДопИнф.СоздатьМенеджерЗаписи();
			МЗ.Заказ = Заказ.Ссылка;
	...
	Для Каждого СтрокаСпецифика из ДеревоСпецифик.Строки Цикл
		...
		НовСтрокаСпецифика.Количество = СтрокаСпецифика.Количество;
		...
		МЗ = РегистрыСведений.ЗаказТоварыДопИнф.СоздатьМенеджерЗаписи();
		МЗ.Заказ = Заказ.Ссылка;
		МЗ.ИдСтроки = НовСтрокаСпецифика.ИдСтроки;
		МЗ.ИдСтрокиВладельца = СтрокаТпТовары.ИдСтроки;
		МЗ.Количество = СтрокаСпецифика.Количество;
		МЗ.Станция = СтрокаТпТовары.Станция;
		МЗ.НомерМарки = СтрокаТпТовары.НомерМарки;
		МЗ.АвторЗаказано = глПользователь;
		МЗ.ВремяДозаказ  = ТекущаяДатаНаСервере();
		МЗ.Статус = СтрокаТпТовары.Статус;
		МЗ.Товар = СтрокаСпецифика.Специфика;
		МЗ.РабочееМесто = СтрокаТпТовары.РабочееМесто;
		МЗ.Записать(Истина);
	...
	Если ТекущийРежим = "ПРОДАЖА" Тогда
		ГлавнаяФорма.ОбновитьВремя(0);
		ОбновитьСтатусыСтрок();
		ПоказатьЗаказНаМонитореГостя(,"ОбновитьСпецификиПоСтрокеТовара()");
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;                   	   	
...
Процедура ИзменитьКурс(РаботаСВыдачей = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(ТекСтатус) И (СтрокаЗаказа.НомерМарки ИЛИ ТекСтатус = СтатусГотово ИЛИ (ТекСтатус = СтатусЗаказано)) Тогда
		ОтметитьВыдачу(СтрокаЗаказа);
		Возврат;
	КонецЕсли;	
	
	Текст1="Нет марок!";
	Текст2="К этой позиции нет марок ...";
	ИнтерфейсРМ.ВопросПредупреждение("Ошибка",Текст1,Текст2,"","ОК","");
	Возврат;
...
Процедура ОтметитьВыдачу(СтрокаТовара,ВыводПредупреждения=Истина)
	...
	Если фПодтвердитьВыдачу Тогда		
		...
		Защита.ПодтверждениеВыдачи(СтрокаТовара.ИдСтроки, Заказ.Ссылка);
		...
		СтрокаТовара.НомерМарки = 0;
		СтрокаТовара.Статус = СтатусВыдано;
		СтрокаТовара.Модифицирована = Истина;
		ЗаписатьЗаказТоварыДопИнф()
	КонецЕсли;
...
Функция ДобавитьНовыеСпецифики(СтрокаТпТовары, ДеревоНовыхСпецифик)
	...
	Если ТипЗнч(ДеревоНовыхСпецифик) = Тип("Массив") Тогда
		Для Каждого Специфика из ДеревоНовыхСпецифик Цикл
			...
			КонецЕсли;
		КонецЦикла;
		Возврат ДеревоНовыхСпецифик.Количество();
	Иначе
	...
	ПрочитатьТекущийДокумент(Истина,Истина);
	ГлавнаяФорма.ОбновитьВремя();
	ПоказатьЗаказНаМонитореГостя(,"ДобавитьНовыеСпецифики()");
	
	Возврат Истина;
	
КонецФункции   

Функция УдалитьОбщиеСпецифики(СтрокаТпТовары, МассивОбщихСпецифик)
	//ис(Новый Структура("СтрокаЗаказа, ДеревоНовыхСпецифик",СтрокаЗаказа, ДеревоНовыхСпецифик));
	...
	СтрокаТпТовары.Модифицирована = 1;
	СтрокаДокЗаказа = Заказ.Товары.Найти(ИдСтроки, "ИдСтроки");
	МассивУдаляемыеСпецифики = Новый Массив;
	
	Для Каждого СтрокаТпСпецифика из СтрокаТпТовары.Строки Цикл
		Если ксТрактир.ОбщаяСпецифика(СтрокаТпСпецифика.Товар) И МассивОбщихСпецифик.Найти(СтрокаТпСпецифика.Товар) = Неопределено Тогда
			МассивУдаляемыеСпецифики.Добавить(СтрокаТпСпецифика);
			Модиф = Истина;
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого Т Из МассивУдаляемыеСпецифики Цикл
		Ст = Заказ.Специфики.Найти(Т.ИдСтроки, "ИдСтроки");	
		Если Ст <> Неопределено Тогда
			Заказ.Специфики.Удалить(Ст);
		КонецЕсли;
		МЗ = РегистрыСведений.ЗаказТоварыДопИнф.СоздатьМенеджерЗаписи();
		МЗ.Заказ = Заказ.Ссылка;
		МЗ.ИдСтроки = Т.ИдСтроки;
		...
		СтрокаТпТовары.Строки.Удалить(Т);
	КонецЦикла;          
...
Процедура ОформлениеСтрокиЗаказа(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	СтрокаТекТовара = ГлавнаяФорма.тпТовары.Строки.Найти(ТекИдСтрокиТовара(), "идСтроки", Ложь);
	КартинкаСтрокиТовара = 0;
	
	// статус
	ЯчейкаСтатус = ОформлениеСтроки.Ячейки.НомерСтроки;
	ЯчейкаСумма = ОформлениеСтроки.Ячейки.СуммаРеализации;
	...
	Иначе
		Если ДанныеСтроки.ЦенаРеализации И ДанныеСтроки.Количество Тогда
	...
	Если ДанныеСтроки.СтатусОплаты = 1 И ДанныеСтроки.СуммаРеализации Тогда
		ЯчейкаСумма.ЦветФона = Метаданные.ЭлементыСтиля.БолееИнтенсивныйЗеленый.Значение;
		ЯчейкаСумма.ЦветТекста  = WebЦвета.Белый;
	КонецЕсли;                                                                   	
	
		
	Если ДанныеСтроки.Уровень() Тогда // это специфика  
		СтрокаТовара = СтрокаТовара.Родитель;
		Фирма = СтрокаТовара.Фирма;
		ОформлениеСтроки.Ячейки.НомерСтроки.ВысотаЯчейки=1;
		ОформлениеСтроки.Ячейки.НомерСтроки.ОтображатьТекст = 0;
		ОформлениеСтроки.Ячейки.НомерСтроки.Шрифт = Новый Шрифт(ОформлениеСтроки.Шрифт,,5,,,,,10);;
		ОформлениеСтроки.Ячейки.Товар.УстановитьТекст("  - " + ДанныеСтроки.Товар);
		
		Если Не СтрокаТовара.Количество Тогда
			ОформлениеСтроки.Ячейки.ЦенаРеализации.ОтображатьТекст = 0;
			ОформлениеСтроки.Ячейки.СуммаРеализации.ОтображатьТекст = 0;
		КонецЕсли;
	Если ДанныеСтроки.Количество = 1 Тогда
		...
		КонецЕсли;
		ОформлениеСтроки.Шрифт = Новый Шрифт(ОформлениеСтроки.Шрифт,,9);
		ЯчейкаСтатус.ОтображатьТекст	= Ложь;
	Иначе
		Фирма = ДанныеСтроки.Фирма;
		ОформлениеСтроки.Шрифт = Новый Шрифт(ОформлениеСтроки.Шрифт,,11);
		ОформлениеСтроки.Ячейки.НомерСтроки.Шрифт = Новый Шрифт(ОформлениеСтроки.Шрифт,,7,,,,,100);;
		ЯчейкаСтатус.ОтображатьТекст	= Истина;
		ОформлениеСтроки.Ячейки.НомерСтроки.ВысотаЯчейки=2;
	...
	ИначеЕсли (ДанныеСтроки.Уровень() И ДанныеСтроки.Родитель.Количество = 0) Или ДанныеСтроки.Количество = 0 Тогда
		ОформлениеСтроки.Шрифт = Новый Шрифт(ОформлениеСтроки.Шрифт,,9,,,,Истина); // зачеркнутый
		ОформлениеСтроки.ЦветТекста = ЦветаСтиля.ЦветТекстаНедоступнойСтроки;
		ОформлениеСтроки.Ячейки.НомерСтроки.ВысотаЯчейки=1;
	КонецЕсли; 
	
	Если ДанныеСтроки.СтатусОплаты >= 0 Тогда
		ОформлениеСтроки.ЦветТекста =  ЦветаСтиля.ЦветТекстаНедоступнойСтроки;
	ИначеЕсли глПараметрыРМ.ФирмыРМ.Найти(Фирма) = Неопределено  Тогда
		ОформлениеСтроки.ЦветТекста = ЦветаСтиля.ЦветТекстаСтрокиЧужойФирмы;
	КонецЕсли;

		
	Колво = СтрокаТовара.Количество;
	Если СтрокаТовара.Уровень() = 0 Тогда
		Статус = СтрокаТовара.Статус;
	Иначе
		Статус = СтрокаТовара.Родитель.Статус;
	КонецЕсли;                                   		
	...
	ЦВ = ОформлениеСтроки.Ячейки.Товар.ЦветФона;
	ЦветВыделеннойСтандарт = Новый Цвет(ЦВ.Красный - 20, ЦВ.Зеленый - 20, ЦВ.Синий - 20);
	ЦветВыделенной = ЦветВыделеннойСтандарт;
	ЦветВыделенной_Статус = Новый Цвет(ЯчейкаСтатус.ЦветФона.Красный - 15, ЯчейкаСтатус.ЦветФона.Зеленый - 15, ЯчейкаСтатус.ЦветФона.Синий - 15);
	Если СтрокаТекТовара = ДанныеСтроки Или (ДанныеСтроки.Уровень() И СтрокаТекТовара = ДанныеСтроки.Родитель) Тогда
		Для Каждого Ячейка Из ОформлениеСтроки.Ячейки Цикл
			Если Ячейка = ЯчейкаСтатус Тогда
				Если ЯчейкаСтатус.ЦветФона.Красный = -1 Тогда
					ЦветВыделенной = ЯчейкаСтатус.ЦветФона;	
				Иначе
					ЦветВыделенной = Новый Цвет(ЯчейкаСтатус.ЦветФона.Красный - 20, ЯчейкаСтатус.ЦветФона.Зеленый - 20, ЯчейкаСтатус.ЦветФона.Синий - 20);		
				КонецЕсли;
			ИначеЕсли	Ячейка = ЯчейкаСумма Тогда
				Если ЯчейкаСумма.ЦветФона.Красный = -1 Тогда
					ЦветВыделенной = ОформлениеСтроки.Ячейки.Товар.ЦветФона;
				Иначе                                    					
					ЦветВыделенной = Новый Цвет(ЯчейкаСумма.ЦветФона.Красный - 20, ЯчейкаСумма.ЦветФона.Зеленый - 20, ЯчейкаСумма.ЦветФона.Синий - 20);
				КонецЕсли;
				
			ИначеЕсли  СтрокаТекТовара = ДанныеСтроки Тогда
					ЦветВыделенной = ЦветВыделеннойСтандарт;	
			Иначе
				ЦветВыделенной = Ячейка.ЦветФона;
				Если ЦветВыделенной.Красный+ЦветВыделенной.Зеленый+ЦветВыделенной.Синий <= 0 Тогда
					ЦветВыделенной = ЦветВыделеннойСтандарт;
				КонецЕсли;
			КонецЕсли;
			Ячейка.ЦветФона = ЦветВыделенной;
		КонецЦикла;
		
		//ОформлениеСтроки.Ячейки.НомерСтроки.ЦветФОна = 
		Элемент.ЦветФонаВыделения = ЦветВыделенной;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ДанныеСтроки.ИдСвязаннойСтроки) И ДанныеСтроки.Строки.Количество() Тогда
		СвязаннаяСтрока = Заказ.Товары.Найти(ДанныеСтроки.ИдСвязаннойСтроки, "ИдСтроки");
		Если Не СвязаннаяСтрока = Неопределено Тогда
			ОформлениеСтроки.Ячейки.Товар.УстановитьТекст("Готовим " + СвязаннаяСтрока.Товар);
		КонецЕсли;
		
	КонецЕсли;
	
	
	ОформлениеСтроки.Ячейки.Модифицирована.ОтображатьТекст = 0;
	...
	Если ЭтоРМСтанция Тогда
		Если (ДанныеСтроки.Количество > 1 Или (ДанныеСтроки.Количество и ДанныеСтроки.Уровень() = 0) ) И Цел(ДанныеСтроки.Количество) = ДанныеСтроки.Количество Тогда
			ОформлениеСтроки.Ячейки.Количество.УстановитьТекст(Формат(ДанныеСтроки.Количество,"ЧДЦ=0"));
		КонецЕсли;
	КонецЕсли;
...
Процедура ИнициализацияРеквизитов()
	...
	ЦветДругое		= Новый Цвет(245,245,245);	// белый
...
Процедура МониторМарок() Экспорт
	...
	Если ТекущийРежим = "ПРОДАЖА" Тогда
		Если ЭтоРМСтанция Тогда
			МониторМарок.СтанцияОтбор = глПараметрыРМ.Станция;	
		КонецЕсли;
...
Процедура ОбновитьИнформациюОВыданныхПейджерах() Экспорт
	Если глПараметрыРМ.ЭтоОтдохни Тогда
		ГлавнаяФорма.ЭлементыФормы.кнПейджер.Видимость = Ложь;
	Иначе
...
Процедура РецептыГостя() Экспорт 
      ИнтерфейсРМЛояльность.ДействиеРецептыГостя(ГлавнаяФорма);  
КонецПроцедуры

Процедура ПроверитьБлюдаДляПриготовления() Экспорт
	
	тпТовары = ГлавнаяФорма.тпТовары;
	фЗаписать = Ложь;
	//:тпТовары = Новый ДеревоЗначений;
	ДанныеСтрок = тпТовары.Скопировать();
	Для Каждого Т Из ДанныеСтрок.Строки Цикл
		Если Т.Статус = Перечисления.СтатусыПозицийЗаказа.ДляПриготовления И не ЗначениеЗаполнено(Т.ИдСвязаннойСтроки) Тогда
			стУслуга = ТоварыДляПриготовления.Найти(Т.Товар,  "Товар");
			Если стУслуга = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			Услуга = стУслуга.Услуга;
			Станция = ОбщегоНазначенияПовтИсп.СтанцияТовара(стУслуга.Услуга);
			Если Не Станция = глПараметрыРМ.Станция Тогда
				Продолжить;
			КонецЕсли;
			
			ТекстВопроса = СтрШаблон("У гостя есть товар для приготовления:
			|%1 x%2
			|Оформить приготовление?", СокрЛП(Т.Товар), ""+Т.Количество + " " + Т.Товар.ЕдиницаИзмерения);	
			СтрокаТ = ГлавнаяФорма.тпТовары.Строки.Найти(Т.идСтроки);
			Если ИнтерфейсРМ.ВопросПредупреждение("" + Услуга, "Вопрос гостю", ТекстВопроса,  "Оформить", , "Отказаться") = "Оформить" Тогда
				МинКоличествоСпецифики = Т.Количество / стУслуга.Кратность;
				Если МинКоличествоСпецифики - Цел(МинКоличествоСпецифики) Тогда
					МинКоличествоСпецифики = МинКоличествоСпецифики + 1;
				КонецЕсли;
				МинКоличествоСпецифики = Цел(МинКоличествоСпецифики);
				идСтроки = ВводНовойСтроки(Услуга, 1,,Истина,Строкат.идСтроки, стУслуга.Специфика, МинКоличествоСпецифики);
				СтрокаТ = ГлавнаяФорма.тпТовары.Строки.Найти(Т.идСтроки);
			
				Если ЗначениеЗаполнено(идСтроки) Тогда                   					
					СтрокаТ.ИдСвязаннойСтроки = идСтроки;
					СтрокаТ.Модифицирована = 1;
					ЗаписатьЗаказТоварыДопИнф();
					фЗаписать = Истина;
				Иначе
					СтрокаТ.Статус = Перечисления.СтатусыПозицийЗаказа.Выдано;
					СтрокаТ.Модифицирована = 1;
					ЗаписатьЗаказТоварыДопИнф();					
					фЗаписать = Истина;
				КонецЕсли;
			Иначе
				...
				СтрокаТ.Модифицирована = 1;
				ЗаписатьЗаказТоварыДопИнф();
				локЗаказ = Заказ.Ссылка.ПолучитьОбъект();
				фЗаписать = Истина;
			КонецЕсли; 
		КонецЕсли;
		
	КонецЦикла;
	Если фЗаписать Тогда
...
ТекущийРежим = "";
ДопПараметрыИнфо	= Новый Структура("НомерСтрокиЗаказа, СуммаПоСтанцииНач, СброситьНастройки, ЛояльностьДанныеЗаказа, ФирмаФильтр");
...
Показатель_ДобавлениеСтроки 			= КонтрольныеПоказатели.КодПоказателя("Добавление строки");
Показатель_ПечатьЧеков 					= КонтрольныеПоказатели.КодПоказателя("Печать чеков");
Показатель_ОбсчетЛОЛ						= КонтрольныеПоказатели.КодПоказателя("Оперативный обсчет заказа лояльностью");
Показатель_УТМ 							= КонтрольныеПоказатели.КодПоказателя("УТМ");
Показатель_ОбновлениеМонитораГостя  = КонтрольныеПоказатели.КодПоказателя("Обновление монитора гостя");

фУстанавливатьТекСтроку = Истина;
