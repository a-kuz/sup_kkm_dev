...
Функция РМсНевыполненнойПроверкой(НаДату)
	Отказ = Ложь;
	ТекстОшибки = "";
...
&НаСервере
Функция УстановитьФлагУтвРезультат(РабочееМесто) Экспорт
	//:РабочееМесто = Справочники.РабочиеМеста.ПустаяСсылка();
	Отказ = Ложь;
	ТекстОшибки = "";
	ТекстЗапроса = СтрШаблон("
	|UPDATE КонтрольныеПроверки_Строки
	|	SET [КонтрольныеПроверки_Строки].[УтвРезультат] = 'Выполнена'	
	|FROM
	|	dbo.[КонтрольныеПроверки] AS КонтрольныеПроверкиТ
	|	LEFT JOIN dbo.[КонтрольныеПроверки_Строки] AS КонтрольныеПроверки_Строки
	|	ON КонтрольныеПроверкиТ.ид_строки = КонтрольныеПроверки_Строки.ид
	...
	|	AND [Оборудование] = %2
	|SELECT 'OK'
	|", Формат(НаДату,"ДФ=yyyyMMdd"), РабочееМесто.НомерККМ);
	
	стрРМ = "";
	ВыборкаРегионы = Справочники.Регионы.Выбрать();
	
	Пока ВыборкаРегионы.Следующий() Цикл
		Если ВыборкаРегионы.Ссылка <> РабочееМесто.ИнформационнаяБаза.Регион Тогда
			Продолжить;
		КонецЕсли;
		Соединение = SQL.ПодключитьсяКСУПцентр(ВыборкаРегионы.Ссылка, "raz_ext_ref");
		SQL.ВыполнитьЗапрос(Соединение, ТекстЗапроса, Отказ, ТекстОшибки);
		//стрРМ = стрРМ + Символы.ПС + СтрСоединить(Результат.ВыгрузитьКолонку(0), Символы.ПС);
		Возврат Не Отказ;
	КонецЦикла;
	Возврат Ложь;	
...
Процедура ОбновитьДанныеКонтрольныхПроверок(Команда)
	...
	МассивРМ = РМсНевыполненнойПроверкой(НаДату);	
	Для Каждого РМ Из МассивРМ Цикл
		...
		//НачатьПроверкуФайла(Нов.ПутьКлокальнойКопии, Нов);
	КонецЦикла;
...
Функция ОбработатьСледующееРМнаСервере()
	...
	Для Каждого Т Из ТаблицаРабочихМест Цикл
		...
		Если Т.ОбработанНаКлиенте Тогда
			Продолжить;
		КонецЕсли;
		...
		Если Сеть.ПодключитьШару(Хост) Тогда
		...
		Т.Обработан = Истина;
		Результат = Истина;
		Если Не ЗначениеЗаполнено(Т.ОписаниеПроблемы) Тогда
			Если ЗначениеЗаполнено(Т.РабочееМесто) Тогда
				УстановитьФлагУтвРезультат(Т.РабочееМесто)
			КонецЕсли;                                    			
		КонецЕсли;
...
&НаКлиенте
Процедура НачатьПроверкуФайла(Путь, СтрокаТЗ = Неопределено, СтрокаДерева = Неопределено)
	Если СтрокаТЗ = Неопределено и СтрокаДерева <> Неопределено Тогда
		СтрокаТЗ = СтрокаДерева;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СтрокаТЗ.РабочееМесто) Тогда
		Хост = сеть.ИмяХостаРМ(СтрокаТЗ.РабочееМесто, Истина);
		ЭтоСервер = Ложь;
	Иначе
		Хост = СтрокаТЗ.ИБ.СерверХост;
		ЭтоСервер = Истина;
	КонецЕсли;
	
	Если Сеть.ПодключитьШару(Хост, ЭтоСервер) Тогда
		
		Файл = Новый Файл(Путь);
		Файл.НачатьПроверкуСуществования(Новый ОписаниеОповещения("ЗакончитьПроверкуФайла", ЭтотОбъект, Новый Структура("Файл, СтрокаТЗ, СтрокаДерева",Файл, СтрокаТЗ, СтрокаДерева)));
	Иначе
		СтрокаТЗ.КаталогДоступен = Ложь;
		СтрокаТЗ.ОписаниеПроблемы = "Каталог локальной копии недоступен.";
		СтрокаТЗ.ОбработанНаКлиенте = Истина;
...
&НаКлиенте
Процедура ЗакончитьПроверкуФайла(ФайлСуществует, ДополнительныеПараметры) Экспорт
	Файл = ДополнительныеПараметры.Файл; //:Файл = Новый Файл;
	Если ФайлСуществует Тогда
		СтрокаТЗ = ДополнительныеПараметры.СтрокаТЗ;
		...
		Если СтрокаТЗ <> Неопределено Тогда
			Т = СтрокаТЗ;
		Иначе
			Сообщить(Файл.ПолноеИмя + ?(ФайлСуществует, " ", " не ") + "существует");
			Возврат;
		КонецЕсли;
		...
		Если ФайлСуществует Тогда			
			...
			КонецЕсли;
		Иначе
			Т.КаталогДоступен = Ложь;
			Т.ОписаниеПроблемы = "Каталог локальной копии недоступен.";
		КонецЕсли;
		Т.ОбработанНаКлиенте = Истина;
		Т.Обработан = Истина;
		Если Не ЗначениеЗаполнено(Т.ОписаниеПроблемы) Тогда
			Если ЗначениеЗаполнено(Т.РабочееМесто) Тогда
				УстановитьФлагУтвРезультат(Т.РабочееМесто)
...
&НаКлиенте
Процедура ПроверитьФайл(Команда)
	Если Элементы.Страницы.ТекущаяСтраница = Элементы.ГруппаДеревоРМ Тогда
		ДС = Элементы.ДеревоРМ.ДанныеСтроки(Элементы.ДеревоРМ.ТекущаяСтрока);
	Иначе
		ДС = Элементы.ТаблицаРабочихМест.ТекущиеДанные;
	КонецЕсли;
	
	Если Не ДС = Неопределено Тогда
		Путь = ДС.ПутьКлокальнойКопии;
		Если Элементы.Страницы.ТекущаяСтраница = Элементы.ГруппаДеревоРМ Тогда
			НачатьПроверкуФайла(Путь,,ДС);
		Иначе
			НачатьПроверкуФайла(Путь,ДС);
		КонецЕсли;
...
&НаСервереБезКонтекста
Процедура СкопироватьЛокальнуюКопиюСсервераТТнаРМНаСервере(ИБ, РабочееМесто)
	ФоновыеЗадания.Выполнить("Сеть.СкопироватьЛокальнуюКопиюСсервераТТнаРМ",Массив(ИБ, РабочееМесто), Строка(ИБ)+">"+Строка(РабочееМесто),"Копирование " + Строка(ИБ)+">"+Строка(РабочееМесто));
КонецПроцедуры

&НаКлиенте
Процедура СкопироватьЛокальнуюКопиюСсервераТТнаРМ(Команда)
	ДС = Элементы.ДеревоРМ.ДанныеСтроки(Элементы.ДеревоРМ.ТекущаяСтрока);
	Если Не ДС = Неопределено Тогда
		СкопироватьЛокальнуюКопиюСсервераТТнаРМНаСервере(ДС.ИБ, ДС.РабочееМесто);
	КонецЕсли;
КонецПроцедуры

&НаСервере
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	НаДату = ТекущаяДата()-4000;
КонецПроцедуры
