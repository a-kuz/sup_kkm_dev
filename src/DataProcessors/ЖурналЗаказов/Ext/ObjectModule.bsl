...
Процедура СоздатьСтруктуруЖурнала() Экспорт
	...
	ИначеЕсли НЕ СводныйЖурнал И НЕ ДоставкаЖурнал Тогда
		...
		ЖурналЗаказов.Колонки.Добавить("Время",,,6);
		ЖурналЗаказов.Колонки.Добавить("Пиктограмма",," ",3);
		ЖурналЗаказов.Колонки.Добавить("Номер",,,8);
		// < КС_ВДВ ------------------------------------------------------------ 
		ЖурналЗаказов.Колонки.Добавить("НомерКарты",,"№ карты",8);
		//Если глПараметрыРМ.ЗКП Тогда
		//
		//	ЖурналЗаказов.Колонки.Добавить("ФИО",,"ФИО",18);

		//
		//КонецЕсли; 
		ЖурналЗаказов.Колонки.Добавить("Пейджер",ПолучитьОписаниеТиповЧисла(2,0),"Пейджер",7);
		ЖурналЗаказов.Колонки.Добавить("Приготовление",ПолучитьОписаниеТиповЧисла(2,0),"Готовится",7);
		ЖурналЗаказов.Колонки.Добавить("КВыдаче",ПолучитьОписаниеТиповЧисла(2,0),"К выдаче",7);
		...
		//----------------------------------------------------------
		ТабПоле.РежимВыделенияСтроки = РежимВыделенияСтрокиТабличногоПоля.Строка;
...
Процедура ЗаполнитьПростойЖурнал(ТекДок, ВыбОтбор=Неопределено, ПодстрокаФильтра=Неопределено) 
	...
	Если РежимНедозакрытых Тогда
		...
		|// < КС_ВДВ ------------------------------------------------------------ 
		|	ПротоколРасчетов.Заказ.КартаДоступа.Код	КАК НомерКарты,
		|	ВЫБОР
	...
	Иначе
		...
		|	ЗаказДопИнф.ДатаОткрытия			КАК ДокДата,
		|	ЗаказДопИнф.Заказ.Автор				КАК Автор,
		|// < КС_ВДВ ------------------------------------------------------------ 
		|	ЗаказДопИнф.Заказ.КартаДоступа.Код	КАК НомерКарты,
		|	ВЫБОР
		|		КОГДА ЗаказДопИнф.Заказ.КартаДоступа.ТипКарты = ЗНАЧЕНИЕ(Перечисление.ТипыКартДоступа.Постоянная)
		|			ТОГДА ЗаказДопИнф.Заказ.Клиент.Наименование
		|		ИНАЧЕ """"
		...
		|	ВЫБОР КОГДА ЗаказДопИнф.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказа.Открыт) ТОГДА 0 ИНАЧЕ выбор когда не ВТПейджеры.Заказ есть null тогда 7 иначе 0 конец КОНЕЦ	КАК Пиктограмма,
		|		выбор когда 
		|ЗаказДопИнф.Заказ.ИтоговаяСумма > 0 тогда 
		|ДОБАВИТЬКДАТЕ(ЗаказДопИнф.ДатаОткрытия, день, -2) иначе  ЗаказДопИнф.ДатаОткрытия конец		
		|ИЗ
		|	РегистрСведений.ЗаказДопИнф КАК ЗаказДопИнф
		|// < КС_ВДВ ------------------------------------------------------------ 
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТПейджеры КАК ВТПейджеры
		...
		|	И ЗаказДопИнф.Заказ.Автор = &Автор", "") + "
		|"+?(ЗначениеЗаполнено(ВыбОтбор), "
		|	И ЗаказДопИнф.ЗначОтбор = &ВыбОтбор", "") + "
		|
		// Список открытых заказов на доставку
		|"+?(глВерсия = 3, " 
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ
		...
		|	ВЫБОР КОГДА ДоставкаДопИнфСрезПоследних.Заказ.ЕстьПресчет ТОГДА 1 ИНАЧЕ 0 КОНЕЦ	КАК Пиктограмма,0
		|ИЗ
		...
		|"+?(ЗначениеЗаполнено(ВыбАвтор), "
		|	И ДоставкаДопИнфСрезПоследних.Заказ.Автор = &Автор", "") + "
		|"+?(ЗначениеЗаполнено(ВыбОтбор), "
