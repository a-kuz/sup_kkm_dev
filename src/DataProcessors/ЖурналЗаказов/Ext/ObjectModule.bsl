...
Процедура СоздатьСтруктуруЖурнала() Экспорт
	...
	ИначеЕсли НЕ СводныйЖурнал И НЕ ДоставкаЖурнал Тогда
		...
		ЖурналЗаказов.Колонки.Добавить("Номер",,"Заказ №",5);
		ЖурналЗаказов.Колонки.Добавить("НомерКартыЛояльности",,"Карта Кегельбум",9);
		ЖурналЗаказов.Колонки.Добавить("НомерКарты",,"Бейдж №",5);
		ЖурналЗаказов.Колонки.Добавить("Пейджер",ПолучитьОписаниеТиповЧисла(2,0),"Пейджер",3);
		ЖурналЗаказов.Колонки.Добавить("Приготовление",ПолучитьОписаниеТиповЧисла(2,0),"Готовится",3);
		ЖурналЗаказов.Колонки.Добавить("КВыдаче",ПолучитьОписаниеТиповЧисла(2,0),"К выдаче",3);
		...
		ЖурналЗаказов.Колонки.Добавить("ВремяБездействия",ПолучитьОписаниеТиповСтроки(5),"Бездействие, мин",4);
		ЖурналЗаказов.Колонки.Добавить("МестоПоследнейАктивности",,"Место последней активности",7);
		Если глПараметрыРМ.ТипТТ = Справочники.ТипыТТ.МОКП ИЛИ глПараметрыРМ.ТипТТ = Справочники.ТипыТТ.КМ Тогда
			...
			ЖурналЗаказов.Колонки.Добавить("ОплаченоМЪ",ПолучитьОписаниеТиповЧисла(12,2),,7);
			ЖурналЗаказов.Колонки.Добавить("ОплаченоО",ПолучитьОписаниеТиповЧисла(12,2),,7);
			ЖурналЗаказов.Колонки.Добавить("ОплаченоКП",ПолучитьОписаниеТиповЧисла(12,2),,7);
		Иначе
		...
		Если глПараметрыРМ.ТипТТ = Справочники.ТипыТТ.МОКП ИЛИ глПараметрыРМ.ТипТТ = Справочники.ТипыТТ.КМ Тогда
			ТабПоле.Колонки.Дата.Видимость = Ложь;
			ТабПоле.Колонки.ОплаченоКП.Видимость = Ложь;
			ТабПоле.Колонки.ОплаченоО.Видимость = Ложь;
			ТабПоле.Колонки.ОплаченоМЪ.Видимость = Ложь;
		КонецЕсли;
...
Процедура ОбновитьСтатистику() Экспорт
	...
	|		ПО ОткрытыеЗаказыПоКартам.Заказ = ЗаказТовары.Ссылка
	|ГДЕ
	|	НеоплаченныеОткрытые.Заказ ЕСТЬ NULL и ЗаказТовары.Ссылка IS NOT NULL
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
Процедура ЗаполнитьПростойЖурнал(ТекДок, ВыбОтбор=Неопределено, ПодстрокаФильтра=Неопределено) 
	...
	Иначе
		...
		|	(Выбор РАЗНОСТЬДАТ(ВТЗаказы.ВремяПоследнейАктивности, &ТекВремя, ДЕНЬ)   
		|		Когда 0 тогда РАЗНОСТЬДАТ(ВТЗаказы.ВремяПоследнейАктивности, &ТекВремя, МИНУТА) 
		|		Иначе 999 конец)
		|		КАК ВремяБездействия,
		...
		|	СуммыПоМестам.ОплаченоМЪ КАК ОплаченоМЪ,
		|	СуммыПоМестам.ОплаченоО КАК ОплаченоО,
		|	СуммыПоМестам.ОплаченоКП КАК ОплаченоКП
		|ИЗ
		...
		|					КОГДА ЗНАЧЕНИЕ(справочник.МестаРеализации.Мяснов)
		|						ТОГДА ВЫБОР
		|								КОГДА ЕСТЬNULL(ЗаказТовары.ДокументОплаты.Фискализирован, 0) = 0
		|									ТОГДА 0
		|								ИНАЧЕ ЗаказТовары.СуммаРеализации
		|							КОНЕЦ
		|				КОНЕЦ) КАК ОплаченоМЪ,
		|			СУММА(ВЫБОР ЗаказТовары.Фирма.МестоРеализации
		|					КОГДА ЗНАЧЕНИЕ(справочник.МестаРеализации.Отдохни)
		|						ТОГДА ВЫБОР
		|								КОГДА ЕСТЬNULL(ЗаказТовары.ДокументОплаты.Фискализирован, 0) = 0
	...
	Пока Выборка.Следующий() Цикл
		...
		//СтрокаЖурнала.ВремяБездействия = (ТекущаяДатаСеанса()- Выборка.ВремяПоследнейАктивности)/60);
		СтрокаЖурнала.ВремяБездействия = Выборка.ВремяБездействия;
