...
Процедура КнопкаУдалитьНажатие(Элемент)
	...
	ТС = ЭлементыФормы.ЖурналЗаказов.ТекущаяСтрока;
	Если ТС = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если ТС.ОплаченоМъ Или ТС.ОплаченоКП Или ТС.ОплаченоО Тогда
		ИнтерфейсРМ.ВопросПредупреждение("Ошибка","Нет доступа","заказ имеет оплаченные строки",,"ОК","");
		Возврат;                                                                                          	
	КонецЕсли;
	...
	Заказ.ПолучитьОбъект().РазблокироватьРедактированиеПоФирме(глПараметрыРМ.Фирма);
	// можно удалять, если не передумают
	Текст1 = "Удаление заказа";
	Текст2 = "Удалить заказ?";
	Если ИнтерфейсРМ.ВопросПредупреждение("Вопрос",Текст1,Текст2,"Да","","Нет")="Нет" Тогда
		Возврат;
	КонецЕсли;
	

	ИнтерфейсРМ.ЗаписьСобытия(Справочники.События.ЗаказУдалениеНеПустого, Заказ, УбратьВсеБуквы(Заказ.Номер));
	ксТрактир.ОсвободитьКартуДоступаОтЗаказа(Заказ.Ссылка);
	ксТрактир.СнятьМаркиСУчета("Заказ", Массив(Заказ));
	ксТрактир.ВозвратПейджеровПоЗаказам(Массив(Заказ),Неопределено,Ложь);			
...
Процедура УстановитьВидимостьКнопок() Экспорт
	Если ЭлементыФормы.ПанельЖурналы.ТекущаяСтраница = ЭлементыФормы.ПанельЖурналы.Страницы.ОткрытыеЗаказы Тогда
		...
		ЭлементыФормы.КнопкаВозврат.Видимость = Истина;	
		ЭлементыФормы.КнопкаУдалить.Видимость = Истина;	
		ОбновитьЖурнал();
	Иначе
		...
		ЭлементыФормы.КнопкаВозврат.Видимость = Ложь;	
...
Процедура КнопкаВыдачаНажатие(Элемент)
	...
	Заказ = ТС.ДокСсылка;
	ЕстьМарки = ксТрактир.ПоЗаказуЕстьМарки(Заказ);
	Если ЕстьМарки Тогда
		БлюдоВыдали = ИнтерфейсРМ.ВопросПредупреждение("Вопрос гостю","Есть невыданные блюда","Удалить марки?", "Да","Нет","",,,,,Заказ);	
		...
		КонецЕсли;		
	Иначе
		ИнтерфейсРМ.ВопросПредупреждение("Марки","Нет марок","По заказу нет открытых марок", "","Ок","",,,,,Заказ);		
	КонецЕсли;
...
Процедура КнопкаПейджерНажатие(Элемент)
	...
	ТаблицаВыданных = ксТрактир.ВыданныеПейджерыПодробно(Заказ);
	
	МассивЗаказов = Массив(Заказ);
	ПейджерыСданы = (ТаблицаВыданных.Количество() = 0);
	
	Если ПейджерыСданы Тогда
		Текст1="Нет невозвращенных!";
		Текст2="По заказу " + УбратьВсеБуквы(Заказ.Номер) + " нет невозвращенных пейджеров!";
