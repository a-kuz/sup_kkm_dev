...
Перем СтанцияОтбор Экспорт;			// текущий фильтр по стнации
...
Процедура ИнициализацияРеквизитов()
	...
	СтанцияОтбор		= ?(ЗначениеЗаполнено(глРабочееМесто.Станция) и (глПараметрыРМ.Тип = Перечисления.ТипыРМ.Производство или глПараметрыРМ.Тип = Перечисления.ТипыРМ.СтанцияПовараМОКП), глРабочееМесто.Станция, Неопределено);
	...
	ЦветФонаМаркиКвыдаче = ЦветФонаМаркиКвыдаче;
	ПараметрыНастройки.Свойство("ЦветФонаМаркиКвыдаче", ЦветФонаМаркиКвыдаче);
	Если ЦветФонаМаркиКвыдаче = Неопределено Тогда
		ЦветФонаМаркиКвыдаче =  Новый Цвет(179,172,134);
	КонецЕсли;
	...
	ОчередьМарок.Колонки.Добавить("Номерок");
	// < КС_ВДВ ------------------------------------------------------------ 
...
Процедура ОбновитьТаймер()
	...
	Для н = 1  По КолвоМарокНаЭкране Цикл
		...
		Попытка
			...
			// часики по блюдам
			ЭлементыФормы["СоставМаарки"+н].ОбновитьСтроки();
		Исключение
		КонецПопытки;
	КонецЦикла; 
...
Процедура ОбновитьОчередьМарок()
	...
	|		ПО (ТоварыЗаказа.Ссылка = ЗаказТоварыДопИнф.Заказ)
	|			И (ТоварыЗаказа.ИдСтроки = ЗаказТоварыДопИнф.ИдСтроки)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЗаказДопИнф КАК ЗаказДопИнф
	|		ПО (ТоварыЗаказа.Ссылка = ЗаказДопИнф.Заказ)
	|ГДЕ
	...
	|	МАКСИМУМ(СписокТоваров_.Номерок) КАК Номерок,
	...
	|	КОНЕЦ КАК Поле1,
	|	СвязанныеТовары.Товар КАК СвязанныйТовар
	|ПОМЕСТИТЬ СписокТоваров
	|ИЗ
	|	СписокТоваров_ КАК СписокТоваров_
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.Заказ.Товары КАК СвязанныеТовары
	|		ПО СписокТоваров_.Заказ = СвязанныеТовары.Ссылка
	|			И СписокТоваров_.ИдСвязаннойСтроки = СвязанныеТовары.ИдСтроки
	|
	|СГРУППИРОВАТЬ ПО
	...
	|		ИНАЧЕ СписокТоваров_.НомерСтроки
	|	КОНЕЦ,
	|	СвязанныеТовары.Товар
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СписокТоваров.Заказ КАК Заказ,
	|	СписокТоваров.Номерок КАК Номерок,
	|	СписокТоваров.ИдСтроки КАК ИдСтроки,
	|	СписокТоваров.ВремяДозаказ КАК ВремяДозаказ,
	|	СписокТоваров.ВремяЗаказано КАК ВремяЗаказано,
	|	СписокТоваров.ВремяГотово КАК ВремяГотово,
	|	СписокТоваров.ВремяВыдано КАК ВремяВыдано,
	|	СписокТоваров.Статус КАК Статус,
	|	СписокТоваров.КодПодтверждения КАК КодПодтверждения,
	|	СписокТоваров.РабочееМесто КАК РабочееМесто,
	|	СписокТоваров.Подача КАК Подача,
	|	СписокТоваров.ГруппаПечати КАК ГруппаПечати,
	|	СписокТоваров.АвторЗаказано КАК АвторЗаказано,
	|	СписокТоваров.АвторГотово КАК АвторГотово,
	|	СписокТоваров.Товар КАК Товар,
	|	МАКСИМУМ(СписокТоваров.НомерСтроки) КАК НомерСтроки,
	|	СписокТоваров.Количество КАК Количество,
	|	СписокТоваров.КоличествоУдалено КАК КоличествоУдалено,
	|	СписокТоваров.ИдСтрокиВладельца КАК ИдСтрокиВладельца,
	|	СписокТоваров.НомерМарки КАК НомерМарки,
	|	СписокТоваров.ВремяМаркиГр КАК ВремяМаркиГр,
	|	СписокТоваров.Порядок КАК Порядок,
	|	СписокТоваров.СвязанныйТовар КАК СвязанныйТовар
	|ИЗ
	|	(ВЫБРАТЬ
	|		СписокТоваров.Заказ КАК Заказ,
	|		СписокТоваров.Номерок КАК Номерок,
	...
	|		СписокТоваров.АвторГотово КАК АвторГотово,
	|		СписокТоваров.Товар КАК Товар,
	|		СписокТоваров.НомерСтроки КАК НомерСтроки,
	|		СписокТоваров.Количество КАК Количество,
	|		СписокТоваров.КоличествоУдалено КАК КоличествоУдалено,
	|		СписокТоваров.ИдСтрокиВладельца КАК ИдСтрокиВладельца,
	|		СписокТоваров.НомерМарки КАК НомерМарки,
	|		СписокТоваров.ВремяМаркиГр КАК ВремяМаркиГр,
	|		ВЫБОР
	|			КОГДА ТИПЗНАЧЕНИЯ(СписокТоваров.Товар) = ТИП(Справочник.Специфики)
	|					И СписокТоваров.Товар.ЕстьЦена
	|				ТОГДА 1
	|			КОГДА ТИПЗНАЧЕНИЯ(СписокТоваров.Товар) = ТИП(Справочник.Специфики)
	|					И НЕ СписокТоваров.Товар.ЕстьЦена
	|				ТОГДА ВЫБОР
	|						КОГДА СписокТоваров.Товар В ИЕРАРХИИ
	|								(ВЫБРАТЬ
	|									константы.ГруппаОбщихСпецифик
	|								ИЗ
	...
	|		ВЫБОР
	|			КОГДА ТИПЗНАЧЕНИЯ(ЗаказТоварыДопИнф.Товар) = ТИП(Справочник.Специфики)
	|					И ЗаказТоварыДопИнф.Товар.ЕстьЦена
	|				ТОГДА 1
	|			КОГДА ТИПЗНАЧЕНИЯ(ЗаказТоварыДопИнф.Товар) = ТИП(Справочник.Специфики)
	|					И НЕ ЗаказТоварыДопИнф.Товар.ЕстьЦена
	|				ТОГДА ВЫБОР
	|						КОГДА ЗаказТоварыДопИнф.Товар В ИЕРАРХИИ
	|								(ВЫБРАТЬ
	|									константы.ГруппаОбщихСпецифик
	|								ИЗ
	|									Константы КАК константы)
	...
	|				И (ЗаказТоварыДопИнф.ГруппаПечати = ЗНАЧЕНИЕ(Справочник.ГруппыПечати.ПустаяСсылка))) КАК СписокТоваров
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	СписокТоваров.Порядок,
	|	СписокТоваров.Подача,
	|	СписокТоваров.СвязанныйТовар
	|
	|УПОРЯДОЧИТЬ ПО
	|	СписокТоваров.Подача,
	|	СписокТоваров.Порядок,
	...
	СХ = Новый СхемаЗапроса;
	СХ.УстановитьТекстЗапроса(Запрос.Текст);
	Пакеты = СХ.ПакетЗапросов;
	ПервыйПакет = Пакеты[0];
	ОператорВыбрать = ПервыйПакет.Операторы[0];
	Если ЗначениеЗаполнено(ЗаказОтбор) Тогда
		ОператорВыбрать.Отбор.Добавить("ЗаказТоварыДопИнф.Заказ = &Заказ");
	КонецЕсли;
	Если ЗначениеЗаполнено(СтанцияОтбор) Тогда
		ОператорВыбрать.Отбор.Добавить("ЗаказТоварыДопИнф.Станция = &Станция");
	КонецЕсли;
	
	Если ПросроченныеОтбор Тогда
		ОператорВыбрать.Отбор.Добавить("ВЫБОР
	...
	ПоследнийПакет.КонтрольныеТочкиИтогов.Добавить(ФильтрПО);
	ПоследнийПакет.ОбщиеИтоги = Ложь;
	
	НовыйТекст = СХ.ПолучитьТекстЗапроса();
	...
	СписокСтатусов = Массив(Перечисления.СтатусыПозицийЗаказа.Заказано, Перечисления.СтатусыПозицийЗаказа.Удалено);
	...
	Для каждого СтрокаДереваМарки Из ДеревоТоваров.Строки Цикл
		...
		Если Марка = Неопределено Тогда
			...
			Марка.Номерок 				= СтрокаДереваМарки.Номерок;
...
Процедура СоставМаркиДобавитьПозиции(Марка, СтрокаДереваТовар, КонтрольноеВремя = 0);
	...
	Если НЕ ТипТовар И СтрокаДереваТовар.Уровень() = 1 Тогда			
		...
		СтрокаМарки.Номер					= СтрокаДереваТовар.НомерМарки;
...
Процедура ЗаполнитьТаблицуГотовых()
	...
	|		ПО ЗаказТоварыДопИнф.Заказ = Марки.Заказ
	|			И ЗаказТоварыДопИнф.НомерМарки = Марки.НомерМарки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.Заказ.Товары КАК ТоварыЗаказа
	|		ПО (ТоварыЗаказа.Ссылка = ЗаказТоварыДопИнф.Заказ)
	|			И (ТоварыЗаказа.ИдСтроки = ЗаказТоварыДопИнф.ИдСтроки)
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ЗаказДопИнф КАК ЗаказДопИнф
	|		ПО ЗаказТоварыДопИнф.Заказ = ЗаказДопИнф.Заказ
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.Заказ.Товары КАК СвязанныеТовары
	|		ПО ЗаказТоварыДопИнф.Заказ = СвязанныеТовары.Ссылка
	|			И ЗаказТоварыДопИнф.ИдСвязаннойСтроки = СвязанныеТовары.ИдСтроки
	|ГДЕ
...
Функция ФорматНаименования(СтрокаДереваТовар)
	...
	Если ТипЗнч(СтрокаДереваТовар.Товар) = Тип("СправочникСсылка.Товары") Тогда
		...
		Если ЗначениеЗаполнено(СтрокаДереваТовар.СвязанныйТовар) Тогда
			ТекНаименование = "Готовим " + ?(ЗначениеЗаполнено(СтрокаДереваТовар.СвязанныйТовар.НаименованиеДляМарки), СтрокаДереваТовар.СвязанныйТовар.НаименованиеДляМарки, СтрокаДереваТовар.СвязанныйТовар.Наименование);
		ИначеЕсли ЗначениеЗаполнено(СтрокаДереваТовар.Товар.НаименованиеДляМарки) тогда
...
Процедура ПоказатьМарки()
	...
	ЭлементыФормы.тКолвоСтраниц.Видимость = ВсегоСтраниц>1;
	ЭлементыФормы.КнопкаЛистатьВперед.Видимость = ТекСтраница < ВсегоСтраниц;
	ЭлементыФормы.КнопкаЛистатьНазад.Видимость = ТекСтраница <> 1;
	...
	Для н = 1 По КолвоМарокНаЭкране Цикл
		...
		Иначе
			...
			Если Марка.Состав.Количество() Тогда
				ЭлементыФормы["тНадписьОплачено"+н].Видимость = СтрокаОплачена(Марка.Заказ, Марка.Состав[0].ИдСтроки);
				ЭлементыФормы["тНадписьОплачено"+н].Заголовок = глСимволРубля; 
			Иначе
				ЭлементыФормы["тНадписьОплачено"+н].Видимость = Ложь;
			КонецЕсли;
			ТабПолеМарки.ЦветФона		= ?(РежимВозврата, ЦветФонаМаркиКвыдаче, Новый Цвет());
			ТабПолеМарки.ЦветФонаПоля	= ?(РежимВозврата, ЦветФонаМаркиКвыдаче, Новый Цвет());
			
			ЭлементыФормы["КнопкаМарки"+н]				.Заголовок = НомерМаркиВОчереди;
			Если ВыбФильтр = "По маркам" Тогда	
				ЭлементыФормы["тНаименованиеМарки"+н]	.Заголовок = "Бейдж " + Марка.Заказ.КартаДоступа.Идентификатор2;
				// < КС_ВДВ ------------------------------------------------------------ 
		...
		Иначе // Режим "Приготовление"
			Если ВыбФильтр <> "По маркам" ИЛИ ОбработкаВсехСтрокМарки Тогда
				...
				Если НЕ ПустаяСтрока(СтрПейджеры) Тогда
					...
					ВыводитьПредупреждениеПейджер=Ложь;
				Иначе
					Марка = ОчередьМарок.Найти(ТабПолеМарки.ТекущаяСтрока.Номер, "Номер");
					Номерок = АЗ(Марка.Заказ,"Номерок");
					Если ЗначениеЗаполнено(Номерок) Тогда
					
						Текст2 = "Необходимо отнести блюда к номерку " + Номерок;	
...
Процедура ОформлениеСтрокиМарки(Элемент, ОформлениеСтроки, ДанныеСтроки) 
	...
	Если НЕ РежимВозврата И ДанныеСтроки.Статус = Перечисления.СтатусыПозицийЗаказа.Удалено Тогда
		ОформлениеСтроки.Ячейки.КонтрольноеВремя.Текст		= "удалено";
		ПолучитьОформленнуюСтроку(ОформлениеСтроки, 3);
		Возврат;
	КонецЕсли;		
	
	// Оформление колонки время приготовления (осталось)
	КолвоСекунд = ДанныеСтроки.КонтрольноеВремя - ТекущаяДата();
	// < КС_ВДВ ------------------------------------------------------------ 
...
Процедура ПриОткрытии()
	...
	ЭтаФорма.ПодключитьОбработчикОжидания("ПоказатьРекламу", 8, 1);
КонецПроцедуры

Процедура ПоказатьРекламу() Экспорт
...
Процедура ПриЗакрытии()
	...
	// < КС_ВДВ ------------------------------------------------------------ 
	ОтключитьВыводНаИнфоДисплей(Истина);
	ЭтаФорма.ОтключитьОбработчикОжидания("ПоказатьРекламу");
...
Процедура КнопкаВозвратПейджераНажатие(Элемент)
	...
	Если ЗначениеЗаполнено(РезОткрытия) И ТипЗнч(РезОткрытия) = Тип("Структура") Тогда
		...
		Заказ = ксТрактир.ЗаказПоКартеДоступа(КартаДоступа);
		Если ЗначениеЗаполнено(Заказ) Тогда
			ксТрактир.ВыполнитьВозвратПейджера(Заказ);	
		КонецЕсли;
...
Функция СтрокаОплачена(Заказ, идСтроки)
	...
	|	И ЗаказТовары.ИдСтроки = &ИдСтроки
	|	И ЗаказТовары.СтатусОплаты = 1");
	Запрос.УстановитьПараметр("Заказ", Заказ);
...
Функция ПолучитьСписокПейджеров(СписокКодовПодтверждений, Станция, ЕстьОплаченные = Ложь, ЕстьДругие = Ложь)
	...
	|	ЗаказТоварыДопИнф.Станция КАК Станция,
	|	ВЫБОР
	|		КОГДА ЗаказТовары.СтатусОплаты = 1
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК Оплачен
	|ПОМЕСТИТЬ ВТМарки
	...
	|		ПО ЗаказТоварыДопИнф.Заказ = ЗаказТовары.Ссылка
	|			И ЗаказТоварыДопИнф.ИдСтроки = ЗаказТовары.ИдСтроки
	|ГДЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТМарки.НомерМарки КАК НомерМарки,
...
Процедура ПриемЗаказаНажатие(Элемент)
	Закрыть();
	глОбработки.ГлавнаяФорма.Заблокировать(Истина);

	//глОбработки.ГлавнаяФорма.СброситьТекущийДокумент();

КонецПроцедуры

Процедура РежимАдминистратораНажатие(Элемент)
	Закрыть();
	глОбработки.главнаяформа.ОткрытьМенюОпераций();
...
ЦветФонаМаркиКвыдаче = Новый Цвет;
БлокироватьПодтверждения = Истина;