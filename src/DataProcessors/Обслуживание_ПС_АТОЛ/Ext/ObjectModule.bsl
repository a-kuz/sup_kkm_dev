...
Функция ОплатитьПлатежнойКартой(ВходныеПараметры,ВыходныеПараметры)
	...
	Если Отдел <> Неопределено Тогда // оплата по отделам
		Попытка
			Ответ = DRV.ОплатитьПлатежнойКартойНомОтдела(Сумма * 100, Отдел, СсылочныйНомер, КодАвторизации, НазваниеКарты, КодХеш, КартаСбербанка, НомерКарты);
		Исключение
			ВыходныеПараметры.Очистить();
	...
	Иначе
		Ответ = ПолучитьСтрокиСлипЧека(СсылочныйНомер,ВыходныеПараметры);
		Если Ответ Тогда
...
Функция ПолучитьСтрокиСлипЧека(КодХеш = Неопределено,ВыходныеПараметры)
	
	СлипЧек = "";
	
	КолСтрокСлип = DRV.КоличествоСтрокСлипЧека;
	Если КодХеш <> Неопределено и КолСтрокСлип = 0 Тогда
		Очистить = истина;
		ИмяФайла = "c:\sup_kkm_addins\p";
		ФайлР = Новый Файл(ИмяФайла);
		Если ФайлР.Существует() Тогда
			Текст = Новый ТекстовыйДокумент;
			Текст.Прочитать(ИмяФайла,КодировкаТекста.OEM);
			Для к = 1 по Текст.КоличествоСтрок() Цикл
				СтрокаФайла = Текст.ПолучитьСтроку(к);
				Если КодСимвола(СтрокаФайла) = 22 Тогда
					СлипЧек = СлипЧек + Символы.ПС + "[отрезка]";
				ИначеЕсли СтрНайти(СтрокаФайла,КодХеш) <> 0 Тогда
					Очистить = Ложь;
				Иначе
					СлипЧек = СлипЧек + СтрокаФайла + ?(к = Текст.КоличествоСтрок(), "",Символы.ПС);
				КонецЕсли;
			КонецЦикла;
		Иначе
			ВыходныеПараметры.Очистить();
...
Функция ВернутьПлатежПоПлатежнойКарте(ВходныеПараметры,ВыходныеПараметры)
	...
	Если Отдел <> Неопределено Тогда // оплата по отделам
		Попытка
			Ответ = DRV.ВернутьПлатежПоПлатежнойКартеНомОтдела(Сумма * 100, Отдел, СсылочныйНомер, КодАвторизации, НазваниеКарты, КодХеш, КартаСбербанка, НомерКарты);
		Исключение
			ВыходныеПараметры.Очистить();
			ВыходныеПараметры.Добавить(ОписаниеОшибки());
			Возврат Ложь;
	...
	Если Не Ответ Тогда
		...
		ВыходныеПараметры.Добавить(" " + КодОшибки + ": " + СтрОшибки);
	Иначе
		Ответ = ПолучитьСтрокиСлипЧека(КодХеш,ВыходныеПараметры);
		Если Ответ Тогда
			Слип = ВыходныеПараметры[0];
			ВыходныеПараметры.Очистить();
...
Функция Ошибка()
	...
	ЗарегистрироватьСобытие("Торговое оборудование.Ошибка",	УровеньЖурналаРегистрации.Предупреждение, ,	ТО.Ссылка, Строка(DRV.ResultCode)+" - "+DRV.ResultDescription);
