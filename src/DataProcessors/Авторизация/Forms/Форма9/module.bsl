...
Функция СотрудникПоследнегоЗаказа() Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
	|	Заказ.Автор КАК Автор
	|ИЗ
	|	Документ.Заказ КАК Заказ
	|ГДЕ
	|	Заказ.РабочееМесто = &РабочееМесто
	|	И Заказ.Автор <> ЗНАЧЕНИЕ(Справочник.Сотрудники.ПустаяСсылка)
	|	И (Заказ.Автор.Наименование <> ""Администратор""
	|			ИЛИ &тест)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Заказ.Дата УБЫВ");
...
Процедура ОбновитьСписокПоследних() Экспорт
	...
	Если Не глПоследниеСотрудники.Количество() Тогда
		Попытка
			СотрудникПоследнегоЗаказа = СотрудникПоследнегоЗаказа();
			Если ЗначениеЗаполнено(СотрудникПоследнегоЗаказа) Тогда
				глПоследниеСотрудники.Добавить(СотрудникПоследнегоЗаказа);
			КонецЕсли;
