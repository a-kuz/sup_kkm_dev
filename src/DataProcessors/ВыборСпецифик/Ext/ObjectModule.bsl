...
Процедура ВыводСпецификНаИнфоДисплей(СброситьНастройки = Ложь) Экспорт
	...
	СтрокаЗаказа.Колонки.Добавить("Кл"			, , "№", 5 );		// < КС_ВДВ ------------------------------------------------------------ 
	...
	НовСтрока.Кл = "*";
	НовСтрока.Наименование = Товар.НаименованиеДляМарки;
	НовСтрока.Количество = УдельныйВесТовар;
	НовСтрока.СуммаРеализации = СуммаТовар;
	ДопПараметрыИнфо.Вставить("СтрокаЗаказа", СтрокаЗаказа);
	
	//ДопПараметрыИнфо.Вставить("ГруппаСпецифик", СтрокаЗаказа);
