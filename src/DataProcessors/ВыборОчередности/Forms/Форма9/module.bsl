...
Процедура ПередОткрытием(Отказ, СтандартнаяОбработка)
	...
	ЭлементыФормы.СправочникСписок.Доступность = ВыдатьПейджер;
	...
	КонецЕсли; 
	Если ЗначениеЗаполнено(Пейджер) и ТипЗнч(Пейджер) = Тип("СправочникСсылка.Пейджеры") Тогда
		ЭлементыФормы.СправочникСписок.ИерархическийПросмотр = Ложь;
		Если ЗначениеЗаполнено(глРабочееМесто.НаборПейджеров) Тогда
			СправочникСписок.Отбор.НаборПейджеров.Установить(глРабочееМесто.НаборПейджеров);
		Иначе	
...
Процедура СправочникСписокПриАктивизацииСтроки(Элемент)
	// Вставить содержимое обработчика.
	СтандартнаяОбработка = Ложь;
	Если Не ТипЗнч(Пейджер) = Тип("СправочникСсылка.Номерки") Тогда
		ВыборПейджера();
	КонецЕсли;
	
КонецПроцедуры

Процедура КнопкаНомерокНажатие(Элемент)
	РезультатВвода = ИнтерфейсРМ.ВводЧисла("Номерок", "Число", 3, 0);
	Если Не ЗначениеЗаполнено(РезультатВвода) Тогда
		Возврат;
	КонецЕсли;
	Номерок = Справочники.Номерки.НайтиПоКоду(РезультатВвода);
	Если Номерок.Пустая() Тогда
...
Процедура ДоступностьЭлементов()
	Если ТипЗнч(Пейджер) = Тип("СправочникСсылка.Номерки") Тогда
		
		ЭлементыФормы.СправочникСписок.Видимость = Ложь;
		ЭлементыФормы.КнопкаНомерок.Заголовок = Пейджер;
		ЭлементыФормы.НадписьПейджер.Заголовок = "Номерок ";
		//СправочникСписок.Отбор.Ссылка.Установить(Неопределено, Истина);				
		
	ИначеЕсли ВыдатьПейджер Тогда
		
		ЭлементыФормы.СправочникСписок.Доступность = Истина;
		ЭлементыФормы.СправочникСписок.Видимость = Истина;
		ЭлементыФормы.КнопкаНомерок.Заголовок = "НОМЕРОК";
		ЭлементыФормы.НадписьПейджер.Заголовок = "№ пейджера ";
		//СправочникСписок.Отбор.Ссылка.Установить(,Ложь);
	Иначе
		
		ЭлементыФормы.СправочникСписок.Доступность = Ложь;
		ЭлементыФормы.КнопкаНомерок.Видимость = Ложь;
		ЭлементыФормы.НадписьПейджер.Заголовок = "№ пейджера ";
		СправочникСписок.Отбор.Ссылка.Установить(Пейджер,Истина);

			
	КонецЕсли;
	
	Если ксТрактир.ВыданныеПейджеры(Заказ).Количество() Тогда
		ЭлементыФормы.КнопкаНомерок.Видимость = Ложь;			
	КонецЕсли;
	...
	ВидимостьЛистания = Товары.Количество() > 8;
	
	ЭлементыФормы.КнопкаВверхСписка.Видимость = ВидимостьЛистания;
	ЭлементыФормы.КнопкаВнизСписка.Видимость 	= ВидимостьЛистания;
КонецПроцедуры
