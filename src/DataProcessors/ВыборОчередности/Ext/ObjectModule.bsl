...
Функция ВыполнитьВыборОчередности(ФормаЗаказа, ЗаказСсылка, Знач ТаблицаЗаказа, Знач Станция, ВыданныйПейджер, Знач ТекущийКурс, Знач СтанцияСуммаНач) Экспорт
	...
	Для Каждого ГруппаОчередности Из ГруппыОчередности Цикл
		...
		Если Не РезультатПоСтанции.ВыборСделан Тогда
			Возврат РезультатПоСтанции;
		КонецЕсли;
		Если Не ЗначениеЗаполнено(ВыданныйПейджер) И ЗначениеЗаполнено(РезультатПоСтанции.Пейджер) И Не ТипЗнч(ВыданныйПейджер) = Тип("СправочникСсылка.Номерки") Тогда
			ВыданныйПейджер = РезультатПоСтанции.Пейджер;
		КонецЕсли;
		...
		Если ГруппаОчередности.Станция <> глРабочееМесто.Станция Тогда
			ИнтерфейсРМ.ВопросПредупреждение("Внимание!","Марка на приготовление отправлена на станцию " + ГруппаОчередности.Станция, "Блюдо будет приготовлено на станции " + ГруппаОчередности.Станция,"","ОК","");	
		КонецЕсли;
	КонецЦикла;	
	
	РезультатПоСтанции.Пейджер = ВыданныйПейджер;
...
Функция ВыполнитьВыборОчередностиПоСтанции(ФормаЗаказа, ЗаказСсылка, ТаблицаЗаказа, Знач Станция, ВыданныйПейджер, Знач ТекущийКурс, Знач СтанцияСуммаНач) Экспорт
	...
	Если ФормаВыбора.ВыборСделан Тогда
		...
		Если ВыдатьПейджер Или ТипЗнч(Пейджер) = Тип("СправочникСсылка.Номерки") Тогда
