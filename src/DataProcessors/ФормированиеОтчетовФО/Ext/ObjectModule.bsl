...
// Описание процедуры
//
Функция СформироватьОтчеты(МестоРеализации, ВыводитьСообщения = Истина) Экспорт
	...
	Если ЗначениеЗаполнено(глПараметрыРМ) Тогда
	...
	Если РежимПереформирования Тогда
		...
		Запрос.Текст = "ВЫБРАТЬ Ссылка ИЗ Документ.ОтчетФО
						...
						|	И Дата МЕЖДУ &ДатаНачала И &ДатаКонца
	...
	Пока ВыборкаПоСменам.Следующий() Цикл
		...
		ТаблицаРеализации = Обработки.ФормированиеОтчетовФО.ТаблицаОперативнойРеализации(ВыборкаПоСменам.Смена, 1).Скопировать(,
		...
		Для Каждого ТипРасхода Из ТипыРасхода Цикл
			...
			ОтчетФО.Фирма = Фирма(МестоРеализации);
			...
			Для Каждого Т Из СтрокиСпецифики Цикл
				...
				Иначе
					...
					Нов.Номенклатура = Т.Товар.Номенклатура;
					Нов.ЕдиницаИзмерения = Нов.Номенклатура.БазоваяЕдиницаИзмерения;
					Нов.ИдентификаторТовара = ИдСтроки(Т.ТоварСпецифики, Т.ВариантОплаты, Т.КассоваяСмена);;
