...
Функция СформироватьОтчетыПоФирме(Фирма, ВыводитьСообщения = Ложь) Экспорт
	...
	Запрос.УстановитьПараметр("ДатаКонца",КонецДня(макс(ДатаКонца,ДатаНачала)));
	...
	|
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(Документ.ПротоколРасчетов.ПустаяСсылка),
	...
	Пока ВыборкаПоСменам.Следующий() Цикл
		...
		ТаблицаРеализации = Обработки.ФормированиеОтчетовФО.ТаблицаОперативнойРеализации(ВыборкаПоСменам.Смена, Истина, Фирма).Скопировать(,
							"ШК,Количество,СуммаРозн,СуммаФакт,СтавкаНДСссылка,СуммаНДС,ТипРасхода,ККМ,Товар,ТоварСпецифики,ВариантОплаты,КассоваяСмена,СуммаКорр,ТипТовара,СуммаСпецифики,СуммаФактСпецифики");
		ТаблицаРеализации.Свернуть("ШК,ТипТовара,Товар,СтавкаНДСссылка,ТипРасхода,ККМ,Товар,ТоварСпецифики,ВариантОплаты,КассоваяСмена","Количество,СуммаРозн,СуммаФакт,СуммаНДС,СуммаКорр,СуммаСпецифики,СуммаФактСпецифики");
		...
		Для Каждого ТипРасхода Из ТипыРасхода Цикл
			...
			ОтчетФО.Товары.Свернуть("ШК,Номенклатура,ЕдиницаИзмерения,Контрагент,ВариантОплаты,Идентификатор,ККМ,КассоваяСмена,СтавкаНДС","Количество,Сумма,СуммаБезСкидки,СуммаБонусов,СуммаКорр,СуммаНДС"); 
