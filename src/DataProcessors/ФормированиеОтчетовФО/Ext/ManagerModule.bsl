Функция ТаблицаОперативнойРеализации(Смена, ЧислаЧислами = 0, Фирма) Экспорт
	Если Справочники.МестаРеализации.КМ = Фирма.МестоРеализации Тогда
		Возврат ТаблицаОперативнойРеализацииКМ(Смена, ЧислаЧислами, Фирма);
	КонецЕсли;
	Запрос = Новый Запрос("ВЫБРАТЬ
	|	докПротоколРасчетов.Заказ КАК Заказ,
	...
	|		КОНЕЦ) КАК СуммаФакт,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(тСуммыПоЗаказам.ИтогСуммаФакт, 0) = 0
	|			ТОГДА 1
	|		ИНАЧЕ СУММА(тчПротокол.СуммаФакт) / тСуммыПоЗаказам.ИтогСуммаФакт
	|	КОНЕЦ КАК Доля,
	...
	|		ПО докПротоколРасчетов.Ссылка = тчПротокол.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	|			СУММА(ПротоколРасчетов.ИтогСуммаФакт) КАК ИтогСуммаФакт,
	|			ПротоколРасчетов.Заказ КАК Заказ
	|		ИЗ
	|			Документ.ПротоколРасчетов КАК ПротоколРасчетов
	|		
	|		СГРУППИРОВАТЬ ПО
	|			ПротоколРасчетов.Заказ) КАК тСуммыПоЗаказам
	|		ПО докПротоколРасчетов.Заказ = тСуммыПоЗаказам.Заказ
	|ГДЕ
	...
	|	докПротоколРасчетов.Заказ,
	|	тСуммыПоЗаказам.ИтогСуммаФакт
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	т.Фирма КАК Фирма,
	|	т.ДокументОплаты КАК ДокументОплаты
	...
	|		докЗаказТовары.Фирма,
	|		докЗаказТовары.ДокументОплаты
	|	ИЗ
	...
	|		ВозвратТовары.Товар.КодСУП,
	|		ВозвратТовары.Фирма,
	|		ВозвратДопИнф.ПротоколРасчетов) КАК т
	|
	|СГРУППИРОВАТЬ ПО
	|	т.Заказ,
	|	т.Товар,
	...
	|	ЕСТЬNULL(ТаблицаОплат.Фирма, ТаблицаРеализацииСвернутая.Фирма) КАК Фирма
...
Функция ТаблицаОперативнойРеализацииКМ(Смена, ЧислаЧислами = 0, Фирма)
	...
	|ИЗ
	|	Документ.Заказ.Товары КАК ЗаказТовары
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПротоколРасчетов.Протокол КАК ПротоколРасчетовПротокол
	|		ПО ЗаказТовары.ДокументОплаты = ПротоколРасчетовПротокол.Ссылка
	|			И (ПротоколРасчетовПротокол.НомерСтроки = 1)
	|ГДЕ
	|	ПротоколРасчетовПротокол.Ссылка.Фискализирован
	|	И ПротоколРасчетовПротокол.Ссылка.Смена = &Смена
	|
	|СГРУППИРОВАТЬ ПО
	|	ЗаказТовары.Фирма,
	...
	|	И ВозвратДопИнф.ПротоколРасчетов.Смена = &Смена
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
