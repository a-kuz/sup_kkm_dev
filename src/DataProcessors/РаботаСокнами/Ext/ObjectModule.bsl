...
Процедура ShowMessageEx(ШиринаОкна, ЦветФона, ТекстСообщения, Событие, Данные, ИдСообщения) Экспорт
	ПоказатьПлашкуПрост(" ", ТекстСообщения, , "1000000", 10, 230);
КонецПроцедуры  
...
Процедура Подсказка(Текст) Экспорт
	ПоказатьПлашкуПрост("-", Текст, 5, "3000",0,200);
КонецПроцедуры
...
// Показывает цветной прямоугольник с заголовком и текстом
// Плашка скрывается при клике на нее либо при истечении таймаута
// При наведении курсора мыши на плашку таймаут сбрасывается
//	
// Параметры:
//  ПараметрыПлашки - Структура:
// 							Заголовок 				- Строка - по умолчанию "(:-)"
//								Текст						- Строка	- по умолчанию "" 
// 							Таймаут					- Число  - по умолчанию 2500ms
//								Лево						- Число  - X-координата, px
//								Верх						- Число  - Y-координата, px
// 							Ширина					- Число	- по умолчанию 300px
//								Высота					- Число  - по умолчанию 180px
//								РазмерЗаголовка		- Число  - по умолчанию 14pt
//								РазмерТекста			- Число  - по умолчанию 9pt
// 							Прозрачность			- Число  - по умолчанию 255 (непрозрачное)
// 							ЦветТекста				- Строка - RGB-HEX цвет
// 							ЦветФона					- Строка - RGB-HEX цвет
//								НомерПлашки				- Число  - идентификатор плашки. 
// 																	  Если нужно вывести 2 плашки одновременно, они должны быть с разними номерами плашки.
// 																	  НомерПлашки передается в качестветпараметра функции УдалитьПлашку(НомерПлашки)
//	Все ключи структуры необязательны
Процедура ПоказатьПлашку(ПараметрыПлашки = Неопределено) Экспорт
	МассивПараметров = СтрРазделить("Заголовок,Текст,Таймаут,НомерПлашки,Прозрачность,Лево,Верх,Ширина,Высота,РазмерТекста,РазмерЗаголовка,ЦветФона,ЦветТекста", ",",Ложь);	
	Заголовок = "(:-)"; Текст = ""; НомерПлашки = 0; Таймаут = "2500"; Лево = 0; Верх = 0; Прозрачность = 255; Ширина = 300; Высота = 180; РазмерЗаголовка = 14; РазмерТекста = 9; ЦветФона = Неопределено; ЦветТекста = "000000"; Отладка = Ложь;
	
	Если ПараметрыПлашки = Неопределено Тогда
		ПараметрыПлашки = Новый Структура;
	КонецЕсли;
	
	Для Каждого ПараметрПлашки Из МассивПараметров Цикл
		Если ПараметрыПлашки.Свойство(ПараметрПлашки) Тогда
			Выполнить(СтрШаблон("%1 = ПараметрыПлашки.%1", ПараметрПлашки));
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого Т Из ПараметрыПлашки Цикл
		Выполнить(СтрШаблон("%1 = ПараметрыПлашки.%1", Т.Ключ));
	КонецЦикла;
	...
	Если AHK_Balloon = Неопределено Тогда
		...
		|Global BGCOLOR := 0x068A3F
		|Global FONTCOLOR := 0x000000
		|
		...
		|Global marginX := 10
		|Global marginY := 25
		|Global fontSize1 := %РазмерЗаголовка%
		|Global fontSize2 := %РазмерТекста%
		|
		...
		|		}
		|
		|		Gui,G%index%: Add, Text, c%FONTCOLOR% W1000, % this.strCaption
		|		Width := WINDOW_WIDTH - 40
		|		if (this.index <= 4) {
		|			Gui,G%index%: Font, s%fontSize2%
		|			Gui,G%index%: Add, Text, c%FONTCOLOR% W%Width% H200 , % this.strText
		|		} else {
		|			Gui,G%index%: Font, s20
		|			Gui,G%index%: Add, Text, CXY W%Width% H200 c%FONTCOLOR% , % this.strText
		|		}
		|		
		|		
		|
		...
		Скрипт = СтрЗаменить(Скрипт, "%РазмерТекста%", Формат(РазмерТекста,"ЧГ=0"));
		Скрипт = СтрЗаменить(Скрипт, "%РазмерЗаголовка%", Формат(РазмерЗаголовка,"ЧГ=0"));
		Если Отладка Тогда
	...
	AHK_Balloon.ahkassign("WINDOW_Y",?(ЗначениеЗаполнено(Верх), Верх, 0));
	...
	|BGCOLOR := 0x%10
	|",
	НомерПлашки, Заголовок, Текст, Формат(Таймаут,"ЧГ=0"), Прозрачность,Ширина,Высота,РазмерЗаголовка, РазмерТекста, ?(ЦветФона=Неопределено,"0", ЦветФона));
	
	Скрипт = Скрипт + "FONTCOLOR := 0x" + ЦветТекста + "
	|";
	Скрипт = Скрипт + "ShowMessage()";
	AHK_Balloon.addScript(Скрипт, 2); 
	
КонецПроцедуры

//Процедура ПоказатьПлашку(Заголовок = "(:-)", Знач Текст = "", НомерПлашки = 0, Таймаут = "2500", Лево = 0, Прозрачность = 255, Ширина = 300, Высота = 180, РазмерЗаголовка = 14, РазмерТекста = 9, Отладка = Ложь) Экспорт
Процедура ПоказатьПлашкуПрост(Заголовок = ":-)", Текст = "",НомерПлашки =0 , Таймаут = 2500, Лево = 0, Прозрачность = 255) Экспорт
	ПараметрыПлашки = Новый Структура("Заголовок, Текст,НомерПлашки, Таймаут, Лево, Прозрачность", Заголовок, Текст,НомерПлашки, Таймаут, Лево, Прозрачность);
	ПоказатьПлашку(ПараметрыПлашки);
КонецПроцедуры
