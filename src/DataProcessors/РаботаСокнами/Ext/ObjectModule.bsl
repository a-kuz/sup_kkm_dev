...
Процедура ПоказатьПлашку(ПараметрыПлашки = Неопределено) Экспорт
	МассивПараметров = СтрРазделить("Заголовок,Текст,Таймаут,НомерПлашки,Прозрачность,Лево,Ширина,Высота,РазмерТекста,РазмерЗаголовка,ЦветФона,ЦветТекста", ",",Ложь);	
	Заголовок = "(:-)"; Текст = ""; НомерПлашки = 0; Таймаут = "2500"; Лево = 0; Прозрачность = 255; Ширина = 300; Высота = 180; РазмерЗаголовка = 14; РазмерТекста = 9; ЦветФона = Неопределено; ЦветТекста = "000000"; Отладка = Ложь;
